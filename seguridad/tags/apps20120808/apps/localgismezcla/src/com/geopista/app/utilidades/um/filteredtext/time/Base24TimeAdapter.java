/**
 * @(#) Base24TimeAdapter.java	1.0	99/06/08
 *
 ...
 * <p>
 * The time format this adapter sticks to is HH:MM:SS. Here the hours should
 ...
 * @author	UnicMan
 */
public class Base24TimeAdapter
	extends		TimeAdapter
	implements	UMConstants
{
	...
	public void keyTyped( KeyEvent keKey )
	{
		...
		if( Character.isDigit(cKey) || cKey == TIME_SEP ||
			cKey == KeyEvent.VK_BACK_SPACE || cKey == KeyEvent.VK_PERIOD
		 )
			return;
		keKey.consume();
	}
	...
	public void focusLost( FocusEvent feEvt )
	{
		...
		if( (cCalendar=TextConverter.getBase24Time(szText)) == null )
		{
			...
			szTime = TextConverter.toBase24String(cCalendar);
