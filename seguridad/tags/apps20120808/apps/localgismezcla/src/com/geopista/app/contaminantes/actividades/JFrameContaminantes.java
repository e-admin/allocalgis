...
import com.geopista.app.contaminantes.CAddressJDialog;
...
import com.geopista.app.contaminantes.util.JInternalFrameShowMap;
import com.geopista.app.utilidades.ShowMaps;
import com.geopista.app.ocupaciones.CMainOcupaciones;
import com.geopista.app.ocupaciones.creacion.CCreacionLicencias;
...
public class JFrameContaminantes extends JInternalFrameShowMap {
	...
	private void initComponents() throws CancelException{//GEN-BEGIN:initComponents
        ...
        //layer = ShowMaps.showLayer(com.geopista.app.contaminantes.init.Constantes.url,geopistaEditor, layerName, permitido, new String[]{"parcelas", "numeros_policia",layerName}, new Color(64, 0, 0));   //"parcelas","base_cartografica",
        ...
        {
            if (ShowMaps.showMap(com.geopista.app.contaminantes.init.Constantes.url,geopistaEditor,
			...
			public void featureAdded(FeatureEvent e) {
				...
				for (Iterator i = features.iterator(); i.hasNext();) {
                    ...
                    if (geoF.getSystemId()==null || geoF.getSystemId().length()<=0 || !idValido)
                    {
                        ...
                        try
                        {
                            new OperacionesContaminantes((com.geopista.app.contaminantes.init.Constantes.url)).gestionarHistorico(new Historico(new Integer(actividad.getId()).intValue(), com.geopista.app.contaminantes.init.Constantes.ADD, com.geopista.app.contaminantes.init.Constantes.CONTAMINANTES));
                        }catch(Exception ex){}
...
//						f.setAttribute(0, new Integer(geoF.getSystemId()));
                    	...
                    	final Runnable runnable = new Runnable(){
                    		...
                    		public void run(){
                    			try {
                                    CResultadoOperacion resultado = (new OperacionesContaminantes(com.geopista.app.contaminantes.init.Constantes.url)).getDireccionMasCercana(actividad.getId());
                                    if (resultado.getResultado()) {
                                        ...
                                        (new OperacionesContaminantes(com.geopista.app.contaminantes.init.Constantes.url)).saveActividad(actividad);
                                    }
                                    else {
...
//						jPanelActividad.load(actividad);
			...
			for (Enumeration e = actividad.getInspecciones().elements(); e.hasMoreElements();) {
				...
				if (inspeccion.getAnexos() != null) {
					...
					for (Enumeration e1 = inspeccion.getAnexos().elements(); e1.hasMoreElements();) {
						...
						if (anexo.getEstado() != com.geopista.protocol.contaminantes.CConstantes.CMD_ANEXO_ADDED) {
							anexo.setEstado(-1);
	...
	private void borrar(boolean mostrarMensaje) {
		...
		try {
			result = (new OperacionesContaminantes(com.geopista.app.contaminantes.init.Constantes.url)).deleteActividad(actividad);
		} catch (Exception e) {
		...
		if (result.getResultado()) {
            ...
            try
            {
                new OperacionesContaminantes((com.geopista.app.contaminantes.init.Constantes.url)).gestionarHistorico(new Historico(new Integer(actividad.getId()).intValue(), com.geopista.app.contaminantes.init.Constantes.BORRAR, com.geopista.app.contaminantes.init.Constantes.CONTAMINANTES));
            }catch(Exception ex){}
	...
	private void salvar() {
		...
		try {
			...
			String sMensaje = "";
			try {
                result = (new OperacionesContaminantes(com.geopista.app.contaminantes.init.Constantes.url)).saveActividad(auxActividad);
			...
			if (result.getResultado()) {
                ...
                try
                {
                    new OperacionesContaminantes((com.geopista.app.contaminantes.init.Constantes.url)).gestionarHistorico(new Historico(new Integer(actividad.getId()).intValue(), com.geopista.app.contaminantes.init.Constantes.MODIFICAR, com.geopista.app.contaminantes.init.Constantes.CONTAMINANTES));
                }catch(Exception ex){}
	...
	public void enabled(boolean bValor) {
     ...
     private void  generarFicha()
     {
         try
         {
		     ...
		     new GeopistaPrintable().printObjeto(FichasDisponibles.fichacontaminante, actividad , Contaminante.class, getGeopistaEditor().getLayerViewPanel(), GeopistaPrintable.FICHA_CONTAMINANTE_PLANOS);
         }catch(Exception e)
         {
