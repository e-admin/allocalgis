    ...
    ACException, Exception
    {
    	...
    	if (aplicacionLocal.isLogged())
    	{
    		...
    		if (aplicacionLocal.isLogged())
    		{
    			...
    			GeopistaLayer localLayer = ((GeopistaFeature) updateFeatures[0])
    			.getLayer();
            ...
            public void componentShown(ComponentEvent e){
                 new Thread(new Runnable(){
                          public void run(){
                              try{
                                  progressDialog.report(aplicacion.getI18nString("cementerio.mapaJPanel.tag3"));
                                  if (com.geopista.app.licencias.CUtilidadesComponentes.showGeopistaMap(desktop, panel, geopistaEditor, mapa, editable)){
                                      GeopistaLayer layer=(GeopistaLayer)geopistaEditor.getLayerManager().getLayer("cementerios");
                                      if (layer!=null){
