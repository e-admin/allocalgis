...
public class MarkCenterTool extends CenterTool {
    ...
    private GeopistaEditor editor = null;
    private double factorTolerance = 2;
    ...
    public MarkCenterTool(WorkbenchContext context){
    	...
    	this.context.getLayerViewPanel().addMouseListener(new MouseAdapter()
        {
    ...
    public MarkCenterTool(GeopistaEditor editor){
    	this.editor = editor;
    	this.editor.getLayerViewPanel().addMouseListener(new MouseAdapter()
        {
            public void mousePressed(MouseEvent e)
            {
                deactivate();
    	...
    	this.editor.getLayerViewPanel().getViewport().addListener(new ViewportListener(){
   ...
   public void paint(Geometry geometry) throws Exception{
	    ...
	    this.originalStroke = new BasicStroke(
	            ...
	            0);   
	    setStroke(originalStroke);
	   	double tolerance = SnapManager.getToleranceInPixels(this.editor.getBlackboard()) / this.editor.getLayerViewPanel().getViewport().getScale();
	   	Geometry buffer = geoms.buffer(1*tolerance/factorTolerance);   
	   	geoms = geoms.union(buffer);
	   	this.geo = geoms;
		this.activate(this.editor.getLayerViewPanel());
		this.redrawShape();
//		this.outlineItemsShape = null;
  }
   
   public void setFactorTolerance(double factor){
	   factorTolerance = factor;
   }
