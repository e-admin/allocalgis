...
public class DataStoreOutputFC implements DataStoreOutputFormat {
     ...
     */
    private FeatureCollection tableToFC(Table table, ParameterList pl,
    FeatureCollection fc) throws Exception {
        ...
        for (int r = 0; r < table.getRowCount(); r++) {
            ...
            for (int i = 0; i < row.length; i++) {
                ...
                // get feature id
                if ( columnNames[i].equalsIgnoreCase( idProp ) )  id = row[i].toString();
                ...
                if ( !(row[i] instanceof ByteArrayOutputStream) )  {
                    if ( row[i] instanceof Table ) {
                        ...
                        FeatureCollection fc_ =
                            factory.createFeatureCollection( td_.getTargetName(), 1000 );
    ...
    private org.deegree.model.feature.FeatureType createFeatureType(FeatureType ft, Table table, 
        ...
        for (int i = 0; i < columnNames.length; i++) {
            ...
            //String pn = ft.getProperty( table.getTableName()+"."+columnNames[i] );
            ...
            if ( td.getGeoFieldIdentifier( columnNames[i] ) == null  ) {
                if ( columnTypes[i].equals( "org.deegree.model.table.Table") ) {
                    ...
                    // column
                    ftp[k++] = factory.createFeatureTypeProperty( pn, columnTypes[i],
                                                                  true );
                }
            } else {
                // create a FeatureTypeProperty with geometry type
                ftp[k++] = factory.createFeatureTypeProperty( pn, "org.deegree.model.geometry.GM_Object",
                                                              true );
            }
        }
     ...
     *
     * @param bos wkb datastructure
     * @param srs spatial reference system used for each feature within the
     *				feature collection.
     */
    ...
    CS_CoordinateSystem srs)  throws Exception {
        ...
        switch ( getGeometryType( wkb ) ) {
            case 1: geo = geoFactory.createGM_Point( wkb, srs ); break;
            case 2: geo = geoFactory.createGM_Curve( wkb, srs); break;
            case 3: geo = geoFactory.createGM_Surface( wkb, srs, new GM_SurfaceInterpolation_Impl(1)); break;
            case 4: geo = geoFactory.createGM_MultiPoint( wkb, srs ); break;
            case 5: geo = geoFactory.createGM_MultiCurve( wkb, srs ); break;
            case 6: geo = geoFactory.createGM_MultiSurface( wkb, srs, new GM_SurfaceInterpolation_Impl(1) ); break;
            default: geo = null;
        }
    ...
    private int getGeometryType(byte[] wkb) throws GM_Exception {
        ...
        if (byteorder == 0) wkbType = ByteUtils.readBEInt(wkb,1);
        else wkbType = ByteUtils.readLEInt(wkb,1);
