...
class CGetFeature extends DBGetFeature {
    ...
    public CGetFeature(PostgisDataStore parent, OGCWebServiceRequest request) {
    ...
    protected OGCWebServiceResponse[] performRequest(OGCWebServiceRequest request) {
        ...
        try {
            ...
            PostgisDBAccess osa = new PostgisDBAccess( con );
            ...
            for (int i = 0; i < queries.length; i++) {
                //System.out.println("queries[i].getTypeName()"+queries[i].exportAsXML());
                if ( parent.isKnownFeatureType( queries[i].getTypeName() ) ) {
                    ...
                    //System.out.println("------------Nombre del tipo de query:"+queries[i].getTypeName());
                    map.put( queries[i].getTypeName()+i, pl );
