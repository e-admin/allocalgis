			...
			if (!((EntidadesSingularesEIELTableModel)((TableSorted)getJTableListaEntidades().getModel()).getTableModel()).getData().isEmpty()){
				if (confirm(I18N.get("LocalGISEIEL","localgiseiel.register.tag2"), 
					...
					if (selectedItem != null && selectedItem instanceof EntidadesSingularesEIEL){
						...
						try {
							Collection lstFeatures = ConstantesLocalGISEIEL.clienteLocalGISEIEL.getIdsFeatures(selectedItem, ConstantesLocalGISEIEL.ENTIDADES_SINGULARES);
							...
							ConstantesLocalGISEIEL.clienteLocalGISEIEL.eliminarElemento(selectedItem, lstFeatures, idLayer, ConstantesLocalGISEIEL.ENTIDADES_SINGULARES );
