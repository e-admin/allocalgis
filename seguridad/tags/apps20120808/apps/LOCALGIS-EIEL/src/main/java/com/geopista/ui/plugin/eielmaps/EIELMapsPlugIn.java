...
public class EIELMapsPlugIn extends AbstractPlugIn
{
	/**
	 * Logger for this class
	 */
	private static final Log	logger	= LogFactory.getLog(EIELMapsPlugIn.class);
    ...
    private JComboBox getComboMapas() {
        ...
        aplicacion.getBlackboard().put(AppContext.MAPAS_COMBO, comboMapas);
    ...
    public static void forceLoadMap(int idMapa, TaskMonitorDialog progressDialog) throws HeadlessException, CancelException{
    	...
    	//Indicamos las capas que no queremos que desaparezcan cuando se cargue el mapa
		...
		{
			new JOptionPane(I18N.get("LocalGISEIEL","localgiseiel.editorimportador.panel.ErrorLoadingMap"),JOptionPane.ERROR_MESSAGE).createDialog(panel, "ERROR").show();
		}
