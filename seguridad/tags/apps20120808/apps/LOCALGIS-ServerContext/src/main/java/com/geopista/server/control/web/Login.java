...
//		}	    
    ...
    private ISesion getSesion(HttpServletRequest request){
        ...
        {
            logger.debug("doPost(HttpServletRequest, HttpServletResponse) - Nombre del user principal:"+ jaasUserPrincipal.getName());
        }
    ...
    private void grabarLogin(ISesion sSesion, HttpServletRequest request){
    	try
        {
	    	...
	    	if(!SSOAuthManager.isSSOActive()){
	            ...
	            COperacionesControl.grabarLogin(sSesion.getIdSesion(), sSesion.getIdUser(),SSOConstants.SSO_NAME);
