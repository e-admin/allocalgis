...
public class SQLParserPostGIS {
    ...
    public PreparedStatement[] newUpdate(int iMunicipio,String sSQL, ACLayer schema, ACFeatureUpload featureUpload, Integer srid_destino, int sridDefecto, int sridInicial) throws SQLException, ACException, FileNotFoundException, IOException{
...
//                    alParams.add(new Integer(srid.getSRID(iMunicipio)));
                    ...
                    if (((Integer)acAtt.getColumn().getType() == ACLayer.TYPE_NUMERIC) && (valorAtributo != null)) //Integer
                    	try{ 
                    	...
                    	if (featureUpload.getAttValues()[iIndex-1] instanceof String && ((Integer)acAtt.getColumn().getType() == ACLayer.TYPE_GEOMETRY) && ((String)featureUpload.getAttValues()[iIndex-1]).endsWith(" EMPTY")){
                    		alParams.add(null);
                    	}else
                    		alParams.add(featureUpload.getAttValues()[iIndex-1]);
                    }
    ...
    /** Sirve para los insert, update y delete. */
    public PreparedStatement[] newUpdate(List listaMunicipios,String sSQL, ACLayer schema, ACFeatureUpload featureUpload, Integer srid_destino, int idMunicipio, int sridDefecto, int sridInicial) throws SQLException, ACException, FileNotFoundException, IOException{
    	//Introduzco el parámetro L para la función length(geometry)
        String sQueries[]=sSQL.split(";");
        PreparedStatement[] apsRet=new PreparedStatement[sQueries.length];
...
//                    alParams.add(new Integer(srid.getSRID(Integer.parseInt(iMunicipio))));
    ...
    public PreparedStatement newSelect(String idEntidad, String iMunicipio,String sSQL, ACLayer schema,Geometry geometry, 
