		...
		systemCertStoreTestButton.addActionListener(new java.awt.event.ActionListener()
			{
				public void actionPerformed(java.awt.event.ActionEvent e){					
					if(!getSystemCertStoreTextField().getText().trim().equals("")){
						Dialog systemConfigPanelAbsolutParent=(Dialog)systemConfigPanel.getParent().getParent().getParent().getParent();
						String password = "";
...
//		getIdMunicipioField().setText(aplicacion.getUserPreference(AppContext.CODIGOMUNICIPIO,"",true)); //$NON-NLS-1$
        ...
        if(CertificateUtils.isDNIeActive()){
	       ...
	       getSystemCertStoreTextField().setText(aplicacion.getUserPreference(CertificateConstants.SYSTEM_CERT_STORE,CertificateUtils.getDefaultWindowsJksSystemCertStore(),true));   
