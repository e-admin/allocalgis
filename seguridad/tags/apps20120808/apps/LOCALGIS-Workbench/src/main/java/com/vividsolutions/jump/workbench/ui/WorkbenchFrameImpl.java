 ...
 */
public class WorkbenchFrameImpl extends WorkbenchFrame implements LayerViewPanelContext,
    ViewportListener, WorkbenchGuiComponent{
                    ...
                    setTitle(
                            (panel.selectedNodes(Category.class).size() != 1)
                                    ? ("("
                                            + panel
                                                    .selectedNodes(
                                                            Category.class).size() + " categories selected)")
                                    : ((Category) panel
                                            .selectedNodes(Category.class)
                ...
                public void popupMenuWillBecomeVisible(PopupMenuEvent e) {
                    ...
                    setTitle(
                            (panel.selectedNodes(Layer.class).size() != 1)
                                    ? ("("
                                            + panel.selectedNodes(Layer.class)
                                                    .size() + " layers selected)")
                                    : ((Layerable) panel
                    ...
                    setTitle(
                            (panel.selectedNodes(WMSLayer.class).size() != 1)
                                    ? ("("
                                            + panel
                                                    .selectedNodes(
                                                            WMSLayer.class).size() + " WMS layers selected)")
                                    : ((Layerable) panel
    ...
    private Map nodeClassToLayerNamePopupMenuMap = CollectionUtil.createMap(
            new Object[]{Layer.class, layerNamePopupMenu, WMSLayer.class,
        wmsLayerNamePopupMenu, Category.class, categoryPopupMenu});
    private int positionIndex = -1;
    private int primaryInfoFrameIndex = -1;

    public WorkbenchFrameImpl(String title, ImageIcon icon,
...
// Aqui no creo que deba ir un TaskFrame por si se quiere activar otro tipo de JinternalFrame
   ...
   // public void addInternalFrame(final TaskComponent internalFrame) {
    ...
    public void addInternalFrame(final JInternalFrame internalFrame,
        ...
        desktopPane.add( (Component) internalFrame, alwaysOnTop
    ...
    private void setClosingBehaviour(final LayerManagerProxy internalFrame) {
        ...
        i.addInternalFrameListener(new InternalFrameAdapter() {
            public void internalFrameClosing(InternalFrameEvent e) {
                if (1 == getInternalFramesAssociatedWith((LayerManager)internalFrame
                        .getLayerManager()).size()) {
                    if (confirmClose("Close Task", internalFrame
                            .getLayerManager()
                            .getLayersWithModifiedFeatureCollections())) {
    ...
    public void flash(final HTMLFrame frame) {
        ...
        new Timer(100, new ActionListener() {
            ...
            public void actionPerformed(ActionEvent e) {
                try {
                    tickCount++;
                    frame.setBackgroundColor(((tickCount % 2) == 0)
                            ? originalColor
                            : Color.yellow);
                    if (tickCount == 2) {
...
//        }
    ...
    public static String toMessage(Throwable t) {
        ...
        if (t.getLocalizedMessage() == null) {
            message = "No description was provided";
                ...
                .indexOf("side location conflict") > -1) {
    ...
    void windowMenu_menuSelected(MenuEvent e) {
        ...
        if (windowMenu.getItemCount() > 0
                ...
                && windowMenu
                        .getItem(0)
                        .getText()
                        .equals(AbstractPlugIn.createName(
                                CloneWindowPlugIn.class))) {
    ...
    private void installKeyboardShortcutListener() {
        addEasyKeyListener(new KeyListener() {
			...
			// Aquino 10/15/2003]
            AbstractPlugIn
                    .toActionListener(plugIn, workbenchContext,
