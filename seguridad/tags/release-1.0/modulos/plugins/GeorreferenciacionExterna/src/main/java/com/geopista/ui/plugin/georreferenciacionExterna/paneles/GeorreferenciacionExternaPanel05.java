...
public class GeorreferenciacionExternaPanel05 extends javax.swing.JPanel implements WizardPanel {
	...
	private void guardarConsulta(){
		...
		if(consultaSelected!=null){
			...
			Test.actualizarConsulta(consultaDatosBean);
		}
		else{
			//es un alta
			Test.guardarConsulta(consultaDatosBean);
		}
	}
	...
	public void obtenerFilas(){
		...
		try{
	    	...
	    	int valor=Test.obtenerTotalFilasBbdd(datos.getBbdd(), datos.getTabla(), datos.getTipoDatos(), 
	    	...
	    	while(var<valor){
	    		...
	    		RowBean[] datosBBDD = Test.obtenerDatosBbdd(
	...
	public void addLayerAttribute(com.localgis.ws.servidor.ServicesStub.RowBean[] rowbeannuevo) throws SecurityException, NoSuchMethodException, IllegalArgumentException, InstantiationException, IllegalAccessException, InvocationTargetException
	{		
		...
		if(capa.isLocal() && dataType != null){//SOLO CUANDO LA CAPA ES LOCAL
			...
			numPaquete++;
			com.localgis.ws.servidor.ServicesStub.ColumnsBean[] colb=null;
			int cont=0;
    							...
    							if(rule.getSymbolizers()!=null){
    								for(int indiceSymbolicers = 0; indiceSymbolicers < rule.getSymbolizers().length; indiceSymbolicers++){
    									if (rule.getSymbolizers()[indiceSymbolicers] instanceof LineSymbolizer_Impl){
    										...
    										if((featureCollection.getFeatureSchema().hasAttribute("TEXTO"))){
