...
public class PMRBasicNetworkFactoryDialog extends JDialog  implements ItemListener{
	...
	private JRadioButton featureRadioButton = null;
	private JRadioButton layerRadioButton = null; 
	private JRadioButton bidirectionRadioButton = null;
	private JRadioButton uniqueDirectionAtoBRadioButton = null;
	private JRadioButton uniqueDirectionBtoARadioButton = null;
	private JComboBox layerComboBox= null;
	private JComboBox impedanciaBA = null;
	private JComboBox impedanciaAB = null;
	private JComboBox description = null;
	private JTextField nombreRedTextField = null;

	private ButtonGroup featuresButtonGroup = new ButtonGroup();
	private ButtonGroup directionButtonGroup = new ButtonGroup();


	private JPanel rootPanel = null; 
	private JPanel featuresSelectPanel = null;
	private JPanel directionSelectPanel = null;
...
/*			rootPanel.add(getDescriptionSelectPanel(), 
	...
	public String getsRedOrigen(){
		return redesPanel.getSubredSelected();
	}
...
/*			featuresSelectPanel.add(getFeatureRadioButton() , 
	...
	public JRadioButton getBidirectionRadioButton() {
		...
		if (bidirectionRadioButton == null){
			...
			this.bidirectionRadioButton.setEnabled(false);
		}
	...
	public JRadioButton getUniqueDirectionAtoBRadioButton() {
		...
		if (uniqueDirectionAtoBRadioButton == null){
			...
			this.uniqueDirectionAtoBRadioButton.setEnabled(false);
		}
	...
	public JRadioButton getUniqueDirectionBtoARadioButton() {
		...
		if (uniqueDirectionBtoARadioButton == null){
			...
			this.uniqueDirectionBtoARadioButton.setEnabled(false);
	...
	public void impedanciasComboBoxController(boolean impedanciaAB, boolean impedanciaBA, boolean cargar ) {
		...
		if ((impedanciaAB || impedanciaBA) && cargar){
			...
			if (impedanciaAB && impedanciaBA){
				com.geopista.app.utilidades.EdicionUtils.cargarLista(this.getImpedanciaAB(), attributesLayerList);
		...
		if (this.getLayerComboBox().getSelectedIndex() >= 0){
			...
			if ( !((GeopistaLayer)selectedLayer).isLocal() && !this.getFeatureRadioButton().isSelected()){
				...
				if (descripcionController && cargar){
					...
					for (int i = 0; i < selectedLayer.getFeatureCollectionWrapper().getFeatureSchema().getAttributeCount(); i++){
						attributesLayerList.add(((GeopistaSchema)selectedLayer.getFeatureCollectionWrapper().getFeatureSchema()).getColumnByAttribute(selectedLayer.getFeatureCollectionWrapper().getFeatureSchema().getAttributeName(i)));
					}
