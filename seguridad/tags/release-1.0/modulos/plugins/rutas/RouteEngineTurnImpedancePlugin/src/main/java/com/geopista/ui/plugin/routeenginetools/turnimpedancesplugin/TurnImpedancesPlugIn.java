			...
			if (dialog.wasOKPressed()){
				...
				if(network.getGraph() instanceof DynamicGraph && ((DynamicGraph)network.getGraph()).getMemoryManager() instanceof SpatialAllInMemoryExternalSourceMemoryManager){
					...
					try{
						...
						nDAO.clearImpedanceMatrix(netName, this.dialog.getNode().getID(), connection);
						nDAO.setImpedanceMatrix(netName, this.dialog.getNode(), connection);
...
//	}
	...
	private Network searchNetworkByName(PlugInContext context,
		...
		Network net1 = FuncionesAuxiliares.getNetworkManager(context).getNetwork(netName);
		Iterator<Network> it = FuncionesAuxiliares.getNetworkManager(context).getNetworks().values().iterator();
		while (it.hasNext()){
			Network net = it.next();
