...
public class LocalGISAppTableModel extends DefaultTableModel {
    ...
    public void setModelData(Hashtable<String,LocalGISApp> listaLocalGISApp, TipoApp tipoApp) {
        try
        {
        	GeopistaAcl defaultAcl;
        	String permisoApp;
            for (Enumeration e=listaLocalGISApp.keys();e.hasMoreElements();) {
            	String keyName = (String) e.nextElement();               	
            	if(listaLocalGISApp.get(keyName).getTipoApp().equals(tipoApp)){
            		defaultAcl =com.geopista.security.SecurityManager.getPerfil(listaLocalGISApp.get(keyName).getNombreAcl());
            		permisoApp = listaLocalGISApp.get(keyName).getPermisoApp();
