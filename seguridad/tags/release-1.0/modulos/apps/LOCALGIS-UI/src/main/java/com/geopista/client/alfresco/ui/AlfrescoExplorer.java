...
public class AlfrescoExplorer extends JDialog {
	...
	private void init() {	
		...
		if(rootNode == null || rootNode.getUserObject() == null){
			JOptionPane.showMessageDialog(this, AppContext.getApplicationContext().getI18nString("alfresco.error.permission"));	
			
			dispatchEvent(new WindowEvent(this.getOwner(), WindowEvent.WINDOW_CLOSING));
			setVisible(false);		
			dispose();
		}	
		else{
	...
	private void createNewDirectory(){		
		String newDirectory = JOptionPane.showInputDialog(this, AppContext.getApplicationContext().getI18nString("alfresco.input.directory"));
	...
	private void renameDirectory(){	
		if(!getTreeSelectedAlfrescoNode().getUuid().equals(((AlfrescoNode)rootNode.getUserObject()).getUuid())){
			String newName = JOptionPane.showInputDialog(this, AppContext.getApplicationContext().getI18nString("alfresco.input.renameDirectory"), getTreeSelectedAlfrescoNode().getName());
			if(newName!=null){
				try {
	...
	private void annexeAlfrescoDocument(){		
	    ...
	    try {
	    	...
	    	if(features != null || (element != null && app.equals(AlfrescoConstants.APP_INVENTORY))){	
	    		...
	    		CementerioDocumentClient documentClient = new CementerioDocumentClient(AppContext.getApplicationContext().getString(AppContext.HOST_ADMCAR) + "/" + WebAppConstants.CEMENTERIOS_WEBAPP_NAME + ServletConstants.ADMINISTRADOR_CARTOGRAFIA_SERVLET_NAME +
				    			...
				    			public void run(){
				    				try {				                                
										if(app.equals(AlfrescoConstants.APP_GENERAL)){
											...
											getDynamicTreePanel().getDragTable().setModel(alfrescoDocumentClient.getChildFiles(new AlfrescoKey(((AlfrescoNode)finalParentNode.getUserObject()).getUuid(), AlfrescoKey.KEY_UUID), new Object [] { element }));
								    	}
		...
		while(itDocuments.hasNext()){
			try {
				String uuid = itDocuments.next();
					...
					if(parentNode != null){
						uuid = parentNode.getReference().getUuid();
						uuidFolders.add(parentNode.getReference().getUuid());
					}
			} catch (Exception e) {
				System.out.println(e);
			}
	...
	private JSplitPane getButtonSplitPane() {
		if (buttonSplitPane == null) {
			...
			buttonSplitPane.setMinimumSize(new Dimension(200, 35));
			buttonSplitPane.setPreferredSize(new Dimension(0, 35));
			buttonSplitPane.setSize(new Dimension(500, 35));
			buttonSplitPane.setDividerSize(0);
			buttonSplitPane.setContinuousLayout(true);
			buttonSplitPane.setDividerLocation(250);			
