		    											...
		    											for(int i=0; i<lstMunicipiosNucleos.size(); i++)
		    											{
		    												...
		    												for (int j=0; j<lstCuadros.size(); j++){
		    										    		...
		    										    		if(cuadro.getNamejrxml().equals("A") || cuadro.getNamejrxml().equals("G")){
		    										    			where = cuadro.getTabla()+".PROVINCIA='"+String.valueOf(municipio.getId()).substring(0, 2)+"'";
				    												
		    										    		}
		    										    		else{
		    										    			where = cuadro.getTabla()+".PROVINCIA='"+String.valueOf(municipio.getId()).substring(0, 2)+"' AND "+
		    										    			cuadro.getTabla()+".MUNICIPIO='"+String.valueOf(municipio.getId()).substring(2, String.valueOf(municipio.getId()).length())+"'";
...
/* CONSTRUCTORES */    
    ...
    private void initialize(String title) throws Exception{
        ...
        clienteLocalGISEIEL = new LocalGISEIELClient(aplicacion.getString(AppContext.GEOPISTA_CONEXION_ADMINISTRADORCARTOGRAFIA) +	"/EIELServlet");
    ...
    private void generarInformeJButtonActionPerformed( String jrxmlfile, HashMap paramHash, String pathDestino, String formato) {
        try{
          //  this.setCursor(Cursor.getPredefinedCursor(Cursor.WAIT_CURSOR));
             ...
             try {
            	 String locale =  AppContext.getApplicationContext().getUserPreference(AppContext.GEOPISTA_LOCALE_KEY, "es_ES", true);
                ...
                logger.error("Error al genera el cuadro mpt",e);
            }
