...
{        
	        		...
	        		if (getJPanelTree().getPatronSelected()==null) return;
	        		 ...
	        		 if (layerName!=null){	 			
	        				TreeRendereEIELDomains renderer=(TreeRendereEIELDomains)getJPanelTree().getTree().getCellRenderer();
	        			 	if (ConstantesLocalGISEIEL.loadedLayers.containsKey(layerName)){
	        			 		if (geopistaLayer!=null){
		        			 		new ReloadLayerPlugIn().loadFeatures(geopistaLayer, GeopistaEditorPanel.getEditor().getLayerViewPanel(),true);
		        			 		renderer.setIconoLoaded(getJPanelTree().getPatronSelected(),false);
		        			 		ConstantesLocalGISEIEL.loadedLayers.remove(layerName);
	        			 		}
	        			 	}
	        			 	else{
	        			 		if (geopistaLayer!=null){
    ...
    public void dominiosJPanel_selectionChanged(ActionEvent e){
	 	...
	 	if (e.getSource() instanceof AbstractSelection){
	 		IAbstractSelection abtractSelection=(IAbstractSelection)e.getSource();
		...
		if (ConstantesLocalGISEIEL.permisos.containsKey(ConstantesLocalGISEIEL.PERM_PUBLICADOR_EIEL)){
		 	...
		 	if (e.getSource() instanceof FeatureEvent){
				...
				try {
	    			...
	    			ArrayList lst = obtenerListaDatos(getJPanelTree().getPatronSelected(),0);
	    			...
	    			if ( (lst!=null) && (lst.size()==1 || (lst.size()==2))){
	    				...
	    				ConstantesLocalGISEIEL.clienteLocalGISEIEL.insertarElemento(workflowEIEL, idLayer, nodoSeleccionado);
...
//            	getJPanelBotonera().getJButtonListarTabla().setEnabled(true);
    		...
    		if((ConstantesLocalGISEIEL.permisos.containsKey(ConstantesLocalGISEIEL.PERM_PUBLICADOR_EIEL)) ||
    	    	...
    	    	try {
					...
					while (enumera.hasMoreElements()){
						...
						if (nodoEIEL!=null){
							if (ConstantesLocalGISEIEL.modelosPatrones.get(nodoEIEL.getClave())!=null){
								if (nodoEIEL.getElementosTemporales()>0 || nodoEIEL.getElementosPublicables()>0||nodoEIEL.getElementosExternos()>0)
									...
									parametros="  (T:"+nodoEIEL.getElementosTemporales()+")"+ " (P:"+nodoEIEL.getElementosPublicables()+") (E:"+nodoEIEL.getElementosExternos()+")";
    	...
    	for (int i=0; i < ((ListaDatosEIELJPanel)getJPanelListaElementos()).getJTableListaDatos().getRowCount(); i++){
    		...
    		//else{
    			...
    			if (elemento != null && elemento.getBloqueado()!=null &&
    						!elemento.getBloqueado().equalsIgnoreCase(com.geopista.security.SecurityManager.getPrincipal()!=null?com.geopista.security.SecurityManager.getPrincipal().getName():AppContext.getApplicationContext().getUserPreference(AppContext.USER_LOGIN,null,false))){
    				color=Color.red;    				
    			}
    ...
    public void listarDatosTabla(boolean activarBoton,int idMunicipio){
    	...
    	if (patternSelected != null && !patternSelected.equals("")){
    		...
    		//GENERICO
    			...
    			ArrayList lst=recargarElementos(patternSelected,activarBoton,idMunicipio);
    			String activarMunicipio=ConstantesLocalGISEIEL.modelosActivarSelectorMunicipio.get(patternSelected);
    			if ((activarMunicipio!=null) && (activarMunicipio.equals("S")))
    				jPanelSelectorMunicipio.setEnabled(true);
    			else
    				jPanelSelectorMunicipio.setEnabled(false);
    			if (lst!=null)
     ...
     * @param idMunicipio 
     */
    private ArrayList recargarElementos(String patternSelected, boolean activarBoton, int idMunicipio){
		...
		String modelo=ConstantesLocalGISEIEL.modelosPatrones.get(patternSelected);
		
		setNameTableModel(modelo);    			
		jPanelListaElementos = null;
		jSplitPane.setRightComponent(getJPanelListaElementos());    		    			
		ArrayList lst = obtenerListaDatos(patternSelected,idMunicipio);
    ...
    private Collection getIdFeatures(Object object, String selectedPattern){
    	...
    	try {
    		...
    		lstIdFeatures = ConstantesLocalGISEIEL.clienteLocalGISEIEL.getIdsFeatures(object, selectedPattern);
    ...
    private ArrayList obtenerListaDatos(String tipo){
    	return obtenerListaDatos(tipo,0);
    }
    
    private ArrayList obtenerListaDatos(String tipo,int idMunicipio){
    	...
    	try {
    		...
    		String nombreTabla=ConstantesLocalGISEIEL.tablasAlfanumericas.get(tipo);    		
    		ArrayList camposEspecificos=ConstantesLocalGISEIEL.camposTablaEspecificos.get(tipo);
    		...
    		lstElements = ConstantesLocalGISEIEL.clienteLocalGISEIEL.getLstElementos(filter, tipo,
    				getJPanelBotonera().getJCheckBoxFiltroGeometrias().isSelected(),idMunicipio,nombreTabla,camposEspecificos);
...
//    			((BotoneraJPanel)getJPanelBotonera()).getJButtonInformes().setEnabled(true);
    ...
    private void seleccionarFeaturesEditor(Object obj, String clave){
		...
		try {
			if (GeopistaEditorPanel.getEditor()!= null){
	    		...
	    		String nombreTabla=ConstantesLocalGISEIEL.tablasAlfanumericas.get(patternSelected);    		

				lstFeatures = ConstantesLocalGISEIEL.clienteLocalGISEIEL.getIdsFeatures(obj, patternSelected,false,nombreTabla);
    	...
    	try {
    		...
    		if (patternSelected != null && !patternSelected.equals("")){
    			...
    			//MARKED
    				...
    				if (obj.getCodINEMunicipio().equals(compareObject.getCodINEMunicipio()) &&
    						obj.getCodINEProvincia().equals(compareObject.getCodINEProvincia()) &&
    							obj.getTipo().equals(compareObject.getTipo()) &&
    								obj.getDenominacion().equals(compareObject.getDenominacion())){
    					return obj;
    				}else{
    					return null;
    				}
    		...
    		try {
                ...
                if (geopistaLayer!=null && geopistaLayer.getFeatureCollectionWrapper()!=null && geopistaLayer.getFeatureCollectionWrapper().getFeatures()!=null){
	                ...
	                if (featuresToDelete.size()>0){
	                	aplicacion.getBlackboard().put(Const.ESTADO_VALIDACION,estadoAPublicar);
	                	GeopistaValidatePlugin.deleteFeatures(featuresToDelete.toArray());
	                	aplicacion.getBlackboard().remove(Const.ESTADO_VALIDACION);
    ...
    private void selectorMunicipioJPanel_actionPerformed() {
        try{
            ...
            listarDatosTabla(false,lcgMunicipio.getIdMunicipio());
    ...
    public void botoneraJPanel_actionPerformed(){
        try{
            ...
            if (getJPanelBotonera().getBotonPressed().equalsIgnoreCase(ConstantesLocalGISEIEL.OPERACION_ELIMINAR)){
            	String bloqueado= ConstantesLocalGISEIEL.clienteLocalGISEIEL.bloqueado(selectedItem, getJPanelTree().getPatronSelected());
            ...
            if (abrirDialogo){
        	        ...
        	        progressDialog.report(I18N.get("LocalGISEIEL","localgiseiel.mensajes.tag10"));
            ...
            if (!cancelado){
	            ...
	            if (lcgMunicipio!=null)
	            	listarDatosTabla(true,lcgMunicipio.getIdMunicipio());
	            else
	            	listarDatosTabla(true,0);
	            reColorearBloqueo(getJPanelTree().getPatronSelected());
            }
    ...
    private void eliminarElementoEntidadesAgrupadas(Object object, String tipoElemento){
    	...
    	try {
    		...
    		if  (object instanceof WorkflowEIEL){
        		...
        		//String clave=ConstantesLocalGISEIEL.modelosCapas.get(patternSelected);        		
    			//lstFeatures = ConstantesLocalGISEIEL.clienteLocalGISEIEL.getIdsFeatures(object, tipoElemento);
    			//GeopistaLayer geopistaLayer= (GeopistaLayer) GeopistaEditorPanel.getEditor().getLayerManager().getLayer(clave);
    		//	String idLayer = geopistaLayer!=null?geopistaLayer.getSystemId():null;
    			
    			
    			ConstantesLocalGISEIEL.clienteLocalGISEIEL.eliminarElemento(object, lstFeatures, null, tipoElemento);
    ...
    private void eliminarElemento(Object object, String tipoElemento){
    	...
    	try {
    		...
    		if  (object instanceof WorkflowEIEL){
    			...
    			ConstantesLocalGISEIEL.clienteLocalGISEIEL.eliminarElemento(object, lstFeatures, idLayer, tipoElemento);
        ...
        progressDialog.addComponentListener(new ComponentAdapter(){
            public void componentShown(ComponentEvent e){
                new Thread(new Runnable(){
                    public void run(){
                        try{
                            ConstantesLocalGISEIEL.clienteLocalGISEIEL.bloquearElemento(elemento, bloquear, tipoElemento);
                        }
                        catch(Exception e){                            
                            ErrorDialog.show(AppContext.getApplicationContext().getMainFrame(), 
                            		I18N.get("LocalGISEIEL","localgiseiel.sqlerror.title"), 
                ...
                new Thread(new Runnable(){
                    public void run(){
                        try{
                        	...
                        	Collection lstFeatures=null;
                        	if (GeopistaEditorPanel.getEditor()!= null){                   			
                    			lstFeatures = ConstantesLocalGISEIEL.clienteLocalGISEIEL.getIdsFeatures(elemento, patternSelected,true,nombreTabla); 
                        	}
                        	
                        	switch(nuevoEstado){
                        		case Const.ESTADO_A_PUBLICAR:
                        			if (!ConstantesLocalGISEIEL.capasEspeciales.containsKey(patternSelected)){
                        				ConstantesLocalGISEIEL.clienteLocalGISEIEL.publicarElemento(elemento, tipoElemento);
                        		...
                        		default:
                        			if (!ConstantesLocalGISEIEL.capasEspeciales.containsKey(patternSelected)){
	                        			ConstantesLocalGISEIEL.clienteLocalGISEIEL.validarElemento(elemento, tipoElemento);                                    
    	...
    	if (selectedPattern.equals(ConstantesLocalGISEIEL.ABASTECIMIENTO_AUTONOMO)){
    		...
    		if (tipoOperacion.equals(ConstantesLocalGISEIEL.OPERACION_ANNADIR)){
    			...
    			if (dialog.getAbastecimientoAutonomoPanel().getOkPressed()){
    				...
    				GeopistaLayer geopistaLayer= (GeopistaLayer) GeopistaEditorPanel.getEditor().getLayerManager().getLayer(ConstantesLocalGISEIEL.NUCLEO_LAYER);
    				String idLayer = geopistaLayer!=null?geopistaLayer.getSystemId():null;
    				ConstantesLocalGISEIEL.clienteLocalGISEIEL.insertarElemento(abst, idLayer, selectedPattern);
    			...
    			if (selectedItem!= null && selectedItem instanceof AbastecimientoAutonomoEIEL){
    				...
    				String bloqueado= ConstantesLocalGISEIEL.clienteLocalGISEIEL.bloqueado(selectedElement,selectedPattern);
    				if (bloqueado!=null && !bloqueado.equalsIgnoreCase(com.geopista.security.SecurityManager.getPrincipal()!=null?com.geopista.security.SecurityManager.getPrincipal().getName():AppContext.getApplicationContext().getUserPreference(AppContext.USER_LOGIN,"",false))){
    		...
    		if (tipoOperacion.equals(ConstantesLocalGISEIEL.OPERACION_ANNADIR)){
    			...
    			if (selectedItem!= null && selectedItem instanceof CaptacionesEIEL){
    				...
    				if (bloqueado!=null && !bloqueado.equalsIgnoreCase(com.geopista.security.SecurityManager.getPrincipal()!=null?com.geopista.security.SecurityManager.getPrincipal().getName():AppContext.getApplicationContext().getUserPreference(AppContext.USER_LOGIN,"",false))){
    					...
    					bloquearElemento(selectedElement, getJPanelTree().getPatronSelected(),true);
    					CaptacionesDialog dialog = new CaptacionesDialog(selectedElement,isEditable(selectedElement));
    					dialog.setVisible(true);  
        		...
        		if (tipoOperacion.equals(ConstantesLocalGISEIEL.OPERACION_ANNADIR)){
        			...
        			if (selectedItem!= null && selectedItem instanceof VertederosEIEL){
        				...
        				if (bloqueado!=null && !bloqueado.equalsIgnoreCase(com.geopista.security.SecurityManager.getPrincipal()!=null?com.geopista.security.SecurityManager.getPrincipal().getName():AppContext.getApplicationContext().getUserPreference(AppContext.USER_LOGIN,"",false))){
        					...
        					bloquearElemento(selectedElement, getJPanelTree().getPatronSelected(),true);
        					VertederosDialog dialog = new VertederosDialog(selectedElement,isEditable(selectedElement));
        					dialog.setVisible(true);  
    		...
    		if (tipoOperacion.equals(ConstantesLocalGISEIEL.OPERACION_ANNADIR)){
    			...
    			if (selectedItem!= null && selectedItem instanceof DepositosEIEL){
    				...
    				if (bloqueado!=null && !bloqueado.equalsIgnoreCase(com.geopista.security.SecurityManager.getPrincipal()
    						!=null?com.geopista.security.SecurityManager.getPrincipal().getName():AppContext.getApplicationContext().getUserPreference(AppContext.USER_LOGIN,"",false))){
    					JOptionPane.showMessageDialog(this, I18N.get("LocalGISEIEL","localgiseiel.mensajes.tag1") + " " 
    							+ bloqueado + "\n" + I18N.get("LocalGISEIEL","localgiseiel.mensajes.tag2"));
    					...
    					bloquearElemento(selectedElement, getJPanelTree().getPatronSelected(),true);
    					DepositosDialog dialog = new DepositosDialog(selectedElement,isEditable(selectedElement));
    					dialog.setVisible(true);  
    					if (dialog.getDepositosPanel().getOkPressed()){
    		...
    		if (tipoOperacion.equals(ConstantesLocalGISEIEL.OPERACION_ANNADIR)){
    			...
    			if (selectedItem!= null && selectedItem instanceof PuntosVertidoEIEL){
    				...
    				if (bloqueado!=null && !bloqueado.equalsIgnoreCase(com.geopista.security.SecurityManager.getPrincipal()
    					...
    					bloquearElemento(selectedItem, getJPanelTree().getPatronSelected(),true);
    					PuntosVertidoDialog dialog = new PuntosVertidoDialog(selectedElement,isEditable(selectedElement));
    					dialog.setVisible(true);  
    					if (dialog.getPuntosVertidoPanel().getOkPressed()){
    		...
    		if (tipoOperacion.equals(ConstantesLocalGISEIEL.OPERACION_ANNADIR)){
    			...
    			if (selectedItem!= null && selectedItem instanceof TratamientosPotabilizacionEIEL){
    				...
    				if (bloqueado!=null && !bloqueado.equalsIgnoreCase(com.geopista.security.SecurityManager.getPrincipal()
    					...
    					bloquearElemento(selectedElement, getJPanelTree().getPatronSelected(),true);
    					TratamientosPotabilizacionDialog dialog = new TratamientosPotabilizacionDialog(selectedElement,isEditable(selectedElement));
    					dialog.setVisible(true);  
    					if (dialog.getTratamientosPotabilizacionPanel().getOkPressed()){
    		...
    		if (tipoOperacion.equals(ConstantesLocalGISEIEL.OPERACION_ANNADIR)){
    			...
    			if (selectedItem!= null && selectedItem instanceof EmisariosEIEL){
    				...
    				if (bloqueado!=null && !bloqueado.equalsIgnoreCase(com.geopista.security.SecurityManager.getPrincipal()!=null?com.geopista.security.SecurityManager.getPrincipal().getName():AppContext.getApplicationContext().getUserPreference(AppContext.USER_LOGIN,"",false))){
    					...
    					bloquearElemento(selectedItem, getJPanelTree().getPatronSelected(),true);
    					EmisoresDialog dialog = new EmisoresDialog(selectedElement,isEditable(selectedElement));
    					dialog.setVisible(true);  
    		...
    		if (tipoOperacion.equals(ConstantesLocalGISEIEL.OPERACION_ANNADIR)){
    			...
    			if (selectedItem!= null && selectedItem instanceof ColectorEIEL){
    				...
    				if (bloqueado!=null && !bloqueado.equalsIgnoreCase(com.geopista.security.SecurityManager.getPrincipal()!=null?com.geopista.security.SecurityManager.getPrincipal().getName():AppContext.getApplicationContext().getUserPreference(AppContext.USER_LOGIN,"",false))){
    					...
    					bloquearElemento(selectedElement, getJPanelTree().getPatronSelected(),true);
    					ColectoresDialog dialog = new ColectoresDialog(selectedElement,isEditable(selectedElement));
    					dialog.setVisible(true);  
    		...
    		if (tipoOperacion.equals(ConstantesLocalGISEIEL.OPERACION_ANNADIR)){
    			...
    			if (selectedItem!= null && selectedItem instanceof TramosConduccionEIEL){
    				...
    				if (bloqueado!=null && !bloqueado.equalsIgnoreCase(com.geopista.security.SecurityManager.getPrincipal()!=null?com.geopista.security.SecurityManager.getPrincipal().getName():AppContext.getApplicationContext().getUserPreference(AppContext.USER_LOGIN,"",false))){
    					...
    					bloquearElemento(selectedElement, getJPanelTree().getPatronSelected(),true);
    					TramosConduccionDialog dialog = new TramosConduccionDialog(selectedElement,isEditable(selectedElement));
    					dialog.setVisible(true);  
...
//    				if (geopistaLayer!=null)
//    					idLayer= geopistaLayer.getSystemId();
    				ConstantesLocalGISEIEL.clienteLocalGISEIEL.insertarElemento(agrup, idLayer, selectedPattern);
    				cancelado=false;
    			}
    		}
    		else if (tipoOperacion.equals(ConstantesLocalGISEIEL.OPERACION_MODIFICAR)){
