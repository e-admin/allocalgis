...
//    private JPanel panelPrincipal = null;
    ...
    public EIELListaTablaVersionDialog() throws HeadlessException {
        ...
        getJPanelFiltro().getJPanelBotones().add(jLabelUsuario, new GridBagConstraints(0,3,1,1,1, 1,GridBagConstraints.WEST,GridBagConstraints.NONE, new Insets(2,5,0,3),0,0));
        getJPanelFiltro().getJPanelBotones().add(getJComboUsuario(), new GridBagConstraints(1,3,1,1,1, 1,GridBagConstraints.WEST,GridBagConstraints.HORIZONTAL, new Insets(2,5,0,3),0,0));
        
        getJPanelFiltro().getJPanelBotones().add(jLabelTabla, new GridBagConstraints(0,4,3,1,1, 1,GridBagConstraints.WEST,GridBagConstraints.NONE, new Insets(2,5,0,3),0,0));
        getJPanelFiltro().getJPanelBotones().add(getJComboTabla(), new GridBagConstraints(1,4,1,1,1, 1,GridBagConstraints.WEST,GridBagConstraints.HORIZONTAL, new Insets(2,5,0,3),0,0));
    ...
    public JComboBox getJComboUsuario(){
    	if (jComboUsuario  == null){
    		...
    		ArrayList<Usuario> usuarios = obtenerListaUsuarios(AppContext.getIdEntidad());
...
//		    		new ObjetoCombo(I18N.get("LocalGISEIEL","localgiseiel.dialog.titulo.np"),ConstantesLocalGISEIEL.NUCLEOS_POBLACION),
    ...
    public void filtroJPanel_actionPerformed(){
        ...
        try{
        	...
        	Date fecha = this.getJPanelFiltro().getJTextFieldFecha().getDate();
        	String hora = this.getJPanelFiltro().getJTextFieldHora().getText();
        	if (fecha !=null){
        		SimpleDateFormat formatter = new SimpleDateFormat("yyyy-MM-dd");
//        		SimpleDateFormat formatter = new SimpleDateFormat("yyyy-MM-dd hh:mm:ss ");
        		fechaVersion =formatter.format(fecha) + " "+hora;
        	}
	...
	private void abrirDialogo(String selectedPattern, String tipoOperacion)
		...
		//- EMISARIOS
			...
			if (tipoOperacion
					.equals(ConstantesLocalGISEIEL.OPERACION_VER_VERSION)) {
				if (selectedElement != null
						&& selectedElement instanceof EmisariosEIEL) {
					EmisoresDialog dialog = new EmisoresDialog(
							(EmisariosEIEL) selectedElement, false);
					dialog.setVisible(true);
				}
			} else if (tipoOperacion
					.equals(ConstantesLocalGISEIEL.OPERACION_FIJAR_VERSION)) {
...
//          String filter = " order by versionesalfa.fecha desc ";
        	...
        	if (fechaVersion!=null && !fechaVersion.equalsIgnoreCase("")){
        		filter += " and versionesalfa.fecha < '" +fechaVersion+"'";
        		anterior = true;
        	}
        	...
        	if (accionVersion!=null && !accionVersion.equalsIgnoreCase("")){
        		if (anterior)
        			filter += " and ";
        		filter += " versionesalfa.tipocambio like '" +accionVersion+"'";
        		anterior = true;
        	}
        	...
        	if (usuarioVersion!=null && !usuarioVersion.equalsIgnoreCase("")){
        		...
        		filter += " versionesalfa.id_autor = " +usuarioVersion;
        	}
        	
            lstElements = ConstantesLocalGISEIEL.clienteLocalGISEIEL.getLstElementosVersionados(filter, tipo,this.elemento,false);
    ...
    private ArrayList<Usuario> obtenerListaUsuarios(int idEntidad){

    	ArrayList<Usuario> lstElements = new ArrayList();
        try {
             if(idEntidad>0) 
            	 lstElements = ConstantesLocalGISEIEL.clienteLocalGISEIEL.getLstUsuariosEntidad(String.valueOf(idEntidad));
