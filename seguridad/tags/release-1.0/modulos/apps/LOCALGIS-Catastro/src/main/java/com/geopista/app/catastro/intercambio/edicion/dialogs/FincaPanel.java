...
import com.geopista.app.catastro.intercambio.edicion.CultivoExtendedInfoDialog;
...
//                	jComboBoxComunidadBienes.addItem(new ComunidadBienes(""));
    ...
    public JPanel getJPanelDatosFinca()
    {
        ...
        {       
            ...
            jPanelDatosFinca.add(jLabelAnioPonencia, 
                    new GridBagConstraints(1,2,1,1,0.1, 0,GridBagConstraints.CENTER, GridBagConstraints.CENTER, new Insets(0,5,0,5),0,0));
            ...
            jPanelDatosFinca.add(getJComboBoxAnioPonencia(), gridBagConstraints4);
            ...
            jButtonNuevaFinca.addActionListener(new java.awt.event.ActionListener()
            {
                public void actionPerformed(java.awt.event.ActionEvent e)
                {
                    ...
                    try
                    {
                        if(fc!=null)
                        {
                            Object parcela = ConstantesRegistroExp.clienteCatastro.existeParcelaEnBD(fc.getRefFinca().getRefCatastral());
    ...
    private JButton getJButtonBuscarVia()
    {
        if (jButtonBuscarVia == null)
        {
            ...
            jButtonBuscarVia.setIcon(IconLoader.icon(GestionExpedientePanel.ICONO_BUSCAR));
    ...
    private JButton getJButtonBuscarParaje()
    {
        if (jButtonBuscarParaje == null)
        {
            ...
            jButtonBuscarParaje.setIcon(IconLoader.icon(GestionExpedientePanel.ICONO_BUSCAR));
                    ...
                    else
                    {
                        ...
                        if (dialog.getConstruccionCatastro(construccion)!=null){
                        	if (construccion.getRefParcela()!=null && construccion.getRefParcela().getRefCatastral()!=null && construccion.getNumOrdenBienInmueble()!=null){
                        		String refFinca = GeopistaFunctionUtils.completarConCeros(construccion.getRefParcela().getRefCatastral(),14);
                        		String numBien = GeopistaFunctionUtils.completarConCeros(construccion.getNumOrdenBienInmueble(),4);
    			...
    			public void actionPerformed(java.awt.event.ActionEvent e)
    			{
    				...
    				if (cons!=null)
    				{
    					...
    					if (cons.getRefParcela()!=null && cons.getRefParcela().getRefCatastral()!=null && cons.getNumOrdenBienInmueble()!=null){
    						String refFinca = GeopistaFunctionUtils.completarConCeros(cons.getFinca().getRefFinca().getRefCatastral(),14);
    						String numBien = GeopistaFunctionUtils.completarConCeros(cons.getNumOrdenBienInmueble(),4);
    					}
    					loadData(cons);
                    }
    			}
...
//          (null, "Comunidad de Bienes", TitledBorder.LEADING, TitledBorder.TOP, new Font(null, Font.BOLD, 12)));
                ...
                public void actionPerformed(java.awt.event.ActionEvent e)
                {
                    ...
                    if (bien!=null)
                    {
                        if(fincaEditada==null)
                        {
                            try
                            {
                                Object bi = ConstantesRegistroExp.clienteCatastro.existeBIEnBD(bien.getBienInmueble().
    ...
    private JPanel getJPanelBotoneraCultivo()
    {
...
//            	 bienInmuebleJuridico.getBienInmueble().getDatosEconomicosBien().setCoefParticipacion(new Float(0));
    ...
    private boolean compruebaNumeroFincaReales(boolean isFinca)
    {
        if(isFinca)
        {
            try
            {
                Object parcela = ConstantesRegistroExp.clienteCatastro.existeParcelaEnBD(fincaEditada.getRefFinca().
                        getRefCatastral());
                if (parcela == null)
                {
                    return true;
                ...
                else
                {
                    ...
                    for(int i= 0; i<lstParcelas.size();i++)
                    {
                        ...
                        parcela = ConstantesRegistroExp.clienteCatastro.existeParcelaEnBD(finca.getRefFinca().
                        getRefCatastral());
                        if(parcela!=null)
                        {
                            numParcelarReales++;    
        ...
        else
        {
            try
            {
                Object bi = ConstantesRegistroExp.clienteCatastro.existeBIEnBD(bienInmuebleJuridico.
                ...
                else
                {
                    ...
                    for(int i= 0; i<lstParcelas.size();i++)
                    {
                        ...
                        bi = ConstantesRegistroExp.clienteCatastro.existeBIEnBD(bij.
                        getBienInmueble().getIdBienInmueble().getIdBienInmueble());
                        if(bi!=null)
                        {
                            numParcelarReales++;
    ...
    private void annadeBI(BienInmuebleJuridico bij) {
        if(fincaEditada!=null){
            ArrayList lstBI = fincaEditada.getLstBienesInmuebles();
    ...
    private void annadeConstruccion(ConstruccionCatastro cons, boolean comun)
    {
    ...
    private BienInmuebleCatastro obtenerBien(String idBienInmueble)
    {
        ...
        if(lstBienes!=null)
        {
            for (Iterator iterLstBienes = lstBienes.iterator();iterLstBienes.hasNext();){
            	...
            	String refFinca = GeopistaFunctionUtils.completarConCeros(bien.getIdBienInmueble().getParcelaCatastral().getRefCatastral(),14);            	
            	String numBien = GeopistaFunctionUtils.completarConCeros(bien.getIdBienInmueble().getNumCargo(),4);
...
//            	String numBien = GeopistaFunctionUtils.completarConCeros(construccion.getIdBienInmueble().getNumCargo(),4);
//            	String id = refFinca + numBien;
    ...
    private void annadeCultivo(Cultivo cult, boolean comun)
    {
					 ...
					 if(gestionExpedientePanel.getExpediente().getTipoExpediente().getConvenio().equals(DatosConfiguracion.CONVENIO_FISICO_ECONOMICO)){
						 ...
						 for(int i=0; i<gestionExpedientePanel.getExpediente().getListaReferencias().size(); i++){
							 ...
							 if(finca.getRefFinca().getRefCatastral().equals(fincaEditada.getRefFinca().getRefCatastral())){
								 ...
								 try {
									 int idFinca = ConstantesRegistroExp.clienteCatastro.getIdFincaCatastro(fincaEditada.getRefFinca().getRefCatastral());
									 coll.add(idFinca);
								 }
								 catch (Exception e1)
				                    {
						 ...
						 DialogoVerInfCatastralFisicoEconomico dialog = new  DialogoVerInfCatastralFisicoEconomico(app.getMainFrame(),
							 true, coll, ConstantesRegistroExp.tipoConvenio, "");
						 ...
						 for(int i=0; i<gestionExpedientePanel.getExpediente().getListaReferencias().size(); i++){
							 ...
							 if(bien.getBienInmueble().getIdBienInmueble().getIdBienInmueble().equals(bienInmuebleJuridico.getBienInmueble().getIdBienInmueble().getIdBienInmueble())){

								 try {
									 int idFinca = ConstantesRegistroExp.clienteCatastro.getIdFincaCatastro(bienInmuebleJuridico.getBienInmueble().getIdBienInmueble().getParcelaCatastral().getRefCatastral());
									 coll.add(idFinca);
								 }
								 catch (Exception e1)
				                    {
