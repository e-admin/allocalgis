...
public class ReferenciarViasPanel extends javax.swing.JPanel implements WizardPanel {
	...
	public static HashMap<GeopistaDatosImportarPadron, ArrayList<PoliciaCoincidencias>> listaCoincidencias = new HashMap<GeopistaDatosImportarPadron, ArrayList<PoliciaCoincidencias>>();
			...
			while (it.hasNext()){
				...
				if (idDomicilio > 0){
					...
					if (numsPolicia!= null && !numsPolicia.isEmpty()){
						int numPolicia = -1;
						try{
							numPolicia = Integer.parseInt(((PortalGeorreferenciado)numsPolicia.get(0)).getPortal());
