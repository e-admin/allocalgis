	...
	public static void validacion(Connection connection, StringBuffer str,String fase){
        ...
        {  
			...
			String sql = "select query_validate_mpt.tipo,query_validate_mpt.nombre,query_validate_mpt.query AS vista,dictionary.traduccion AS descripcion from query_validate_mpt" +
							" left join dictionary on query_validate_mpt.id_descripcion=dictionary.id_vocablo AND locale='"+AppContext.getApplicationContext().getUserPreference("locale", "es_ES", false)+"'";
				...
				if (vista.getVista().equals("check_mpt_alumbrado")){
					...
					while (rs.next()) {	
						...
						if(rs.getString("PRODU_BASU")!=null&&!rs.getString("PRODU_BASU").equals(""))
							v_recogida_basura_bean.setProdu_basu(LocalGISEIELServerUtils.redondear(Double.parseDouble(rs.getString("PRODU_BASU")),1));
						else
					...
					while (rs.next()) {	
						...
						if(rs.getString("LONGITUD")!=null && !rs.getString("LONGITUD").equals(""))	
							tramo_carretera_bean.setLongitud(LocalGISEIELServerUtils.redondear(Double.parseDouble(rs.getString("LONGITUD")) ,1));	
						else
					...
					while (rs.next()) {	
						...
						if(rs.getString("BO")!=null&&!rs.getString("BO").equals(""))
							plan_urbanistico_bean.setBo(LocalGISEIELServerUtils.formatFecha(rs.getDate("BO")));
						else
			...
			for (Enumeration<String> iterator = datosMPTErroneos.keys(); iterator.hasMoreElements();) {
				...
				for (int i = 0; i < datosErroneosByType.size(); i++) {
					...
					String rowToInsertInFile=LocalGISEIELServerUtils.getRowToMPT(fase, "|", valores);//la fase se recogerá como parametro.
