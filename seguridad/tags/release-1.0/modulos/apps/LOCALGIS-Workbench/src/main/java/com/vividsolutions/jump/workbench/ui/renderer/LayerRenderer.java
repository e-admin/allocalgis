...
public class LayerRenderer extends FeatureCollectionRenderer {
    ...
    public static boolean render(Layerable layerable, ILayerViewPanel panel) {
        if (!layerable.isVisible()) {
            return false;
        }     
        if (layerable.getLayerManager() == null)
        	return false;
     ...
     */
    private static double scale(ILayerViewPanel panel) {
    ...
    protected Map layerToFeaturesMap() {
        ...
        if (layer instanceof DynamicLayer && layer.getFeatureCollectionWrapper() == null){
	            ...
	            driverProperties.put("filtrogeometrico",EnvelopeUtil.toGeometry(viewportEnvelope));
	    		...
	    		FeatureCollection featureCollection = geopistaConnection.executeQuery(layer.getDataSourceQuery().getQuery(),exceptions,null);
		        return Collections.singletonMap(layer, featureCollection.getFeatures());
