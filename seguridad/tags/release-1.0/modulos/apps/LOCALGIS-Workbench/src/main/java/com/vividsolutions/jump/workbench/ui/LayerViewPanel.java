...
public class LayerViewPanel extends JPanel implements LayerListener,
    ...
    /* (non-Javadoc)
	 * @see com.vividsolutions.jump.workbench.ui.ILayerViewPanel#dispose()
	 */
    @Override
	public void dispose() {
	...
	public double getNormalizedScale(double scale) {
		double ppm = (Double.parseDouble(AppContext.getApplicationContext().
				getUserPreference("ancho.monitor", "30.5", true))/100)
	...
	private void seleccionAutomaticaMunicipio(Point2D p){
		...
		try{
			if (AppContext.getApplicationContext().getBlackboard().get(AppContext.SEL_MUNI_AUTO)!=null && ((Boolean)AppContext.getApplicationContext().getBlackboard().get(AppContext.SEL_MUNI_AUTO)).booleanValue()){
	            ...
	            if (inCoord.getEPSGCode() != outCoord.getEPSGCode()){
	            	Reprojector.instance().reproject(point,inCoord, outCoord);
		        ...
		        for (int i=0;i<n;i++){
		        	...
		        	if (geom!=null){
			        	...
			        	if (geom.contains(point)){
			        		JComboBox jComboBox = (JComboBox)AppContext.getApplicationContext().getBlackboard().get(AppContext.MUNI_COMBO);
