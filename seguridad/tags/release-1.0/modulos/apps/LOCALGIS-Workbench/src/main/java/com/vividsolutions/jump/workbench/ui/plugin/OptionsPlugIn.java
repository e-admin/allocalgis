...
public class OptionsPlugIn extends AbstractPlugIn {
    ...
    public void initialize(PlugInContext context) throws Exception {
        dialog(context)
                .addTab(appContext.getI18nString("viewEditOptionsPlugIn"), 
                		new EditOptionsPanel(
                			context.getWorkbenchContext().getIWorkbench().getBlackboard(), 
							(JDesktopPane) context.getWorkbenchContext().getIWorkbench().getGuiComponent().getDesktopPane()));
        dialog(context)
                .addTab(appContext.getI18nString("snapVerticesOptionsPlugIn"), GUIUtil.resize(
                        IconLoader.icon("QuickSnap.gif"), 16),
                        new SnapVerticesToolsOptionsPanel(context
                                .getWorkbenchContext().getIWorkbench()
                                .getBlackboard()))        ;
