...
class CTransaction extends WFSMainLoop {
     ...
     * @param request 
     */
    public CTransaction( PostgisDataStore parent, OGCWebServiceRequest request ) {
    ...
    /**
     * performs the passed transaction request encapsulated in a 
     * <tt>OGCWebServiceRequest</tt>.
     *
    ...
    private String[] getAffectedFeatureTypes( WFSOperation[] operations ) {
        ...
        for ( int i = 0; i < operations.length; i++ ) {
            if ( operations[i] instanceof WFSInsert ) {
                ...
                list.add( ((WFSUpdate)operations[i]).getTypeName() );
            } else if ( operations[i] instanceof WFSDelete ) {
                list.add( ((WFSDelete)operations[i]).getTypeName() );
            } else {
                // native request
            }
        }
    ...
    */
     ...
     */
    public ArrayList performInsert( InsertTree tree, PostgisDBAccess osa )
                            throws Exception {
        ...
        for ( int ii = 0; ii < idValues.size(); ii++ ) {
                    ...
                    switch ( type ) {
                        ...
                        case org.deegree.services.wfs.configuration.FeatureType.GEOMETRY:
                        {
                            if ( ( (String)vals.get( ii ) ).length() == 0 ) {
                                ...
                                valueList.append( getGeometryFromText( (String)vals.get( ii ) ) );
                            }
            ...
            StringBuffer stmt = new  StringBuffer( valueList.length() + 1000 );
            stmt.append( "INSERT INTO " ).append( table ).append( " " )
                .append( fieldList ).append( " " ).append( valueList );
     ...
     */
     private boolean performUpdate (WFSUpdate update) throws Exception {
        ...
        try {
            ...
            if (!fIDs.isEmpty ()) {
                ...
                PostgisRowSelector selector = new PostgisRowSelector (
     ...
     boolean performUpdate (PostgisRowSelector selector, HashMap changes)
        throws Exception {  

         PostgisRowSelector [] referencedRows = 
            (PostgisRowSelector[])selector.getReferencedRows ();
    ...
    private void performDelete (FeatureType ft, HashMap tables) throws Exception {
        ...
        while (!tables.isEmpty ()) {
            ...
            while (it.hasNext ()) {
                String name = (String) it.next ();
                PostgisRowSelector selector = (PostgisRowSelector) tables.get (name);
    ...
    private Set getFeatureIDs (FeatureType ft, Filter filter, DBAccess osa) throws Exception {
        ...
        AbstractSQLBuilder sqlBuilder = new PostgisSQLBuilder (ft);
    ...
    */
    ...
    /**
     * generates a sql fragment from the passed GML geometry that can be used to
     * create insert and update statementens for the postgis database
     */
    private static String getGeometryFromText( String gml ) throws Exception {
        Debug.debugMethodBegin( "CTransaction", "getGeometryFromText" );
     
        GM_Object geo = GMLAdapter.wrap( gml );
        StringBuffer s = WKTAdapter.export( geo );
        int crs = extractCRS( geo );
        StringBuffer sb = new StringBuffer( s.length() + 50 );
    ...
    /**
     * extracts a valid postgis crs code from the passed geometry. if no valid
     * code can be extracted '-1', indicating 'no CRS' will be returned.
     */
    private static int extractCRS( GM_Object geom ) {
        Debug.debugMethodBegin( "CTransaction", "extractCRS" );
        
        int icrs = -1;
        String crs = null;        
        try {
            crs = geom.getCoordinateSystem().getName();
        } catch (Exception e) {}
        if ( crs != null ) {
            int pos = crs.lastIndexOf( ':' );
            if  ( pos > -1 ) {
                crs = crs.substring( pos+1, crs.length() );
            }
            try {
