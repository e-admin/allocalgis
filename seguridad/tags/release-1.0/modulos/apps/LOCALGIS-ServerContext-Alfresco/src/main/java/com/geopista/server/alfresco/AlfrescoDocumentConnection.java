			...
			if (queryResult != null
				...
				for (ResultSetRow row : rows) {
					...
					if (AlfrescoManagerUtils.isDirectory(resultSetRowNode)) {	
						try {
							Node node = AlfrescoManagerImpl.getInstance().getNode(new AlfrescoKey(resultSetRowNode.getId(), AlfrescoKey.KEY_UUID));
							AlfrescoNode nodeDirectory = new AlfrescoNode(AlfrescoManagerUtils .getPropertyFromNode(node, Constants.PROP_NAME), resultSetRowNode.getId());
...
//						tableModel.addColumn("");
//					}
					Node node = AlfrescoManagerImpl.getInstance() .getNode(new AlfrescoKey(resultSetNode.getId(), AlfrescoKey.KEY_UUID));
					AlfrescoNode nodeDirectory = new AlfrescoNode(AlfrescoManagerUtils.getPropertyFromNode(node, Constants.PROP_NAME), node.getReference().getUuid());
					tableModel.addRow(new Object[] { nodeDirectory , getAnnexationStatus(nodeDirectory.getUuid(), elements)});
					//tableModel.addRow(new Object[] { nodeDirectory , ""});
				}
			}
     ...
     */
    private Color getAnnexationStatus(String uuid, Object [] elements){
