...
public class RendererDocumentos extends DefaultListCellRenderer
{
           ...
           Object value, int i, boolean s, boolean f)
    {
            ...
            if(extension.equalsIgnoreCase(WORD))
                label.setIcon(DocumentBean.iconWord);
            else if(extension.equalsIgnoreCase(PPT))
                label.setIcon(DocumentBean.iconPPT);
            else if(extension.equalsIgnoreCase(PDF))
                label.setIcon(DocumentBean.iconPDF);
            else if(extension.equalsIgnoreCase(TXT))
                label.setIcon(DocumentBean.iconTxt);
            else if(extension.equalsIgnoreCase(XML))
                label.setIcon(DocumentBean.iconXML);
            else if(extension.equalsIgnoreCase(HTML))
                label.setIcon(DocumentBean.iconHTML);
            else
                label.setIcon(DocumentBean.iconDefault);
