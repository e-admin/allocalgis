...
public class UtilRegistroExp
{
     ...
     * */
    //LCGIII.Desplazado al paquete LOCALGIS-UI
    ...
    public static Date getDate(String fecha)
    {
		try {
			if (esDate(fecha))
				return ConstantesRegistroExp.df.parse(fecha);
			else
				return null;
		} catch (Exception e)
        {
    ...
    public static boolean esDate(String fecha)
    {
		try
        {
			Date date = ConstantesRegistroExp.df.parse(fecha);
		}
                                          ...
                                          boolean editable, String visible, String novisible,TaskMonitor monitor)
    {
          try
          {
            ...
            geopistaEditor.addCursorTool("Dynamic fence tool", "com.vividsolutions.jump.workbench.ui.cursortool.UpdateDynamicFenceTool");
             ...
             if (ShowMaps.showMap(CConstantesComando.adminCartografiaUrl,geopistaEditor, id_map, editable,padre,monitor)!=null)
             {
                try
                {
...
//            }
    ...
    {
        ConstantesRegistroExp.botonesGestExp[0] = false;
        ...
        ConstantesRegistroExp.botonesGestExp[5] = false;
    }
    ...
    {
        ArrayList aux = ConstantesRegistroExp.tiposExpedientes;
