...
public class ASCPanel extends JPanel implements FeatureExtendedPanel {
	...
	private JScrollPane getJScrollPaneLinderos() {
		if (jScrollPaneLinderos == null) {
			...
			// jScrollPaneLinderos.setPreferredSize(new Dimension(400, 300));
	...
	public void enter() {
		...
		if (this.gestionExpedientePanel != null) {
			...
			if (this.gestionExpedientePanel.getExpediente().getIdEstado() >= ConstantesRegistroExp.ESTADO_FINALIZADO) {
				...
				new Thread(new Runnable() {
					...
					public void run() {
						try {
							...
							String textFxcc = oper.obtenerDxf(id_Expediente,
									ref_Catastral);
...
//								PlantaSignificativaInfo pInfo = (PlantaSignificativaInfo) this.infoplantas.get(getPlantaCategory(actualLayerAU));
...
//			ascWrite.write(asc, geopistaEditor.getLayerManager());
	...
	//	public void loadTablePlantas(ArrayList construcciones) {
	//		// TODO Auto-generated method stub
	//		Iterator<PlantaCatastro> itPlantas = obtenerPlantasCatastro(construcciones).iterator();
	//
	//		while (itPlantas.hasNext()) {
	//				((TablePlantasModel)((TableSorted)jTableListaPlantas.getModel()).getTableModel()).anniadirPlanta(itPlantas.next());
	//		}
	//
	//		// Seleccionar el indez para mostrar usos.
	//		jTableListaPlantas.getSelectionModel().setSelectionInterval(0, 0);
	//
	//	}
