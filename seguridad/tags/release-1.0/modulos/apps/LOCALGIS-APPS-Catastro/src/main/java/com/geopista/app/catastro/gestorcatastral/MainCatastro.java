 ...
 *
 */
public class MainCatastro extends CMain implements IMainCatastro {
...
//            showAuth();
			...
			SSOAuthManager.ssoAuthManager(ConstantesRegistroExp.idApp);								
            ...
            Municipio municipio = (new OperacionesAdministrador(com.geopista.protocol.CConstantesComando.loginLicenciasUrl)).getMunicipio(new Integer(ConstantesRegistroExp.IdMunicipio).toString());
            if (municipio!=null)
            {
            	ConstantesRegistroExp.Municipio = municipio.getNombre();
            	ConstantesRegistroExp.Provincia= municipio.getProvincia();

            	com.geopista.security.GeopistaPrincipal principal= com.geopista.security.SecurityManager.getPrincipal();
            	if(principal!=null)
            		setTitle(getTitle()+ " - " + ConstantesRegistroExp.Municipio + " (" + ConstantesRegistroExp.Provincia+") - " + I18N.get("RegistroExpedientes","modulocatastral.cauthdialog.jlabelnombre") + principal);
            	com.geopista.security.SecurityManager.setIdMunicipio(new Integer(ConstantesRegistroExp.IdMunicipio).toString());
            }
                    ...
                    new Thread(new Runnable()
                    {
                        public void run()
                        {
                            try
                            {
                                ...
                                String idUser = ConstantesRegistroExp.clienteCatastro.inicializaBlackBoard();
                                aplicacion.getBlackboard().put("idUserCatastro", idUser);
                                DatosConfiguracion datos = ConstantesRegistroExp.clienteCatastro.getParametrosConfiguracion();
                                if(datos!=null)
                                {
                                    ConstantesRegistroExp.modoTrabajo = datos.getModoTrabajo();
                                    ConstantesRegistroExp.tipoConvenio = datos.getTipoConvenio();
                                    ConstantesRegistroExp.formaConvenio = datos.getFormaConvenio();
                                }
                                if(!ConstantesRegistroExp.tipoConvenio.equalsIgnoreCase(DatosConfiguracion.CONVENIO_NINGUNO))
                                {
                                    ...
                                    ConstantesRegistroExp.tiposExpedientes = ConstantesRegistroExp.clienteCatastro.
                                            getTiposExpedientes(ConstantesRegistroExp.tipoConvenio);
                                    ConstantesRegistroExp.maquinaEstadosFlujo = new MaquinaEstadoFlujo();
                                    expUsuario = (ArrayList)ConstantesRegistroExp.clienteCatastro.
                                    							getExpedientesUsuario("false", ConstantesRegistroExp.tipoConvenio, ConstantesRegistroExp.modoTrabajo);
    ...
    private JMenuBar getJMenuBarCatastro()
    {
        if(jMenuBarCatastro==null)
        {
            ...
            //jMenuBarCatastro.add(getJMenuIdiomas());
        	...
        	jMenuItemImportarSubparcelas.addActionListener(new ActionListener()
                ...
                public void actionPerformed(ActionEvent e)
                {
                    openWizard (new WizardPanel[] {
                            ...
                            new GeopistaMostrarSubParcelas()}, true);
                }
                    });
        }
        	...
        	jMenuItemImportarDatosCatastro.addActionListener(new ActionListener()
                ...
                public void actionPerformed(ActionEvent e)
                {
                    openWizard (new WizardPanel[] {
                    		...
                    		new GeopistaMostrarParcelas()}, true);
    ...
    private void gestionExpMenuItemActionPerformed(Expediente exp)
    {
        ...
        gest.addInternalFrameListener(new javax.swing.event.InternalFrameListener()
        {
            ...
            public void internalFrameClosed(javax.swing.event.InternalFrameEvent evt)
            {
                ...
                siguienteFrame(null,ConstantesRegistroExp.FRAME_ORIGEN_GESTION_EXP_ACCION_CERRAR,gest);
            }
        ...
        linkVerEstadoBotonGestionExpPanel.addActionListener(new ActionListener()
        {
            public void actionPerformed(java.awt.event.ActionEvent evt)
            {
                ...
                siguienteFrame(exp,ConstantesRegistroExp.FRAME_ORIGEN_GESTION_EXP_VER_ESTADO,gest);
            }
        });
        ...
        linkAsociarParcelasBotonGestionExpPanel.addActionListener(new ActionListener()
        {
            ...
            {
                Expediente exp = gest.getExpediente();
                siguienteFrame(exp,ConstantesRegistroExp.FRAME_ORIGEN_GESTION_EXP_ASOCIAR_PARCELA_BIEN_INMUEBLE,gest);
            }
        });
        ...
        linkModificarBotonGestionExpPanel.addActionListener(new ActionListener()
        {
            ...
            {
            	 Expediente exp = gest.getExpediente();
            	siguienteFrame(exp,ConstantesRegistroExp.FRAME_ORIGEN_GESTION_EXP_ACCION_MODIFICAR_INF_CATASTRAL,gest);
            }
        });
        ...
        linkAsociarDatosGraficosBotonGestionExpPanel.addActionListener(new ActionListener()
        {
            ...
            {
                Expediente exp = gest.getExpediente();
                siguienteFrame(exp,ConstantesRegistroExp.FRAME_ORIGEN_GESTION_EXP_ACCION_ASOCIAR_DATOS_GRAFICOS,gest);
            }
        });
        ...
        linkAsociarBotonGestionExpPanel.addActionListener(new ActionListener()
            ...
            public void actionPerformed(java.awt.event.ActionEvent evt)
            {
                ...
                final COperacionesDGC geoConn= new COperacionesDGC();
 
                try {
                	
                	HashMap certificadoHashMap = (HashMap) aplicacion.getBlackboard().get(ConstantesRegistroExp.CERTIFICADO_DATOS);
            		...
            		if(certificadoHashMap == null){
	                	...
	                	if(expediente.getIdEstado() == ConstantesRegistroExp.ESTADO_REGISTRADO){
	                		...
	                		String nombreSolicitante = (String)certificadoHashMap.get(ConstantesRegistroExp.CERTIFICADO_NOMBRE_SOLICITANTE);
	                		String nifSolicitante = (String)certificadoHashMap.get(ConstantesRegistroExp.CERTIFICADO_DNI_SOLICITANTE);
	                    	CatastroWS catastrows = new CatastroWS();
	                        String XML = catastrows.buildCreacionExpedienteRequest(expediente, geoConn,
	                        		nombreSolicitante, nifSolicitante);
	                        System.out.println(XML);                           
	                        TestClient test = new TestClient(certificadoHashMap);
                    		datosWsResponse = test.asociacionExpediente(XML, aplicacion, expediente);
		 	                    ...
		 	                    if (datosWsResponse.getRespuesta().getLstUnidadError() == null ||
	 	                    		...
	 	                    		expediente.setReferenciaExpedienteGerencia(datosWsResponse.getRespuesta().getExpediente().getReferenciaExpedienteGerencia());
		 	                    	 ...
		 	                    	 siguienteFrame(expediente,
		 	                    			 ConstantesRegistroExp.FRAME_ORIGEN_ASOCIADO_EXP,gest);
	                		...
	                		String XML = XML = catastrows.buildConsultaCatastroRequest(expediente, geoConn, 
	                				nombreSolicitante, nifSolicitante);
	                		System.out.println(XML);
	                		TestClient test = new TestClient(certificadoHashMap);
	                		datosWsResponse = test.consultaCatastro(XML, aplicacion, expediente);
	                		...
	                		if(datosWsResponse != null){
		    	                ...
		    	                if (datosWsResponse.getRespuesta().getLstUnidadError() == null ||
		    	                	...
		    	                	siguienteFrame(expediente,ConstantesRegistroExp.FRAME_ORIGEN_OBTENER_INF_CATASTRO,gest);
            ...
            public void actionPerformed(java.awt.event.ActionEvent evt)
            {
            	...
            	try {
            		...
            		if(certificadoHashMap == null){
	            		...
	            		CatastroWS catastrows = new CatastroWS();
	               	 	String XML = catastrows.buildActualizaCatastroRequest(expediente, geoConn,nombreSolicitante, nifSolicitante);
	               	 	System.out.println(XML);
	                     TestClient test = new TestClient(certificadoHashMap);
	                     datosWsResponse = test.actualizacionCatastro(XML, aplicacion, expediente);
	                     ...
	                     if(datosWsResponse != null){
		                      ...
		                      if (datosWsResponse.getRespuesta().getLstUnidadError() == null ||
		                    	 ...
		                    	 Expediente exp = gest.getExpediente();
		                    	 siguienteFrame(exp,ConstantesRegistroExp.FRAME_ORIGEN_ACTUALIZAR_CATASTRO,gest);
		                    	  ...
		                    	  for(int i=0; i<datosWsResponse.getRespuesta().getLstUnidadError().size(); i++){
		                    		  ...
		                    		  for(int j=0; j<unidadError.getLstErrores().size(); j++){
		                    			  ...
		                    			  if(errorWS!= null && errorWS.getCodigo() != null && errorWS.getCodigo().equals(ErrorWSBean.WS_ERROR_REFERENCIA_NO_ACTUALIZADA) ){
		                    				  isInfoCatModif = true;
		                    				  expediente.setIdEstado(ConstantesRegistroExp.ESTADO_RELLENADO);
		                    				  ...
		                    				  //siguienteFrame(expediente,ConstantesRegistroExp.FRAME_ORIGEN_ACTUALIZAR_CATASTRO,gest);
		                    			  }
		                    		  }
            ...
            public void actionPerformed(java.awt.event.ActionEvent evt)
            {
            	...
            	try {
            		...
            		if(certificadoHashMap == null){
	            		...
	            		CatastroWS catastrows = new CatastroWS();
	               	 	String XML = catastrows.buildConsultaEstadoExpedienteRequest(expediente, geoConn, nombreSolicitante, nifSolicitante);
	               	 	System.out.println(XML);
	               	 	TestClient test = new TestClient(certificadoHashMap);
	                     datosWsResponse = test.consultaEstadoExpediente(XML, aplicacion, expediente);
	                     ...
	                     if(datosWsResponse != null){
	                    	 ...
	                    	 //	se han producido errores
	                          ...
	                          if (datosWsResponse.getRespuesta().getLstUnidadError() == null ||
	                        	  ...
	                        	  if (datosWsResponse.getRespuesta().getUdsa().getExpediente().getTipoDeIntercambio().equals(ConstantesRegistroExp.EXP_FINALIZADO)){
	...
	//		                    	 wsOper.insertarDatosCatastrales(datosWsResponse.getRespuesta().getUdsa().getUnidadDatosIntercambio());
			                    	 ...
			                    	 actualizaCatastroTemporal(exp, ConstantesRegistroExp.tipoConvenio);
			                    	 
			                    	 //se actualiza el estado del expediente
			                         siguienteFrame(exp,ConstantesRegistroExp.FRAME_ORIGEN_GESTION_EXP_CONSULTA_ESTADO,gest);
	                        	  }
		                     }	
	                     }
            		}
        ...
        linkConstularBotonGestionExpPanel.addActionListener(new ActionListener()
            ...
            {
                Expediente exp = gest.getExpediente();
                siguienteFrame(exp,ConstantesRegistroExp.FRAME_ORIGEN_GESTION_EXP_ACCION_MODIFICAR_DATOS_EXP,gest);
            }
                });
        ...
        linkExportarFicheroBotonGestionExpPanel.addActionListener(new ActionListener()
        {
            ...
            {
                Expediente exp = gest.getExpediente();
                siguienteFrame(exp,ConstantesRegistroExp.FRAME_ORIGEN_GESTION_EXP_ACCION_EXPORTAR_FICHERO,gest);
            }
        });
        ...
        linkBotonSiguienteCrearExpPanel.addActionListener(new ActionListener()
        {
            public void actionPerformed(java.awt.event.ActionEvent evt)
            {
                ...
                {	
                    ...
                    if(exp!=null)
                    {
                        siguienteFrame(exp,ConstantesRegistroExp.FRAME_ORIGEN_CREAR_EXP_ACCION_SIGUIENTE,crearExp);
                    }
                }
            }
        });
        ...
        linkBotonGuardarCrearExpPanel.addActionListener(new ActionListener()
        {
            public void actionPerformed(java.awt.event.ActionEvent evt)
            {
                if(crearExp.recopilarDatos())
                {
                    ...
                    if(exp!=null)
                    {
                        siguienteFrame(exp,ConstantesRegistroExp.FRAME_ORIGEN_CREAR_EXP_ACCION_GUARDAR,crearExp);
                    }
                }
            }
        });
    ...
    private void muestraBuscarExpedientesUsuario(ArrayList expedientes)
    {
        ...
        linkBotonBuscarBuscarPanel.addActionListener(new ActionListener()
        {
            public void actionPerformed(java.awt.event.ActionEvent evt)
            {
                siguienteFrame(null, ConstantesRegistroExp.FRAME_ORIGEN_BUSQUEDA_ACCION_NUEVO, busqueda);
            }
        });
             	...
             	try {
             		...
             		if(certificadoHashMap == null){
                		...
                		String nifSolicitante = (String)certificadoHashMap.get(ConstantesRegistroExp.CERTIFICADO_DNI_SOLICITANTE);
	             		...
	             		CatastroWS catastrows = new CatastroWS();
	                	 	String XML = catastrows.buildConsultaEstadoExpedienteRequest(expediente, geoConn, 
	                	 			nombreSolicitante, nifSolicitante);
	                	 	System.out.println(XML);
	                	 	TestClient test = new TestClient(certificadoHashMap);
	                      datosWsResponse = test.consultaEstadoExpediente(XML, AppContext.getApplicationContext(), expediente);
	                     	 ...
	                     	 //	se han producido errores
	                           ...
	                           if (datosWsResponse.getRespuesta().getLstUnidadError() == null ||
	                        	  ...
	                        	  if (datosWsResponse.getRespuesta().getUdsa().getExpediente().getTipoDeIntercambio().equals(ConstantesRegistroExp.EXP_FINALIZADO)){
			                     	 ...
			                     	 //se actualiza el estado del expediente
			                         siguienteFrame(exp,ConstantesRegistroExp.FRAME_ORIGEN_BUSQUEDA_CONSULTA_ESTADO,busqueda);
	                        	  }
	                           }
	                      }	
            		}
        ...
        linkBotonAceptarBuscarPanel.addActionListener(new ActionListener()
        {
            public void actionPerformed(java.awt.event.ActionEvent evt)
            {
                ...
                if(exp!=null)
                {
                    siguienteFrame(exp, ConstantesRegistroExp.FRAME_ORIGEN_BUSQUEDA_ACCION_ACEPTAR, busqueda);
                }
                else
                {
    ...
    private void mostrarAsociarParcelas(Expediente exp)
    {
            ...
            public void actionPerformed(java.awt.event.ActionEvent evt)
            {
                if(mosImagenEst.checkeaReferEstado())
                {
                    ...
                    if(exp!=null)
                    {
                        ...
                        siguienteFrame(exp, ConstantesRegistroExp.FRAME_ORIGEN_ASOCIAR_PARCELA_ACCION_GUARDAR, mosImagenEst);
                    }
                }
                else
                {
        ...
        linkBotonSalirAsociarParcelasPanel.addActionListener(new ActionListener()
        {
            public void actionPerformed(java.awt.event.ActionEvent evt)
            {
                ...
                {
                    Expediente exp = mosImagenEst.getExpediente();
                    siguienteFrame(exp, ConstantesRegistroExp.FRAME_ORIGEN_ASOCIAR_PARCELA_ACCION_SALIR, mosImagenEst);
                }
            }
        });
    ...
    private void mostrarAsociarBienInmueble(Expediente exp)
    {
            ...
            public void actionPerformed(java.awt.event.ActionEvent evt)
            {
                if(asociarBI.checkeaReferEstado())
                {
                    ...
                    if(exp!=null)
                    {
                        ...
                        siguienteFrame(exp, ConstantesRegistroExp.FRAME_ORIGEN_ASOCIAR_BIEN_INMUEBLE_ACCION_GUARDAR, asociarBI);
                    }
                }
                else
                {
        ...
        linkBotonSalirAsociarParcelasPanel.addActionListener(new ActionListener()
        {
            public void actionPerformed(java.awt.event.ActionEvent evt)
            {
                ...
                {
                    Expediente exp = asociarBI.getExpediente();
                    siguienteFrame(exp, ConstantesRegistroExp.FRAME_ORIGEN_ASOCIAR_BIEN_INMUEBLE_ACCION_SALIR, asociarBI);
                }
            }
        });
    ...
    private void mostrarModificarExpediente(Expediente exp)
    {
        ...
        linkBotonGuardarModificarExpPanel.addActionListener(new ActionListener()
        {
            public void actionPerformed(java.awt.event.ActionEvent evt)
            {
                if(modificarExp.recopilarDatos())
                {
                    ...
                    if(exp!=null)
                    {
                        siguienteFrame(exp, ConstantesRegistroExp.FRAME_ORIGEN_MODIFICAR_EXP_ACCION_GUARDAR, modificarExp);
                    }
                }
            }
        });
        ...
        linkBotonSalirModificarExpPanel.addActionListener(new ActionListener()
        {
        	public void actionPerformed(java.awt.event.ActionEvent evt)
        	{
        		...
        		Expediente exp = modificarExp.getExpediente();
        		siguienteFrame(exp, ConstantesRegistroExp.FRAME_ORIGEN_MODIFICAR_EXP_ACCION_SALIR, modificarExp);
                    ...
                    btnSalir.addActionListener(new ActionListener()
                        ...
                        {  
                            if(exp!=null)
                            {
                                try
                                {
                                    Expediente expediente= (Expediente)ConstantesRegistroExp.clienteCatastro.getParcelasExpediente(exp, ConstantesRegistroExp.tipoConvenio);
                                    siguienteFrame(expediente, ConstantesRegistroExp.FRAME_ORIGEN_MODIFICAR_INF_CATASTRAL_ACCION_SALIR, desktopPane.getAllFrames()[0]);
                                }
                                catch(ACException e)
                                {
    ...
    private void mostrarExportacionMasiva()
    {
        if (desktopPane.getAllFramesInLayer(JDesktopPane.getLayer(new JInternalFrame())).length>0) return;
        final ExportacionMasiva expMas = new ExportacionMasiva(this);
        ...
        linkBotonGenerarExportaMasivoPanel.addActionListener(new ActionListener()
        {
        	 public void actionPerformed(java.awt.event.ActionEvent evt)
             {
                 ...
                 {
                     exportacionMasivaDesacoplado(expMas, directorio, nombreFinEntrada, nombreVARPAD);
    ...
    private void mostrarAsociarUsuariosExpediente()
    {
        ...
        try
        {
            ArrayList expedientes = (ArrayList)ConstantesRegistroExp.clienteCatastro.
            										getExpedientesUsuario("false", ConstantesRegistroExp.tipoConvenio,ConstantesRegistroExp.modoTrabajo);
    ...
    private void mostrarAsociarDatosGraficos(Expediente exp)
    {
        try
        {
            ...
            openModalComponent(I18N.get("ModuloCatastral","dialog.titulo.infografica"),
                    sc);
        }
        catch (Exception e){
            e.printStackTrace();
    ...
    {
        ConstantesRegistroExp.Locale= "es_ES";
        setLang(ConstantesRegistroExp.Locale);
    ...
    {
        ConstantesRegistroExp.Locale= "ca_ES";
        setLang(ConstantesRegistroExp.Locale);
    ...
    {
        ConstantesRegistroExp.Locale= "va_ES";
        setLang(ConstantesRegistroExp.Locale);
    ...
    {
        ConstantesRegistroExp.Locale= "gl_ES";
        setLang(ConstantesRegistroExp.Locale);
	...
	public boolean mostrarAyudaActionPerformed()
    {
		...
		try
        {
            String helpSetFile = "help/catastro/CatastroHelp_" + ConstantesRegistroExp.Locale.substring(0,2) + ".hs";
			url = HelpSet.findHelpSet(loader, helpSetFile);
			if (url == null)
            {
                url=new URL("help/catastro/CatastroHelp_" + ConstantesRegistroExp.LocalCastellano.substring(0,2) + ".hs");
   			}
            hs = new HelpSet(loader, url);

			// ayuda sensible al contexto
			hs.setHomeID(ConstantesRegistroExp.helpSetHomeID);
    ...
    private void comprobarActualizacionesYEnvios()
    {
        try
        {
            ConstantesRegistroExp.clienteCatastro.comprobarActualizacionYEnvios();
        }
        catch(ACException e)
        {
     ...
     * en ConstantesRegistroExp indicando de donde proviene este expediente. Con esos datos se llama a maquinaEstadosFlujo
    ...
    private void siguienteFrame(Expediente exp, int frameOrigen, JInternalFrame iFrame)
    {
        ...
        switch(resul)
        {
            case ConstantesRegistroExp.FRAME_DESTINO_GESTION_EXP_CREA_EXP:
            {
            ...
            case ConstantesRegistroExp.FRAME_DESTINO_GESTION_EXP_UPDATE_EXP:
            {
            ...
            case ConstantesRegistroExp.FRAME_DESTINO_GESTION_EXP:
            {
                desktopPane.getDesktopManager().closeFrame(iFrame);
                gestionExpMenuItemActionPerformed(exp);
                break;
            }
            case ConstantesRegistroExp.FRAME_DESTINO_ASOCIAR_PARCELAS_CREAR_EXP:
            {
                exp = insertaExpedienteEnBD(exp);
                if(exp!=null)
                {
            ...
            case ConstantesRegistroExp.FRAME_DESTINO_ASOCIAR_BIEN_INMUEBLE_CREAR_EXP:
            {
                exp = insertaExpedienteEnBD(exp);
                if(exp!=null)
                {
            ...
            case ConstantesRegistroExp.FRAME_DESTINO_ASOCIAR_PARCELAS:
            {
                desktopPane.getDesktopManager().closeFrame(iFrame);
                mostrarAsociarParcelas(exp);
                break;
            }
            case ConstantesRegistroExp.FRAME_DESTINO_ASOCIAR_BIEN_INMUEBLE:
            {
                desktopPane.getDesktopManager().closeFrame(iFrame);
                mostrarAsociarBienInmueble(exp);
                break;
            }
            case ConstantesRegistroExp.FRAME_DESTINO_VER_ESTADO:
            {
            ...
            case ConstantesRegistroExp.FRAME_DESTINO_CREAR_EXP:
            {
                desktopPane.getDesktopManager().closeFrame(iFrame);
                crearExpMenuItemActionPerformed();
                break;
            }
            case ConstantesRegistroExp.FRAME_ACCION_DEFAULT:
            {
                desktopPane.getDesktopManager().closeFrame(iFrame);
                UtilRegistroExp.menuBarSetEnabled(true, this);
                break;
            }
            case ConstantesRegistroExp.FRAME_DESTINO_MODIFICAR_DATOS_EXP:
            {
                desktopPane.getDesktopManager().closeFrame(iFrame);
                mostrarModificarExpediente(exp);
                break;
            }
            case ConstantesRegistroExp.FRAME_DESTINO_MODIFICAR_INF_CATASTRAL:
            {
                desktopPane.getDesktopManager().closeFrame(iFrame);
                mostrarModificarInfCatastral(exp);
                break;
            }
            case ConstantesRegistroExp.FRAME_DESTINO_MODIFICAR_INF_CATASTRAL_SINCRONIZAR: {
                ...
                if(sincronizarExpediente(exp)){
                    ...
                    if(escribeCatastroTemporal(exp,ConstantesRegistroExp.tipoConvenio, ConstantesRegistroExp.modoTrabajo)){
                        exp.setIdEstado(ConstantesRegistroExp.maquinaEstadosFlujo.estadoSiguiente(exp.getIdEstado()));
            ...
            case ConstantesRegistroExp.FRAME_DESTINO_MODIFICAR_INF_CATASTRAL_ESCRIBIR_CATASTRO_TEMPORAL:
            {
...
//                	}
            ...
            case ConstantesRegistroExp.FRAME_DESTINO_GESTION_EXP_EXPORTAR:
            {
            
                try{
                   //if(exportacionMasiva(exp, "", "", ""))
                    	
                    if(exportacionMasiva(exp, "C:\\LocalGIS\\datos", "FinEntrada", "VARPAD")){
                    
                        mostrarMensajeDialogo(I18N.get("RegistroExpedientes",
                                "Catastro.RegistroExpedientes.MainCatastro.exportacionCorrecta"));
                      //  exp.setIdEstado(ConstantesRegistroExp.maquinaEstadosFlujo.estadoSiguiente(exp.getIdEstado()));
                       // updateExpedienteEnBD(exp);
                      //  ConstantesRegistroExp.maquinaEstadosFlujo.actualizaBotones((int)exp.getIdEstado());
            ...
            case ConstantesRegistroExp.FRAME_DESTINO_GESTION_EXP_UPDATE_EXP_ELIMINA_CATASTRO_TEMPORAL:
            {
            ...
            case ConstantesRegistroExp.FRAME_DESTINO_ASOCIAR_DATOS_GRAFICOS:
            {
                mostrarAsociarDatosGraficos(exp);
                break;
            }
            case ConstantesRegistroExp.FRAME_DESTINO_ASOCIAR_DATOS_GRAFICOS_SINCRONIZAR:
            {
                //TODO llamar a los metodos de Importacion para el FIN Retorno de sincronizacion
            ...
            case ConstantesRegistroExp.FRAME_DESTINO_ASOCIAR_DATOS_GRAFICOS_ESCRIBIR_CATASTRO_TEMPORAL:
            {
                if(escribeCatastroTemporal(exp,ConstantesRegistroExp.tipoConvenio, ConstantesRegistroExp.modoTrabajo))
                {
                    exp.setIdEstado(ConstantesRegistroExp.maquinaEstadosFlujo.estadoSiguiente(exp.getIdEstado()));
            ...
            case ConstantesRegistroExp.FRAME_DESTINO_BUSCAR_EXPEDIENTES:
            {
                desktopPane.getDesktopManager().closeFrame(iFrame);
                final JFrame desktop= (JFrame)this;
                final TaskMonitorDialog progressDialog= new TaskMonitorDialog(desktop, null);
            ...
            case ConstantesRegistroExp.FRAME_DESTINO_GESTION_EXP_ACTUALIZA_CATASTRO:
            {
            	updateExpedienteEnBD(exp);
            	desktopPane.getDesktopManager().closeFrame(iFrame);
                gestionExpMenuItemActionPerformed(exp);
            ...
            case ConstantesRegistroExp.FRAME_DESTINO_GESTION_EXP_CONSULTA_ESTADO_CATASTRO:
            {
            	updateExpedienteEnBD(exp);
            	desktopPane.getDesktopManager().closeFrame(iFrame);
                gestionExpMenuItemActionPerformed(exp);
            ...
            case ConstantesRegistroExp.FRAME_DESTINO_BUSQUEDA_CONSULTA_ESTADO:
            {
            	updateExpedienteEnBD(exp);
            	desktopPane.getDesktopManager().closeFrame(iFrame);
            ...
            case ConstantesRegistroExp.FRAME_DESTINO_GESTION_EXP_CONSULTA_CATASTRO:
            {
            	updateExpedienteEnBD(exp);
            	desktopPane.getDesktopManager().closeFrame(iFrame);
            	gestionExpMenuItemActionPerformed(exp);
    ...
    public Expediente insertaExpedienteEnBD(Expediente expediente)
    {
        try
        {
            expediente= (Expediente)ConstantesRegistroExp.clienteCatastro.crearExpediente(expediente);
            return expediente;
        }
        catch(ACException e)
        {
     ...
     * los datos del expediente_finca_catastro o expediente_bieninmueble.
     */
    ...
    public void updateIdentificadoresDialogoExpediente(ArrayList lstIdentificadoresDialogo, Expediente exp)
    {
        try
        {
             ConstantesRegistroExp.clienteCatastro.updateIdentificadoresDialogoExpediente(lstIdentificadoresDialogo, exp);
        }
        catch(ACException e)
        {
            JOptionPane.showMessageDialog(this,e.getCause().getMessage());
    ...
    public boolean exportacionMasiva(Expediente exp, String directorio, String nombreFicheroFinEntrada, String nombreVARPAD) {
        try
        {
            ...
            ConstantesRegistroExp.clienteCatastro.exportacionMasiva(exp, ConstantesRegistroExp.modoTrabajo,
                    directorio, nombreFicheroFinEntrada, nombreVARPAD, ConstantesRegistroExp.tipoConvenio);
    ...
    public boolean escribeCatastroTemporal(Expediente exp, String convenio, String modoTrabajo) {
        try {
            ...
            ConstantesRegistroExp.clienteCatastro.escribirEnCatastroTemporal(exp, convenio,modoTrabajo);
            return true;
    ...
    public boolean existeReferenciaCatastroTemporal(Expediente exp) {
        try {
            ...
            boolean existe = ConstantesRegistroExp.clienteCatastro.existeReferenciaCatastroTemporal(exp);
            return existe;
    ...
    public boolean actualizaCatastroTemporal(Expediente exp, String convenio) {
        try {
            ...
            ConstantesRegistroExp.clienteCatastro.actualizaCatastroTemporal(exp, convenio);
            return true;
    ...
    public boolean sincronizarExpediente(Expediente exp)
    {
        try
        {
            ConstantesRegistroExp.clienteCatastro.sincronizarExpediente(exp);
            return true;
        }
        catch(Exception e)
        {
    ...
    public boolean eliminarCatastroTemporal(Expediente exp)
    {
        try
        {
            ConstantesRegistroExp.clienteCatastro.elimiarCatastroTemporalExp(exp);
            return true;
        }
        catch(Exception e)
        {
    ...
    private boolean configureApp()
    {
        try
        {
            ...
            try
            {
               com.geopista.protocol.CConstantesComando.loginLicenciasUrl = aplicacion.getString("geopista.conexion.servidorurl")+"catastro";
//               ConstantesRegistroExp.clienteCatastro = new CatastroClient(aplicacion.getString(AppContext.GEOPISTA_CONEXION_ADMINISTRADORCARTOGRAFIA) +
//                "/CatastroServlet");
               ConstantesRegistroExp.clienteCatastro = new CatastroClient(aplicacion.getString("geopista.conexion.servidorurl")+"Catastro/CatastroServlet");
				
               ConstantesRegistroExp.Locale = aplicacion.getString(AppContext.GEOPISTA_LOCALE_KEY,"es_ES");

               try
               {
                   ConstantesRegistroExp.IdMunicipio=new Integer(aplicacion.getString("geopista.DefaultCityId")).intValue();
               }
               catch (Exception e)
               {
    ...
    private boolean showAuth()
    {
        try
        {
            CAuthDialog auth = new CAuthDialog(this, true,
                        CConstantesComando.loginLicenciasUrl,ConstantesRegistroExp.idApp,
                        ConstantesRegistroExp.IdMunicipio, aplicacion.getI18NResource());
