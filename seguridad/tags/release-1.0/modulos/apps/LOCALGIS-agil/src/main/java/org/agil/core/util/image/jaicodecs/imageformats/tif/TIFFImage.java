...
import org.agil.core.util.image.jaicodecs.image.RasterFactory;
...
/**Hereda de SimpleRenderedImage, que a su vez hereda
 * de RenderedImage.
 * Por tanto es un RenderedImage (no un Tiled)
 *
 * Parece ser que carga todos los datos bajo demanda.
 * Solo hay que hacer que cargue los tiles requeridos.
 *
 * (ver como funciona)
 */
public class TIFFImage extends SimpleRenderedImage {
	...
	/**Stream de donde se van a leer los datos*/
	...
	/*byte order:diferencia entre el procesador*/
	boolean isBigEndian;
	...
	public TIFFImage(SeekableStream stream,TIFFDecodeParam param,int directory)throws IOException {
		...
		if (dir.getField(TIFFImageDecoder.TIFF_TILE_WIDTH) != null) {
			...
			//Estamos leyendo una imagen TIFF con tiles
...
//				System.out.println("La imagen original esta dividida en TILES");
//				System.out.println("width="+tileWidth+" height="+tileHeight);

		} else {
//				System.out.println("La imagen original no esta tileada");
				...
				if (field == null) {
					...
					/*Si el tag TIFF_ROWS_PER_STRIP es null, se considerara un solo tile
					(la imagen entera). Dicen que se puede hacer un mejor trabajo??
					*/
//					System.out.println("la imagen tampoco esta stripeada");
					tileHeight = height;
				} else {
...
//						System.out.println("la imagen esta stripeada (cada fila un strip)");
						tileHeight = (int)l;
					}
			}
	...
	switch(photometric_interp) {
		...
		case 0:
//			System.out.println("se trata de una escala de grises");
			bands = 1;
...
//			System.out.println("se trata de una escala de grises");
	...
	case 2:
		// RGB full color
//		System.out.println("es una imagen rgb");
	...
	case 3:
//		System.out.println("se trata de una imagen con paleta de color");
		// Palette color
	...
	case 4:
//		System.out.println("se trata de una imagen con transparencia, no contemplada");
	...
	default:
//		System.out.println("se trata de un tipo no contemplado:raro....");
...
//System.out.println("calculado el numero de tiles:"+tilesX+" "+tilesY);
	...
	if (compField != null) {
//		System.out.println("utiliza compresion");
		compression = compField.getAsInt(0);
		// Fax T.4 compression options
		if (compression == 3) {
//			System.out.println("compresion FAX T.4");
		...
		if (compression == 4) {
//			System.out.println("compresion FAX T.6");
...
//			System.out.println("compresion LZW");
		...
		if (compression != COMP_NONE && compression != COMP_PACKBITS &&
...
//		System.out.println("no utiliza compresion");
		compression = COMP_NONE;
	}
//	System.out.println("fin de lectura de la cabecera");
	...
	public synchronized Raster getTile(int tileX, int tileY) {
		...
		WritableRaster tile =(WritableRaster)RasterFactory.createWritableRaster(sampleModel,
																					buffer,
		...
		try {
			...
			//cuando un TIFF esta tileado, tiene un campo que es tileOffsets que
			//indica los bytes que hay que saltar. Asi, lo que hace es saltar un numero
			//determinado.
		...
		//Se obtienen las coordenadas pixels de los tiles abarcados
