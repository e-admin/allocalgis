...
public class EntirePage implements ActionListener {
	private PrintLayoutFrame frame;

	private final int hauteurMax = 600;

	private final int largeurMax = 800;

	public EntirePage(PrintLayoutFrame parent) {
		frame = parent;
	}

	public void actionPerformed(ActionEvent e) {
		if (frame.getZoomActif() == PrintLayoutFrame.LARGEUR_PAGE) {
			frame.setZoomActif(PrintLayoutFrame.PAGE_ENTIERE);
			frame.getPrintLayoutPreviewPanel().getPreviewPanel().getPage()
					.resize(frame.getPageFormat());
			frame.getPrintLayoutPreviewPanel().setPreview(
					new PreviewPanel(frame, frame.getPrintLayoutPreviewPanel()
