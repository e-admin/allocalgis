			...
			if (SwingUtilities.isLeftMouseButton(e))
			{
				if (e.getClickCount() == 2)
				{
					...
					if (dlg.isOkpressed())
					{
						...
						//((PrintableZone)MapFrame.this.getParent().getPlugInContext().getLayerViewPanel().getParent()).setScaleFactor(factor)
...
//		setCorner(points);
	...
	{
		double extent = 0;
		if (getCurrentExtent() == MapFrame.FULL_EXTENT
				|| getCurrentExtent() == MapFrame.CURRENT_EXTENT)
		{			
