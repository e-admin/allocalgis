...
public class DrawPointPlugIn extends AbstractPlugIn
{
    private boolean selectDrawPointButtonAdded = false;
    ...
    public void addButton(final ToolboxDialog toolbox)
    {
        if (!selectDrawPointButtonAdded)
        {
            MouseEvent event = new MouseEvent(toolbox,
 	               ...
 	               0, 0,1,
 	              true);		   
 		QuasimodeTool drawPointTool = new QuasimodeTool(DrawPointTool.create(toolbox.getContext())).add(
                    new QuasimodeTool.ModifierKeySpec(true, false, false),
                    null);		   		   
 		drawPointTool.mousePressed(event);
 			
         toolbox.add(drawPointTool);
            toolbox.finishAddingComponents();
            toolbox.validate();
            selectDrawPointButtonAdded = true;
        }
    }
}
