...
public class DrawPolygonPlugIn extends AbstractPlugIn
{
    private boolean selectDrawPolygonButtonAdded = false;
    ...
    {
    	if (!selectDrawPolygonButtonAdded)
    	{
    		...
    		MouseEvent event = new MouseEvent(toolbox,
    				...
    				0, 0,1,
    				true);		   
    		QuasimodeTool drawPolygonTool = new QuasimodeTool(DrawPolygonTool.create(toolbox.getContext())).add(
    				new QuasimodeTool.ModifierKeySpec(true, false, false),
    				null);		   		   
    		drawPolygonTool.mousePressed(event);

    		toolbox.add(drawPolygonTool);
    		toolbox.finishAddingComponents();
    		toolbox.validate();
    		selectDrawPolygonButtonAdded = true;
    	}        
