...
public class AlfrescoRepositoryManagerImpl implements AlfrescoRepositoryManager {
	 ...
	 */
	public QueryResult getParentNode(AlfrescoKey key) throws RepositoryFault, RemoteException {
	...
	public boolean renameNode(AlfrescoKey key, String newName) throws RepositoryFault, RemoteException{
		return moveNode(AlfrescoManagerUtils.getParentReferenceFromParent(new AlfrescoKey(getParentNode(key).getResultSet().getRows(0).getNode().getId(),AlfrescoKey.KEY_UUID), newName), key);
	}
