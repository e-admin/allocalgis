...
public class StringDomain extends PatternBasedDomain {
	 ...
	 */
	private static final Log logger = LogFactory.getLog(StringDomain.class);
 ...
 * @param Description
 */
	public StringDomain(String pattern, String Description) {
       
		super(pattern, Description);
         if ("".equals(pattern)) setPattern("?[.*]");
		}
	/**
 * Llamado por Java2XML
 */
public StringDomain() {
	 ...
	 * 
	 */
	protected boolean validateLocal(Feature feature, String Name, Object objValue) {
		if (objValue==null) 
			objValue = feature.getString(Name);
    ...
    objValue = objValue.toString();
		...
		if (maxLength>-1&&((String)objValue).length()>maxLength) 
			{
				setLastErrorMessage("STRING MAXLENGTH EXCEEDED");
				return false;
			}
		boolean pasa=false;		
//		if (getFormat()!=null && !getFormat().equals(""))// hay un patron especificado
		...
		if (!pasa)
			setLastErrorMessage("STRING PATTERN MISMATCH");
		return pasa;
	}
