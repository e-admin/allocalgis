...
public class TIFFDirectory extends Object {
	...
	public int[] getTags() {
		...
		Enumeration enumerationElement = fieldIndex.keys();
		int i = 0;

		while (enumerationElement.hasMoreElements()) {
			tags[i++] = ((Integer)enumerationElement.nextElement()).intValue();
		}
	...
	public static int getNumDirectories(SeekableStream stream) throws IOException{
		...
		int endian = stream.readUnsignedShort();
		if (!isValidEndianTag(endian)) {
//			System.out.println("endianTag no valido");
			throw new IllegalArgumentException(JaiI18N.getString("TIFFDirectory1"));
		}
...
//		System.out.println("directorios leidos");
