...
public class Condition  {
	...
	private boolean test(Geometry g) throws Exception {
		...
		if (pos==0) {
			...
			List layersList = context.getLayerNamePanel().getLayerManager().getLayers();
			Iterator enLayersList = layersList.iterator();
			Layer layer = null;
			while (enLayersList.hasNext()){
				layer = (Layer)enLayersList.next();
				if (layer.getName().equals((String)query.valueCB.getSelectedValue()))
					break;
			}
