...
//	private String traduccionSelected;
	...
	public FichasFilterPanel(JFrame desktop, String tipo, HashMap<String,String> listaFiltros, String locale) throws Exception{
        ...
        plantillasClient= new LocalGISPlantillasClient(aplicacion.getString(AppContext.GEOPISTA_CONEXION_ADMINISTRADORCARTOGRAFIA) + "/PlantillasServlet");
        initComponents();
	...
	private void initComponents(){		
        ...
        if (!tipo.equals(ConstantesLocalGISEIEL.PATRON_FICHA_MUNICIPAL)){
        	...
        	add(contenedorInferiorInformePanel, new org.netbeans.lib.awtextra.AbsoluteConstraints(0, 80, 750, 80));
...
//			    guardarSubreports();
			    ...
			    obtenerSubreports(jasperDesign, listaSubReports);
			    ...
			    if (!previsualizar){
			        ...
			        resultado=JREIEL.generarReportConnectDB(jrxmlfile, pathDestino, 
			    			...
			    			locale,codEntidad,codNucleo,nucleoParaImprimir,imprimirImagenes,usarWMSExternos,this);
			    }else{
			        /** Previsualizamos el documento */
			    	resultado=JREIEL.generarReportConnectDB(jrxmlfile, null, ""+ConstantesLocalGISEIEL.formatoPREVIEW, 
							  listaFiltros, nodo,desktop, locale,codEntidad,codNucleo,nucleoParaImprimir,imprimirImagenes,usarWMSExternos,this);
	...
	private void obtenerSubreports(JasperDesign jasperDesign, ArrayList<String> listaSubReports) {
				...
				if(hijo instanceof JRDesignSubreport) {
					...
					if(nombre != null) {
						...
						if(!listaSubReports.contains(nombre)) {
							...
							try {
								guardarCompilarSubreport(nombre);
								jasperDesignSub = JRXmlLoader.load(path + nombre);
								obtenerSubreports(jasperDesignSub, listaSubReports);
							} catch(Exception e) {
    ...
    private void guardarCompilarSubreport(String subreportname) throws Exception {
        ...
        for(int i = 0; i < subreports.length; i++) {
        	...
        	if(nombre.equalsIgnoreCase(subreportname)) {
                ...
                JasperCompileManager.compileReportToFile((new File(path)).getAbsolutePath() + File.separator + nombre);
                return;
        	}
        }
    }
