...
public class MapasTematicosPanel extends JPanel{
    ...
    private void mostrarMapaJButtonActionPerformed() {
        try{
            ...
            if (mapaSeleccionado!=null){
	            int idMapa=eielClient.getIdMapa(mapaSeleccionado);
	            if (idMapa!=-1){
    ...
    private void mostrarMapasDisponiblesJButtonActionPerformed() {
        try{
            ...
            if (patronMapas!=null){
	            ArrayList listaMapas=eielClient.getMapas(patronMapas,ConstantesLocalGISEIEL.Locale);
	            if (listaMapas!=null){
   ...
   /* private void getNucleosEIEL(){
    ...
    private Collection getIndicadoresEIEL(){
		...
		Collection<Object> c=null;
        try {
			c = eielClient.getIndicadoresEIEL();
		} catch (Exception e) {
			e.printStackTrace();
		}
