...
public abstract class PolygonTool extends MultiClickTool {
    public PolygonTool() {
    	...
    	ResourceBundle bundle2 = ResourceBundle.getBundle("com.geopista.ui.cursortool.languages.PolygonTooli18n",loc,this.getClass().getClassLoader());    	
        I18N.plugInsResourceBundle.put("PolygonTool",bundle2);
    }
    ...
    protected boolean checkPolygon() throws NoninvertibleTransformException {
        if (getCoordinates().size() < 3) {
            getPanel().getContext().warnUser(I18N.get("PolygonTool","ThePolygonMustHaveAtLeast3Points"));
