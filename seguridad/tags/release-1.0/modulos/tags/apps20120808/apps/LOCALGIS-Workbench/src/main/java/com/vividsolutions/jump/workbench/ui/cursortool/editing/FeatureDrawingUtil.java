...
public class FeatureDrawingUtil {
        ...
        if (layerNamePanelProxy.getLayerNamePanel().chooseEditableLayer() == null) {
            Layer layer =
                layerViewPanel.getLayerManager().addLayer(
                    ...
                    aplicacion.getI18nString("GeopistaAddNewLayerPlugIn.New"),
                    GeopistaAddNewLayerPlugIn.createBlankFeatureCollection());
            layer.setEditable(true);
            if(layer instanceof GeopistaLayer)
            {
              ((IGeopistaLayer) layer).setLocal(true);
            }
            layerViewPanel.getContext().warnUser(
                aplicacion.getI18nString("NingunaCapaEditable"));
        }
    ...
    public UndoableCommand createAddCommand(
        ...
        ((GeopistaFeature) feature).setLayer((IGeopistaLayer) layer);
