...
import com.geopista.protocol.cementerios.*;
 ...
 */
public class JROperacionesCDataSource {
...
//            value= inmueble.getEdificabilidad()!=-1?new Double(inmueble.getEdificabilidad()).toString():"";
...
//            try{value= derecho.getFormaAdquisicion()!=null?Estructuras.getListaFormaAdquisicion().getDomainNode(derecho.getFormaAdquisicion()).getTerm(locale):"";}catch(Exception e){value= "";}
...
//    }
    ...
    private static Object getDocumentoValue(JRField field, ElemCementerioBean elem) throws Exception{
        if (field == null) return null;
        Object value= null;
        CementerioDocumentClient documentClient= new CementerioDocumentClient(((AppContext) AppContext.getApplicationContext()).getString(AppContext.GEOPISTA_CONEXION_ADMINISTRADORCARTOGRAFIA) +
                DocumentManagerPlugin.DOCUMENT_SERVLET_NAME);

        Collection c= documentClient.getAttachedDocuments(elem.getId());
