...
import com.geopista.server.metadatos.db.*;
               ...
               while (rsSQL.next())
               {
                     ...
                     if ((oldCapa==null) || (!oldCapa.getId().equals(newId)))
                     {
                                 ...
                                 if (oldCapa.getIdAcl()!=null)
                                 {
                                     try
                                     {
                                        GeopistaAcl acl =sSesion.getPerfil(new Long(oldCapa.getIdAcl()).longValue());
                                        if (!acl.checkPermission(new GeopistaPermission(GeopistaPermission.LEER_CAPA)))
                                        {
                                           logger.warn("El usuario "+sSesion.getUserPrincipal().getName()+" NO tiene parmiso para leer en la capa "+oldCapa.getId());
                                           noAcl=true;
                                        }
