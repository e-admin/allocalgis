...
public class COperacionesDGC
{
	...
	private Persona getRepresentante(String nifRepresentante, String id_bienInmueble) throws SQLException
	{
		...
		try {
			...
			while (rs.next())
			{
				...
				idProv = GeopistaUtil.completarConCeros(idProv, 2);

				idMun = GeopistaUtil.completarConCeros(idMun, 3);
	...
	private ArrayList getTitular(String referenciaExpediente, String id_bienInmueble) throws SQLException
	{
		...
		if (id_bienInmueble.length()<20){
			GeopistaUtil.completarConCeros(id_bienInmueble, 20);
		}
		...
		try{
			...
			while (rs.next())
			{
				...
				if (!nif.startsWith("E") ||
					...
					GeopistaUtil.completarConCeros(idMun, 3);


					idProv = GeopistaUtil.completarConCeros(idProv.trim(), 2);

					idMun = GeopistaUtil.completarConCeros(idMun.trim(), 3);					
	...
	private ArrayList getConstrucciones(FincaCatastro finca) throws Exception
	{
		...
		try
		{
			...
			while(rs.next())
			{
				...
				construccionCatastro.setNumOrdenConstruccion(GeopistaUtil.completarConCeros(rs.getString("numero_orden_construccion"), 4));
		...
		try
		{
			...
			if(rs.next())
			{
				...
				if (elemRep.getId() != null && elemRep.getId().length()<18){
					elemRep.setId(GeopistaUtil.completarConCeros(elemRep.getId(), 18));
				}
	...
	private Expediente completarFincaCatastroExpedienteDB(Expediente exp, String idMunicipioFinca) throws Exception{
		...
		try {
			...
			while(rs.next()) {
				...
				String idM = GeopistaUtil.completarConCeros(String.valueOf(idMunicipio), 5);               
