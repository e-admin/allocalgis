...
public class JPanelDominios extends JPanel implements TreeSelectionListener{
	...
	private void initComponents(ResourceBundle messages)
	{
		...
		jTextFieldValorMáximo = new JNumberTextField(JNumberTextField.NUMBER, new Integer(999999999)); //999999999999999999
		...
		}catch(Exception ex)
		{
			...
			PrintWriter pw = new PrintWriter(sw);
			ex.printStackTrace(pw);
			JOptionPane.showMessageDialog(frame, "Excepción al cargar el Renderer:\n"+sw.toString());
			logger.error("Exception: " + sw.toString());
		...
		jPanelNumberDomain.add(jTextFieldValorMáximo, new org.netbeans.lib.awtextra.AbsoluteConstraints(30, 100, 100, -1));
				...
				if(modoNuevo)
				{
					...
					if (jTextFieldAnadir.getText().length()>0)
					{
						if (jTextFieldPatronAnadir.getText().length() > Constantes.PATRON_MAX_SIZE) {
							JOptionPane.showMessageDialog(frame, "Patrón demasiado largo.");
			...
			if (inputValue!=null && inputValue.length()>0)
			{
				...
				if (resultado.getResultado())
				{
					...
					/*DefaultTreeModel treeModel=(DefaultTreeModel)jTreeArbol.getModel();
                     ...
                     jTreeArbol.scrollPathToVisible(new TreePath(hojaDominio.getPath()));
                     //Lo meto también en la lista particular del municipio
                      Domain domainParticular=new Domain(newDomain.getIdDomain(), newDomain.getName());
	...
	private void enabledComponents(boolean bEnabled)
	{
		...
		jTextFieldValorMáximo.setEditable(bEnabled);
	...
	private String obtenerPatronNumber()
	{
		...
		if (jTextFieldValorMinimo.getText()!=null&&jTextFieldValorMinimo.getText().length()>0)
			try {sValorMinimo=jTextFieldValorMinimo.getNumber().toString();}catch(Exception e){sValorMinimo="";}
				...
				sValorMinimo="-INF";
		String sValorMaximo="";
		if (jTextFieldValorMáximo.getText()!=null&&jTextFieldValorMáximo.getText().length()>0)
			try {sValorMaximo=jTextFieldValorMáximo.getNumber().toString();}catch(Exception e){sValorMaximo="";}
			else
				sValorMaximo="INF";
	...
	private void mostrarPatronNumber(String sPatron)
	{
		try
		{
			...
			try
			{
				...
				jTextFieldValorMáximo.setText(sValorMaximo);
			}catch(Exception e){
				jTextFieldValorMáximo.setText("");
			}
		}catch(Exception e)
	...
	private void pintaLeyenda()
	{
		...
		}catch(Exception ex)
		{
			...
			PrintWriter pw = new PrintWriter(sw);
			ex.printStackTrace(pw);
			JOptionPane.showMessageDialog(frame, "Excepción al cargar la leyenda:\n"+sw.toString());
			logger.error("Exception: " + sw.toString());
		}
	}
	...
	private com.geopista.app.utilidades.JNumberTextField jTextFieldValorMáximo;
