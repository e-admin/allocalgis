...
//	}
	...
	@RequestMapping(method=RequestMethod.GET,value={"/eliminarUsuario/{id}"})
	public ResponseEntity<EncapsuladorIntegerSW> eliminarUsuario(@PathVariable("id") Long id){
		try {
			return new ResponseEntity<EncapsuladorIntegerSW>(gestorCUUsuario.borra(id)?new EncapsuladorIntegerSW(1):new EncapsuladorIntegerSW(0),responseHeaders, HttpStatus.OK);
