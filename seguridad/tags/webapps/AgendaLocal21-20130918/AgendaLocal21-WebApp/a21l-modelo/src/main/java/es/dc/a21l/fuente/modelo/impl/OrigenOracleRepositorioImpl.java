...
public class OrigenOracleRepositorioImpl extends RepositorioBaseImpl<Fuente> implements OrigenOracleRepositorio{
	private static final Logger log = LoggerFactory.getLogger(OrigenOracleRepositorioImpl.class);
	private static final String DRIVER_ORACLE = "oracle.jdbc.OracleDriver"; 
	private static final String CADENA_BD_ORACLE="jdbc:oracle:thin:";
	private static final String queryTablas = "select object_name as table_name  from user_objects where object_type = 'TABLE'";
	...
	public EncapsuladorListSW<TablaFuenteDatosDto> listarTablasFuenteExterna(Fuente fuente) {
		...
		if(!cadenaConexion.contains(CADENA_BD_ORACLE)){
			cadenaConexion=CADENA_BD_ORACLE.concat(cadenaConexion);
		}
    ...
    private TipoAtributoFD obtenerTipoColumna(int tipo) {
    	try {
	    	...
	    	else if ( tipo == Types.DECIMAL || tipo == Types.DOUBLE || tipo == Types.FLOAT || tipo == Types.INTEGER || tipo == Types.NUMERIC || tipo == Types.REAL || 
	    			tipo == Types.SMALLINT || tipo == Types.TINYINT || tipo == Types.BIGINT || tipo==Types.BOOLEAN || tipo==Types.BIT ||tipo==Types.NUMERIC )
