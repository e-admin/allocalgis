...
public class FuenteRepositorioImpl extends RepositorioBaseImpl<Fuente> implements FuenteRepositorio{
	...
	private static final String SELECT_FUENTES_USUARIO="select f from Fuente f where f.registradaPor.id= :idUsuario";
	...
	public List<Fuente> cargaFuentesAsociadasUsuario(Long idUsuario) 
	{
		List<Fuente> listFuentes = getEntityManager().createQuery(SELECT_FUENTES_USUARIO).setParameter("idUsuario", idUsuario).getResultList();
		return listFuentes;
	}
	
	public boolean isUsuarioConFuentes(Long idUsuario) 
	{
		List<Fuente> listFuentes = getEntityManager().createQuery(SELECT_FUENTES_USUARIO).setParameter("idUsuario", idUsuario).getResultList();
		return listFuentes.size()>0;
	}
