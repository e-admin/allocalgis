...
public class GestorCUCriterioImpl implements GestorCUCriterio {
	...
	private Boolean operaAritmetica(TipoOperacionCriterioEmun operacion,Double valorIzq ,Double valorDch) throws Exception{
		...
		if(operacion.equals(TipoOperacionCriterioEmun.CONTAINS)){
			return valorIzq.toString().indexOf(valorDch.toString())!=-1;
		}
		if(operacion.equals(TipoOperacionCriterioEmun.LIKE)){
			return valorIzq.toString().indexOf(valorDch.toString())!=-1;
		}
	...
	private Boolean operaTexto(TipoOperacionCriterioEmun operacion, String valorIzq ,String valorDch) throws Exception{
		...
		if(operacion.equals(TipoOperacionCriterioEmun.CONTAINS)){
			return valorIzq.toLowerCase().indexOf(valorDch.toLowerCase())!=-1;
		}
		if(operacion.equals(TipoOperacionCriterioEmun.LIKE)){
			if(valorDch.startsWith("%") && valorDch.endsWith("%")){
				valorDch=valorDch.replaceAll("%","").trim();
				return valorIzq.toLowerCase().indexOf(valorDch.toLowerCase())!=-1;
			}else{
				if(valorDch.startsWith("%")){
					valorDch=valorDch.replaceAll("%","").trim();
					return valorIzq.toLowerCase().endsWith(valorDch.toLowerCase());
				}else{
					if(valorDch.endsWith("%")){
						valorDch=valorDch.replaceAll("%","").trim();
						return valorIzq.toLowerCase().startsWith(valorDch.toLowerCase());
					}else{
						valorDch=valorDch.replaceAll("%","").trim();
						return valorIzq.toLowerCase().equals(valorDch.toLowerCase());
					}
				}
			}
		}	
