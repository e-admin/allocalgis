...
public class CategoriaControladorSW {
	...
	@RequestMapping(method=RequestMethod.GET,value={"/cargaCategoriasPermisosUsuario/{idUsuario}"})
	public ResponseEntity<EncapsuladorListSW<CategoriaDto>> cargaCategoriasPermisosUsuario(@PathVariable("idUsuario") Long idUsuario){
		return new ResponseEntity<EncapsuladorListSW<CategoriaDto>>(gestorCUCategoria.cargaCategoriasPermisosUsuario(idUsuario), responseHeaders, HttpStatus.OK);
	}
}