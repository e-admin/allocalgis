...
<div class="cuerpoprincipal">
	...
	<div class="areacentral">
		...
		<div class="fizq ancho80">
			...
			<!-- MAPA -->
			<div id="map" style="width: 550px; height: 550px; border-style:solid; border-width:1px;border-color:#666666;">
				<div class="popup" id="tooltip" style="display:inline-block; visibility: hidden; position: relative;padding: 5px;font-size: 8pt;z-index: 1000; background-color: white;"></div>
			</div>
        ...
        <div style="padding: 0.2em 0.7em;" class=""> 
            ...
            <div>
            	<form id="formularioEstiloVisualizacion" action="indicadores.htm" method="post">
	                ...
	                <div class="linea">
		            	<label class="label" for="inicio"><spring:message code="jsp.visualizacion.inicio.valorMinimo"/>:&nbsp;${valorMinimo}</label>
		            </div>
		            <div class="linea">
		            	<label class="label" for="inicio"><spring:message code="jsp.visualizacion.inicio.valorMaximo"/>:&nbsp;${valorMaximo}</label>
		            </div>
	                <div class="linea">
...
<script type="text/javascript">
	...
	var valorProyeccionOSM="EPSG:900913";
	...
	function init() {
		...
		var bounds = new OpenLayers.Bounds(475774.9406515219, 4629122, 686857, 4849600);
...
// 			maxResolution: 861.2421875,
		...
		/*Código incluido en optións para el tema del boundigbox */
// 		maxResolution: "auto",
// 		minResolution: "auto",
...
// 		/* Código que centra el mapa en un BoundingBox concreto. Para que funcione se ha modificado maxResolution en la definición del mapa a "auto" */
		/* Creación del objeto Bounds (left (minX), bottom (minY), rigth (maxX), top (maxY)) */
 		var limites = new OpenLayers.Bounds(${indicadorDto.minX}, ${indicadorDto.minY}, ${indicadorDto.maxX}, ${indicadorDto.maxY});
 		var zoom = map.getZoomForExtent(limites); //This function will return an integer from 0 -> maxZoom (default 16) at which the bounds will fit into the map window. 
 	    var lonlat = limites.getCenterLonLat().transform(new OpenLayers.Projection("EPSG:4326"), map.getProjectionObject());
// 	    //var lonlat = new OpenLayers.LonLat(13.41,52.52).transform(new OpenLayers.Projection("EPSG:4326"),map.getProjectionObject()); //esta prueba con zoom = 15
 	    map.setCenter(lonlat,zoom);	   

// 	    //Sólo cuando el mapa no tenga centro se hará un zoomToExtend(bounds) (Zoom to the passed in bounds, recenter)
