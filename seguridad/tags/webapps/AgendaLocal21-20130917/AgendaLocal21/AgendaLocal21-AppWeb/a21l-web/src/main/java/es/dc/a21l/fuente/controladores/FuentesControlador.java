...
public class FuentesControlador extends GenericAbstractController {
	...
	public String edita(@RequestParam(value = "id") long id, Model model,HttpServletRequest request,@RequestParam(value="errorTamanhoArchivo",required=false)Boolean errorTamanhoArchivo) {
		...
		cargaCaracteresSeparadores(model);
		cargaTiposCodificacion(model);
		return "fuenteTile";
	}
	...
	public String verDatosTablaFuente(@RequestParam(value = "id") long id,@RequestParam(value = "esquema") String esquema, @RequestParam(value = "tabla") String tabla, 
		...
		if ( datos.getContenido().containsKey(ERROR_PARSEO_FICHERO)) {
			model.addAttribute("errorFichero","jsp.fuente.error.permisos");
		}
	...
	private void cargaTiposCodificacion(Model model){
		UrlConstructorSW url = new UrlConstructorSWImpl(getServicioConfiguracionGeneral().getUrlBaseSW());
		url.setParametroCadena("fuentes").setParametroCadena("cargaTiposCodificacion");
		List<TipoCodificacionDto>listaTipoCodificacion=new ArrayList<TipoCodificacionDto>();
		listaTipoCodificacion.addAll(getRestTemplate().getForEntity(url.getUrl(),
				EncapsuladorListSW.class).getBody());
		model.addAttribute("listaTiposCodificacion", listaTipoCodificacion);

	}
	
	private void cargaCaracteresSeparadores(Model model){
		UrlConstructorSW url = new UrlConstructorSWImpl(getServicioConfiguracionGeneral().getUrlBaseSW());
		url.setParametroCadena("fuentes").setParametroCadena("cargaCaracterSeparador");
		List<CaracterSeparadorDto>listaCaracterSeparador=new ArrayList<CaracterSeparadorDto>();
		listaCaracterSeparador.addAll(getRestTemplate().getForEntity(url.getUrl(),
				EncapsuladorListSW.class).getBody());
		model.addAttribute("listaCaracterSeparador", listaCaracterSeparador);
