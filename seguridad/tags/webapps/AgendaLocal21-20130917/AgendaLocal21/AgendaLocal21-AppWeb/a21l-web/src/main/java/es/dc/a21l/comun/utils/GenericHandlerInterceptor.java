...
public class GenericHandlerInterceptor extends HandlerInterceptorAdapter {
	...
	public boolean preHandle(HttpServletRequest request,HttpServletResponse response, Object handler) throws Exception {
		...
		request.setAttribute("usuarioAdministrador",getUserAdmin());
	...
	public Boolean getUserAdmin(){
		Long id=servicioSeguridad.getUserDetails().getId();
		UsuarioDto usuario=cargaUsuarioPorIdSW(id);
		if(usuario.getEsAdmin()!=servicioSeguridad.getUserDetails().getEsAdmin()){
			servicioSeguridad.getUserDetails().setEsAdmin(usuario.getEsAdmin());
		}
		return usuario.getEsAdmin();
	}
	
	protected UsuarioDto cargaUsuarioPorIdSW(Long id) {
		UrlConstructorSW url = new UrlConstructorSWImpl(servicioConfiguracionGeneral.getUrlBaseSW()).setParametroCadena("usuarios")
				.setParametroCadena("usuario").setParametroCadena(id);
		ResponseEntity<UsuarioDto> respuesta = restTemplate.getForEntity(url.getUrl(), UsuarioDto.class);
