...
<script src="js/dataTables/js/jquery.tablesorter.min.js"></script>
			...
			<div class="areacompleta">
				...
				<div class="areacentral">
					...
					<c:if test="${indicadorDto.resultadoOperacion.resultado == 'exitoCrear' || indicadorDto.resultadoOperacion.resultado == 'exitoGuardar'}">
		     		...
		     		<c:if test="${calculoBoundingBox}">
		     			<div class="controlAlertas">	
							<div class="ui-widget">
		            			<div class="success ui-corner-all"> 
									<p>
										<span class="ui-icon ui-icon-circle-check fizq"></span>
						          		<spring:message code="jsp.indicadores.indicador.calculobounding" />
					         		</p>
					  	 		</div>
		         			</div>
		         		</div>
		     		...
		     		<form:form  modelAttribute="indicadorDto" id="formularioIndicador" action="indicadores.htm">
						...
						<div class="clear"></div>
						<div id="areabounding">
							<p class="mayuscula sup40 fizq clear"><spring:message code="jsp.indicador.bounding.a.mostrar"/>:</p>
							<div class="clear"></div>
							<div id="coordenadas_x" class="linea fizq">
								<label class="alinea_inf" style="font-size: 0.9em;"><spring:message code="jsp.indicador.bounding.coordenadas.x"/>:</label>
								<form:input path="minX" cssClass="input_bounding" cssStyle="margin-left:12px;" maxlength="20" />
								<form:input path="maxX" cssClass="input_bounding" cssStyle="margin-left:10px;" maxlength="20" />
							</div>
							<div class="clear"></div>
							<div id="coordenadas_y" class="linea fizq">
								<label class="alinea_inf" style="font-size: 0.9em;"><spring:message code="jsp.indicador.bounding.coordenadas.y"/>:</label>
								<form:input path="minY" cssClass="input_bounding" cssStyle="margin-left:10px;" maxlength="20" />
								<form:input path="maxY" cssClass="input_bounding" cssStyle="margin-left:10px;" maxlength="20" />
							</div>					
						</div>
					</form:form>				
				<!-- areacentral -->
...
$(document).ready(function() {
	...
	if (indicadores.comprobarSiExisteTablaLista('tabla_campos_mostrar')) {	
		indicadores.ordenarListaCamposMostrar();
	}
...
var indicadores = {
	...
	/*
	 * SE COMPRUEBA SI EXISTE LA LISTA DE ELEMENTOS A MOSTRAR	 
	*/
	comprobarSiExisteTablaLista: function(tabla) {
		var resp = false;
		$('#div_tabla_campos_mostrar table tbody tr').each(function() {			
				resp = true;
		});
		return resp;
	},
	
	/* Método que ordena la lista de campos a mostrar */
	ordenarListaCamposMostrar: function() {
		$("#tabla_campos_mostrar").tablesorter();
		$("#tabla_campos_mostrar").trigger("update");
		var sorting = [[3,0]]; 
		$("#tabla_campos_mostrar").trigger("sorton",[sorting]);
	},
	...
	*/
	...
	rellenarCamposParaFormula: function() {
		...
		$('#zona_columnas table').each(function(){
			...
			$("[id='"+$(this).attr("id")+"'] tr td").each(function(){
				var nombreCol = $(this).html();
				if ( $(this).parent().attr("data-tipo") == "VALORFDNUMERICO" || $(this).parent().attr("data-tipo") == "VALORFDGEOGRAFICO"  ) {
