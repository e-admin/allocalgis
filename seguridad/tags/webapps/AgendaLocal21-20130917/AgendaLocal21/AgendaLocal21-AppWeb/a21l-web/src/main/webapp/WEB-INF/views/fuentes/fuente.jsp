    ...
    <div class="cuerpoprincipal">
		...
		<div class="areacentral">
			...
			<div id="alertasJS" class="controlAlertas oculto">	
				...
				<input id="error_caracterSeparador" type="hidden" value="<spring:message code="validacion.fuentes.caracterSeparador"/>">
				<input id="error_tipoCodificacion" type="hidden" value="<spring:message code="validacion.fuentes.tipoCodificacion"/>">
				<div class="ui-widget">
					<div class="ui-state-error ui-corner-all"> 
						<p><span class="ui-icon ui-icon-alert fizq"></span> 
						<strong><spring:message code="global.formulario.erros.titulo"/></strong>
			...
			<form:form id="formulario"  modelAttribute="fuenteDto" action="fuentes.htm" enctype="multipart/form-data" method="post">
				...
				<div class="clear"></div>
					...
					<div id="csv" class="oculto conexion">
						<div class="linea">
							<label for="caracterSeparador" class="label fizq"><spring:message code="jsp.fuentes.caracter"/></label>
							<select name="idCaracterSeparador" id="caracterSeparador" >
								<option value="" selected="selected">&nbsp;</option>
								<c:forEach items="${listaCaracterSeparador}" var="caracter">
		                      	<option value="${caracter.id}" <c:if test="${fuenteDto.idCaracterSeparador eq caracter.id}">selected="selected"</c:if>><spring:message code="${caracter.caracter}"/></option>
		                       </c:forEach>
		                    </select>
						</div>
						<div class="linea">
							<label for="tipoCodificacion" class="label fizq"><spring:message code="jsp.fuentes.tipoCodificacion"/></label>
							<select name="idTipoCodificacion" id="tipoCodificacion" >
								<option value="" selected="selected">&nbsp;</option>
								<c:forEach items="${listaTiposCodificacion}" var="tCodificacion">
		                      	<option value="${tCodificacion.id}" <c:if test="${fuenteDto.idTipoCodificacion eq tCodificacion.id}">selected="selected"</c:if>><spring:message code="${tCodificacion.nombre}"/></option>
		                       </c:forEach>
		                    </select>
						</div>
					</div>
				...
				<div id="mensaje_ayuda" class="linea" style="color:red;">
					<label class="oculto mensaje_ayuda" id="msg_WFS"><spring:message  code="jsp.fuentes.ayuda.WFS"/></label>
					<label class="oculto mensaje_ayuda" id="msg_GML"><spring:message  code="jsp.fuentes.ayuda.GML"/></label>
					<label class="oculto mensaje_ayuda" id="msg_CSV"></label>
					<label class="oculto mensaje_ayuda" id="msg_ODBC"><spring:message  code="jsp.fuentes.ayuda.ODBC"/></label>
					<label class="oculto mensaje_ayuda" id="msg_PostGis"><spring:message  code="jsp.fuentes.ayuda.PostGis"/></label>
					<label class="oculto mensaje_ayuda" id="msg_MySQL"><spring:message  code="jsp.fuentes.ayuda.MySQL"/></label>
					<label class="oculto mensaje_ayuda" id="msg_Oracle"><spring:message  code="jsp.fuentes.ayuda.Oracle"/></label>
				</div>
			</form:form>
			</div>
			<%-- zona indicadores --%>
	...
	$(function(){
		$("#formulario").submit(function() {
			...
			if (tipo.length == 0) {
				...
				if(tipo==4 && $('#caracterSeparador').val().length==0){
					arrayErros.push($('#error_caracterSeparador').val());
				}
				if(tipo==4 && $('#tipoCodificacion').val().length==0){
					arrayErros.push($('#error_tipoCodificacion').val());
				}
					if($("#idDto").val()==0){
