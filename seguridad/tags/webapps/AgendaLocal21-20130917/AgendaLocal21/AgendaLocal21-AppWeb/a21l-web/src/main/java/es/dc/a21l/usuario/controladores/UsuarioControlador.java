...
public class UsuarioControlador extends GenericAbstractController {
	...
	public String eliminarUsuario(@RequestParam(value="id") Long id, Model model, HttpServletRequest request,HttpServletResponse response) throws Exception{
		...
		UrlConstructorSW url= new UrlConstructorSWImpl(getServicioConfiguracionGeneral().getUrlBaseSW()).setParametroCadena("usuarios").setParametroCadena("eliminarUsuario").setParametroCadena(id).setParametroCadena(getServicioSeguridad().getUserDetails().getId());
