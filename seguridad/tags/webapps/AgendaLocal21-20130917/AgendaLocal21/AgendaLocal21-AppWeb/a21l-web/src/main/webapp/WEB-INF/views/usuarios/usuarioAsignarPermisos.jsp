...
<div class="cuerpoprincipal">
	...
	<div class="areacentral">
		...
		<br/>
		<c:if test="${usuarioDto.esAdmin}">
			<p style="color:red;"><spring:message code="jsp.usuarios.usuarioAsignarPermisos.esAdmin"/></p>
			<br/>
		</c:if>
	...
	function InicializarTabla()
	{
		<c:forEach items="${mapaCategoriasTabla[0]}" var="categoria">
	 	...
	 	<c:forEach items="${mapaIndicadoresTabla[0]}" var="indicador">
			dibujarFilaIndicadorPub('tablaIndicadores', '0', '${indicador.id}', '${indicador.nombre}','${indicador.urlPublicacion}', '${indicador.urlPublicacionCorta}', '${indicador.numUsuariosPublicacion}', '${indicador.loginUltimaPeticion}', 20, '${indicador.publico}', '${indicador.pteAprobacionPublico}');
		</c:forEach>
	}

	function dibujarFilaIndicadorPub(idTabla, idCategoriaPadre, idIndicador, nombreIndicador, urlPublicacion, urlPublicacionCorta, numUsuarios, loginUsuario, margenIzq, publico, pteAprobacionPublico)
	{
...
// 		// [4] Indicador público
		...
		if(publico=="true" && pteAprobacionPublico=="false"){
			cell4.innerHTML="<img alt='permisos' src='images/interrogacion_publi.png' title='<spring:message code='jsp.usuarios.usuarioAsignarPermisos.elemento.publico'/>'/>";
		}
	}
	...
	function dibujarFilaCategoriaPub(idTabla, idCategoriaPadre, idCategoria, nombreCategoria, margenIzq)
	{
		...
		//Inserto una celda vacía para que la tabla no quede descompensada con Indicadores (celda indicadores públicos)
		var cell4 = row.insertCell(3);
		 ...
		 <c:forEach items="${mapaIndicadoresTabla}" var="entry">
		 	if (idCategoria == '${entry.key}')
		 	{
		 		<c:forEach items="${entry.value}" var="indicadorHijo">
					dibujarFilaIndicadorPub(idTabla, idCategoria, '${indicadorHijo.id}', '${indicadorHijo.nombre}','${indicadorHijo.urlPublicacion}', '${indicadorHijo.urlPublicacionCorta}', '${indicadorHijo.numUsuariosPublicacion}', '${indicadorHijo.loginUltimaPeticion}', margenIzq + 20, '${indicadorHijo.publico}', '${indicadorHijo.pteAprobacionPublico}');
				</c:forEach>
			}
		</c:forEach>
	}
