...
public class IndicadorDtoValidador extends ValidadorDtoBase<IndicadorDto> {
	...
	protected void aplicaValidacion(IndicadorDto dto,EncapsuladorErroresSW erros) {
		...
		if ((dto.getMinX()!=null) && (!dto.getMinX().matches("[0-9]*\\.[0-9]*"))) {
			erros.addError(IndicadorDtoFormErrorsEmun.ERROR_VALOR_NUMERICO_MINX);
		}
		if ((dto.getMinY()!=null) && (!dto.getMinY().matches("[0-9]*\\.[0-9]*"))) {
			erros.addError(IndicadorDtoFormErrorsEmun.ERROR_VALOR_NUMERICO_MINY);
		}
		if ((dto.getMaxX()!=null) && (!dto.getMaxX().matches("[0-9]*\\.[0-9]*"))) {
			erros.addError(IndicadorDtoFormErrorsEmun.ERROR_VALOR_NUMERICO_MAXX);
		}
		if ((dto.getMaxY()!=null) && (!dto.getMaxY().matches("[0-9]*\\.[0-9]*"))) {
			erros.addError(IndicadorDtoFormErrorsEmun.ERROR_VALOR_NUMERICO_MAXY);
		}
	}
}