...
public class ExpresionDtoValidador extends ValidadorDtoBase<ExpresionDto> {
	...
	protected void aplicaValidacion(ExpresionDto dto,EncapsuladorErroresSW erros) {
		...
		if((!dto.getTipoOperacion().equals(TipoOperacionEmun.ABS) && !dto.getTipoOperacion().equals(TipoOperacionEmun.AREA)&&!dto.getTipoOperacion().equals(TipoOperacionEmun.LONGITUD)) && dto.getTipoOperandoDch().equals(TipoOperandoEmun.SIN_OPERANDO)){
			erros.setHashErrors(true);
			return;
		}
		...
		if((dto.getTipoOperacion().equals(TipoOperacionEmun.AREA)||dto.getTipoOperacion().equals(TipoOperacionEmun.LONGITUD)||dto.getTipoOperacion().equals(TipoOperacionEmun.TRANSFORMANULO))&& !dto.getTipoOperandoIzq().equals(TipoOperandoEmun.FUENTE_DATOS)){
			erros.setHashErrors(true);
			return;
		}
