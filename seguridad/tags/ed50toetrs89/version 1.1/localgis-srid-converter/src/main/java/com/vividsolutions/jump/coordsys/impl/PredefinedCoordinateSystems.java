...
public class PredefinedCoordinateSystems {
	private static PrecisionModel precisionModelGeographic = getGeometryFactory(
			100000000).getPrecisionModel();
	private static PrecisionModel precisionModelUTM = getGeometryFactory(100)
			.getPrecisionModel();

	public static final CoordinateSystem BC_ALBERS_NAD_83 = new CoordinateSystem(
			"BC Albers", 42102, "North_American_Datum_1983", 0,
			precisionModelUTM);
	public static final CoordinateSystem GEOGRAPHICS_WGS_84 = new CoordinateSystem(
			"Geographics WGS84", 4326, "WGS84", 0, precisionModelGeographic);
	public static final CoordinateSystem GEOGRAPHICS_ETRS89 = new CoordinateSystem(
			"Geographics ETRS89", 4258, "ETRS89", 0, precisionModelGeographic);
	public static final CoordinateSystem GEOGRAPHICS_ED50 = new CoordinateSystem(
			"Geographics ED50", 4230, "ED50", 0, precisionModelGeographic);
	...
	public static CoordinateSystem createUTMNorth(final int zone) {
		...
		return new CoordinateSystem("UTM " + (zone < 10 ? "0" : "") + zone
				+ "N", 32600 + zone, "WGS84", zone, precisionModelUTM);
	}
	...
	public static CoordinateSystem createED50(final int zone) {
		...
		// Pad with zero to facilitate sorting [Jon Aquino]
		return new CoordinateSystem("UTM " + (zone < 10 ? "0" : "") + zone
				+ "N ED50", 23000 + zone, "ED50", zone, precisionModelUTM);
	}
	...
	public static CoordinateSystem createETRS89(final int zone) {
		...
		// Pad with zero to facilitate sorting [Jon Aquino]
		return new CoordinateSystem("UTM " + (zone < 10 ? "0" : "") + zone
				+ "N ETRS89", 25800 + zone, "ETRS89", zone, precisionModelUTM);
	}

	public static GeometryFactory getGeometryFactory(int precision) {
	...
	public static CoordinateSystem getCoordinateSystem(int epsgCode) throws ParseException {
		...
		if (epsgCode == GEOGRAPHICS_WGS_84.getEPSGCode()) {
			...
			throw new ParseException("Sistema de proyección desconocido");
		}
