...
public class UTMED50ToGeoETRS89Converter {
				...
				if (cmd.hasOption("t")) {
					uTMED50ToGeoED50Converter
							.transformDataBase(
									...
									properties.getProperty(UNIQUE_KEY),properties
									.getProperty(DROP_CONSTRAINT) , con);
					...
					Ed50toetrs89Utils.dropBackupColumn(con, Arrays.asList(properties
							.getProperty(DDBB_GEOM_COLUMNS_TRANSFORM).split(";")));
	...
	private void transformDataBase(String columnsToTransform,
			String tablesNotIncludesTransform, String uniqueKeys, String dropConstraint, Connection con)
		...
		String[] listDropConstraint = dropConstraint.split(";");
					...
					if (tableConversion) {
						...
						try {
							makeConversionWithListKeys(con, tabla,
									geometryColumnName, listUniqueKeys, listDropConstraint);
	...
	private static void makeConversionWithListKeys(Connection con,
			String table, String geometryColumnName, String[] listUniqueKeys, String[] listDropConstraint)
			...
			try {
				...
				if (e.getMessage().contains("no existe la columna")) {
					UTMED50ToGeoETRS89Converter.printMessage("La tabla "
							+ table + " no tiene la columna de clave única "
							+ listUniqueKeys[m]);
				} else {
					e.printStackTrace();
					printMessage(Messages
