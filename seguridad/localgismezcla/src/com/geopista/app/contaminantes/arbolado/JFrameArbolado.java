...
public class JFrameArbolado extends JInternalFrameShowMap implements LayerListener {
	...
	private void salvar() {
		...
		try {
			...
			CResultadoOperacion resultadoOperacion = new OperacionesContaminantes(com.geopista.app.contaminantes.init.Constantes.url).saveArbolado(arbolado);
			if (!resultadoOperacion.getResultado()) {
				logger.warn("resultadoOperacion.getDescripcion(): " + resultadoOperacion.getDescripcion());
				return;
			}
            new OperacionesContaminantes((com.geopista.app.contaminantes.init.Constantes.url)).gestionarHistorico(new Historico(new Integer(arbolado.getId()).intValue(), com.geopista.app.contaminantes.init.Constantes.MODIFICAR, com.geopista.app.contaminantes.init.Constantes.ARBOLEDAS));
	...
	private void delete(boolean bMostrarMensaje) {
		...
		try {
			...
			arbolado.setObs(jTextAreaObs.getText());
			arbolado.setExt(jTextFieldExtension.getNumber().floatValue());
			CResultadoOperacion resultadoOperacion = new OperacionesContaminantes(com.geopista.app.contaminantes.init.Constantes.url).deleteArbolado(arbolado);
			if (!resultadoOperacion.getResultado()) {
				logger.warn("resultadoOperacion.getDescripcion(): " + resultadoOperacion.getDescripcion());
			}
            ...
            new OperacionesContaminantes((com.geopista.app.contaminantes.init.Constantes.url)).gestionarHistorico(new Historico(new Integer(arbolado.getId()).intValue(), com.geopista.app.contaminantes.init.Constantes.BORRAR, com.geopista.app.contaminantes.init.Constantes.ARBOLEDAS));
		} catch (Exception ex) {
	...
	private void initComponents() throws CancelException{//GEN-BEGIN:initComponents
        ...
        jComboBoxTipo = new ComboBoxEstructuras(com.geopista.app.contaminantes.estructuras.Estructuras.getListaTipoZonaArbolada(),
                          null,com.geopista.app.contaminantes.init.Constantes.Locale);
			...
			public void featureAdded(FeatureEvent e) {
				...
				for (Iterator i = features.iterator(); i.hasNext();) {
                    ...
                    {	JOptionPane optionPane = new JOptionPane(messages.getString("JFrameContaminantes.insertError"), JOptionPane.ERROR_MESSAGE);
                        ...
                        try
                        {
                            new OperacionesContaminantes((com.geopista.app.contaminantes.init.Constantes.url)).gestionarHistorico(new Historico(new Integer(arbolado.getId()).intValue(), com.geopista.app.contaminantes.init.Constantes.ADD, com.geopista.app.contaminantes.init.Constantes.ARBOLEDAS));
                        }catch(Exception ex){}
	...
	private void initListas() {
		...
		try {
			listaArbolados = (new OperacionesContaminantes(com.geopista.app.contaminantes.init.Constantes.url)).getArbolados();
    ...
    private void  generarFicha()
    {
        try
        {
            ...
            new GeopistaPrintable().printObjeto(FichasDisponibles.fichaarbolado, getArbolado(arboladoID) , Arbolado.class, getGeopistaEditor().getLayerViewPanel(), GeopistaPrintable.FICHA_CONTAMINANTE_ARBOLADO);
        }catch(Exception e)
        {
