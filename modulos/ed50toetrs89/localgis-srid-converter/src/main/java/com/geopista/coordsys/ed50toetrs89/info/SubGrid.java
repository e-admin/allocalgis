...
public class SubGrid {
	...
	/**
	 * Creates a SubGrid from the specified SubGrid name, parent file name,
	 * created date, updated date, south latitude, north latitude, east
	 * longitude, west longitude, latitude increase, longitude increase,
	 * subgrid's nodes count.
	 * 
	 * @param subName
	 *            the sub grid name.
	 * @param parent
	 *            the grid name if it exists.
	 * @param created
	 *            the date when the grid file was created.
	 * @param update
	 *            the date when the grid file was updated for the last time.
	 * @param lat1
	 *            the south (lower) latitude of the grid.
	 * @param lat2
	 *            the north (upper) latitude of the grid.
	 * @param long1
	 *            the east (more eastern) longitude of the grid.
	 * @param long2
	 *            the west (more western) longitude of the grid.
	 * @param latInc
	 *            the latitude increase of the grid.
	 * @param longInc
	 *            the longitude increase of the grid.
	 * @param gsCount
	 *            number of nodes in the grid.
	 */
	public SubGrid(String subName, String parent, String created,
			String update, Double lat1, double lat2, double long1,
			double long2, double latInc, double longInc, long gsCount) {
		super();
	 ...
	 * 
	 * @param l
	 *            the position in bytes to read in to the grid file.
	 */
	 ...
	 * 
	 * @param nodes
	 *            4 nodes to make the conversion.
	 */
	 ...
	 * 
	 * @param buf
	 *            {@link RandomAccessFile} to read the sub grid.
	 */
	public void readSubGrid(RandomAccessFile buf) {
		...
		try {
			...
			while (true) {
				...
				this.subName = String.valueOf(UtilsToReadGridFile
						.readChain(buf));
	 ...
	 * (non-Javadoc)
	 * 
	 * @see java.lang.Object#toString() pass to string all the sub grid's
	 * information.
	 */
	...
	/**
	 * Reads the nodes for make the transformation, using the posicion of this
	 * class.
	 * 
	 * @param bufin
	 *            file to read the nodes
	 * @param p
	 *            nodes to store the read data
	 * @param lonlat
	 *            longitude and latitude to read nodes.
	 */
	 ...
	 * @return true if the coordinate matches with the sub grid and false in any
	 *         other case.
	 */
	public boolean isInside(Double lon, Double lat) {
		return (lon <= eLong && lon >= wLong && lat <= nLat && lat >= sLat) ? true
				: false;
	}
