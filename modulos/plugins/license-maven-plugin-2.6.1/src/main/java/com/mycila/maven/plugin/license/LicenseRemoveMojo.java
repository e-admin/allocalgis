...
public final class LicenseRemoveMojo extends AbstractLicenseMojo {
    ...
    private void remove(Document document) {
		//System.out.println("Checking header from:"+document.getFile());
        document.parseHeader();		
        if (document.headerDetected()){

			//Guardamos el fichero original
			/*String newName = document.getFile().getName() + ".original_license";
			File copyNewName = new File(document.getFile().getParentFile(), newName);
			document.saveTo(copyNewName);*/
			...
			//System.out.println("No Checking header detected in:"+document.getFile());
			return;
		}
        if (!dryRun) {
            document.save();
