...
public final class HeaderParser {
    ...
    private int findBeginPosition() {
    	//System.out.println("Obteniendo posicion inicial");
        int beginPos = 0;
        line = fileContent.nextLine();
        //System.out.println("Linea:<"+line+">");
        if (headerDefinition.getSkipLinePattern() == null)
            return beginPos;
        //System.out.println("Siguiendo busqueda:");
        ...
        line = fileContent.nextLine();
        //System.out.println("Devolviendo:"+beginPos);
        return beginPos;
    }
    ...
    private boolean hasHeader() {
		...
		int originalBeginPosition=beginPosition;
		try{
			// skip blank lines
			//System.out.println("ENTERING: con "+originalBeginPosition);
			while (line != null && "".equals(line.trim()))
				line = fileContent.nextLine();
				
			//beginPosition=fileContent.getPosition();
			boolean headerEncontrado=false;
			while (!headerEncontrado && line!=null){
				//System.out.println("BUSCANDO EN:<"+line+">");
				if (headerDefinition.isFirstHeaderLine(line.trim())) {
					//System.out.println("Existe header en line");
					
					headerEncontrado=true;
				}
				else{
					beginPosition=fileContent.getPosition();
					//System.out.println("No Existe header en line");
					line=null;
					 line = fileContent.nextLine();
					 //System.out.println("LINE BUCLE:"+line);
					}
			}		
			posicionEncontrada=beginPosition;
			// check if there is already a header
			gotHeader = false;
			//System.out.println("Header Definition:" +" LINE:"+line);
			if (headerDefinition.isFirstHeaderLine(line)) {
				...
				do {
					...
					if (headerDefinition.isMultiLine() && headerDefinition.isLastHeaderLine(line)) {
						foundEnd = true;
						//System.out.println("END ENCONTRADO:"+line);
						break;
					}
				}
				//while (line != null && line.trim().startsWith(before.trim()));
				while (line != null && (line.trim().startsWith(before.trim()) ||
						(line.trim().startsWith("USA."))));
				...
				gotHeader = false;
				//System.out.println("A Punto de devolver");
				//System.out.println("InPlaceHeader:"+inPlaceHeader);
				for (String keyword : keywords) {
					//System.out.println("Keyword:"+keyword);
					
					if (inPlaceHeader.indexOf(keyword.toLowerCase()) != -1) {
						//System.out.println("Existe Header a borrar en:"+beginPosition);
						beginPosition=posicionEncontrada;
						gotHeader = true;						
						break;
					}
					else{						
						beginPosition=originalBeginPosition;
						//System.out.println("Devolviendo:"+beginPosition);
			...
			//System.out.println("GotHeader:"+gotHeader+ " con:"+beginPosition);
			return gotHeader;
		}
		finally{
