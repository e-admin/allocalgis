...
public class TSPCerradoPlugIn extends ThreadedBasePlugIn implements ThreadedPlugIn,
ActionListener {
	...
	private HashMap<Node,VirtualNodeInfo> nodeInfoVirtualList = null;
	...
	public boolean execute(PlugInContext context) throws Exception {
		...
		this.rutas_examinadas = 0;// número de rutas examinadas
		...
		if (tspPointsLayer != null){
			...
			if (graphsubred == null){
				...
				ventanaerror
				.addText("Error en la configuracion. Inicie el Configurador de Rutas");
		...
		if (!nodeInfoVirtualList.isEmpty()){
				((LocalGISNetworkManager)networkMgr).endUseOfVirtualNodes(TSPCerradoPlugIn.class);
		}
		return false;
	 ...
	 * por "," y la transforma en un vector de enteros que será el parametro
	 * devuelto
	 */
...
//				feature.setAttribute("nodeId", new Integer(node.getID()));
		...
		Layer nodesLayer = context.addLayer("TSP",
				"Nodos visitados en la ruta cíclica", nodesFeatureCol);
...
//		nodesLayer.addStyle(labelStyle);
	 ...
	 * start es el índice de un elemento en camino desde el cual se realizaran
	 * todas las permutaciones posibles n n-umero de elemento de camino
	 */
	...
	throws WrongPathException, ElementNotFoundException {
		...
		rutas_examinadas++;
		// monitor.report(rutas_examinadas,(camino.length),
		// "...Calculando la ruta cí½clica");
		System.out.println("");
		...
		for (int w = 0; w < camino.length; w++) {
			...
			if (source == null || end == null) {
				...
				if (path == null) {
					...
					TSPAlgorithm tspAlg = new TSPAlgorithm(networkMgr, path, false, null);
//					AStarFunctions afunc = new AStarFunctions(
//							end) {
//
//						public double cost(AStarNode n1, AStarNode n2) {
//							// Se pasan nodos contiguos
//							Edge edge = null;
//							;
//							edge = n1.getNode().getEdge(n2.getNode());
//							double peso = ((EdgeWithCost) edge).getCost();
//							if (peso < 0) {
//								System.out.print("Error en el peso");
//								throw new NegativeCostNotSupportedException("Offending edge "+edge);
//							} else {
//								return peso;
//							}
//						}
...
//
//							return NodeUtils.euclideanDistance(
//									(XYNode) n, (XYNode) end);
//						}
//
//						@Override
...
//
//					};
//					AStarShortestPathFinder pfAstar = new AStarShortestPathFinder(
//							graph, source, end, afunc);
//					pfAstar.calculate();
//					path = pfAstar.getPath();
					
					tsp.addAll(tspAlg.getTSP());
	...
	throws Exception {
		...
		if (ruta_fallida != 0) {
			...
			dibujarCaminos(subred, algorithm.getTSP(), coste_ruta_opt, monitor);
			...
			for(int i=0; i < algorithm.getTSP().size(); i++){
//			for (Iterator<RoutePath> itr = algorithm.getTSP().iterator(); itr.hasNext();){ï¿½
				monitor.report("Generando informe ruta: " + (i+1));
				Path path = algorithm.getTSP().get(i);
				VirtualNodeInfo startNodeInfo = null;
				VirtualNodeInfo endNodeInfo=null;
				if (path!=null && path.isValid() && !path.isEmpty()){
					...
					if(context.getWorkbenchFrame()!= null){
						context.getWorkbenchFrame().getOutputFrame().addText("<b>Camino " + (i+1) + "</b>");
	...
	public ImageIcon getIcon() {
		return IconLoader.icon(I18N.get("tsp","routeengine.calcularruta.iconfile_c"));
	}
	...
	public void addButton(final ToolboxDialog toolbox)
	{
		...
		{
			//			toolbox.addToolBar();
			TSPCerradoPlugIn explode = new TSPCerradoPlugIn();                 
