...
public class TSPAbiertoPlugIn extends ThreadedBasePlugIn implements ThreadedPlugIn,
ActionListener {
	...
	public boolean execute(PlugInContext context) throws Exception {
		...
		if (!nodeInfoVirtualList.isEmpty()){
				((LocalGISNetworkManager)networkMgr).endUseOfVirtualNodes(TSPAbiertoPlugIn.class);
		}
		return false;
...
//					path = pfAstar.getPath();
	...
	throws Exception {
		...
		if (ruta_fallida != 0) {
			...
			// eliminamos la ultima ruta para abierto
...
//			for (Iterator<RoutePath> itr = algorithm.getTSP().iterator(); itr.hasNext();){ï¿½
	...
	public ImageIcon getIcon() {
		return IconLoader.icon("tsp_open.png");
	}
	...
	public void addButton(final ToolboxDialog toolbox)
	{
		...
		{
			//			toolbox.addToolBar();
			TSPAbiertoPlugIn explode = new TSPAbiertoPlugIn();                 
