...
public class CalcularRutaClick extends ThreadedBasePlugIn {
	...
	public boolean execute(PlugInContext context) throws Exception {
		...
		if (configProperties.getRedesNames().length <= 0){
			context.getLayerViewPanel().getContext().warnUser("Error en la configuración. Inicie el configurador de rutas");
		}
	...
	public static void dibujarCamino(String Algoritmo, Path p,double coste_ruta,Integer numRuta) {
		...
		if (p != null && p.isValid()){
			...
			if (p.getEdges().size() > 2){
				...
				if (edgesFeatureCol == null){
					if (firstNodeInfo.getEdge() instanceof LocalGISDynamicEdge){
						edgesFeatureCol = EdgesFeatureCollections.getLocalGISDynamicEdgeFeatureCollection();
	 ...
	 * 	Si hay numero, realiza la siguiente operación. 
	 ...
	 *  	Cuando se agregue el nuevo tramo, agregará en donde estaba la ruta 1 , dos rutas nuevas. Por eso necesita dos
	...
	public void addButton(final ToolboxDialog toolbox)
	{
		...
		{
			//			toolbox.addToolBar();
			CalcularRutaClick explode = new CalcularRutaClick();                 
	...
	protected static void CalculateRouteTwoLastNodes(Integer routeSelected) throws ElementNotFoundException {
		...
		LocalGISNetworkManager netMngr = (LocalGISNetworkManager) FuncionesAuxiliares.getNetworkManager(context);
		try{
			...
			if (source != null && end != null){
				...
				try {
					...
					if (configProperties.getDisabilityType() != null && !configProperties.getDisabilityType().equals("")){
						weighter = new LocalGISPMRWeighter(Double.parseDouble(configProperties.getPavementWidth()), Double.parseDouble(configProperties.getTransversalSlope()), Double.parseDouble(configProperties.getLongitudinalSlope()));
					}
