...
public class RouteEngineDragDropPointTool extends NClickTool {
	private static final int PIXEL_RANGE = 5;
	private EnableCheckFactory checkFactory;

	public static Point pointToDrag = null;
	public static Point pointDroped = null;

	public RouteEngineDragDropPointTool(EnableCheckFactory checkFactory) {
		super(1);
	...
	private Coordinate modelClickFirstCoordinate() throws NoninvertibleTransformException {
		return (Coordinate) getCoordinates().get(0);
	}
	
	private Coordinate modelClickEndCoordinate() throws NoninvertibleTransformException {
	...
	private LineSegment segmentInRange(Geometry geometry, Coordinate target) {
		//It's possible that the geometry may have no segments in range; for example, if it
	...
	private Coordinate newVertex(LineSegment segment, Coordinate target) {
		...
		//No good to make the new vertex one of the endpoints. If the segment is
	...
	protected void gestureFinished() throws java.lang.Exception {
		...
		SegmentContext segment =
			findSegment(layerToFeaturesInRangeMap, modelClickFirstCoordinate());
		if (segment == null) {
		...
		EditTransaction transaction =
			new EditTransaction(
					...
					false,
					getPanel());
		final Coordinate newVertex = newVertex(segment.getSegment(), modelClickFirstCoordinate());
		final Coordinate dropedVertex = newVertex(segment.getSegment(), modelClickEndCoordinate());
		...
		transaction.commit(new EditTransaction.SuccessAction() {
			public void run() {
				try {
					...
					Animations.drawExpandingRing(
							getPanel().getViewport().toViewPoint(dropedVertex),
							false,
							Color.green,
							getPanel(),
							null);
	...
	private HashMap layerToFeaturesInRangeMap() throws NoninvertibleTransformException {
		...
		i.hasNext();
			...
			Collection featuresInRange = featuresInRange(modelClickFirstCoordinate(), editableLayer);
	...
	protected Point getPointToDrag() throws NoninvertibleTransformException {
		return new GeometryFactory().createPoint((Coordinate)getCoordinates().get(0));
	}	
	
	protected Point getPointDroped() throws NoninvertibleTransformException{
		return new GeometryFactory().createPoint((Coordinate)getCoordinates().get(getCoordinates().size()-1));
	}
