...
public class InfoRouteDialog extends JDialog{
	...
	private Collection<RouteStretchInterface> infoRoutesStretchBeansCollection = null;
	...
	public InfoRouteDialog(final PlugInContext context, String title, ArrayList<RouteStretchInterface> list)
	{
		...
		this.addComponentListener(new ComponentListener()
			...
			{
			    // restaura el cursor tool 
//			    if (context.getLayerViewPanel().getCurrentCursorTool() instanceof RouteEngineDrawPointTool)
//				    {
					// Reactiva el CursorTool
					context.getLayerViewPanel().setCurrentCursorTool(context.getLayerViewPanel().getCurrentCursorTool());
//				    }
	...
	private JList getInfoRouteJList(){
		if (infoRouteJList == null){
			...
			if (this.infoRoutesStretchBeansCollection!= null && !this.infoRoutesStretchBeansCollection.isEmpty()){
				Iterator<RouteStretchInterface> it = this.infoRoutesStretchBeansCollection.iterator();
				while(it.hasNext()){
...
//					 }
	...
	private void onZoomToRouteButtonDo() {
		if (this.pluginContext != null){
			...
			if (this.infoRoutesStretchBeansCollection!=null && !this.infoRoutesStretchBeansCollection.isEmpty()){
				Iterator<RouteStretchInterface> it = this.infoRoutesStretchBeansCollection.iterator();
				while(it.hasNext()){
					InfoRouteStretchBean actualStretchBean = (InfoRouteStretchBean) it.next();
					if (actualStretchBean!=null && actualStretchBean.getGeometries()!=null && !actualStretchBean.getGeometries().isEmpty()){
						geoArrayList.addAll(actualStretchBean.getGeometries());
					}
				}
	...
	private void generarInformeImprimibleRuta(Collection<RouteStretchInterface> infoRoutesStretchBeansCollection2){
		try{
			...
			if ((infoRoutesStretchBeansCollection2 != null) && !infoRoutesStretchBeansCollection2.isEmpty() ){
				...
				if (encontrado){          
					try {
						Map parametros = obtenerParametros(((LayerViewPanel)this.pluginContext.getLayerViewPanel()).createImage(200, 200),"hola \n /n hola \n /n");
