 ...
 * 
 */
public class CalcularRutaPlugIn extends ThreadedBasePlugIn implements ActionListener,
ItemListener {
	...
	private static Log LOG = LogFactory.getLog(CalcularRutaPlugIn.class);
...
//		progressDialog.setVisible(true);
	...
	private void generarInformeRuta(PlugInContext context, 
		...
		writeRoutePathInformation(context.getWorkbenchFrame().getOutputFrame(), path.getEdges(), path);
	}

	private void writeRoutePathInformation(HTMLFrame outputFrame, List<Edge> edges, Path p) {
...
//			e.printStackTrace();
	...
	public void run(TaskMonitor monitor, PlugInContext context) throws Exception {
		...
		ArrayList<IdEdgeNetworkBean> temporalIncidentes = (ArrayList<IdEdgeNetworkBean>) aplicacion.getBlackboard().get("temporalincidents");
		if (configProperties.getTipoVehiculo().equals("En Coche")){
			if(temporalIncidentes == null)
				weighter = new LocalGISStreetWeighter();
			else
				weighter = new LocalGISStreetWeighter(temporalIncidentes,networkMgr);
		} else{
			if(temporalIncidentes == null)
				weighter = new LocalGISWeighter();
			else
				weighter = new LocalGISWeighter(temporalIncidentes,networkMgr);
		}

		AStarFunctions afunc=null;

		String disabilityType = configProperties.getDisabilityType();
		if (disabilityType != null && !disabilityType.equals(""))
		    {
			weighter = new LocalGISPMRWeighter(configProperties);
		    }
