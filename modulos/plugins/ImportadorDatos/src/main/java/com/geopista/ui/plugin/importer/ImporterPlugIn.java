...
public class ImporterPlugIn extends AbstractPlugIn 
{
	...
	/**
	 * Al menos debe de haber creadas dos capas.
	...
	{	
		...
		context.getWorkbenchContext().getIWorkbench().getGuiComponent().getToolBar(appContext.getString("CalculateFeature.category")).addPlugIn(
	      ...
	      featureInstaller.addMainMenuItem(this,
	            new String[] { appContext.getI18nString("Tools"), I18N.get("ImporterPlugIn","ImporterPlugIn.ImporterEditorPanel.boton.importar") },
	            I18N.get("ImporterPlugIn","ImporterPlugIn.plugin.nombre"), false, null,
	            createEnableCheck(context.getWorkbenchContext()));
		...
		try
		{
			...
			try
			{
				ConstantesImporter.clienteImporter = new ImporterClient(
						appContext.getString(UserPreferenceConstants.LOCALGIS_SERVER_ADMCAR_SERVLET_URL) + ServletConstants.CSERVLETIMPORTER_SERVLET_NAME);
				ConstantesImporter.Locale = appContext.getString(UserPreferenceConstants.DEFAULT_LOCALE_KEY,"es_ES");

				try
				{
					ConstantesImporter.IdMunicipio=new Integer(appContext.getString(UserPreferenceConstants.DEFAULT_MUNICIPALITY_ID)).intValue();
				}
				catch (Exception e)
				{
					System.out.println("Valor de id municipio no valido:"+e.toString()+appContext.getString(UserPreferenceConstants.DEFAULT_MUNICIPALITY_ID));
					logger.error("Valor de id municipio no valido:"+e.toString()+appContext.getString(UserPreferenceConstants.DEFAULT_MUNICIPALITY_ID));
					System.exit(-1);
				}
			}
			catch(Exception e)
