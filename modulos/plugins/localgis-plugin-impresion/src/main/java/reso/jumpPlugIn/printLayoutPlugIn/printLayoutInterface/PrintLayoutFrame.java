...
public class PrintLayoutFrame extends JFrame
{
	...
	public void resetPage () {
		//Eliminamos componentes graficos de la pagina
		List lst = this.getPrintLayoutPreviewPanel().getPreviewPanel().getPage().getGraphicElement();
		for (int i = 0; i < lst.size(); i++)
			this.getPrintLayoutPreviewPanel().getPreviewPanel().getPage().remove((GraphicElements)lst.get(i));
		this.getPrintLayoutPreviewPanel().getPreviewPanel().repaint();
		System.gc();
	}

	public JLayeredPane getPageLegende() {
		return (this.getPrintLayoutPreviewPanel().getPreviewPanel().getPage().getPageForPrint());
	}
	...
	public PrintLayoutFrame(PlugInContext context)
	{
		...
		setIconImage(IconLoader.icon(I18NPlug.get(PrintLayoutPlugIn.name, "JUMPWorkbench.app-icon.gif")).getImage());
		setName(I18NPlug.get(PrintLayoutPlugIn.name,"reso.jumpPlugIn.printLayoutPlugIn.printLayoutInterface.PrintLayoutFrame.Name"));
		setTitle(I18NPlug.get(PrintLayoutPlugIn.name,"reso.jumpPlugIn.printLayoutPlugIn.printLayoutInterface.PrintLayoutFrame.Name"));
	...
	public void setMapsExtents()
	{
		...
		for (Iterator iter = elements.iterator(); iter.hasNext();)
		{
			...
			if (element instanceof MapFrame)
			{
				...
				Map parameters = extentManager.getParameters();
				parameters.put(UtilsPrintPlugin.PARAM_CONFIG_PAGE_COUNT, Integer.toString(getExtentManager().getExtentCount()));
				parameters.put(UtilsPrintPlugin.PARAM_CONFIG_CURRENT_PAGE, Integer.toString(getCurrentPage()+1));
