...
public class GraphicText extends GraphicElements {
	...
	private Border border = BorderFactory.createLineBorder(borderColor, borderThickness);
	...
	private Map paramSubstitutions = null;
	...
	public void initCornerPoint() {
		...
		BottomRightCorner = new Point((int) onScreen.getBounds().getWidth(), (int) onScreen.getBounds().getHeight());
	}
	...
	public void setUnderline(boolean isUnderline) {
		...
		setText(UtilsPrintPlugin.formatTextStyleUnderline(text, isUnderline));
	}
	...
	private Map getMapParamSubstitutions() {
		//Obtenemos parametros enviados para sustituciones
		if (paramSubstitutions == null && parent != null) 
			paramSubstitutions = parent.getExtentManager().getParameters();
		return paramSubstitutions;
	}
	 ...
	 * @return
	 */
	 private String substitutionsParameters(String value, Map substitutionMap)
	 {
		 String result = null;
		 Object objResult = null; 
		 try
		 {
			 if (value != null && substitutionMap != null && substitutionMap.containsKey(value)) {
				 objResult = substitutionMap.get(value);
				 //Si es un array son elementos de repeticion que se muestran en cada hoja (uno por cuadricula)
				 if ((objResult instanceof ArrayList)  && ((ArrayList) objResult).size() >= parent.getExtentManager().getCurrentExtent())
					 result = (String) ((ArrayList) objResult).get(parent.getExtentManager().getCurrentExtent());
				 else 
	  ...
	  * </ul>
	  * <br>
	  * @see GraphicText#getMapParamSubstitutions()
	  * @param text Text to set
	  */
	 public void setText(String text) {
		 this.text = text;
		 
		 //Eliminar efectos a aplicar una vez reemplazados los parametros: subrayado y mayusculas
		 String textProcess = UtilsPrintPlugin.formatTextStyleUnderline(text, false);
		 //Reemplazo parametros
		 textProcess = substitutionsParameters(textProcess, getMapParamSubstitutions());
		 //Restaurar estilos
		 textProcess = UtilsPrintPlugin.formatTextStyleUnderline(textProcess, this.isUnderline);
		 textProcess = ((this.isUpperCase)? textProcess.toUpperCase() : textProcess);
		 
		 //Actualizar vistas
		 onScreen.setText(textProcess);
		 forPrint.setText(textProcess);
