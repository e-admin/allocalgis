	...
	public class ScaleBarFrameOnScreen extends JComponent {
		...
		public void paint (Graphics g)
		{
			...
			if (render!=null)
			{
				...
				double scale;
				if (mf==null)
					scale=parent.getPlugInContext().getLayerViewPanel().getViewport().getScale();
				else
					scale=mf.getScale();
...
public class ScaleBarFrameForPrint extends JComponent {
	...
	public void resize(int newForPrintWidth, int oldForPrintWidth,
			int newForPrintHeight, int oldForPrintHeight, int widthOnScreen,int heightOnScreen) {
		forPrint.setBounds(
		...
		forPrint.setLocation((int) forPrint.getBounds().getX(), (int) forPrint.getBounds().getY());
		switch (parent.getZoomActif()) {
		case PrintLayoutFrame.PAGE_ENTIERE:
			onScreen.setBounds(
			...
			onScreen.setLocation((int) onScreen.getBounds().getX(), (int) onScreen.getBounds().getY());
			break;
		case PrintLayoutFrame.LARGEUR_PAGE:
			onScreen.setBounds(
					(int) forPrint.getBounds().getX(),
					(int) forPrint.getBounds().getY(), 
					(int) forPrint.getBounds().getWidth(), 
					(int) forPrint.getBounds().getHeight());
			onScreen.setLocation((int) forPrint.getBounds().getX(), (int) forPrint.getBounds().getY());
			break;
		}
		//		resizeIcon();
	...
	private MapFrame getMap(int number)
	{
		List elem;
		if (parent != null){
			elem=parent.getPage().getGraphicElement();
			int cont=1;
