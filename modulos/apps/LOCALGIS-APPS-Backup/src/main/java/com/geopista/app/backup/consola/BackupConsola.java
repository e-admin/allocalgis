...
public class BackupConsola {
     ...
     * args[3] --> Ruta destino de los backup comun
     * args[4] --> id de entidad o * en caso de backup completo
     * @param args
     */
	public static void main(String[] args) {
		switch  (args.length){
			...
			case 4:
				try{
					int idEntidad = Integer.parseInt(args[4]);
					BackupConsola backupConsola = new BackupConsola(args[0],args[1]);
			        backupConsola.execute(args[2],args[3],idEntidad, null, true);
				}catch(Exception ex){
					showHelp();
				}
				break;
			case 5:
				try{
					int idEntidad = Integer.parseInt(args[4]);
					BackupConsola backupConsola = new BackupConsola(args[0],args[1]);
			        backupConsola.execute(args[2],args[3],idEntidad, args[5], true);
				}catch(Exception ex){
					showHelp();
				}
				break;
	...
	private static void backupCompleto(String usuarioBD, String passwordBD, String rutaDestinoBackup){
		...
		for(Enumeration<Integer> e=listaAyuntamientos.keys();e.hasMoreElements();){
			...
			String rutaDestinoBackupComun=rutaDestinoBackup + File.separator + "comun_" + diaSemana;
			...
			try{
		        ...
		        backupConsola.execute(rutaDestinoBackupEntidad, rutaDestinoBackupComun, idEntidad,null, crearBackupComun);
		        crearBackupComun = false;
		        //backupConsola = null; //Ayudar GC
				System.out.println(getLogHeader()+ "[FIN] OK: "+ rutaDestinoBackupEntidad);
				System.out.println(getLogHeader()+ "[FIN] OK: "+ rutaDestinoBackupComun);
			}catch(Throwable th){
				System.out.println(getLogHeader()+ "[FIN] ERROR: "+ rutaDestinoBackupEntidad);
				System.out.println(getLogHeader()+ "[FIN] ERROR: "+ rutaDestinoBackupComun);
				th.printStackTrace();
			}
		}
		System.out.println("Backup finalizado con EXITO");
        backupConsola = null; //Ayudar GC
        System.exit(0);
	}
    ...
    public void execute( String rutaDestinoBackupEntidad, String rutaDestinoBackupComun,int idEntidad,String rutaPostgres, boolean crearBackupComun) throws IOException, InterruptedException {
    	//*******************************
        ...
        ResourceBundle bundle = ResourceBundle.getBundle("com.geopista.app.backup.language.BackupSupraMunicipali18n", loc, this.getClass().getClassLoader());
        ...
        try {
        	...
        	String directorioComun = "";
        	
    		directorioEntidad = rutaDestinoBackupEntidad.replace("\\", "\\\\");
       
            File directorioBaseEntidad= new File(directorioEntidad);
            deleteFiles(directorioBaseEntidad);
            try{directorioBaseEntidad.mkdirs();}catch(Exception ex){}
            
            if (crearBackupComun){
            	directorioComun = rutaDestinoBackupComun.replace("\\", "\\\\");

            	File directorioBaseComun= new File(directorioComun);
            	deleteFiles(directorioBaseComun);
            	try{directorioBaseComun.mkdirs();}catch(Exception ex){}
            }
        		
    		File logBackup = new File(directorioEntidad, BackupConfig.FILELOG);
    		logBackup.createNewFile();
    		logger = new PrintStream(new FileOutputStream(logBackup, false), true);
		
    		File error = new File(directorioEntidad, BackupConfig.FILEERROR);
    		error.createNewFile();
            errorLog = new PrintStream(new FileOutputStream(error, false), true);

    		backupEntidad = new File(directorioEntidad, BackupConfig.FILEENTIDAD+BackupConfig.FILEEXTENSION);
            ...
            backupEntidad_UTF8 = new File(directorioEntidad, BackupConfig.FILEENTIDAD_UTF8+BackupConfig.FILEEXTENSION);
    		backupEntidad_UTF8.createNewFile();
            salidaEntidadSql_UTF8 = new GZIPOutputStream(new FileOutputStream(backupEntidad_UTF8, false));
            if (crearBackupComun){
            	backupComun_UTF8 = new File(directorioComun, BackupConfig.FILECOMUN_UTF8+BackupConfig.FILEEXTENSION);
             ...
             */
            if (crearBackupComun){
            	backupComun = new File(directorioComun, BackupConfig.FILECOMUN+BackupConfig.FILEEXTENSION);
            ...
            errorLog.println(getLogHeader()+"Ruta Entidad: "+rutaDestinoBackupEntidad);
            if (crearBackupComun){
            	errorLog.println(getLogHeader()+"Ruta Comun: "+rutaDestinoBackupComun);
            }
             ...
             */
    		InformacionTabla[] tablasComunes = null;
    		if (crearBackupComun){
    			logger.println(getLogHeader()+"Paso 1");
    			tablasComunes = backupAdapter.obtenerTablasComunes(connection,isVersion8);
    			logger.println(getLogHeader()+I18N.get("BackupSupraMunicipal", "geopista.app.backup.tablasComunes"));
    		}
    		else{
    			logger.println(getLogHeader()+"Paso 1: Ya se ha generado el backup comun, por lo que no se vuelve a generar");
    		}
			...
			logger.println(getLogHeader()+I18N.get("BackupSupraMunicipal", "geopista.app.backup.tablasMunicipio"));
	  	 	...
	  	 	if (crearBackupComun){
    			...
    			logger.println(getLogHeader()+"Paso 3: Ya se ha generado el backup comun, por lo que no se vuelve a generar");
    		}
	  	 	if (crearBackupComun){
    			...
    			logger.println(getLogHeader()+"Paso 3a: Ya se ha generado el backup comun, por lo que no se vuelve a generar");
    		}
	  	 	if (crearBackupComun){
    			...
    			logger.println(getLogHeader()+"Paso 3b: Ya se ha generado el backup comun, por lo que no se vuelve a generar");
    		}
	  	 	if (crearBackupComun){
    			...
    			logger.println(getLogHeader()+"Paso 3c: Ya se ha generado el backup comun, por lo que no se vuelve a generar");
    		}
	        /*
	         ...
	         */
	  	 	if (crearBackupComun){
    			...
    			logger.println(getLogHeader()+"Paso 3d: Ya se ha generado el backup comun, por lo que no se vuelve a generar");
    		}    
