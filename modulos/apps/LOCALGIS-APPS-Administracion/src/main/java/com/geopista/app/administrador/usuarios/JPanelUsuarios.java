...
public class JPanelUsuarios extends javax.swing.JPanel {
	...
	public final static int TAMANIO_MINIMO_PASSWORD = 8;
	public final static int TAMANIO_MINIMO_USER = 7;
	/**
			...
			for (Enumeration e = listaGruposPermisos.gethGroupPermisos()
					...
					try{
						if (auxGrupo.getBotonRol().isSelected()) {
							...
							if (auxGrupo.getBotonPermitir().isSelected() || auxGrupo.getBotonDenegar().isSelected()){
								Permiso auxPermiso = auxUsuario.getPermisos().get(auxGrupo.getIdPerm(),
										aclSelected.getId());	
								
								if(auxPermiso == null){									
									auxPermiso = new Permiso(auxGrupo.getIdPerm(),
											aclSelected.getId(), auxGrupo.getBotonPermitir().isSelected());
									auxUsuario.getPermisos().add(auxPermiso);
								}
								else{
									auxPermiso.setAplica(auxGrupo.getBotonPermitir().isSelected());							
								}
							}
						}
								
//								if (auxGrupo.getBotonDenegar().isSelected()){
//	//								auxUsuario.getPermisos().remove(auxGrupo.getIdPerm(),
//	//										aclSelected.getId());
//	
//									Permiso permiso = auxUsuario.getPermisos().get(auxGrupo.getIdPerm(),
//											aclSelected.getId());
//									if(permiso != null){
//										Permiso auxPermiso = new Permiso(auxGrupo.getIdPerm(),
//												aclSelected.getId(), true);
//										
//									}
//									permiso.setAplica(false);
//								}
//								else{
//									Permiso auxPermiso = new Permiso(auxGrupo.getIdPerm(),
...
//									auxUsuario.getPermisos().add(auxPermiso);
//									
//	//								if(auxGrupo.getBotonPermitir().isSelected()){
//	//									auxUsuario.getPermisos().get(auxGrupo.getIdPerm(),
//	//											aclSelected.getId()).setAplica(true);
//	//								}
//								}
//							}
//						}
						...
						logger.error("Error al guardar los cambios:", ex);
					}
			}
		} catch (Exception e) {
	...
	private void inicializaComboAcl() {
		if(jComboBoxACLs != null && jComboBoxACLs.getItemCount()>0)
			jComboBoxACLs.setSelectedIndex(0);
	}

	private void inicializaComboEntidades() {
		if(jComboBoxEntidades != null && jComboBoxEntidades.getItemCount()>0)
	...
	public void aceptarActionPerformed() {
		try {
			...
			if (this.jTextFieldUsuNombre.getText().toUpperCase().equals("SYSSUPERUSER")){
				...
				//la entidad 0
				if (auxUsuario.getId_entidad()!=null && !auxUsuario.getId_entidad().equals("0"))
					if (((Entidad) this.jComboBoxEntidades.getSelectedItem()).getId()
							.equals("")) {
							JOptionPane optionPane = new JOptionPane(mensajesUsuario
			...
			if (result.getResultado()) {
				...
				actualizarModelo();
				try{
					jTableUsuarios.getSelectionModel().setSelectionInterval(filaSeleccionada, filaSeleccionada);
				}
				catch(Exception e){}
			} else {
