...
import com.geopista.app.administrador.sigm.CSigmFrame;
import com.geopista.app.administrador.usuarios.CUsuariosFrame;
import com.geopista.app.catastro.gestorcatastral.CAcercaDeJDialog;
 ...
 * @author avivar
 */
public class CMainAdministrador extends CMain implements ISecurityPolicy{
    ...
    public CMainAdministrador() {
  	...
  	public CMainAdministrador(boolean fromInicio) {	
  		aplicacion.setUrl(aplicacion.getString(UserPreferenceConstants.LOCALGIS_SERVER_URL) + WebAppConstants.ADMINISTRACION_WEBAPP_NAME);
...
//				Constantes.url = AppContext.getApplicationContext().getString(SSOAuthManager.SSO_SERVER_URL);			
            ...
            /*if (!AppContext.seleccionarMunicipio((Frame)this)){
	            ...
	            Constantes.idEntidad = Integer.parseInt(((ISesion)AppContext.getApplicationContext().getBlackboard().get(UserPreferenceConstants.SESION_KEY)).getIdEntidad());
	            com.geopista.security.SecurityManager.setIdEntidad(String.valueOf(Constantes.idEntidad));
	...
	*/            
	            ...
	            //BORRAR
	            //jMenuItemGestionSigmActionPerformed();
        }catch(Exception ex)
        {
	...
	private boolean configureApp() {
		...
		try {
			...
			//*******************************************************
			try{PropertyConfigurator.configureAndWatch("log4j.ini", 3000);}catch(Exception e){};
    ...
    private void initComponents() {//GEN-BEGIN:initComponents
       ...
       jMenuItemGestionDominios.addActionListener(new java.awt.event.ActionListener() {
        ...
        jMenuGestionSigm = new JMenu();
        jMenuItemGestionSigm = new JMenuItem();
        
        jMenuGestionSigm.setMnemonic('S');
        jMenuItemGestionSigm.setMnemonic('s');
        jMenuItemGestionSigm.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
            	jMenuItemGestionSigmActionPerformed();
            }
        });
        jMenuGestionSigm.add(jMenuItemGestionSigm);
        menuBar.add(jMenuGestionSigm);      
        ...
        castellanoJMenuItem.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
            	castellanoJMenuItem.setEnabled(false);
...
//        }
    ...
    private void jMenuItemGestionDominiosActionPerformed() {//GEN-FIRST:event_jMenuItemGestionDominiosActionPerformed
        ...
        {
            messages = ResourceBundle.getBundle("locale.administrador", new Locale(Constantes.LOCALE_CASTELLANO));
        }
    ...
    private void jMenuItemGestionEntidadesActionPerformed() {
        ...
        try {
            ...
            messages = ResourceBundle.getBundle("locale.administrador", new Locale(
    ...
    private void jMenuItemGestionSigmActionPerformed() {
        if (desktopPane.getAllFramesInLayer(JDesktopPane.getLayer(new JInternalFrame())).length > 0)
            return;
        Container c = this.getContentPane();
        c.setCursor(Cursor.getPredefinedCursor(Cursor.WAIT_CURSOR));
        ...
        sigmFrame = new CSigmFrame(messages, this);
        sigmFrame.editable(aclAdministracion.checkPermission(new com.geopista.security.GeopistaPermission(com.geopista.security.GeopistaPermission.EDITAR_ADMINITRACION)));
        
        mostrarJInternalFrame(sigmFrame);
        c.setCursor(Cursor.getPredefinedCursor(Cursor.DEFAULT_CURSOR));
        c.setCursor(Cursor.getDefaultCursor());
    }
	...
	*/
	...
	private boolean setLang(String locale) {
		...
		try {
			...
			castellanoJMenuItem.setEnabled(true);
			catalanJMenuItem.setEnabled(true);
			euskeraJMenuItem.setEnabled(true);
			gallegoJMenuItem.setEnabled(true);
			valencianoJMenuItem.setEnabled(true);
			
			if (locale.equals(Constantes.LOCALE_CASTELLANO))
				castellanoJMenuItem.setEnabled(false);
			else if (locale.equals(Constantes.LOCALE_CATALAN))
				catalanJMenuItem.setEnabled(false);
			else if (locale.equals(Constantes.LOCALE_EUSKEDA))
				euskeraJMenuItem.setEnabled(false);
			else if (locale.equals(Constantes.LOCALE_GALLEGO))
				gallegoJMenuItem.setEnabled(false);
			else if (locale.equals(Constantes.LOCALE_VALENCIANO))
				valencianoJMenuItem.setEnabled(false);
	...
	private boolean changeScreenLang(ResourceBundle messages) {
        ...
        jMenuGestionSigm.setText(messages.getString("CMainAdministrador.jMenuGestionSigm"));
        jMenuItemGestionSigm.setText(messages.getString("CMainAdministrador.jMenuItemGestionSigm"));        
    ...
    private JMenu jMenuGestionSigm;
    private JMenuItem jMenuItemGestionSigm;
