...
import com.geopista.app.contaminantes.panel.JDialogBuscarArbolado;
import com.geopista.app.contaminantes.panel.JDialogBuscarContaminantes;
import com.geopista.app.contaminantes.panel.JDialogBuscarVertedero;
...
public class JFrameGeneracionPlanos extends javax.swing.JInternalFrame {
	...
	private boolean configureComponets () throws CancelException{
        ...
        if ((geopistaEditor.getLayerManager().getLayers() != null) && (geopistaEditor.getLayerManager().getLayers().size() > 0)){
            ...
            if (ShowMaps.showMap(com.geopista.app.contaminantes.Constantes.url,geopistaEditor, idMap, false,padre,null)!=null){
                layerActividades=(GeopistaLayer)geopistaEditor.getLayerManager().getLayer(layerNameActividades);
                layerVertederos=(GeopistaLayer)geopistaEditor.getLayerManager().getLayer(layerNameVertederos);
                layerArboleda=(GeopistaLayer)geopistaEditor.getLayerManager().getLayer(layerNameArboleda);
                ShowMaps.allLayersVisible(geopistaEditor);
    ...
    private void generar() {
        try
        {
            ...
            if (plantilla!=null)
                new GeopistaPrintableContaminantes().printObjeto(plantilla, null , null, geopistaEditor.getLayerViewPanel(), GeopistaPrintableContaminantes.FICHA_CONTAMINANTE_PLANOS);
            else
            {
    ...
    private void generarFicha() {
		...
		try {
			...
			if (jRadioButtonContaminantes.isSelected() && actividad != null) {
				...
				new GeopistaPrintableContaminantes().printObjeto(FichasDisponibles.fichacontaminante, actividad , Contaminante.class, geopistaEditor.getLayerViewPanel(), GeopistaPrintableContaminantes.FICHA_CONTAMINANTE_PLANOS);
			} else if (jRadioButtonVertederos.isSelected() && vertedero != null) {
                new GeopistaPrintableContaminantes().printObjeto(FichasDisponibles.fichavertedero, vertedero , Vertedero.class, geopistaEditor.getLayerViewPanel(), GeopistaPrintableContaminantes.FICHA_CONTAMINANTE_PLANOS);
   			} else if (jRadioButtonArboledas.isSelected() && zona_arbolada!=null) {
                new GeopistaPrintableContaminantes().printObjeto(FichasDisponibles.fichaarbolado, zona_arbolada , Arbolado.class, geopistaEditor.getLayerViewPanel(), GeopistaPrintableContaminantes.FICHA_CONTAMINANTE_PLANOS);
			}
		} catch (Exception ex) {
