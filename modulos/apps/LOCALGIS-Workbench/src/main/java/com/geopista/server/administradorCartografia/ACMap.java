...
public class ACMap implements Serializable, IACMap{
    ...
    public ACMap(IGeopistaMap gpMap) throws IOException{
        ...
        this.MapServerLayers = new ArrayList<ACWMSLayer>();
        ...
        for (int i=0; i<lstLayerablesOriginal.size(); i++){
        	...
        	}else if (anObjectTemp instanceof ExternalLayer){
        		ExternalLayer exLayer = (ExternalLayer) anObjectTemp;
        		ACWMSLayer acwmsLayer=new ACWMSLayer(exLayer, ++iFamilyPos);
        		MapServerLayers.add(acwmsLayer);
        		
        	}
        }
    }

    public ACMap(ACMapBase acMapSimple) {
		this.name=acMapSimple.getName();
		this.image=acMapSimple.getImage();
		this.id=acMapSimple.getId();
		this.xml=acMapSimple.getXml();
		this.idMunicipio=acMapSimple.getIdMunicipio();
		this.layerFamilies=acMapSimple.getLayerFamilies();
		this.MapServerLayers=acMapSimple.getMapServerLayers();
		this.timeStamp=acMapSimple.getTimestamp();
		this.locale=acMapSimple.getLocale();
		this.idEntidad=acMapSimple.getIdEntidad();
	}
	...
	public GeopistaMap convert(Hashtable htLayers,Hashtable htStyleXMLs, Date hora, String sLocale){
        ...
        if (layerFamilies!=null){
            ...
            int posicion = 1;
            while (iFamilies>0){
            	ACLayerFamilyBase familyBase = (ACLayerFamilyBase) layerFamilies.get(new Integer(index));
            	...
            	// ex: If a layer is at the position 2 of the hashtable, it is the second layer shown to the user. 
            	if (familyBase!=null){
            		family = new ACLayerFamily(familyBase);
        ...
        if(MapServerLayers != null){
            ...
            for (Iterator it = MapServerLayers.iterator(); it.hasNext();){
            	...
            	if (object instanceof ACWMSLayer){
	                ...
	                ACLayerUtil.convert(acwmsLayer,layerManager);
            	}if (object instanceof ACWMSLayerBase){
                    ACWMSLayer acwmsLayer=null;
                    
	                acwmsLayer=new ACWMSLayer((ACWMSLayerBase)object);
	                ...
	                ACLayerUtil.convertWMS(dynamicLayer,(LayerManager)layerManager,features);
            	}
            	//TRATAMIENTO PARA CAPAS EIEL
            	/*else if(acwmsLayer.getService().equals(ACWMSLayer.EIEL_SERVICE)){//CAPA EXTERNA
                	try{
                	EIELLayer eielLayer = acwmsLayer.convert ();
                 ...
                 LoadEIELDataProxy eielProxy=new LoadEIELDataProxy();
   		    	 ...
   		    	 System.out.println("EIEL: Vamos a cargar una capa de la EIEL, cuyo idMunicipio es: "+idMunicipio);
	    		 ...
	    		 if(completeEIELLayer!=null){
	    			 final ExternalLayer externalLayer=new ExternalLayer(
	    			  ...
	    			  boolean isFiringEvents1 = layerManager.isFiringEvents();
	    			  layerManager.setFiringEvents(false);
	    			  externalLayer.setVisible(acwmsLayer.isVisible());
	    	          layerManager.setFiringEvents(isFiringEvents1);
	    			
	    			 
	    			 final List styles=completeEIELLayer.getStyle();
	 
	    		            	//añadimos la capa al gestor de capas
	    			 			layerManager.addCategory(externalLayer.getName(), acwmsLayer.getPosition()-1);
	    		                layerManager.addLayerable(externalLayer.getName(), externalLayer);
                	...
                	e.printStackTrace();
                	System.out.println("Se ha producido un error en la llamada al servicio de capas externas, éstas no se añadirán al mapa");
                }
                }//FIN CAPA EXTERNA
                */
