...
public class FeatureDrawingUtil {
    ...
    public FeatureDrawingUtil(LayerNamePanelProxy layerNamePanelProxy) {
    	...
    	ResourceBundle bundle2 = ResourceBundle.getBundle("com.geopista.ui.cursortool.editing.languages.FeatureDrawingUtili18n",loc,this.getClass().getClassLoader());    	
        I18N.plugInsResourceBundle.put("FeatureDrawingUtil",bundle2);
    }
        ...
        if (layerNamePanelProxy.getLayerNamePanel().chooseEditableLayer() == null) {
            Layer layer =
                layerViewPanel.getLayerManager().addLayer(
                    ...
                    I18N.get("FeatureDrawingUtil","GeopistaAddNewLayerPlugIn.New"),
            ...
            layerViewPanel.getContext().warnUser(
            		I18N.get("FeatureDrawingUtil","NingunaCapaEditable"));
        }
        return layerNamePanelProxy.getLayerNamePanel().chooseEditableLayer();
    }
    
    private Layer getOrCreateLocal(LayerViewPanel layerViewPanel){
    	
    	Layer layerLocal=layerViewPanel.getLayerManager().getLayer(I18N.get("FeatureDrawingUtil","GeopistaAddNewLayerPlugIn.New"));
    	if (layerLocal==null){
	    	Layer layer =
	                layerViewPanel.getLayerManager().addLayer(
	                    StandardCategoryNames.WORKING,
    ...
    public UndoableCommand createAddCommand(
        ...
        if (rollingBackInvalidEdits && !geometry.isValid()) {
            layerViewPanel.getContext().warnUser(I18N.get("FeatureDrawingUtil","DrawFeatureToolTopologyError"));
            return null;
        }
        //Don't want viewport to change at this stage. [Jon Aquino]
        layerViewPanel.setViewportInitialized(true);
        
        //Si hay seleccionada alguna capa (solo una) intentamos escribir en dicha capa si esta editable
        Layer layerSelected=getSelectedLayer();
        
        final Layer layer;
        
        if (layerSelected!=null){
        	if (layerSelected.isEditable()){
        		layer=layerSelected;
        	}
        	else{
        		layer=getOrCreateLocal(layerViewPanel);
        	}
        }
        else{
	        //Obtenemos la capa donde tenemos que pintar si esta editable o creamos una nueva
	        layer = layer(layerViewPanel);	        	       
        }
    ...
    /**
     * De las capas seleccionada obtenemos la primera editable
     * @return
     */
    private Layer getSelectedLayer(){
    	    	
        Layer[] listaLayers=layerNamePanelProxy.getLayerNamePanel().getSelectedLayers();      
        for (int i=0;i<listaLayers.length;i++){
        	Layer layerTemp=listaLayers[i];
        	//if (layerTemp.isEditable())
        	return layerTemp;
        }
        return null;
    }
