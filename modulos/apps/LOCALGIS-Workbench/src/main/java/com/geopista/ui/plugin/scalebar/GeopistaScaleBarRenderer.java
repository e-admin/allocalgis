...
public class GeopistaScaleBarRenderer extends SimpleRenderer {
	...
	public GeopistaScaleBarRenderer(ILayerViewPanel panel) {
	...
	public static void setEnabled(boolean enabled, ILayerViewPanel panel) {
	...
	public void paint(Graphics2D g, double scale) {
		...
		RoundQuantity increment = new IncrementChooser().chooseGoodIncrement(new MetricSystem(1).createUnits(), idealIncrement / scale);
		if (paintingArea)
	...
	{	
		...
		try{
			if (((LayerViewPanel)panel).getParent() instanceof IPrintableZone)
			{
