...
public class FilteredAttributeTablePanel extends JPanel {
        ...
        public TableCellRenderer getCellRenderer(int row, int column) {
            if (!isEditButtonColumn(column)) {
                ...
                renderer.setBackground(
                    (FilteredAttributeTablePanel.this.getModel().getLayer().isEditable()
                        || ((row % 2) == 0))
                        ? Color.white
                        : LIGHT_GRAY);
     ...
     */
    public FilteredAttributeTablePanel(
                ...
                public void mouseClicked(MouseEvent e) {
                    try {
                        ...
                        if (isEditButtonColumn(column)) {
                        	...
                        	if (ComboEditor.currentCell!=null){
                        		ComboEditor.currentCell.editingCanceled(null);
                        	}
                        }
                    } catch (Throwable t) {
                        workbenchContext.getErrorHandler().handleThrowable(t);
    ...
    private FilteredAttributeTablePanel() {
        try {
            jbInit();
        } catch (Exception ex) {
            ex.printStackTrace();
     ...
     * @param listener 
     */
    public void addListener(FilteredAttributeTablePanelListener listener) {
    ...
    private void filterColumns() {
    	...
    	LayerTableModel layerTableModel = getModel();
    	
    	if (layerTableModel.getLayer().getFeatureCollectionWrapper().getFeatureSchema() instanceof GeopistaSchema)
    	{
	    	GeopistaSchema schema = (GeopistaSchema)
	    		layerTableModel.getLayer().getFeatureCollectionWrapper().getFeatureSchema();
    ...
    private boolean isColumnFiltered(int columnIndex){ 
    	...
    	if (layerTableModel.getLayer().getFeatureCollectionWrapper().getFeatureSchema() instanceof GeopistaSchema) 
    	{
    		GeopistaSchema schema = (GeopistaSchema)layerTableModel.getLayer().getFeatureCollectionWrapper().getFeatureSchema();
	    	String modelColumnName = layerTableModel.getColumnName(columnIndex);
	    	Column geopistaColumn = schema.getColumnByAttribute(modelColumnName);
	    	if (geopistaColumn != null){
	    		String geopistaColumnName = geopistaColumn.getName();
	    		if ((geopistaColumnName != null && attributeFilter.isBlackListed(geopistaColumnName))
	    				|| (geopistaColumnName == null && attributeFilter.isBlackListed(modelColumnName))){
	    			return true;  				
	    		}    		
	    	}
	    	else if (attributeFilter.isBlackListed(modelColumnName)){
	    		return true;	
	    	}
    	}
    ...
    private void fireSelectionReplaced() {
        for (Iterator i = listeners.iterator(); i.hasNext();) {
            FilteredAttributeTablePanelListener listener = (FilteredAttributeTablePanelListener) i.next();
            listener.selectionReplaced(this);
        }
    }
    private static class SelectionModelWrapper implements ListSelectionModel {
        private FilteredAttributeTablePanel panel;
        private ListSelectionModel selectionModel;
        public SelectionModelWrapper(FilteredAttributeTablePanel panel) {
            this.panel = panel;
	...
	private void setCellEditors(final Layer layer) {
		...
		if (layer.getFeatureCollectionWrapper().getFeatureSchema() instanceof GeopistaSchema) {
			MySchema = (GeopistaSchema) layer.getFeatureCollectionWrapper()
					.getFeatureSchema();
			MySchema.setGeopistalayer((GeopistaLayer) layer);
			Domain domain;
				...
				if (MySchema instanceof GeopistaSchema
					...
					if (domainType == Domain.CODEDENTRY
						...
						try {
							...
							ComboEditor elEditor=null;
							ComboRender elRender=null;
							if (EditorCombo.getCombo()!=null){
								elEditor = new ComboEditor(EditorCombo
										.getCombo(), EditorCombo.getValues());
								elRender = new ComboRender(EditorCombo
										.getCombo(), EditorCombo.getValues());
							}
