...
public class GeopistaPreferencesPanel  extends JPanel implements OptionsPanel
{
	...
	private int colorSeleccion = Integer.parseInt(UserPreferenceStore.getUserPreference("color.feature.seleccion","3",true));
	private double anchoMonitor = Double.parseDouble(UserPreferenceStore.getUserPreference("ancho.monitor","30.5",true));
	...
	private int fuente = Integer.parseInt(UserPreferenceStore.getUserPreference(UserPreferenceConstants.DEFAULT_MENU_FONT,"0",true));
	private String tamanno = UserPreferenceStore.getUserPreference(UserPreferenceConstants.DEFAULT_MENU_SIZE,"12",true);
	private int estilo = Integer.parseInt(UserPreferenceStore.getUserPreference(UserPreferenceConstants.DEFAULT_MENU_STYLE,"0",true));
	...
	public void okPressed() {
		...
		UserPreferenceStore.setUserPreference(UserPreferenceConstants.DEFAULT_FEATURE_SELECTED_COLOR,String.valueOf(cmbColorSeleccion.getSelectedIndex()));
		UserPreferenceStore.setUserPreference(UserPreferenceConstants.DEFAULT_MENU_FONT,String.valueOf(cmbFuente.getSelectedIndex()));
		UserPreferenceStore.setUserPreference(UserPreferenceConstants.DEFAULT_MENU_STYLE,String.valueOf(cmbEstilo.getSelectedIndex()));
		UserPreferenceStore.setUserPreference(UserPreferenceConstants.DEFAULT_MENU_SIZE,txtTamanno.getText());

		UserPreferenceStore.setUserPreference(UserPreferenceConstants.DEFAULT_UNIT_NAME,unitTextField.getText());
		UserPreferenceStore.setUserPreference(UserPreferenceConstants.DEFAULT_UNIT_EQUIVALENCE,equivalenceTextField1.getText());

		UserPreferenceStore.setUserPreference(SLDStyleImpl.SYMBOLIZER_SIZES_REFERENCE_SCALE,String.valueOf(pintadoEscalableValor));            
		appContext.getBlackboard().put(SLDStyleImpl.SYMBOLIZER_SIZES_REFERENCE_SCALE,pintadoEscalableValor);
		UserPreferenceStore.setUserPreference(UserPreferenceConstants.DEFAULT_SCREEN_WEIGHT,String.valueOf(anchoMonitorValor));    
		
		if (file!=null)// el usuario ha elegido un fichero específico
		{
			String path = UserPreferenceStore.getUserPreference(UserPreferenceConstants.PREFERENCES_DATA_PATH_KEY,UserPreferenceConstants.DEFAULT_DATA_PATH,false)+file.getName();
			GeopistaFunctionUtils.copyFile(new File(path),file);
			UserPreferenceStore.setUserPreference(UserPreferenceConstants.PREFERENCES_APPICON_KEY,file.getName());
			//appContext.getMainFrame().setIconImage(Toolkit.getDefaultToolkit().getImage(file.getAbsolutePath()));
		}
	}
	public void init() {
		appIcon=UserPreferenceStore.getUserPreference(UserPreferenceConstants.PREFERENCES_APPICON_KEY,null,false);
		if (appIcon==null)//icono por defecto
			lblFile.setText(AppContext.getMessage("GeopistaPreferencesPanel.DefaultIcon"));
		lblFile.setText(UserPreferenceStore.getUserPreference(UserPreferenceConstants.PREFERENCES_DATA_PATH_KEY,UserPreferenceConstants.DEFAULT_DATA_PATH,false)+
				UserPreferenceStore.getUserPreference(UserPreferenceConstants.PREFERENCES_APPICON_KEY,"",false));
		String escala=UserPreferenceStore.getUserPreference(SLDStyleImpl.SYMBOLIZER_SIZES_REFERENCE_SCALE,"null",false);            
	...
	boolean openFile()
	{
		...
		fc.setCurrentDirectory(new File(UserPreferenceStore.getUserPreference(UserPreferenceConstants.PREFERENCES_LAST_IMAGE_FOLDER_KEY,".",false)));
		...
		}else if( result == JFileChooser.APPROVE_OPTION)
		{
			...
			UserPreferenceStore.setUserPreference(UserPreferenceConstants.PREFERENCES_LAST_IMAGE_FOLDER_KEY,file.getPath());
		}else
		{
			return false;
		}
