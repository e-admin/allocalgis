...
public class GeopistaConnection implements Connection, IGeopistaConnection
{
    ...
    public GeopistaConnection(DriverProperties driverProperties)
        {
            ...
            String sUrlPrefix = aplicacion.getString(UserPreferenceConstants.LOCALGIS_SERVER_ADMCAR_SERVLET_URL);
	...
	public FeatureCollection executeQuery(String query, Collection exceptions, TaskMonitor monitor)
    {
        ...
        try
        {
            ...
            if (driverProperties.get(UserPreferenceConstants.VERSION) != null){
            	Version version = (Version)driverProperties.get(UserPreferenceConstants.VERSION);
            ...
            GeopistaLayer localLayer = administradorCartografiaClient.loadLayer(map, idLayer, UserPreferenceStore.getUserPreference(UserPreferenceConstants.DEFAULT_LOCALE_KEY, "es_ES", true), g, filter, srid_destino,monitor);
	...
	public FeatureCollection executeQueryLayer(String query, Collection exceptions, TaskMonitor monitor, IGeopistaLayer localLayer)
    {
        ...
        try
        {
            ...
            localLayer = administradorCartografiaClient.loadLayer(map, localLayer, UserPreferenceStore.getUserPreference(UserPreferenceConstants.DEFAULT_LOCALE_KEY, "es_ES", true), g, filter, true,srid_destino);
            this.layer = localLayer;
	...
	public List loadFeatures(IGeopistaLayer localLayer, Object[] featureIds)
    {
        try
        {
            ...
            if (driverProperties.get("srid_destino") instanceof Integer)
            	...
            	srid_destino = new Integer((String)driverProperties.get("srid_destino"));
            return administradorCartografiaClient.loadFeatures(localLayer,Arrays.asList(featureIds),UserPreferenceStore.getUserPreference(UserPreferenceConstants.DEFAULT_LOCALE_KEY, "es_ES", true),srid_destino);
        } catch (Exception e)
        {
            return null;
	...
	public ArrayList executeUpdate(String update, FeatureCollection featureCollection, TaskMonitor monitor) throws Exception
    {
        ...
        try {
			originalEPSG=(CoordinateSystem)driverProperties.get("selectedImportEPSG");
			if (originalEPSG!=null)
				driverProperties.put("srid_inicial",originalEPSG.getEPSGCode());
		} catch (Exception e1) {
		}
        ...
        if (driverProperties.get("srid_inicial") != null){
		    AppContext.getApplicationContext().getBlackboard().put(UserPreferenceConstants.SRID_INICIAL, driverProperties.get("srid_inicial"));
        }else
		    AppContext.getApplicationContext().getBlackboard().put(UserPreferenceConstants.SRID_INICIAL, null);
        ...
        String language = UserPreferenceStore.getUserPreference(UserPreferenceConstants.DEFAULT_LOCALE_KEY, "es_ES", true);

        ArrayList features = (ArrayList) administradorCartografiaClient.uploadFeatures(language, featureCollection.getFeatures().toArray(),
        																refreshFeatures, validateFeatures, monitor, srid_destino);
        return features;
