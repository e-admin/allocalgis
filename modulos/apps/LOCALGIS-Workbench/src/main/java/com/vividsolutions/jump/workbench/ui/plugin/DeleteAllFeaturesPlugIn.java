...
public class DeleteAllFeaturesPlugIn extends AbstractPlugIn {
	...
	public boolean execute(final PlugInContext context) throws Exception {
		...
		//Confirmar eliminar features
		if (confirmarEliminar(context) == true) 
		{
			...
			result = EditTransaction.commit(transactions);
		}
	...
	private EditTransaction createTransaction(Layer layer, PlugInContext context) {
		EditTransaction transaction = new EditTransaction(new ArrayList(),  getName(), layer, 
	...
	private boolean confirmarEliminar (PlugInContext plugInContext) {
		
		Object[] options = {"Si", "No"};
		int result = JOptionPane.showOptionDialog(
				plugInContext.getWorkbenchFrame(),
				AppContext.getApplicationContext().getI18nString("DeleteAllFeatures.mensajeConfirmacion"),
				AppContext.getApplicationContext().getI18nString("DeleteAllFeatures.title"),					
				JOptionPane.YES_NO_OPTION,
				JOptionPane.QUESTION_MESSAGE,null,     //do not use a custom Icon
		        options,  //the titles of buttons
		        options[1]); //default button title);

		boolean continuar = (result == JOptionPane.YES_OPTION);
