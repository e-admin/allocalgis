...
public class SchemaPanel extends JPanel {
    ...
    private EnableCheck basicEnableCheck = new EnableCheck() {
        public String check(JComponent component) {
            if (!isEditableAndSystemLayer(layer)) {
                return aplicacion.getI18nString("LayerEditableAndLocal");
            }
            if (rowsToActOn().length == 0) {
                return aplicacion.getI18nString("AtLeast1RowMustSelected");
            }
            return null;
        }
    };
    ...
    private MyPlugIn moveUpPlugIn = new MyPlugIn() {
        ...
        public MultiEnableCheck createEnableCheck() {
            return super.createEnableCheck().add(new EnableCheck() {
                public String check(JComponent component) {
                    return min(rowsToActOn()) == 0 ? aplicacion.getI18nString("FieldIsAlreadyAtTheTop") : null;
    ...
    private void updateComponents() {
        table.setShowGrid(isEditableAndSystemLayer(layer));
        table.setEnabled(isEditableAndSystemLayer(layer));
        applyButton.setEnabled(isEditableAndSystemLayer(layer));
        revertButton.setEnabled(isEditableAndSystemLayer(layer));
        forceInvalidConversionsToNullCheckBox.setEnabled(isEditableAndSystemLayer(layer));
    ...
    void jbInit() throws Exception {
        ...
        statusLabel.setText(" ");
        applyButton.setText(aplicacion.getI18nString("ApplyChanges"));
        ...
        forceInvalidConversionsToNullCheckBox.setToolTipText(
            aplicacion.getI18nString("LeaveUncheckedConversionErrors"));
        forceInvalidConversionsToNullCheckBox.setText(aplicacion.getI18nString("ForceInvalidConversionsToNull"));
        buttonPanel.setLayout(gridBagLayout3);
        buttonPanel.setBorder(BorderFactory.createLoweredBevelBorder());
        revertButton.setText(aplicacion.getI18nString("RevertChanges"));
    ...
    public String validateInput() {
        ...
        if (geometryCount() > 1) {
            return aplicacion.getI18nString("OnlyOneGeometryFieldIsAllowed");
        }

        if (geometryCount() == 0) {
            return aplicacion.getI18nString("GeometryFieldMustBeDefined");
        }
    ...
    private String validate(int row, String name) {
        if (name.trim().length() == 0) {
            return aplicacion.getI18nString("FieldNameCannotBeBlank");
        }
        ...
        for (int i = 0; i < getModel().getRowCount(); i++) {
            ...
            if (getModel().get(i).getName().equalsIgnoreCase(name.trim())) {
                return "FieldNameAlreadyExists" + name;
            }
        }
        ...
        public Component getTableCellRendererComponent(
            ...
            if (!isSelected) {
                ...
                component.setBackground(
                    (isEditableAndSystemLayer(layer) || ((row % 2) == 0)) ? Color.white : LIGHT_GRAY);
            }
    ...
    public static boolean isEditableAndSystemLayer(Layer layer)
    {
        if(!layer.isEditable()) return false;
        if(layer instanceof GeopistaLayer)
        {
            return ((IGeopistaLayer) layer).isLocal();
        }
        return true;
