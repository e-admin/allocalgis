...
import com.vividsolutions.jump.workbench.ui.renderer.style.EIELStyle.SizedStrokeFillStyle;

public abstract class EIELVertexStyle implements Style, SizedStrokeFillStyle {
    // UT
    ...
    protected EIELVertexStyle() {
    }

    // UT made RectangularShape shape a Shape
    protected EIELVertexStyle(Shape shape) {
        this.shape = shape;
    }
    ...
    public void initialize(ILayer layer) {
        ...
        fillColor = GUIUtil.alphaColor(((BasicStyle)layer.getBasicStyle()).getFillColor(), ((BasicStyle)layer.getBasicStyle()).getAlpha());
        strokeColor = GUIUtil.alphaColor(((BasicStyle)layer.getBasicStyle()).getLineColor(), ((BasicStyle)layer.getBasicStyle()).getAlpha());

    }

    public void paint(Feature f, Graphics2D g, IViewport viewport) throws Exception {
