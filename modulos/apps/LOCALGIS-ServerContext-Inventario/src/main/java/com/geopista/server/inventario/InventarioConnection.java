...
import com.geopista.app.inventario.LCGIII_DatosAmortizacionConst;
...
//    private static String sRevisionActual = "";
    ...
    public void returnInmuebles(ObjectOutputStream oos, String superpatron, String patron, 
    		...
    		ConfigParameters configParameters,boolean withAllData) throws Exception{
           try{
               for (Iterator it=getInmuebles(superpatron, patron, cadena, filtro, idLayers, idFeatures,null, 
            		   userSesion,withAllData,configParameters).iterator();it.hasNext();){
    ...
    public void returnMuebles(ObjectOutputStream oos, String superpatron, String patron, String cadena, Object[] filtro, Object[] idLayers, 
    		Object[] idFeatures, Sesion userSesion, ConfigParameters configParameters,boolean withAllData) throws Exception{
           try{
        	   boolean withData=false;
        	   if (withAllData)
        	   	withData=true;
        	   else
        		   withData=(idFeatures==null||idFeatures.length==0)?false:true;
               for (Iterator it=getMuebles(superpatron, patron, cadena, 
            		   filtro, idLayers, idFeatures,null, 
            		   userSesion, withData,configParameters).iterator();it.hasNext();){
    ...
    public void returnDerechosReales(ObjectOutputStream oos, String superpatron, String patron, String cadena, Object[] filtro, Object[] idLayers, Object[] idFeatures,
                                     Sesion userSesion, ConfigParameters configParameters,boolean withAllData) throws Exception{
           try{
    ...
    public void returnValoresMobiliarios(ObjectOutputStream oos, String superpatron, String patron, String cadena, Object[] filtro, Object[] idLayers, 
           ...
           try{
               for (Iterator it=getValoresMobiliarios(superpatron, patron, cadena,
            		   filtro, idLayers, idFeatures, null,userSesion, withAllData,configParameters).iterator();it.hasNext();){
    ...
    public void returnVias(ObjectOutputStream oos, String superpatron, String patron, String cadena, Object[] filtro, Object[] idLayers, 
           ...
           try{
               for (Iterator it=getVias(superpatron, patron, cadena, filtro, idLayers, 
            		   idFeatures, null,userSesion, withAllData,configParameters).iterator();it.hasNext();){
                   oos.writeObject((ViaBean)it.next());
               }
           }catch(Exception e){
...
//		                inmueble.setVersionado(true);
        ...
        if ((vNumInventarios.size()>0) && ((configParameters!=null) && !configParameters.isBusquedaIndividual())){
...
//            docConn.borrarFicherosEnDisco(bien.getId(), bien.getDocumentos());
     ...
     */
	 private Boolean eliminarTodoInventarioBienRevertible(Connection conn,Object obj) throws Exception{
		...
		String sSQLDeleteVersionesBienRevertibleBien=null;
		if (bien==null){
			sSQLDeleteVersionesBienRevertibleBien="delete from versiones where revision in " +
				"(select revision_actual FROM bien_revertible WHERE ID_MUNICIPIO='"+municipio+"'"+
				" group by revision_actual) and id_table_versionada="+TABLE_INVENTARIO_REVERTIBLES;			
		}
	    ...
	    try {
	        ...
	        if (sSQLDeleteVersionesBienRevertibleBien!=null){
		        ps= conn.prepareStatement(sSQLDeleteVersionesBienRevertibleBien);
	...
	private Boolean eliminarTodoInventarioBienes(Connection conn,Object obj) throws Exception{
		...
		String sSQLDeleteVersionesCreditoDerecho=null;
		String sSQLDeleteVersionesVehiculo=null;
		String sSQLDeleteVersionesInmuebleUrbanos=null;
		String sSQLDeleteVersionesInmuebleRusticos=null;
		String sSQLDeleteVersionesMuebles=null;
		String sSQLDeleteVersionesValoresMoviliarios=null;
		String sSQLDeleteVersionesViasUrbanas=null;
		String sSQLDeleteVersionesViasRusticas=null;
		String sSQLDeleteVersionesSemovientes=null;
		if (bien==null){
			sSQLDeleteVersionesCreditoDerecho="delete from versiones where revision in " +
				"(select revision_actual FROM bienes_inventario WHERE ID_MUNICIPIO='"+municipio+"' and tipo='"+Const.PATRON_CREDITOS_DERECHOS_PERSONALES+"'"+
				" group by revision_actual) and id_table_versionada="+TABLE_INVENTARIO_CREDITOS;			
			sSQLDeleteVersionesVehiculo="delete from versiones where revision in " +
					"(select revision_actual FROM bienes_inventario WHERE ID_MUNICIPIO='"+municipio+"' and tipo='"+Const.PATRON_VEHICULOS+"'"+
					" group by revision_actual) and id_table_versionada="+TABLE_INVENTARIO_VEHICULOS;			
			sSQLDeleteVersionesInmuebleUrbanos="delete from versiones where revision in " +
					"(select revision_actual FROM bienes_inventario WHERE ID_MUNICIPIO='"+municipio+"' and tipo='"+Const.PATRON_INMUEBLES_URBANOS+"'"+
					" group by revision_actual) and id_table_versionada="+TABLE_INVENTARIO_INMUEBLES;			
			sSQLDeleteVersionesInmuebleRusticos="delete from versiones where revision in " +
					"(select revision_actual FROM bienes_inventario WHERE ID_MUNICIPIO='"+municipio+"' and tipo='"+Const.PATRON_INMUEBLES_URBANOS+"'"+
					" group by revision_actual) and id_table_versionada="+TABLE_INVENTARIO_INMUEBLES;	
			sSQLDeleteVersionesMuebles="delete from versiones where revision in " +
					"(select revision_actual FROM bienes_inventario WHERE ID_MUNICIPIO='"+municipio+"' and tipo='"+Const.PATRON_MUEBLES_HISTORICOART+"'"+
					" group by revision_actual) and id_table_versionada="+TABLE_INVENTARIO_MUEBLES;	
			sSQLDeleteVersionesValoresMoviliarios="delete from versiones where revision in " +
					"(select revision_actual FROM bienes_inventario WHERE ID_MUNICIPIO='"+municipio+"' and tipo='"+Const.PATRON_VALOR_MOBILIARIO+"'"+
					" group by revision_actual) and id_table_versionada="+TABLE_INVENTARIO_VALORES;	
			sSQLDeleteVersionesViasUrbanas="delete from versiones where revision in " +
					"(select revision_actual FROM bienes_inventario WHERE ID_MUNICIPIO='"+municipio+"' and tipo='"+Const.PATRON_VIAS_PUBLICAS_URBANAS+"'"+
					" group by revision_actual) and id_table_versionada="+TABLE_INVENTARIO_VIAS;	
			sSQLDeleteVersionesViasRusticas="delete from versiones where revision in " +
					"(select revision_actual FROM bienes_inventario WHERE ID_MUNICIPIO='"+municipio+"' and tipo='"+Const.PATRON_VIAS_PUBLICAS_RUSTICAS+"'"+
					" group by revision_actual) and id_table_versionada="+TABLE_INVENTARIO_VIAS;	
			sSQLDeleteVersionesSemovientes="delete from versiones where revision in " +
					"(select revision_actual FROM bienes_inventario WHERE ID_MUNICIPIO='"+municipio+"' and tipo='"+Const.PATRON_SEMOVIENTES+"'"+
					" group by revision_actual) and id_table_versionada="+TABLE_INVENTARIO_SEMOVIENTES;
		}
	    ...
	    try {
	        ...
	        ps.close();
	        if (sSQLDeleteVersionesCreditoDerecho!=null){
		        ps= conn.prepareStatement(sSQLDeleteVersionesCreditoDerecho);
		        resultado=ps.execute();
		        ps.close();
	        }
	        ...
	        if (sSQLDeleteVersionesVehiculo!=null){
		        ps= conn.prepareStatement(sSQLDeleteVersionesVehiculo);
		        resultado=ps.execute();
		        ps.close();
	        }
		    ...
		    if (sSQLDeleteVersionesInmuebleUrbanos!=null){
		        ps= conn.prepareStatement(sSQLDeleteVersionesInmuebleUrbanos);
		        resultado=ps.execute();
		        ps.close();
	        }
		    ...
		    if (sSQLDeleteVersionesInmuebleRusticos!=null){
		        ps= conn.prepareStatement(sSQLDeleteVersionesInmuebleRusticos);
		        resultado=ps.execute();
		        ps.close();
	        }
		    ...
		    if (sSQLDeleteVersionesMuebles!=null){
		        ps= conn.prepareStatement(sSQLDeleteVersionesMuebles);
		        resultado=ps.execute();
		        ps.close();
	        }
		    ...
		    if (sSQLDeleteVersionesValoresMoviliarios!=null){
		        ps= conn.prepareStatement(sSQLDeleteVersionesValoresMoviliarios);
		        resultado=ps.execute();
		        ps.close();
	        }
		    ...
		    if (sSQLDeleteVersionesViasUrbanas!=null){
		        ps= conn.prepareStatement(sSQLDeleteVersionesViasUrbanas);
		        resultado=ps.execute();
		        ps.close();
	        }
		    if (sSQLDeleteVersionesViasRusticas!=null){
		        ps= conn.prepareStatement(sSQLDeleteVersionesViasRusticas);
		        resultado=ps.execute();
		        ps.close();
	        }
		    ...
		    if (sSQLDeleteVersionesSemovientes!=null){
		        ps= conn.prepareStatement(sSQLDeleteVersionesSemovientes);
		        resultado=ps.execute();
		        ps.close();
	        }
		    ...
		    resultado=ps.execute();
		    ps.close();
		    logger.info("borrado tabla observaciones:"+resultado);
...
//            insertInventarioFeature(valor.getId(), idLayers, idFeatures,sRevisionActual.replaceAll("\\?", "valor_mobiliario"), userSesion, conn);
    ...
    private synchronized void insertVersion(Connection conn, String table, Sesion userSesion) throws PermissionException,LockException,Exception{
    	...
    	ResultSet rs= null;
    	try{
    		StringBuffer sbSQL = new StringBuffer("select revision from versiones");
			sbSQL = sbSQL.append(" where revision=");
			sbSQL = sbSQL.append(this.sRevisionActual.replaceAll("\\?", table));
			sbSQL = sbSQL.append(" and id_table_versionada=");
			sbSQL = sbSQL.append("(select id_table from tables_inventario where name='");
			sbSQL = sbSQL.append(table);
			sbSQL = sbSQL.append("');");   
            ps= conn.prepareStatement(sbSQL.toString());
            rs= ps.executeQuery();
            if (!rs.next()){
        		sbSQL = new StringBuffer("insert into versiones (revision,id_autor,fecha,id_table_versionada) values (");
    		sbSQL = sbSQL.append(this.sRevisionActual.replaceAll("\\?", table));
    		sbSQL = sbSQL.append(",");
    		...
    		sbSQL = sbSQL.append("'));");    		
        		...
        		ps2.executeUpdate();
        		ps2.close();
        		logger.info("SQL insert version:"+sbSQL.toString());
        		logger.info("Version introducida para la tabla:"+table);
            }
            else{
            	logger.error("Ya existe el registro en la tabla. Procedemos a actualizar:"+table);
				sbSQL = new StringBuffer("update versiones set id_autor=");
				sbSQL = sbSQL.append(userSesion.getIdUser());
				sbSQL = sbSQL.append(",fecha=");
				...
				sbSQL = sbSQL.append(table);
				sbSQL = sbSQL.append("');");    
				logger.info("SQL update version:"+sbSQL.toString());
				logger.info("Version introducida para la tabla:"+table);
				ps2 = conn.prepareStatement(sbSQL.toString());
				ps2.executeUpdate();
				ps2.close();
    		...
    		logger.error("Error al introducir la actualizacion para la tabla. Abortamos el registro:"+table);    	
    	}finally{
...
//            insertInventarioFeature(semoviente.getId(), idLayers, idFeatures, sRevisionActual.replaceAll("\\?", "semoviente"), userSesion, conn);
    ...
    private void insertVia(Connection conn, ViaBean via, String revision, Sesion userSesion) throws PermissionException,LockException,Exception{
        ...
        try{
	        ...
	        ps.execute();
	        ps.close();
	        logger.info("SQL Ejecutando query Insert Via: "+((org.enhydra.jdbc.standard.StandardXAPreparedStatement)ps).ps.toString());
...
//            insertInventarioFeature(via.getId(), idLayers, idFeatures, sRevisionActual.replaceAll("\\?", "vias_inventario"),userSesion, conn);
    ...
    private void insertVehiculo(Connection conn, VehiculoBean vehiculo, String revision, Sesion userSesion) throws PermissionException,LockException,Exception{
        ...
        try{
	        ...
	        ps.execute();
	        ps.close();
	        logger.info("SQL Ejecutando query Insert Vehiculo: "+((org.enhydra.jdbc.standard.StandardXAPreparedStatement)ps).ps.toString());
    ...
    private String getSelectInmueble(String superpatron, String patron, Object[] filtro, Object[] idLayers, Object[] idFeatures, Sesion userSesion, String addToFrom) throws Exception {
    	...
    	for(int i=0; i<idFeatures.length; i++){
    		...
    		if (i==0) 
	    		...
	    		"inventario_feature.revision_expirada=9999999999 AND "+
...
//        sql+= "UPPER(vehiculo.fecha_adquisicion) like UPPER('%"+cadena+"%') OR ";
    ...
    private String addPartOfWhere(CampoFiltro cf) throws Exception {
        if (cf.isVarchar() || cf.isDominio()){
           ...
           // return " AND to_date("+getAliasTabla(cf.getTabla())+"."+cf.getNombre()+", 'yyyy-MM-dd') "+cf.getOperador()+" to_date('"+cf.getValorVarchar()+"', '"+Const.pattern+"')";
        	return " AND "+"DATE("+getAliasTabla(cf.getTabla())+"."+cf.getNombre()+") "+cf.getOperador()+" to_date('"+cf.getValorVarchar()+"', '"+Const.pattern+"')";
    ...
    public void returnBienesRevertibles(ObjectOutputStream oos, String patron, String cadena, 
    		Object[] filtro, Object[] idLayers, Object[] idFeatures, Sesion userSesion,ConfigParameters configParameters,boolean withAllData) throws Exception{
           try{
    	...
    	if (features != null){
    		for (int i=0;i<layers.length;i++){
    			...
    			if (feature != null){ 					
                	...
                	rs= ps.executeQuery();
                	if (rs.next()){
                		//El driver de postgres y el de postgis no pueden estar en todas las aplicaciones. Se deben de copiar en el directorio lib/ext
                		//del jetty para que funciones porque en caso contrario la siguiente linea da una excepcion y en lugar de devolver un objeto
                		//de tipo PGGeometry devuelve un PGObject y la aplicacion ya no funciona
...
//	                    hab.setValorAmor(rs.getDouble("total_amortizado"));
	                    if(rs.getLong("ID_CA")!=0 && rs.getLong("id_cuenta_amortizacion")!= rs.getLong("ID_CA")){
	                    	  hab.setValorAmorAnio(rs.getDouble("total_amortizado_anio")+calcularTotalAmortizadoPorAnio(hab.getFechaAdquisicion(),anio, hab.getCoste(), hab.getCuentaAmortizacion(), LCGIII_DatosAmortizacionConst.POR_ANNOS));
			                   hab.setValorAmorPorc(rs.getDouble("total_amortizado_porcentaje")+calcularTotalAmortizadoPorAnio(hab.getFechaAdquisicion(),anio, hab.getCoste(), hab.getCuentaAmortizacion(), LCGIII_DatosAmortizacionConst.POR_PORCENTAJE));
			                   cuenta.setId(rs.getLong("id_cuenta_amortizacion"));	                   
	                    }
	                    else{ 
	                    	 hab.setValorAmorAnio(calcularTotalAmortizado(formatoDeFecha.parse("31/12/"+Integer.toString(anio)),hab.getFechaAdquisicion(), hab.getCoste(), hab.getCuentaAmortizacion(), LCGIII_DatosAmortizacionConst.POR_ANNOS));
			                   hab.setValorAmorPorc(calcularTotalAmortizado(formatoDeFecha.parse("31/12/"+Integer.toString(anio)),hab.getFechaAdquisicion(), hab.getCoste(), hab.getCuentaAmortizacion(), LCGIII_DatosAmortizacionConst.POR_PORCENTAJE));
		             	  }
	                    if(tipoEspecificoBien!=null)
		...
		private ListaHistoricoAmortizaciones getListaHistoricoAmortizables(
				...
				String table=null;
				  ...
				  if (patron.equalsIgnoreCase(Const.PATRON_INMUEBLES_URBANOS)){
			    	...
			    	else if (patron.equalsIgnoreCase(Const.PATRON_BIENES_MUEBLES)){
			    		tipoBien=Const.MUEBLES;
			    		coste="coste_adquisicion";
			    		table="12004";
			    	}
