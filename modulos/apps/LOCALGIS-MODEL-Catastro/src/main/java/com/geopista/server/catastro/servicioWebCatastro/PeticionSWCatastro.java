...
public class PeticionSWCatastro
{
    ...
    public static String crearCuerpoFinEntradaRequest(ObjectOutputStream oos, Expediente expediente, ArrayList arrayXmlExp) throws Exception{
        ...
        try{
            ...
            if(expediente!=null && arrayXmlExp!=null) {
            	if(expediente.isTipoTramitaExpSitFinales()){
               ...
               // Java2XMLCatastro parser = new Java2XMLCatastro();
                ...
                if(arrayXmlExp.size()>1 && arrayXmlExp.get(0) instanceof ArrayList && arrayXmlExp.get(1) instanceof ArrayList) {
                    ...
                    if(xmlFX_CC.isEmpty() || ConstantesCatastro.modoGeneracion.equalsIgnoreCase(DatosConfiguracion.MODO_GENERAR_FXCC_DIRECTORIO))
                        expediente.setExistenciaInformacionGrafica("N");
                    else
                        expediente.setExistenciaInformacionGrafica("S");
                    ...
                    for(int j = 0; j<xmlParcelasExp.size();j++){
                    	if(expediente.isTipoTramitaExpSitFinales()){
                    		...
                    		//expediente de variaciones
                    		AppContext.getApplicationContext().getBlackboard().put("expSitFinales",false);
                    		final ArrayList instancias = new ArrayList();
                    		XMLReader parserVariaciones = new SAXParser();

                    		parserVariaciones.setFeature("http://apache.org/xml/features/validation/schema", true);
                    		parserVariaciones.setFeature("http://xml.org/sax/features/validation", true);

                    		parserVariaciones.setContentHandler(new SituacionXMLHandler(parserVariaciones, instancias, "elemf"));
                    		parserVariaciones.parse(new InputSource(new ByteArrayInputStream(((String)xmlParcelasExp.get(j)).getBytes("UTF-8"))));
            				...
            				String fincaXMLVariaciones =aux2.write(fc,"elemv");
                    		
            				finEntrada.append(fincaXMLVariaciones);
                    		
                    		
                    		//finEntrada.append((String)xmlParcelasExp.get(j));
                    	}

                        if (ConstantesCatastro.modoGeneracion.equalsIgnoreCase(DatosConfiguracion.MODO_GENERAR_FXCC_FICHERO)){
                        	String fxcc = (String)xmlFX_CC.get(j);
                        	String img = null;
                        	if (xmlIMG != null && xmlIMG.size()>j){
                        		img = (String)xmlIMG.get(j);
                ...
                if(!xmlBienesInmueblesExp.isEmpty()){
                    ...
                    for(int j = 0; j<xmlBienesInmueblesExp.size();j++){
                    	...
                    	//expediente de situaciones finales
                    		...
                    		parserVariaciones.setContentHandler(new SituacionXMLHandler(parserVariaciones, instancias, "elemv"));
                    		parserVariaciones.parse(new InputSource(new ByteArrayInputStream(((String)xmlBienesInmueblesExp.get(j)).getBytes("UTF-8"))));
                    		...
                    		//la gestion de pantalla al modificar/dar de alta, de baja un BienInmueble o actuar sobre un titular
                    		// dandolo de alta de baja o modificandolo, se actua sobre la variable tipoMovimiento actualizandoló al tipo
                    		// de movimiento generado, Para un convenio de titularidad el movimiento siempre es I, asi que acutalizamos
                    		//en este momento ese valor. PAra un expediente de variaciones
                    		if(bij != null){
                    			bij.getBienInmueble().setTipoMovimiento(BienInmuebleCatastro.TIPO_MOVIMIENTO_VARIACIONES_TITULARIDAD);
                    			bij.setTIPO_MOVIMIENTO(BienInmuebleJuridico.TIPO_MOVIMIENTO_VARIACIONES_TITULARIDAD);
                    			if(expediente.getTipoExpediente().getConvenio().equalsIgnoreCase(DatosConfiguracion.CONVENIO_TITULARIDAD)){
                    				if(bij.getLstTitulares() != null && !bij.getLstTitulares().isEmpty()){
                    					for(int w=0; w<bij.getLstTitulares().size();w++){
                    						Titular titular = (Titular)bij.getLstTitulares().get(w);
                    						titular.setTIPO_MOVIMIENTO(Titular.TIPO_MOVIMIENTO_VARIACIONES_TITULARIDAD);	
                    					}
                    				}
                    			}
                    		}
            				...
            				Java2XMLCatastro aux2 = new Java2XMLCatastro();
            				String bienXML =aux2.write(bij,"bij");
            				bienXML = "<elemv>\n<lbicenv>\n" + bienXML + "</lbicenv>\n</elemv>\n";
            				...
            				String bienXMLVariaciones =aux2.write(bij,"elemv");*/
            				
            				varpad.append(bienXML);
