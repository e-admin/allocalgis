/**
 * GeopistaEditarRedSaneamientoPanel.java
...
public class GeopistaEditarRedSaneamientoPanel
{
	private ApplicationContext appcont = AppContext.getApplicationContext();
  private boolean acceso;
  private JOptionPane OpCuadroDialogo;
  
  public GeopistaEditarRedSaneamientoPanel()
  {
      final String[] parametros = new String[6];
      try
      {
        GeopistaPermission geopistaPerm = new GeopistaPermission("Geopista.Infraestructuras.Editar.Red.Saneamiento");
        acceso = appcont.checkPermission(geopistaPerm,"Infraestructuras");

        if(acceso)
        {
          String rutaMapa = appcont.getString("url.mapa.infraestructuras.saneamiento");
          parametros[0] = "-map";
