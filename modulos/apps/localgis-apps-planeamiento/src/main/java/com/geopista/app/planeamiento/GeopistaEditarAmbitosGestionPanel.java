/**
 * GeopistaEditarAmbitosGestionPanel.java
...
public class GeopistaEditarAmbitosGestionPanel
{
  private boolean acceso;
  private ApplicationContext appcont = AppContext.getApplicationContext();
  private JOptionPane OpCuadroDialogo;

  public GeopistaEditarAmbitosGestionPanel()
   {

    final String[] parametros = new String[6];
    try
    {
      GeopistaPermission geopistaPerm = new GeopistaPermission("Geopista.Planeamiento.Editar Ambitos de Gestión del Suelo");
      acceso = appcont.checkPermission(geopistaPerm,"Planeamiento");
      if(acceso)
      {
        String rutaMapa = appcont.getString("url.mapa.planeamiento");
        parametros[0] = "-map";
