/**
 * VerEntidadesAsociadasToDeterminacionDialog.java
...
public class VerEntidadesAsociadasToDeterminacionDialog extends JDialog{
	...
	private String titleCampo1;
	 ...
	 private int idElementoSelectedArbolDeterminaciones;
	...
	public static final int DIM_X = 630;
	 ...
	 public VerEntidadesAsociadasToDeterminacionDialog(FipPanelBean fipPanelBean, 
			 int idElementoSelectedArbolDeterminaciones, 
		 ...
		 this.title = title;
		 this.idElementoSelectedArbolDeterminaciones = idElementoSelectedArbolDeterminaciones;
	    ...
	    EntidadBean[] lstEntidades = consultarEstidadesAsociadasToDeterminacion();
	    if(lstEntidades != null){
	    	tablaElementosAsociados.cargaDatosTabla(lstEntidades);
	    }
	    else{
	    	dispose();
	    }
	...
	*/
	private void inicializaElementos()
	{
		...
		asociadosPanel.add(getBotoneraPanel(), 
				new GridBagConstraints(0,1,2,1, 1, 0.5,GridBagConstraints.EAST,
	...
	private EntidadBean[] consultarEstidadesAsociadasToDeterminacion(){
		
		EntidadBean[] lstEntidades = null;
		try {
			lstEntidades = ClientGestorFip.obtenerEntidadesAsociadasToDeterminacion(
					idElementoSelectedArbolDeterminaciones, application);
	...
	private void verCasos_actionPerformed(){
		...
		if(lstIdSelected.length >1){
			...
			try {
				CondicionUrbanisticaCasoBean[] lstcuc = ClientGestorFip.obtenerCasosCondicionUrbanistica(idElementoSelectedArbolDeterminaciones, 
						lstIdSelected[0], application);
	...
	private void verRegimenes_actionPerformed(){
		...
		if(lstIdCasos.size() == 1){
			...
			if(lstCUCR != null && lstCUCR.length!= 0){
				...
				det.setId(idElementoSelectedArbolDeterminaciones);
				lstDet[0] = det;
				...
				JOptionPane.showMessageDialog(this, 
						I18N.get("LocalGISGestorFipAsociacionDeterminacionesEntidades",
									"gestorFip.asociaciondeterminacionesentidades.regimenes.noAsociadoRegimen"));
			}
