...
public class TablaElementosAsociados extends JPanel{
    ...
    private String titleCampo1;
	  ...
	  public TablaElementosAsociados(String titleCampo1)
	    {
	        this.titleCampo1 = titleCampo1;
	        inicializaTabla();
	       /* if(lstAsociados!=null && lstAsociados.length != 0)
	        {
	            cargaDatosTabla(this.lstAsociados);
	        }*/
	    }
	  ...
	  /**
	    ...
	    private void inicializaTabla()
	    {
	        ...
	        tablaScrollPanel.setPreferredSize(new Dimension(250,300));
	 ...
	 // Regardless of sort order (ascending or descending), null values always appear last.
	     ...
	     */
	    public void cargaDatosTabla(Object[] lstAsociados)
	    {
	        editors.clear();
	        ...
	        if(lstAsociados != null){
	        	...
	        	datos= new String[lstAsociados.length][2];
	 	        for(int i=0; i< lstAsociados.length ;i++)
	 	        {
	 	        	String nombre = "";
	 	        	int id = 0;
	 	        	if(lstAsociados[i] instanceof DeterminacionBean){
	 	        		nombre = ((DeterminacionBean)lstAsociados[i]).getNombre(); 
	 	        		id = ((DeterminacionBean)lstAsociados[i]).getId();
	 	        	}
	 	        	else if(lstAsociados[i] instanceof EntidadBean){
	 	        		nombre = ((EntidadBean)lstAsociados[i]).getNombre();
	 	        		id = ((EntidadBean)lstAsociados[i]).getId();
	 	        	}
	 	        	//String nombreElemento = (String)lstNombreElementos.get(i);
	     ...
	     * */    
	     ...
	     public int[] getIdElementoSeleccionado()
	     {
	    	 ...
	    	 if (elementosTable.getSelectedRow() == -1){
	    		 ...
	    		 int[] selectedRows = elementosTable.getSelectedRows();
	    		 lstId = new int[selectedRows.length];
	    		 for(int i=0; i< selectedRows.length; i++){
	    			 Vector fila =(Vector) modelo.getDataVector().elementAt(selectedRows[i]) ;
	    			 idElemento = new Integer((String)fila.elementAt(0)).intValue();
	    			 lstId[i] = idElemento;
	    		 }
