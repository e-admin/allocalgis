/**
 * MigracionAsistidaUsosPanel.java
...
import com.gestorfip.app.planeamiento.dialogs.migracionasistida.TableRenderUsosMA;
...
public class MigracionAsistidaUsosPanel extends JPanel implements WizardPanel
{
	...
	private TableRenderUsosMA[] lstTableRenderer = null;
    ...
    public MigracionAsistidaUsosPanel(String localId, String nextId)
    {   
    	this.nextID = nextId;
		...
		this.lstTableRenderer = new TableRenderUsosMA[numLayers];
    ...
    {  
        ...
        TitledBorder title;
        title = BorderFactory.createTitledBorder(I18N.get("MigracionAsistidaLayers","gestorFip.migracionasistida.title.usos"));
            	...
            	new Thread(new Runnable()
              {
		          public void run()
		          {
		              try
		              {
		            	  ...
		            	  JLabel desc = new JLabel(I18N.get("MigracionAsistidaLayers","gestorFip.migracionasistida.usos.descripcion"));
		            	  JLabel desc1 = new JLabel(I18N.get("MigracionAsistidaLayers","gestorFip.migracionasistida.usos.descripcion1"));
		            	  JLabel desc2 = new JLabel(I18N.get("MigracionAsistidaLayers","gestorFip.migracionasistida.usos.descripcion2"));
		            	  JLabel desc3 = new JLabel(I18N.get("MigracionAsistidaLayers","gestorFip.migracionasistida.usos.descripcion3"));
		            	  add(desc,  
    ...
    public JTabbedPane getTabbedPane() {
    	...
    	if(tabbedPane == null){
			...
			for(int i=0; i<lstAplicaLayer.length; i++){
				...
				if(lstAplicaLayer[i]){
					lstTableRenderer[i] = new TableRenderUsosMA(lstConfLayerBean[i].getLstConfUsosRegulaciones());
  		...
  		for(int i=0; i<lstAplicaLayer.length; i++){
  			if(lstAplicaLayer[i]){
  				 for (int h = 0; h < lstTableRenderer[i].getData().length; h++) {
  					 ...
  					 if((Boolean)obj[TableRenderUsosMA.POSCAMPOSEL]){
  						 //elemento seleccionado
  						 String alias = (String)obj[TableRenderUsosMA.POSCAMPOALIAS];
...
//  							 if(validateAlias((String)obj[TableRenderUsosMA.POSCAMPOALIAS])){
//  								 if(validateLayer[i]){
//  									 validateLayer[i] = true;
//  								 }
//  							 }
...
//  						//elemento seleccionado
  						...
  						for (int j=0; j<this.lstConfLayerBean[i].getLstConfUsosRegulaciones().length; j++){
  							...
  							if(this.lstConfLayerBean[i].getLstConfUsosRegulaciones()[j].getDeterminacionUso().getCodigo()
										.equals((String)obj[TableRenderUsosMA.POSCAMPOCODIGO])){
  								this.lstConfLayerBean[i].getLstConfUsosRegulaciones()[j].setAlias((String)obj[TableRenderUsosMA.POSCAMPOALIAS]);
