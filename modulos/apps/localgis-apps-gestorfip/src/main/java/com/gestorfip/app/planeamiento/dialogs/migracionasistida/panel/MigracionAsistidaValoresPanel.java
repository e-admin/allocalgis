/**
 * MigracionAsistidaValoresPanel.java
...
public class MigracionAsistidaValoresPanel extends JPanel implements WizardPanel
{
	...
	private TableRenderValoresMA[] lstTableRenderer = null;
    ...
    public MigracionAsistidaValoresPanel(String localId, String nextId)
    {   
    	this.nextID = nextId;
		...
		this.lstTableRenderer = new TableRenderValoresMA[numLayers];
    ...
    {  
        ...
        TitledBorder title;
        title = BorderFactory.createTitledBorder(I18N.get("MigracionAsistidaLayers","gestorFip.migracionasistida.title.valores"));
            ...
            {            
            	...
            	new Thread(new Runnable()
              {
		          public void run()
		          {
		              try
		              {
		            	  JLabel desc = new JLabel(I18N.get("MigracionAsistidaLayers","gestorFip.migracionasistida.valores.descripcion"));
		            	  JLabel desc1 = new JLabel(I18N.get("MigracionAsistidaLayers","gestorFip.migracionasistida.valores.descripcion1"));
		            	  JLabel desc2 = new JLabel(I18N.get("MigracionAsistidaLayers","gestorFip.migracionasistida.valores.descripcion2"));
		            	  JLabel desc3 = new JLabel(I18N.get("MigracionAsistidaLayers","gestorFip.migracionasistida.valores.descripcion3"));
		            	  JLabel desc4 = new JLabel(I18N.get("MigracionAsistidaLayers","gestorFip.migracionasistida.valores.descripcion4"));
		            	  add(desc,  
    ...
    public JTabbedPane getTabbedPane() {
    	...
    	if(tabbedPane == null){
			...
			for(int i=0; i<lstAplicaLayer.length; i++){
				...
				if(lstAplicaLayer[i]){
					lstTableRenderer[i] = new TableRenderValoresMA(lstConfLayerBean[i].getLstConfLayerDeterminacionAplicada());
    ...
    public void enteredFromLeft(Map dataMap)
    {
        ...
        for(int i=0; i< lstAplicaLayer.length; i++){
        	if(lstAplicaLayer[i]){
        		lstTableRenderer[i].setLstDeterminacionesAplic(this.lstConfLayerBean[i].getLstConfLayerDeterminacionAplicada());
...
//  	}
    ...
    private boolean validateData(){
  		...
  		boolean validateDataValoresPanel = true;
  		boolean validateValoresLayer[] = new boolean[numLayers];
  		for(int i=0; i<validateValoresLayer.length; i++){
  			validateValoresLayer[i] = true;
  		}
  		...
  		for(int i=0; i<validateValoresLayer.length; i++){
  			if(!validateValoresLayer[i] && lstAplicaLayer[i]){			
  				txtMsgLayers.append("   *").append(lstConfLayerBean[i].getNameLayer()); 
  				validateDataValoresPanel = false;
  			}
  		}
  		
  		if(!validateDataValoresPanel){
  		...
  		return validateDataValoresPanel;
  	}
		...
		for(int i=0; i<lstAplicaLayer.length; i++){
  			if(lstAplicaLayer[i]){
  				...
  				for (int j=0; j<this.lstConfLayerBean[i].getLstConfLayerDeterminacionAplicada().length; j++){
  					// ponemos todas las determinaciones a false para que luego solo esten en estado true los que
  					// tengan algun valor seleccionado.
  					this.lstConfLayerBean[i].getLstConfLayerDeterminacionAplicada()[j].setSelected(false);
  					this.lstConfLayerBean[i].setAplicada(false);
  				
					if (this.lstConfLayerBean[i].getLstConfLayerDeterminacionAplicada()[j].getLstValores() != null &&
								this.lstConfLayerBean[i].getLstConfLayerDeterminacionAplicada()[j].getLstValores()[0] != null){
						for (int t=0; t<this.lstConfLayerBean[i].getLstConfLayerDeterminacionAplicada()[j].getLstValores().length; t++){
							this.lstConfLayerBean[i].getLstConfLayerDeterminacionAplicada()[j].getLstValores()[t].setSelected(false);
						}
						
					}
					if (this.lstConfLayerBean[i].getLstConfLayerDeterminacionAplicada()[j].getLstValoresReferencia() != null &&
								this.lstConfLayerBean[i].getLstConfLayerDeterminacionAplicada()[j].getLstValoresReferencia()[0] != null){
						for (int t=0; t<this.lstConfLayerBean[i].getLstConfLayerDeterminacionAplicada()[j].getLstValoresReferencia().length; t++){
							this.lstConfLayerBean[i].getLstConfLayerDeterminacionAplicada()[j].getLstValoresReferencia()[t].setSelected(false);
						}
						
					}
					if (this.lstConfLayerBean[i].getLstConfLayerDeterminacionAplicada()[j].getLstRegimenesEspecificos() != null &&
							this.lstConfLayerBean[i].getLstConfLayerDeterminacionAplicada()[j].getLstRegimenesEspecificos()[0] != null){
						for (int t=0; t<this.lstConfLayerBean[i].getLstConfLayerDeterminacionAplicada()[j].getLstRegimenesEspecificos().length; t++){
							this.lstConfLayerBean[i].getLstConfLayerDeterminacionAplicada()[j].getLstRegimenesEspecificos()[t].setSelected(false);
						}
  				 ...
  				 for (int h = 0; h < lstTableRenderer[i].getData().length; h++) {
  					...
  					if((Boolean)obj[TableRenderValoresMA.POSCAMPOSEL]){
  						...
  						for (int j=0; j<this.lstConfLayerBean[i].getLstConfLayerDeterminacionAplicada().length; j++){
  							...
  							String idvalor = (String)obj[TableRenderValoresMA.POSCAMPOID];
  							if(this.lstConfLayerBean[i].getLstConfLayerDeterminacionAplicada()[j].getDeterminacionLayer().getCodigo()
  										.equals((String)obj[TableRenderValoresMA.POSCAMPOCODDETSEL])){
  								
  								if(TableRenderValoresMA.TYPE_VALOR.equals((String)obj[TableRenderValoresMA.POSCAMPOTIPOVALOR])){
	  								for (int t=0; t<this.lstConfLayerBean[i].getLstConfLayerDeterminacionAplicada()[j].getLstValores().length; t++){
	  									if(idvalor.equals(String.valueOf(this.lstConfLayerBean[i].getLstConfLayerDeterminacionAplicada()[j].getLstValores()[t].getId()))){
	  									
	  										this.lstConfLayerBean[i].getLstConfLayerDeterminacionAplicada()[j].setSelected(true);
	  										this.lstConfLayerBean[i].getLstConfLayerDeterminacionAplicada()[j].getLstValores()[t].setAlias((String)obj[TableRenderValoresMA.POSCAMPOALIAS]);
	  										this.lstConfLayerBean[i].getLstConfLayerDeterminacionAplicada()[j].getLstValores()[t].setSelected(true);
	  										this.lstConfLayerBean[i].setAplicada(true);
	  									}
	  								}
  								}
  								
  								else if(TableRenderValoresMA.TYPE_VALOR_REFERENCIA.equals((String)obj[TableRenderValoresMA.POSCAMPOTIPOVALOR])){
  									for (int t=0; t<this.lstConfLayerBean[i].getLstConfLayerDeterminacionAplicada()[j].getLstValoresReferencia().length; t++){
  										if(idvalor.equals(this.lstConfLayerBean[i].getLstConfLayerDeterminacionAplicada()[j].getLstValoresReferencia()[t].getDeterminacionLayer().getCodigo())){
  											
  											this.lstConfLayerBean[i].getLstConfLayerDeterminacionAplicada()[j].setSelected(true);
  											this.lstConfLayerBean[i].getLstConfLayerDeterminacionAplicada()[j].getLstValoresReferencia()[t].setAlias((String)obj[TableRenderValoresMA.POSCAMPOALIAS]);
  											this.lstConfLayerBean[i].getLstConfLayerDeterminacionAplicada()[j].getLstValoresReferencia()[t].setSelected(true);
  											this.lstConfLayerBean[i].setAplicada(true);
  										}
  									}
  								}
  								
  								else if(TableRenderValoresMA.TYPE_REGIMEN_ESPECIFICO.equals((String)obj[TableRenderValoresMA.POSCAMPOTIPOVALOR])){
  									for (int t=0; t<this.lstConfLayerBean[i].getLstConfLayerDeterminacionAplicada()[j].getLstRegimenesEspecificos().length; t++){
  										if(idvalor.equals(String.valueOf(this.lstConfLayerBean[i].getLstConfLayerDeterminacionAplicada()[j].getLstRegimenesEspecificos()[t].getId()))){
  											
  											this.lstConfLayerBean[i].getLstConfLayerDeterminacionAplicada()[j].setSelected(true);
  											this.lstConfLayerBean[i].getLstConfLayerDeterminacionAplicada()[j].getLstRegimenesEspecificos()[t].setAlias((String)obj[TableRenderValoresMA.POSCAMPOALIAS]);
  											this.lstConfLayerBean[i].getLstConfLayerDeterminacionAplicada()[j].getLstRegimenesEspecificos()[t].setSelected(true);
  											this.lstConfLayerBean[i].setAplicada(true);
  										}
  									}
  								}
