    ...
    public void loadData(Cultivo cultivo)
    {
        if (cultivo !=null)
    	{
    		...
    		if (((Cultivo)cultivo).getIntensidadProductiva() != null){
    			...
    			intensidadProductiva = GeopistaUtil.completarConCeros(intensidadProductiva, 2);
		...
		if(okPressed)
		{
            if(cultivo==null)
            {
                ...
                if(!(Boolean)AppContext.getApplicationContext().getBlackboard().get("expSitFinales")){
                	cultivo.setTIPO_MOVIMIENTO(Cultivo.TIPO_MOVIMIENTO_ALTA);
            	}
            }
            else{
            	if(!(Boolean)AppContext.getApplicationContext().getBlackboard().get("expSitFinales")){
            		cultivo.setTIPO_MOVIMIENTO(Cultivo.TIPO_MOVIMIENTO_MODIF);
            	}
            }
            
            cultivo.setElementoModificado(true);
		...
		if(okPressed)
		{
            ...
            if(!(Boolean)AppContext.getApplicationContext().getBlackboard().get("expSitFinales")){
            	if(!cultivo.getTIPO_MOVIMIENTO().equals(Cultivo.TIPO_MOVIMIENTO_ALTA)){
            		cultivo.setTIPO_MOVIMIENTO(tipoMovimiento);
            	}
        	}
            cultivo.setElementoModificado(true);
