    ...
    protected void formaReparto_actionPerformed(ActionEvent e) {
    	...
    	if (jComboBoxFormaReparto.getSelectedIndex()>0){
    		...
    		if(patron!=null){
    			...
    			JTableHeader th = jTableReparto.getTableHeader();
    			TableColumnModel tcm = th.getColumnModel();
    			TableColumn tc = tcm.getColumn(2);
    			if(patron.equals("TC")){
    				tc.setHeaderValue(I18N.get("Expedientes","tabla.reparto.columna.cargo"));
    				th.repaint(); 
    				...
    				tc.setHeaderValue(I18N.get("Expedientes","tabla.reparto.columna.local"));
    				th.repaint(); 
    				if(comun instanceof ConstruccionCatastro){
            	...
            	if (obj instanceof ConstruccionCatastro){
            		...
            		if((Boolean)AppContext.getApplicationContext().getBlackboard().get("expSitFinales")){
            			...
            			if(construccion.getTIPO_MOVIMIENTO().equals(ConstruccionCatastro.TIPO_MOVIMIENTO_ALTA)){
            				reparto.setTIPO_MOVIMIENTO(RepartoCatastro.TIPO_MOVIMIENTO_ALTA);
            			}
            			else if(construccion.getTIPO_MOVIMIENTO().equals(ConstruccionCatastro.TIPO_MOVIMIENTO_FINAL) ||
            					construccion.getTIPO_MOVIMIENTO().equals(ConstruccionCatastro.TIPO_MOVIMIENTO_MODIF)){
            				reparto.setTIPO_MOVIMIENTO(RepartoCatastro.TIPO_MOVIMIENTO_MODIF);
            			}
            		}
            		...
            		if((Boolean)AppContext.getApplicationContext().getBlackboard().get("expSitFinales")){
            			...
            			//expediente orientado a variaciones
            			if(cultivo.getTIPO_MOVIMIENTO().equals(ConstruccionCatastro.TIPO_MOVIMIENTO_ALTA) ||
            					cultivo.getTIPO_MOVIMIENTO().equals(ConstruccionCatastro.TIPO_MOVIMIENTO_FINAL)){
            				reparto.setTIPO_MOVIMIENTO(RepartoCatastro.TIPO_MOVIMIENTO_ALTA);
            			}
            			else if(cultivo.getTIPO_MOVIMIENTO().equals(ConstruccionCatastro.TIPO_MOVIMIENTO_MODIF)){
            				reparto.setTIPO_MOVIMIENTO(RepartoCatastro.TIPO_MOVIMIENTO_MODIF);
            			}
            		}
            	}
    					...
    					for (Iterator iteratorConstrucciones = reparto.getLstLocales().iterator();iteratorConstrucciones.hasNext();){
    						...
    						if (cc.getRefParcela().getRefCatastral()!=null && cc.getNumOrdenConstruccion()!=null){
    							if (cc.getRefParcela().getRefCatastral().length()<14){
    								cc.getRefParcela().setRefCatastral(GeopistaUtil.completarConCeros(cc.getRefParcela().getRefCatastral(),14));
    							}
    							if (cc.getNumOrdenConstruccion().length()<4){
    								cc.setNumOrdenConstruccion(GeopistaUtil.completarConCeros(cc.getNumOrdenConstruccion(), 4));
    							}
