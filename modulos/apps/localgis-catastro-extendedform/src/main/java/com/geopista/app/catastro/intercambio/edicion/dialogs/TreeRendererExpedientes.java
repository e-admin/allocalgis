...
public class TreeRendererExpedientes extends DefaultTreeCellRenderer{
    ...
    public TreeRendererExpedientes() {}
        ...
        if (row>=0)
        {
            ...
            case TIPO_PARCELA:
                ...
                if(ConstantesCatastro.modoTrabajo.equals(DatosConfiguracion.MODO_TRABAJO_ACOPLADO)){
	                if(((FincaCatastro)obj).isActualizadoOVC()){
	                	this.setBorder(BorderFactory.createLineBorder(Color.WHITE));
	                }
	                else{
            ...
            case TIPO_CARGO:
                ...
                sTitle.append("Cargo:").append(" ").append(((BienInmuebleJuridico)obj).getBienInmueble().getIdBienInmueble().getNumCargo()); 
                if(ConstantesCatastro.modoTrabajo.equals(DatosConfiguracion.MODO_TRABAJO_ACOPLADO)){
	                if(((BienInmuebleJuridico)obj).getBienInmueble().isActualizadoOVC()){
	                	this.setBorder(BorderFactory.createLineBorder(Color.WHITE));
	                }
	                else{
	                	this.setBorder(BorderFactory.createLineBorder(Color.red));
            ...
            case TIPO_LOCAL:
                        ...
                        && ((ConstruccionCatastro)obj).getDatosEconomicos().getCodModalidadReparto()!=null)
                {
                    ...
                    sTitle.append("Local :").append(" ").append(GeopistaUtil.completarConCeros(((ConstruccionCatastro)obj).getNumOrdenConstruccion(), 4));
                }
                else
                {
            ...
            case TIPO_SUELO:
                ...
                sTitle.append("Suelo:").append(" ").append(GeopistaUtil.completarConCeros(((SueloCatastro)obj).getNumOrden(),4));
                this.setBorder(BorderFactory.createLineBorder(Color.WHITE));
                break;
                
            case TIPO_UC:
                setIcon(IconLoader.icon(ICONO_UC)); 
                sTitle.append("U.Constructiva:").append(" ").append(GeopistaUtil.completarConCeros(((UnidadConstructivaCatastro)obj).getCodUnidadConstructiva(),4));
                this.setBorder(BorderFactory.createLineBorder(Color.WHITE));
                break;
