...
public class SueloPanel extends JPanel implements FeatureExtendedPanel
{
	...
	private JButton jButtonModificarSuelo = null;
    ...
    public SueloCatastro getSuelo ()
    {
        if(suelo!=null)
        {
            ...
            if(jComboBoxTramo.getSelectedItem()!=null){ 
            	...
            	suelo.getDatosEconomicos().setCodTramoPonencia(((EstructuraDB)jComboBoxTramo.getSelectedItem()).getPatron());
    ...
    private JTextField getJTextFieldLongitudFachada()
    {
        ...
        {
            jTextFieldLongitudFachada = new TextField(6);//new JNumberTextField(JNumberTextField.NUMBER);//, new Integer(99999));
            jTextFieldLongitudFachada.addCaretListener(new CaretListener()
                    {
                public void caretUpdate(CaretEvent evt)
                {
                    EdicionUtils.chequeaLongYDecimalCampoEdit(jTextFieldLongitudFachada,6, aplicacion.getMainFrame());
                    if(!jTextFieldLongitudFachada.getText().equals("")){
	                    ...
	                    if(longFachada >= 1000){
	                    	JOptionPane.showMessageDialog(aplicacion.getMainFrame(), 
	                    			I18N.get("Expedientes","suelo.panel.datosfisicos.fachada.longitud.maxima") + " 999.99 " +
	                    			I18N.get("Expedientes","suelo.panel.datosfisicos.metros"));
    ...
    private JPanel getJPanelFachada()
    {
        if (jPanelFachada == null)
        {
            ...
            JLabel jLabelLongitud = new JLabel (I18N.get("Expedientes", "suelo.panel.datosfisicos.fachada.longitud")+" ("+I18N.get("Expedientes","suelo.panel.datosfisicos.metros")+")"); 
	...
	private JPanel getJPanelBotonera()
    {
        if (jPanelBotonera  == null)
        {
        	...
        	if(!(Boolean)AppContext.getApplicationContext().getBlackboard().get("expSitFinales")){
        		// solo se incluye el boton cuando es un expediente de variaciones
        		jPanelBotonera.add(getJButtonModificarSuelo(), 
	...
	private JButton getJButtonModificarSuelo(){
		if (jButtonModificarSuelo   == null)
        {
			jButtonModificarSuelo = new JButton();
			jButtonModificarSuelo.setText(I18N.get("Expedientes", "suelo.panel.boton.modificarsuelo"));
			
			jButtonModificarSuelo.addActionListener(new java.awt.event.ActionListener()
            {
            	public void actionPerformed(java.awt.event.ActionEvent e)
            	{
            		EdicionUtils.enablePanel(SueloPanel.this,true);
            		if(!suelo.getTIPO_MOVIMIENTO().equals(SueloCatastro.TIPO_MOVIMIENTO_ALTA)){
            			suelo.setTIPO_MOVIMIENTO(SueloCatastro.TIPO_MOVIMIENTO_MODIF);
            		}
            		suelo.setElementoModificado(true);
            	}
            });
        }
		jButtonModificarSuelo.setName("_modificarSuelo");
		return jButtonModificarSuelo;
	}
        ...
        if (jButtonNuevoSuelo    == null)
        {
        	...
        	jButtonNuevoSuelo.addActionListener(new java.awt.event.ActionListener()
            {
            	public void actionPerformed(java.awt.event.ActionEvent e)
            	{
            		...
            		if(!(Boolean)AppContext.getApplicationContext().getBlackboard().get("expSitFinales")){
            			EdicionUtils.enablePanel(SueloPanel.this, true);  
            			EdicionUtils.disabledButtons(SueloPanel.this, "_eliminarSuelo");
            		}
            		EdicionUtils.clearPanel(SueloPanel.this);     
            	}
            });
        }
        jButtonNuevoSuelo.setName("_nuevoSuelo");
	...
	private JButton getJButtonEliminarSuelo()
    {
        ...
        jButtonEliminarSuelo.setName("_eliminarSuelo");
	...
	private JButton getJButtonAniadirSuelo()
    {
        ...
        jButtonAniadirSuelo.setName("_anadirSuelo");
    ...
    private void annadeSuelo()
    {
    	if(this.datosMinimosYCorrectosSuelo()){
	        ...
	        if(!(Boolean)AppContext.getApplicationContext().getBlackboard().get("expSitFinales")){
	        	suelo.setElementoModificado(true);
	        	suelo.setTIPO_MOVIMIENTO(SueloCatastro.TIPO_MOVIMIENTO_ALTA);
	        }
	        lstSuelos.add(suelo);
	        ...
	        //expediente de variaciones
	        	EdicionUtils.enablePanel(SueloPanel.this, true);
	        	SueloPanel.this.getJButtonEliminarSuelo().setEnabled(true);
	        }
    	}else{
        ...
        for(int i = 0; i< lstSuelos.size() && continua;i++)
        {
            		...
            		&&suelo.getNumOrden().equalsIgnoreCase(sue.getNumOrden()))
            {
            	...
            	if((Boolean)AppContext.getApplicationContext().getBlackboard().get("expSitFinales")){
	        		...
	        		//expediente de variaciones
	        		if(suelo.getTIPO_MOVIMIENTO().equals(UnidadConstructivaCatastro.TIPO_MOVIMIENTO_ALTA)){
	        			lstSuelos.remove(i);
	            		continua = false;
	            		suelo.setElementoModificado(false);
	            		TreePath path = new TreePath(arbol.getSelectionPath().getParentPath().getLastPathComponent());
	                    panelExpedientes.removeObject(arbol.getSelectionPath().getLastPathComponent());
	                    arbol.setSelectionPath(path);
	        		}
	        		else{
	        			suelo.setTIPO_MOVIMIENTO(SueloCatastro.TIPO_MOVIMIENTO_BAJA);
	        			suelo.setElementoModificado(false);
	        		}
            	}
            }
