...
public class LocalExtendedInfoPanel extends JPanel
{
    ...
    public void loadData(ConstruccionCatastro construc)
    {
        if (construc !=null)
        {
            ...
            if(((ConstruccionCatastro)construc).getDatosEconomicos().getCorrectorApreciacion()!=null)
            	jTextFieldCoefAprecEcon.setText(((ConstruccionCatastro)construc).getDatosEconomicos().getCorrectorApreciacion().toString());
	...
	private JPanel getJPanelDatosEconomicos() {
		if (jPanelDatosEconomicos == null) {
            ...
            jLabelUso.setText(UtilRegistroExp.getLabelConAsterisco(I18N.get("Expedientes", "local.panel.datoseconomicos.uso")));
	...
	private JComboBox getJComboBoxTipologia() {
		if (jComboBoxTipologia == null) {
            ...
            jComboBoxTipologia.setKeySelectionManager(new ComboBoxKeyTipoDestinoSelectionManager());
		}
	...
	public ConstruccionCatastro getConstruccionCatastro(ConstruccionCatastro cons, String tipoMovimiento)
	{
        if (okPressed)
        {
        	...
        	if(!(Boolean)AppContext.getApplicationContext().getBlackboard().get("expSitFinales")){
        		if(!cons.getTIPO_MOVIMIENTO().equals(ConstruccionCatastro.TIPO_MOVIMIENTO_ALTA)){
        			cons.setTIPO_MOVIMIENTO(tipoMovimiento);
        		}
        	}
        	
        	cons.setElementoModificado(true);
            ...
            {
                datosEconomicosConstruccion.setCorrectorApreciacion(new Float(jTextFieldCoefAprecEcon.getText()));
            }
            ...
            cons.setDatosFisicos(datosFisicosConstruccion);
            cons.setNumOrdenBienInmueble(GeopistaUtil.completarConCeros(jTextFieldCargo.getText(),4));
	...
	public ConstruccionCatastro getConstruccionCatastro(ConstruccionCatastro cons)
	{
        if (okPressed)
        {
        	if(cons==null)
            {
                cons = new ConstruccionCatastro();
                if(!(Boolean)AppContext.getApplicationContext().getBlackboard().get("expSitFinales")){
                	cons.setTIPO_MOVIMIENTO(ConstruccionCatastro.TIPO_MOVIMIENTO_ALTA);
            	}  
            }
        	else{
        		if(!(Boolean)AppContext.getApplicationContext().getBlackboard().get("expSitFinales")){
        			cons.setTIPO_MOVIMIENTO(ConstruccionCatastro.TIPO_MOVIMIENTO_MODIF);
            	}
        	}
