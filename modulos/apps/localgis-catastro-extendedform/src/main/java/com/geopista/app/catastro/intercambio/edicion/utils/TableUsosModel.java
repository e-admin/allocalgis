...
public class TableUsosModel extends AbstractTableModel {
	...
	/**
	 * Devuelve la UsoCatastro completa de la fila seleccionada
	 * 
	 * @param row
	 *            Índice de la fila cuyo objeto UsoCatastro se solicita
	 * @return UsoCatastro completa
	 */
	public UsoCatastro getValueAt(int row) {
	...
	public boolean modificarrUso(UsoCatastro usoOld, UsoCatastro usoNew) {
		try {
			int fila = lstElementos.indexOf(usoOld);
			
			((UsoCatastro)lstElementos.get(fila)).setCodigoUso(usoNew.getCodigoUso());
			((UsoCatastro)lstElementos.get(fila)).setSuperficieDestinada(usoNew.getSuperficieDestinada());
