...
//	private TablePlantasModel tableplantasmodel;
	...
	private void loadData() {
		...
		this.getJTextFieldEscalaCaptura().setText(this.asc.getEscalaCaptura());
		this.getJTextFieldFechaCaptura().setText(this.asc.getFechaCaptura());
		
		if(this.asc.getLstLinderos().isEmpty()){
			((TableLinderosModel)((TableSorted)this.getJTableLinderos().getModel()).getTableModel()).setData(new ArrayList());
		}
		else{
			ArrayList lstLinderos = new ArrayList();
			for(int i=0; i<this.asc.getLstLinderos().size(); i++){
				LinderoCatastro lindero = (LinderoCatastro)this.asc.getLstLinderos().get(i);
				if(!(lindero.getCodVia()<= 0 && lindero.getNumVia() <= 0 &&
						(lindero.getLetraDuplicado() == null || lindero.getLetraDuplicado().equals("")) &&
						(lindero.getNombreVia() == null || lindero.getNombreVia().equals("") && lindero.getNumVia() <= 0) &&
						(lindero.getSiglaVia() == null || lindero.getSiglaVia().equals("")))){
					lstLinderos.add(lindero);
				}
			}
			
			((TableLinderosModel)((TableSorted)this.getJTableLinderos().getModel()).getTableModel()).setData(lstLinderos);
		}
...
//			jButtonBuscarVia.setName("_buscarvia");
	...
	private JTextField getJTextFieldCodDelegacion() {
		if (codDelegacionTextField == null) {
			...
			codDelegacionTextField.setDocument(new LimitadorCaracteres(codDelegacionTextField,3));
