...
import com.geopista.app.catastro.intercambio.edicion.CultivoComunExtendedInfoDialog;
...
import com.geopista.app.catastro.intercambio.edicion.LocalComunExtendedInfoDialog;
import com.geopista.app.catastro.intercambio.edicion.LocalExtendedInfoDialog;
...
import com.geopista.ui.plugin.infcattitularidad.dialogs.DialogoVerInfCatastralTitularidad;
...
public class FincaPanel extends JPanel implements FeatureExtendedPanel
{
    ...
    public void loadData(Object o)
    {
        ...
        if (o instanceof FincaCatastro)
        {
           ...
           if((BienInmuebleJuridico)o != null){
...
//                	jComboBoxComunidadBienes.addItem(new ComunidadBienes(""));
    ...
    public JPanel getJPanelBotoneraFinca()
    {
        if (jPanelBotoneraFinca == null)
        {
            ...
            jPanelBotoneraFinca.add(getjButtonActualizar(), 
                    new GridBagConstraints(0, 0, 1, 1, 1.0, 1.0,
                            GridBagConstraints.CENTER, GridBagConstraints.NONE,
                            new Insets(0, 3, 0, 0), 0, 0));
            
            jPanelBotoneraFinca.add(getjButtonConsultar(), 
        ...
        if (jButtonMasDatosFinca == null)
        {
            ...
            jButtonMasDatosFinca.addActionListener(new java.awt.event.ActionListener()
                ...
                public void actionPerformed(java.awt.event.ActionEvent e)
                {
                    ...
                    FincaExtendedInfoDialog feif = null;
                    if((Boolean)AppContext.getApplicationContext().getBlackboard().get("expSitFinales")){
                    	feif = new FincaExtendedInfoDialog(fincaEditada, isEditable, false);
                    }
                    else{
                    	boolean editFinca = isEditable;
                    	if(fincaEditada.getTIPO_MOVIMIENTO_DELETE().equals(FincaCatastro.TIPO_MOVIMIENTO_BAJA)){
                    		editFinca= false;
                    	}
                    	feif = new FincaExtendedInfoDialog(fincaEditada, editFinca, false);
                    }
                    ...
                    if (fc!=null)
                    {
                    	...
                    	//expediente de variaciones
	                    	if(fc.isElementoModificado()){
	                    		EdicionUtils.enablePanel(FincaPanel.this.getJPanelDatosFinca(), true);
	                    	}
	                    	else{
	                    		EdicionUtils.enablePanel(FincaPanel.this.getJPanelDatosFinca(), false);
	                    	}
                    	}
    ...
    public JPanel getJPanelBienInmueble()
    {
        if (jPanelBienInmueble == null)
        {
            ...
            /*jPanelBienInmueble.add(getJPanelDatosBienInmueble(), 
                    new GridBagConstraints(0,1,6,1,0.1, 0.2,GridBagConstraints.NORTH,
                            GridBagConstraints.HORIZONTAL, new Insets(5,5,5,5),0,0));
            jPanelBienInmueble.add(getJPanelBotoneraBienInmueble(), 
                    new GridBagConstraints(0,4,6,1,0.1, 0.1,GridBagConstraints.NORTH, 
                            GridBagConstraints.HORIZONTAL, new Insets(5,5,5,5),0,0));*/
    ...
    private JButton getJButtonBuscarVia()
    {
        if (jButtonBuscarVia == null)
        {
            ...
            jButtonBuscarVia.setIcon(IconLoader.icon(GestionExpedienteConst.ICONO_BUSCAR));
            ...
            jButtonMasDatosLocales.addActionListener(new java.awt.event.ActionListener()
            {
                public void actionPerformed(java.awt.event.ActionEvent e)
                {
                    ...
                    if (blackboard.get("Comun")!=null && blackboard.getBoolean("Comun"))
                    {
                        ...
                        dialog.setVisible(true);
                        ConstruccionCatastro cons = null;
                        if((Boolean)AppContext.getApplicationContext().getBlackboard().get("expSitFinales")){ 
                        // expediente de situaciones finales
                        	cons = dialog.getConstruccionCatastro(construccion);
                        }
                        else{
                        	// expediente de variaciones
                        	cons = dialog.getConstruccionCatastro(construccion, ConstruccionCatastro.TIPO_MOVIMIENTO_MODIF);
                        }
        				if (cons!=null)
                        {
                            ...
                            if(!(Boolean)AppContext.getApplicationContext().getBlackboard().get("expSitFinales")){
	                            if(construccion.isElementoModificado()){
	                            	EdicionUtils.enablePanel(FincaPanel.this.getJPanelDatosLocal(), true);
	                            }
	                        }
                    ...
                    {
                        LocalExtendedInfoDialog dialog = null;
                        if((Boolean)AppContext.getApplicationContext().getBlackboard().get("expSitFinales")){
                        	dialog = new LocalExtendedInfoDialog(construccion,isEditable);
                        }
                        else{
                        	boolean editLocal = isEditable;
                        	if(construccion.getTIPO_MOVIMIENTO().equals(ConstruccionCatastro.TIPO_MOVIMIENTO_BAJA)){
                        		editLocal = false;
                        	}
                        	dialog = new LocalExtendedInfoDialog(construccion,editLocal);
                        }
                        dialog.setVisible(true);

                        ConstruccionCatastro constru = null;
                        if((Boolean)AppContext.getApplicationContext().getBlackboard().get("expSitFinales")){
                        	constru = dialog.getConstruccionCatastro(construccion);
                        }
                        else{
                        	constru =dialog.getConstruccionCatastro(construccion, ConstruccionCatastro.TIPO_MOVIMIENTO_MODIF);
                        }
            ...
            jButtonNuevoLocalComun.addActionListener(new java.awt.event.ActionListener()
            {
                public void actionPerformed(java.awt.event.ActionEvent e)
                {
    				...
    				//expediente de situaciones finales
    					...
    					//expediente de variaciones
    					cons = dialog.getConstruccionCatastro(cons, ConstruccionCatastro.TIPO_MOVIMIENTO_ALTA);;
    				}
                    ...
                    if (cons!=null)
                    {
                        ...
                        annadeConstruccion(cons, true, (HideableNode)arbol.getSelectionPath().getLastPathComponent());
                        loadData(cons);
                    }
                }
            });
            ...
            jButtonNuevoCultivoComun.addActionListener(new java.awt.event.ActionListener()
            {
                public void actionPerformed(java.awt.event.ActionEvent e)
                {
                    ...
                    if((Boolean)AppContext.getApplicationContext().getBlackboard().get("expSitFinales")){
                    	...
                    	cultivo = dialog.getCultivo(cultivo, Cultivo.TIPO_MOVIMIENTO_ALTA);
                    }
                    ...
                    if (cultivo != null)
                    {
                        ...
                        if(!(Boolean)AppContext.getApplicationContext().getBlackboard().get("expSitFinales")){
	                        if(cultivo.isElementoModificado()){
	    						EdicionUtils.enablePanel(FincaPanel.this.getJPanelDatosCultivo(), true);
	    					}
	                    }
                        annadeCultivo(cultivo, true, (HideableNode)arbol.getSelectionPath().getLastPathComponent());
                        loadData(cultivo);
                    }
                }
            });
        ...
        if (jButtonNuevoLocal == null)
        {
            ...
            jButtonNuevoLocal.addActionListener(new java.awt.event.ActionListener()
    		{
    			public void actionPerformed(java.awt.event.ActionEvent e)
    			{
    				...
    				if (cons!=null)
    				{
    					annadeConstruccion(cons, false, (HideableNode)arbol.getSelectionPath().getLastPathComponent());
    					if (cons.getRefParcela()!=null && cons.getRefParcela().getRefCatastral()!=null && cons.getNumOrdenBienInmueble()!=null){
    						String refFinca = GeopistaUtil.completarConCeros(cons.getRefParcela().getRefCatastral(),14);
                    ...
                    if(construccion!=null && construccion.getNumOrdenConstruccion()!=null)
                    {
                                ...
                                JOptionPane.YES_NO_OPTION) == JOptionPane.YES_OPTION)
                        {
                        	if((Boolean)AppContext.getApplicationContext().getBlackboard().get("expSitFinales")){
                        		...
                        		if(construccion.getTIPO_MOVIMIENTO().equals(Titular.TIPO_MOVIMIENTO_ALTA)){
	                        		eliminaConstruccion();
		                            construccion = null;
		                            
                        		}
                        		else{
                        			construccion.setTIPO_MOVIMIENTO(Titular.TIPO_MOVIMIENTO_BAJA);
                        			construccion.setElementoModificado(false);
                        		}
                        	}
                        }
                    }
        ...
        if (jButtonMasDatosTitular == null)
        {
            ...
            jButtonMasDatosTitular.addActionListener(new java.awt.event.ActionListener()
            {
                public void actionPerformed(java.awt.event.ActionEvent e)
                {
                    ...
                    {
                    	if(!(Boolean)AppContext.getApplicationContext().getBlackboard().get("expSitFinales")){
	                    	if(personaTitular.isElementoModificado()){
	                    		EdicionUtils.enablePanel(FincaPanel.this.getJPanelDatosTitular(), true);
	                    	}
	                    }
                        loadData(titular);
                    }
            ...
            jButtonNuevoTitular.addActionListener(new java.awt.event.ActionListener()
            {
                public void actionPerformed(java.awt.event.ActionEvent e)
                {
                    ...
                    if (personaTitular != null)
                    {
                        ...
                        annadeTitular(personaTitular, (HideableNode)arbol.getSelectionPath().getLastPathComponent());
                    ...
                    if(titular!=null && titular.getNif()!=null)
                    {
                                ...
                                JOptionPane.YES_NO_OPTION) == JOptionPane.YES_OPTION)
                        {
                        	if((Boolean)AppContext.getApplicationContext().getBlackboard().get("expSitFinales")){
                        		...
                        		eliminaTitular();
                        		titular = null;
                        		//titular.setElementoModificado(false);
                        	}
                        	else{
                        		//expediente de variaciones
                        		if(titular.getTIPO_MOVIMIENTO().equals(Titular.TIPO_MOVIMIENTO_ALTA)){
                        			...
                        			titular.setTIPO_MOVIMIENTO(Titular.TIPO_MOVIMIENTO_BAJA);
                        			titular.setElementoModificado(false);
                        			if(!bienInmuebleJuridico.getTIPO_MOVIMIENTO().equals(BienInmuebleJuridico.TIPO_MOVIMIENTO_ALTA) &&
                        					!bienInmuebleJuridico.getTIPO_MOVIMIENTO().equals(BienInmuebleJuridico.TIPO_MOVIMIENTO_BAJA)){
                        				bienInmuebleJuridico.setTIPO_MOVIMIENTO(BienInmuebleJuridico.TIPO_MOVIMIENTO_MODIF);
                        			}
                        		}
                        	}
                        }
        ...
        if (jButtonMasDatosCultivo == null)
        {
        	...
        	jButtonMasDatosCultivo.addActionListener(new ActionListener()
            {
                ...
                {                    
                    ...
                    {
                        blackboard.put("finca", fincaEditada);
                        CultivoComunExtendedInfoDialog dialog = null;
                        if((Boolean)AppContext.getApplicationContext().getBlackboard().get("expSitFinales")){
                        	dialog =  new CultivoComunExtendedInfoDialog(cultivo,isEditable);
                        }
                        else{
                        	boolean editDatosCultivo = isEditable;
                        	if(cultivo.getTIPO_MOVIMIENTO().equals(Cultivo.TIPO_MOVIMIENTO_BAJA)){
                        		editDatosCultivo = false;
                        	}
                        	dialog =  new CultivoComunExtendedInfoDialog(cultivo,editDatosCultivo);
                        }
                        dialog.setVisible(true);
                        Cultivo cult = null;
                        if((Boolean)AppContext.getApplicationContext().getBlackboard().get("expSitFinales")){
                        	cult = dialog.getCultivo(cultivo);
                        }
                        else{
                        	cult = dialog.getCultivo(cultivo, Cultivo.TIPO_MOVIMIENTO_MODIF);
                        }
                    ...
                    {
                    	CultivoExtendedInfoDialog dialog = null ;
                    	if((Boolean)AppContext.getApplicationContext().getBlackboard().get("expSitFinales")){
                         dialog = new CultivoExtendedInfoDialog(cultivo,isEditable);
                    	}
                    	else{
                        	boolean editDatosCultivo = isEditable;
                        	if(cultivo.getTIPO_MOVIMIENTO().equals(Cultivo.TIPO_MOVIMIENTO_BAJA)){
                        	...
                        	dialog = new CultivoExtendedInfoDialog(cultivo,editDatosCultivo);
                    	}
        ...
        if (jButtonMasDatosInmueble == null)
        {
            ...
            jButtonMasDatosInmueble.addActionListener(new ActionListener()
            {
                public void actionPerformed(java.awt.event.ActionEvent e)
                {
                    ...
                    BienInmuebleExtendedInfoDialog dialog = null;
                    if((Boolean)AppContext.getApplicationContext().getBlackboard().get("expSitFinales")){
                    	dialog = new BienInmuebleExtendedInfoDialog(bienInmuebleJuridico.getBienInmueble(), isEditable);
                    }
                    else{
                    	boolean editBien =isEditable;
                    	if(bienInmuebleJuridico.getTIPO_MOVIMIENTO().equals(BienInmuebleJuridico.TIPO_MOVIMIENTO_BAJA)){
                    		editBien= false;
                    	}
                    	dialog = new BienInmuebleExtendedInfoDialog(bienInmuebleJuridico.getBienInmueble(), editBien);
                    }
                    dialog.setVisible(true);                      
                   
                    BienInmuebleJuridico bien = null;
                    if((Boolean)AppContext.getApplicationContext().getBlackboard().get("expSitFinales")){
                    	bien = dialog.getBienInmueble(bienInmuebleJuridico);
                    }
                    else{
                    	bien = dialog.getBienInmueble(bienInmuebleJuridico, ConstruccionCatastro.TIPO_MOVIMIENTO_MODIF);
                    }
                    
                    if (bien!=null)
                    {
                    	if(!(Boolean)AppContext.getApplicationContext().getBlackboard().get("expSitFinales")){
	                    	if(bien.isElementoModificado()){
	                    		// marcamos al objeto que es una modificacion
	                    		dialog.setBienModificado(bienInmuebleJuridico);
	                    		EdicionUtils.enablePanel(FincaPanel.this.getJPanelDatosBienInmueble(), true);
	                    	}
	                    }
            ...
            jButtonDefinirRepresentante.addActionListener(new java.awt.event.ActionListener()
            {
                public void actionPerformed(java.awt.event.ActionEvent e)
                {
                    ...
                    if(bienInmuebleJuridico!=null && bienInmuebleJuridico.getBienInmueble().getIdBienInmueble().getIdBienInmueble()!=null)
                    {
                        ...
                        {
                        	if(!(Boolean)AppContext.getApplicationContext().getBlackboard().get("expSitFinales")){
                        		representante.setElementoModificado(true);
                ...
                public void actionPerformed(java.awt.event.ActionEvent e)
                {
                	...
                	if (getJPanelDatosFinca().getParent()!=null)
                    {
                        ...
                        if(fincaEditada!= null){
                            ...
                            if(!(Boolean)AppContext.getApplicationContext().getBlackboard().get("expSitFinales")){
                            //expediente de variaciones
                            	bic.setTipoMovimiento(BienInmuebleCatastro.TIPO_MOVIMIENTO_ALTA);
                        	}
                            else{
                            	//expediente de situaciones finales
                            	bic.setTipoMovimiento(BienInmuebleCatastro.TIPO_MOVIMIENTO_FINAL);
                            	if(tipoExpediente.getConvenio().equalsIgnoreCase(DatosConfiguracion.CONVENIO_TITULARIDAD)){
                            		bic.setTipoMovimiento(BienInmuebleCatastro.TIPO_MOVIMIENTO_VARIACIONES_TITULARIDAD);
                            	}
                            }
                        }
                    }
                    ...
                    //expediente de situaciones finales
                    	...
                    	//expediente de variaciones
                    	bien = dialog.getBienInmueble(bien, BienInmuebleJuridico.TIPO_MOVIMIENTO_ALTA);
                    }
                    if (bien!=null)
                    {
                        if(fincaEditada==null)
                        ...
                        if(!(Boolean)AppContext.getApplicationContext().getBlackboard().get("expSitFinales")){
	                        if(bic.isElementoModificado()){
	                        	EdicionUtils.enablePanel(FincaPanel.this.getJPanelDatosBienInmueble(), true);
	                        }
	                    }
                        BienInmuebleJuridico bij = bien.clone(bien);
                        annadeBI(bien, (HideableNode)arbol.getSelectionPath().getLastPathComponent());
                        bien.getBienInmueble().setDomicilioTributario(bij.getBienInmueble().getDomicilioTributario());
                        loadData(bien);
                    }
                }
           });
                                ...
                                JOptionPane.YES_NO_OPTION) == JOptionPane.YES_OPTION)
                        {
                        	if((Boolean)AppContext.getApplicationContext().getBlackboard().get("expSitFinales")){
                        		...
                        		if(bienInmuebleJuridico.getTIPO_MOVIMIENTO().equals(BienInmuebleJuridico.TIPO_MOVIMIENTO_ALTA)){
                        			...
                        			//compruebaEliminarBI();
                        			bienInmuebleJuridico.setTIPO_MOVIMIENTO(BienInmuebleJuridico.TIPO_MOVIMIENTO_BAJA);
                        			bienInmuebleJuridico.getBienInmueble().setTipoMovimiento(BienInmuebleCatastro.TIPO_MOVIMIENTO_BAJA);     			
                        			//panelExpedientes.eliminaBienInmueble(bienInmuebleJuridico);
                        			eliminarBienInmueble();
            ...
            jButtonNuevoCultivo.addActionListener(new java.awt.event.ActionListener()
            {
                public void actionPerformed(java.awt.event.ActionEvent e)
                {
                    ...
                    if((Boolean)AppContext.getApplicationContext().getBlackboard().get("expSitFinales")){
                    	...
                    	//expediente de variaciones
                    	cult = dialog.getCultivo(cult, Cultivo.TIPO_MOVIMIENTO_ALTA);
                    }
                    
                    if (cult != null)
                    {
                    	annadeCultivo(cult,false, (HideableNode)arbol.getSelectionPath().getLastPathComponent());
                    	loadData(cult);
                    	if(!(Boolean)AppContext.getApplicationContext().getBlackboard().get("expSitFinales")){
	                    	if(cultivo.isElementoModificado()){
	    						EdicionUtils.enablePanel(FincaPanel.this.getJPanelDatosCultivo(), true);
                    ...
                    if(cultivo!=null && cultivo.getCodSubparcela()!=null)
                    {
                                ...
                                JOptionPane.YES_NO_OPTION) == JOptionPane.YES_OPTION)
                        {
                        	if((Boolean)AppContext.getApplicationContext().getBlackboard().get("expSitFinales")){
                        		...
                        		if(cultivo.getTIPO_MOVIMIENTO().equals(Titular.TIPO_MOVIMIENTO_ALTA)){
                        			eliminaCultivo();
                        			cultivo = null;
                        		}
                        		else{
                        			cultivo.setTIPO_MOVIMIENTO(Titular.TIPO_MOVIMIENTO_BAJA);
                        			cultivo.setElementoModificado(false);
                        			EdicionUtils.enablePanel(getJPanelDatosCultivo(), false);
                        		}
                        	}
                        }
		        	}
	          ...
	          if (direccionFinca != null)
	          {
	        	  ...
	        	  if (jTextFieldCodigoVia.getText() != null){
	        		  ...
	        		  if(jTextFieldCodigoVia.getText().equals("")){
	        			  Integer codigoVia = new Integer(-1);
...
//            	 bienInmuebleJuridico.getBienInmueble().getDatosEconomicosBien().setCoefParticipacion(new Float(0));
    ...
    public void actualizaArbol(HideableNode nodoNuevo)
    {
    ...
    private void eliminarBienInmueble()
    {
    	
    	
    	if (((HideableNode)arbol.getSelectionPath().getLastPathComponent()).getUserObject() instanceof BienInmuebleJuridico){
    		
    		panelExpedientes.eliminaBienInmuebleExpVariaciones((BienInmuebleJuridico)((HideableNode)arbol.getSelectionPath().getLastPathComponent()).getUserObject(),
    				(FincaCatastro)((HideableNode)arbol.getSelectionPath().getParentPath().getLastPathComponent()).getUserObject());
    		...
    		//TreePath path = new TreePath(arbol.getSelectionPath().getParentPath());
    		arbol.setSelectionPath(path);
    ...
    private void compruebaEliminarFinca()
    {
        if (compruebaNumeroFincaReales(true))
        {
            try
            {
            	...
            	if(!(Boolean)AppContext.getApplicationContext().getBlackboard().get("expSitFinales") &&
            			((FincaCatastro)((HideableNode)arbol.getSelectionPath().getLastPathComponent()).getUserObject()).getTIPO_MOVIMIENTO().equals(FincaCatastro.TIPO_MOVIMIENTO_ALTA)){
            		// expediente de variaciones
        ...
        else
        {
        	...
        	if(!(Boolean)AppContext.getApplicationContext().getBlackboard().get("expSitFinales")){
        		...
        		for(int i=0; i< lstBi.size() && continua;i++)
	            {
	                ...
	                {
	                	if(bienInmuebleJuridico.getTIPO_MOVIMIENTO().equals(BienInmuebleJuridico.TIPO_MOVIMIENTO_ALTA)){
	                		// es un elemento que se dio de alta previamente, si se elimina no debe existir
	                		lstBi.remove(i);
	 	                    continua = false;
	                	}
	                	else{
    ...
    public void annadeBI(BienInmuebleJuridico bijCopy, HideableNode nodoOrigen) {
        if(fincaEditada!=null){
        	
        	BienInmuebleJuridico bij = bijCopy.clone(bijCopy);
        	

        	bij.getBienInmueble().getIdBienInmueble().getParcelaCatastral().setRefCatastral(
        			((FincaCatastro)nodoOrigen.getUserObject()).getRefFinca().getRefCatastral());
        	bij.getBienInmueble().getIdBienInmueble().setIdBienInmueble(
        			((FincaCatastro)nodoOrigen.getUserObject()).getRefFinca().getRefCatastral()+
        			bij.getBienInmueble().getIdBienInmueble().getNumCargo()+
        			bij.getBienInmueble().getIdBienInmueble().getDigControl1()+
        			bij.getBienInmueble().getIdBienInmueble().getDigControl2());
        	
        	bienInmuebleJuridico = bij;
            ArrayList lstBI = fincaEditada.getLstBienesInmuebles();
            ...
            HideableNode nodo = panelExpedientes.addObject(nodoOrigen,bij,true);
//            HideableNode nodo = panelExpedientes.addObject((HideableNode)arbol.getSelectionPath().getLastPathComponent(),bij,true);
			...
			ArrayList lstCons = new ArrayList();
			for (int h=0; h<bij.getBienInmueble().getLstConstrucciones().size();h++){
				lstCons.add(bij.getBienInmueble().getLstConstrucciones().get(h));
			}
			
			//Iterator itConstrucciones = bij.getBienInmueble().getLstConstrucciones().iterator();
			Iterator itConstrucciones = lstCons.iterator();
...
//				nodo.add(nodoConstruccion); 
				...
				if (construccion.getDatosEconomicos().getCodTipoValor()!=null
						&& !construccion.getDatosEconomicos().getCodTipoValor().equals("")
						&& construccion.getDatosEconomicos().getCodModalidadReparto()!=null)
				{   

					annadeConstruccion(construccion, true, nodo);	
				
				}
				else{

					annadeConstruccion(construccion, false, nodo);
				}
	
				
			}
			
			ArrayList lstCultivo = new ArrayList();
			for (int h=0; h<bij.getBienInmueble().getLstCultivos().size();h++){
				lstCultivo.add(bij.getBienInmueble().getLstCultivos().get(h));
			}
			
			
			Iterator itCultivos = lstCultivo.iterator();
...
//				nodo.add(nodoCultivo);
				...
				{  
					annadeCultivo(cultivo, true, nodo);
				}	
				else{
					annadeCultivo(cultivo, false, nodo);
				}
			}
        ...
        {
            HideableNode nodo = panelExpedientes.addObject((DefaultMutableTreeNode)arbol.getModel().getRoot(),bijCopy,true);
            panelExpedientes.annadeFincaOBI(bijCopy);
            actualizaArbol(nodo);
        }
    }
    
    public void annadeUnidadConstructivaCatastro(UnidadConstructivaCatastro ucc, HideableNode nodoOrigen) {
    	
    	
    	if(fincaEditada!=null){
    		
    		UnidadConstructivaCatastro uccCopy = ucc.clone(ucc);
    		uccCopy.setRefParcela(((FincaCatastro)nodoOrigen.getUserObject()).getRefFinca());
    		...
    		HideableNode nodo = panelExpedientes.addObject(nodoOrigen,uccCopy,true);
            actualizaArbol(nodo);
    ...
    private void annadeOActualizaRepresentante(Titular representante)
    {
        ...
        if(!(Boolean)AppContext.getApplicationContext().getBlackboard().get("expSitFinales")){
    		representante.setElementoModificado(true);
    		EdicionUtils.enablePanel(FincaPanel.this.getJPanelDatosBienInmueble(), true);
    		bienInmuebleJuridico.setElementoModificado(true);
    		if(!bienInmuebleJuridico.getTIPO_MOVIMIENTO().equals(BienInmuebleJuridico.TIPO_MOVIMIENTO_ALTA) &&
    				!bienInmuebleJuridico.getTIPO_MOVIMIENTO().equals(BienInmuebleJuridico.TIPO_MOVIMIENTO_BAJA)){
    			bienInmuebleJuridico.setTIPO_MOVIMIENTO(BienInmuebleJuridico.TIPO_MOVIMIENTO_MODIF);
  
    			if(bienInmuebleJuridico.getBienInmueble()!= null){
    				if(!bienInmuebleJuridico.getBienInmueble().getTipoMovimiento().equals(BienInmuebleCatastro.TIPO_MOVIMIENTO_ALTA) &&
    						!bienInmuebleJuridico.getBienInmueble().getTipoMovimiento().equals(BienInmuebleCatastro.TIPO_MOVIMIENTO_BAJA)){
    					bienInmuebleJuridico.getBienInmueble().setTipoMovimiento(BienInmuebleJuridico.TIPO_MOVIMIENTO_MODIF);
    				}
    ...
    public void annadeConstruccion(ConstruccionCatastro cons, boolean comun, HideableNode nodoOrigen)
    {
    	ConstruccionCatastro consCopy = cons.clone(cons);
...
//            	String id = refFinca + numBien;
    ...
    public void annadeCultivo(Cultivo cult, boolean comun, HideableNode nodoOrigen)
    {
    	
    	Cultivo cultivoCopy = cult.clone(cult);
    ...
    public void annadeTitular(Titular personaTitular, HideableNode nodoOrigen)
    {
        if(bienInmuebleJuridico!=null)
        {
            ...
            Titular personaTitularCopy = personaTitular.clone(personaTitular);
            lstTitulares.add(personaTitularCopy);
//            HideableNode nodo = panelExpedientes.addObject((HideableNode)arbol.getSelectionPath().getLastPathComponent()
//                    ,personaTitular,true);
            
            HideableNode nodo = panelExpedientes.addObject(nodoOrigen,personaTitularCopy,true);
            //HideableNode nodo = panelExpedientes.addObject(nodoOrigen,personaTitular,true);
    ...
    public void annadeSueloCatastro(SueloCatastro sueloCatastro, HideableNode nodoOrigen)
    {
    
    	if(fincaEditada!=null){
    		
    		SueloCatastro sueloCatastroCopy = sueloCatastro.clone(sueloCatastro);
    		sueloCatastroCopy.setRefParcela(((FincaCatastro)nodoOrigen.getUserObject()).getRefFinca());
    		...
    		lstSuelo.add(sueloCatastroCopy);
    		//fincaEditada.setLstUnidadesConstructivas(lstSuelo);
    		fincaEditada.setLstSuelos(lstSuelo);
    		HideableNode nodo = panelExpedientes.addObject(nodoOrigen,sueloCatastroCopy,true);
    ...
    private void annadirComunidadBienes(Titular cm)
    {
        ...
        if (bienInmuebleJuridico != null)            
        {
            ...
            if(!(Boolean)AppContext.getApplicationContext().getBlackboard().get("expSitFinales")){
            	...
            	//hay que marcar todos los titulares del bien como modificados
            	if(bienInmuebleJuridico.getLstTitulares()!= null && !bienInmuebleJuridico.getLstTitulares().isEmpty()){
            		for(int h=0; h<bienInmuebleJuridico.getLstTitulares().size(); h++){
            			Titular titular = (Titular)bienInmuebleJuridico.getLstTitulares().get(h);
            			if(!titular.getTIPO_MOVIMIENTO().equals(Titular.TIPO_MOVIMIENTO_ALTA) &&
            					!titular.getTIPO_MOVIMIENTO().equals(Titular.TIPO_MOVIMIENTO_BAJA)){
            				titular.setTIPO_MOVIMIENTO(Titular.TIPO_MOVIMIENTO_MODIF);
            				titular.setElementoModificado(true);
            			}
            		}
            ...
            for (int j=0;j<lstComunidadBienes.size();j++)
            {
                ...
                if (comunidadBien.getNif()!=null &&comunidadBien.getNif().equalsIgnoreCase(cm.getNif()))
                {
                	if(!(Boolean)AppContext.getApplicationContext().getBlackboard().get("expSitFinales")){
                    	//expediente de variaciones
                    	if(!comunidadBien.getTIPO_MOVIMIENTO().equals(ComunidadBienes.TIPO_MOVIMIENTO_ALTA) &&
                    			!comunidadBien.getTIPO_MOVIMIENTO().equals(ComunidadBienes.TIPO_MOVIMIENTO_BAJA)){
                    		comunidadBien.setTIPO_MOVIMIENTO(ComunidadBienes.TIPO_MOVIMIENTO_MODIF);
                    	}
            ...
            if(!(Boolean)AppContext.getApplicationContext().getBlackboard().get("expSitFinales")){
            	...
            	com.setTIPO_MOVIMIENTO(ComunidadBienes.TIPO_MOVIMIENTO_ALTA);
            }
	 ...
	 public JTree getArbol() {
		...
		public JButton getjButtonActualizar() {
			if(jButtonActualizar == null){
				...
				jButtonActualizar.setText(I18N.get("Expedientes","finca.boton.datosfinca.actualizar"));
				jButtonActualizar.setName("_actualizarParcela");	 
		...
		public JButton getjButtonConsultar() {
			if(jButtonConsultar == null){
				...
				jButtonConsultar.setText(I18N.get("Expedientes","finca.boton.datosfinca.concultar"));
				jButtonConsultar.setName("_consultarParcela");
				jButtonConsultar.addActionListener(new java.awt.event.ActionListener()
	             {
					 public void actionPerformed(java.awt.event.ActionEvent e)
					 {
											 
						 if(gestionExpedientePanel.getExpediente().getTipoExpediente().getConvenio().equals(DatosConfiguracion.CONVENIO_FISICO_ECONOMICO)){
							 Collection coll = new ArrayList();
							
							 for(int i=0; i<gestionExpedientePanel.getExpediente().getListaReferencias().size(); i++){
								 
								 FincaCatastro finca = (FincaCatastro)gestionExpedientePanel.getExpediente().getListaReferencias().get(i);
								 
								 if(finca.getRefFinca().getRefCatastral().equals(fincaEditada.getRefFinca().getRefCatastral())){

									 try {
										 int idFinca = ConstantesRegExp.clienteCatastro.getIdFincaCatastro(fincaEditada.getRefFinca().getRefCatastral());
										 coll.add(idFinca);
									 }
									 catch (Exception e1)
					                    {
							 ...
							 DialogoVerInfCatastralFisicoEconomico dialog = new  DialogoVerInfCatastralFisicoEconomico(app.getMainFrame(),
								 true, coll, ConstantesCatastro.tipoConvenio, "");
							
							 dialog.setSize(1000, 700);
							 dialog.setVisible(true); 
						 }
						 else if(gestionExpedientePanel.getExpediente().getTipoExpediente().getConvenio().equals(DatosConfiguracion.CONVENIO_TITULARIDAD)){
							 Collection coll = new ArrayList();
							 for(int i=0; i<gestionExpedientePanel.getExpediente().getListaReferencias().size(); i++){
								 String idBienInmueble = "";
								 if(gestionExpedientePanel.getExpediente().getListaReferencias().get(i) instanceof BienInmuebleCatastro){
									 BienInmuebleCatastro bien = (BienInmuebleCatastro)gestionExpedientePanel.getExpediente().getListaReferencias().get(i);
									 idBienInmueble = bien.getIdBienInmueble().getIdBienInmueble();
								 }
								 else if(gestionExpedientePanel.getExpediente().getListaReferencias().get(i) instanceof BienInmuebleJuridico){
									 BienInmuebleJuridico bien = (BienInmuebleJuridico)gestionExpedientePanel.getExpediente().getListaReferencias().get(i);
									 idBienInmueble = bien.getBienInmueble().getIdBienInmueble().getIdBienInmueble();
								 }
								
								 						 
								 if(idBienInmueble.equals(bienInmuebleJuridico.getBienInmueble().getIdBienInmueble().getIdBienInmueble())){

									 try {
										 int idFinca = ConstantesRegExp.clienteCatastro.getIdFincaCatastro(bienInmuebleJuridico.getBienInmueble().getIdBienInmueble().getParcelaCatastral().getRefCatastral());
										 coll.add(idFinca);
									 }
									 catch (Exception e1)
					                    {
							 ...
							 DialogoVerInfCatastralTitularidad dialog = new DialogoVerInfCatastralTitularidad(app.getMainFrame(),
								 true, coll, ConstantesCatastro.tipoConvenio, "");
							 dialog.setSize(1000, 700);
							 dialog.setVisible(true); 
						}
		...
		public void setjButtonConsultar(JButton jButtonConsultar) {
		  ...
		  public FincaCatastro getFincaEditada() {
				return fincaEditada;
			}

			public void setFincaEditada(FincaCatastro fincaEditada) {
				this.fincaEditada = fincaEditada;
			}
