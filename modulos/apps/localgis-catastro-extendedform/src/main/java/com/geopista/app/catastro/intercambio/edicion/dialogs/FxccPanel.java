...
import com.geopista.ui.plugin.synchronizeautocad.SynchronizeAutoCADPlugIn;
...
public class FxccPanel extends JPanel implements FeatureExtendedPanel{
	...
	public String lastRefCatastral = "";
    	...
    	if(file != null && !file.equals("") && !file.equals(" ")){
	    	...
	    	for (Iterator i = dxfLoad.getDataSourceQueries(fileDXF).iterator(); i.hasNext();) {
	    		...
	    		try {
	    			...
	    			if (dataset != null) {
	    				Layer currentLayer = geopistaEditor.getLayerManager().addLayer(
	    						chooseCategory(dataSourceQuery),
	    						dataSourceQuery.toString(), dataset).setDataSourceQuery(dataSourceQuery).setFeatureCollectionModified(false);
	    				...
	    				if(dxfLoad instanceof GeopistaLoadDxfQueryChooser)
	    				{
	    					if(currentLayer instanceof GeopistaLayer)
	    					{
	    						...
	    						String logFilePath = (String)dataSourceQuery.getDataSource().getProperties().get(SynchronizeAutoCADPlugIn.ORIGINAL_FILE_KEY);
...
//    			return I18N.get("Expedientes","fxcc.panel.CategoriaPS");
    ...
    private String getNombrePlantaASC(String nomPlantaDXF){
    	...
    	String nombreASC = "";
    	String nombreDXF = "";
    	if(nomPlantaDXF.contains("-")){
    		nombreDXF = nomPlantaDXF.substring(2,nomPlantaDXF.indexOf("-"));
    	}
    	else{
    		nombreDXF = nomPlantaDXF.substring(2,4);
    	}
    ...
    public void initDXF() {
        ...
        progressDialog.report(I18N.get("Expedientes","fxcc.panel.ComprobandoFicheroDXF"));
    			...
    			new Thread(new Runnable()
    			{
    				public void run()
    				{
    					...
    					{   
    						...
    						if(textFxcc == null){
    							textFxcc = "<fxcc><dxf> </dxf><asc> </asc></fxcc>";	
    						}
    						...
    						if(textFxcc!=null){
    							...
    							if(dxf!=null){
    								...
    								progressDialog.report(I18N.get("Expedientes","fxcc.panel.CargandoFicheroDXF"));
    								...
    								try {
    									...
    									if (textoAscTemp!=null && !textoAscTemp.equals("") && !textoAscTemp.equals(" ")){
...
//    	}
	...
	private void setLstPlantasSignificativas(){
					...
					if (actualCategory.getName().startsWith("PS")){
						...
						if (namesCategory != null && namesCategory.length>0){
							...
							if (nameCategory != null && !nameCategory.equals("")){
								PlantaSignificativaInfo pInfo = new PlantaSignificativaInfo();
								pInfo.setNombrePlantas(nameCategory);
								pInfo.setNumPlantas(1);
								ASCPanel.get_instance().infoplantas.put(pInfo.getNombrePlantas(), pInfo);
							}
						}
					}
				}
