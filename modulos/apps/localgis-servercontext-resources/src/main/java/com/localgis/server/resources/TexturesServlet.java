 ...
 *
 */
public class TexturesServlet extends HttpServlet
{
     ...
     */
    private static final Log logger = LogFactory.getLog(TexturesServlet.class);
    ...
    {
    	try{
	    	byte [] zip = ZIPCreatorUtils.createZip(ZIPConstants.TEXTURAS_ZIPNAME, 
	    			new File(this.getClass().getResource(ZIPConstants.TEXTURAS_DIR).getPath()),false);
	        ServletOutputStream sos = response.getOutputStream();
	        response.setContentType("application/zip");      
	        response.setHeader("Content-Disposition", "attachment; filename=\"" + ZIPConstants.TEXTURAS_ZIPNAME + "\"");
	        sos.write(zip);
	        sos.flush();
    	}
        catch(Exception ex){
