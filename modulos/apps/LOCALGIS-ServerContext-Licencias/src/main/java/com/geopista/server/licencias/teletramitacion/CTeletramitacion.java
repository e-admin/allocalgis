		...
		try {
			...
			while (rs.next()) {
                ...
                if (retorno == 0) {
                    ...
                    if (formularioSolicitudOk(hCampos)){
                        ...
                        if (!procesaRegistroTelematico(registroTeletramitacion)) {
                            ...
                            COperacionesDatabaseLicencias.insertarNotifTelematico(null, registroTeletramitacion.getNifSolicitante(), cnom, "Solicitud Rechazada", "Acuse Recibo", "Solicitud Rechazada", CConstantesTeletramitacion.VU_NOTIF_CRUTAACC, CConstantesTeletramitacion.HITOESTEXP_MESSAGE_CAMPOS_OBLIGATORIOS, "GeoPISTA");
                        } else
                        ...
                        /** Insertamos una notificacion en el sistema de teletramitacion */
                        COperacionesDatabaseLicencias.insertarNotifTelematico(null, nifSolicitante, cnom, "Solicitud Rechazada", "Acuse Recibo", "Solicitud Rechazada", CConstantesTeletramitacion.VU_NOTIF_CRUTAACC, CConstantesTeletramitacion.HITOESTEXP_MESSAGE_FORMULARIO_ERRONEO, "GeoPISTA");
                    }
                }
                ...
                if (retorno != 2) {
                    ...
                    COperacionesDatabaseLicencias.changeEstadoTelematico(solicitudLicencia, 1);
                }
			}
	...
	public static boolean procesaRegistroTelematico(CRegistroTeletramitacion registroTeletramitacion) {
		...
		try {
			...
			try {
				...
				expediente.setVU("1");
				CResultadoOperacion resultado = COperacionesDatabaseLicencias.crearExpedienteLicencias(solicitud, expediente, null, solicitud.getIdMunicipio());
			...
			try {
				...
				if (!rs.next()) {
					...
					if (num_expediente == null || num_expediente.length() <= 0) {
						COperacionesDatabaseLicencias.insertarNotifTelematico(null, nifSolicitante, cnom, "Aportación de documentación denegada", "Acuse Recibo", "Expediente inexistente", CConstantesTeletramitacion.VU_NOTIF_CRUTAACC, "No se puede efectuar la aportación de documentación porque el número de expediente " + num_expediente + " no existe en el sistema.", "GeoPISTA");
					} else {
						long secuencia = COperacionesDatabaseLicencias.insertarNotifTelematico(num_expediente, nifSolicitante, cnom, "Aportación de documentación denegada", "Acuse Recibo", "Expediente inexistente", CConstantesTeletramitacion.VU_NOTIF_CRUTAACC, null, "GeoPISTA");
					}
					return isAportacion;
				}
				...
				if (iIdEstado != CConstantesComando.ESTADO_MEJORA_DATOS && iIdEstado != CConstantesComando.ESTADO_ESPERA_ALEGACIONES) {
					...
					long secuencia = COperacionesDatabaseLicencias.insertarNotifTelematico(num_expediente, nifSolicitante, cnom, "Aportación de documentación denegada", "Acuse Recibo", "Estado del expediente incorrecto", CConstantesTeletramitacion.VU_NOTIF_CRUTAACC, null, cproc);
					return isAportacion;
				}
				...
				if (iIdEstado == CConstantesComando.ESTADO_MEJORA_DATOS) {
					...
					COperacionesDatabaseLicencias.insertarMejora(connection,mejora);
					*/
					/** Solo hay una mejora, pero en una version anterior podia haber varias mejoras. */
					Vector mejoras = COperacionesDatabaseLicencias.getMejorasSolicitud(connection, lIdSolicitud);
					if ((mejoras != null) && (mejoras.size() > 0)) {
						...
						//mejora.setAnexos(anexos);
						COperacionesDatabaseLicencias.modificaAnexosSolicitud(lIdSolicitud, mejora.getAnexos(), mejora, null);
						...
						COperacionesDatabaseLicencias.insertEventoExpediente(expe, "Mejora de datos insertada por teletramitación.", true, false);
				...
				if (iIdEstado == CConstantesComando.ESTADO_ESPERA_ALEGACIONES) {
					...
					Alegacion alegacion = COperacionesDatabaseLicencias.getAlegacion(connection, num_expediente);
					if (alegacion != null) {
						...
						COperacionesDatabaseLicencias.modificaAnexosSolicitud(lIdSolicitud, alegacion.getAnexos(), null, alegacion);
						CExpedienteLicencia expe = new CExpedienteLicencia();
						expe.setNumExpediente(num_expediente);
						expe.setIdSolicitud(lIdSolicitud);
						expe.setEstado(new CEstado(iIdEstado));
						COperacionesDatabaseLicencias.insertEventoExpediente(expe, "Alegación insertada por teletramitación.", true, false);
						...
						for (int i = 0; i < children.length; i++) {
							...
							if (file.isFile()) {
								if (file.getName().equalsIgnoreCase(nombre)) {
									...
									byte[] content = COperacionesDatabaseLicencias.getBytesFromFile(file);
