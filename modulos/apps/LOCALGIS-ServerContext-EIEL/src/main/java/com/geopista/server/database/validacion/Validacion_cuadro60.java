...
public class Validacion_cuadro60 {
	
	private static org.apache.log4j.Logger logger = org.apache.log4j.Logger
			.getLogger(Validacion_cuadro60.class);
	...
	public static void validacion(Connection connection, StringBuffer str, ArrayList lstValCuadros){
		...
		try
        {  
			str.append(Messages.getString("cuadro60") + "\n");
			str.append("______________________________________________________________________\n\n");
			String sql = "select * from v_CENTRO_ENSENANZA";
	        String sql_1 = "select * from v_NIVEL_ENSENANZA";
			...
			for (int i = 0; i < lstcentro_ensenanza_bean.size(); i++)
            {
				...
				if(centro_ensenanza_bean.getEstado().equals("")){
				
					if (contTexto == 0)
					 {
						 str.append(Messages.getString("falsoErrorMPT") + " " +Messages.getString("cuadro60.falsoerror.V_1") + "\n");
						 str.append("\n");
						 contTexto++;
					 }
			...
			if(lstValCuadros.contains("v01")){
				...
				for (int i = 0; i < lstcentro_ensenanza_bean.size(); i++)
	            {
					...
					if(centro_ensenanza_bean.getS_cubi() == 0 && centro_ensenanza_bean.getS_aire() == 0 &&
							!centro_ensenanza_bean.getEstado().equals("E"))	{
					
						if (contTexto == 0)
						 {
							 str.append(Messages.getString("errorMPT") + " " +Messages.getString("cuadro60.V_01") + "\n");
							 str.append("\n");
							 contTexto++;
						 }
						 str.append( centro_ensenanza_bean.getClave() + centro_ensenanza_bean.getProvincia() + 
			...
			if(lstValCuadros.contains("v02")){
				...
				for (int i = 0; i < lstcentro_ensenanza_bean.size(); i++)
	            {
					...
					if( (centro_ensenanza_bean.getS_cubi() + centro_ensenanza_bean.getS_aire()) <= 0 )	{
						
					
						if (contTexto == 0)
						 {
							 str.append(Messages.getString("errorMPT") + " " +Messages.getString("cuadro60.V_02") + "\n");
							 str.append("\n");
							 contTexto++;
						 }
						 str.append( centro_ensenanza_bean.getClave() + centro_ensenanza_bean.getProvincia() + 
        ...
        catch (Exception e)
        {
        	...
        	str.append(Messages.getString("exception") + " " +Validacion_cuadro60.class + e.getMessage());
        	str.append("\n\n");
        }      	
        finally{
