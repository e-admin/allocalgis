...
public class Validacion_cuadro51 {
	
	private static org.apache.log4j.Logger logger = org.apache.log4j.Logger
			.getLogger(Validacion_cuadro51.class);
	public static void validacion(Connection connection, StringBuffer str, ArrayList lstValCuadros){
		...
		try
        {  
			str.append(Messages.getString("cuadro51") + "\n");
			str.append("______________________________________________________________________\n\n");
			String sql = "select * from v_CENT_CULTURAL";
			String sql_v02 = "select * from v_CENT_CULTURAL where S_AIRE > S_SOLA";
			String sql_v03 = "select * from v_CENT_CULTURAL where (S_AIRE+S_CUBI)<=0";
			String sql_1 = "select * from v_CENT_CULTURAL_USOS";
			
			ArrayList lstcent_cultural = new ArrayList();
			ArrayList lstcent_cultural_v02 = new ArrayList();
			ArrayList lstcent_cultural_v03 = new ArrayList();
			...
			while (rs.next()) {	
				...
				lstcent_cultural_v02.add(cent_culturalBean);
			...
			while (rs.next()) {	
				...
				lstcent_cultural_v03.add(cent_culturalBean);
			...
			for (int i = 0; i < lstcent_cultural.size(); i++)
            {
				...
				if(cent_cultural_bean.getOrden_cent().equals(""))
				{
					if (contTexto == 0)
					 {
						 str.append(Messages.getString("falsoErrorMPT") + " " +Messages.getString("cuadro51.falsoerror.V_2") + "\n");
						 str.append("\n");
						 contTexto++;
					 }
					 str.append( cent_cultural_bean.getClave() + cent_cultural_bean.getProvincia() + 
							 cent_cultural_bean.getMunicipio()  + cent_cultural_bean.getPoblamient()+
							 cent_cultural_bean.getOrden_cent()+"\t");
					 error = true;
	            }
            }	
			if (error)
			...
			for (int i = 0; i < lstcent_cultural.size(); i++)
            {
				...
				if(cent_cultural_bean.getEstado().equals(""))
				{
					if (contTexto == 0)
					 {
						 str.append(Messages.getString("falsoErrorMPT") + " " +Messages.getString("cuadro51.falsoerror.V_1") + "\n");
						 str.append("\n");
						 contTexto++;
					 }
					 str.append( cent_cultural_bean.getClave() + cent_cultural_bean.getProvincia() + 
			...
			if(lstValCuadros.contains("v01")){
				...
				for (int i = 0; i < lstcent_cultural.size(); i++)
	            {
					...
					if(cent_cultural_bean.getS_cubi() == 0 && cent_cultural_bean.getS_aire() == 0 &&
							!cent_cultural_bean.getEstado().equals("E"))	{
						if (contTexto == 0)
						 {
							 str.append(Messages.getString("errorMPT") + " " +Messages.getString("cuadro51.V_01") + "\n");
							 str.append("\n");
							 contTexto++;
						 }
						 str.append( cent_cultural_bean.getClave() + cent_cultural_bean.getProvincia() + 
			...
			if(lstValCuadros.contains("v02")){
				//ERROR DEL MPT -> (V_02) 
				for (int i = 0; i < lstcent_cultural_v02.size(); i++)
	            {
					V_cent_cultural_bean cent_cultural_bean    = (V_cent_cultural_bean)lstcent_cultural_v02.get(i);
					
					if (contTexto == 0)
					 {
						 str.append(Messages.getString("errorMPT") + " " +Messages.getString("cuadro51.V_02") + "\n");
						 str.append("\n");
						 contTexto++;
					 }
					 str.append( cent_cultural_bean.getClave() + cent_cultural_bean.getProvincia() + 
			...
			if(lstValCuadros.contains("v03")){
				//ERROR DEL MPT -> (V_03) 
				for (int i = 0; i < lstcent_cultural_v03.size(); i++)
	            {
					V_cent_cultural_bean cent_cultural_bean    = (V_cent_cultural_bean)lstcent_cultural_v03.get(i);
					
					if (contTexto == 0)
					 {
						 str.append(Messages.getString("errorMPT") + " " +Messages.getString("cuadro51.V_03") + "\n");
						 str.append("\n");
						 contTexto++;
					 }
					 str.append( cent_cultural_bean.getClave() + cent_cultural_bean.getProvincia() + 
			...
			if(lstValCuadros.contains("v04")){
				...
				for (int i = 0; i < lstcent_cultural.size(); i++)
	            {
					...
					if (count == 0){
						if (contTexto == 0)
						 {
							 str.append(Messages.getString("errorMPT") + " " +Messages.getString("cuadro51.V_04") + "\n");
							 str.append("\n");
							 contTexto++;
						 }
						 str.append( cent_cultural_bean.getClave() + cent_cultural_bean.getProvincia() + 
				...
				for (int i = 0; i < lstcent_cultural.size(); i++)
	            {
					...
					if (count != 0){
						if(suma != cent_cultural_bean.getS_cubi()){
							if (contTexto == 0)
							 {
								 str.append(Messages.getString("errorMPT") + " " +Messages.getString("cuadro51.V_05") + "\n");
								 str.append("\n");
								 contTexto++;
							 }
							 str.append( cent_cultural_bean.getClave() + cent_cultural_bean.getProvincia() + 
        ...
        catch (Exception e)
        {
        	...
        	str.append(Messages.getString("exception") + " " +Validacion_cuadro51.class + e.getMessage());
        	str.append("\n\n");
        }      	
        finally{
