...
public class Validacion_cuadro04 {
	private static org.apache.log4j.Logger logger = org.apache.log4j.Logger
			.getLogger(Validacion_cuadro04.class);
		...
		try
        {
			...
			for (int i = 0; i < lsttramo_carretera.size(); i++)
            {
				...
				if(lstValCuadros.contains("v02")){
					...
					float diff = (float) ((((V_tramo_carretera_bean)lsttramo_carretera.get(i)).getPk_final() * 1000) -
							(((V_tramo_carretera_bean)lsttramo_carretera.get(i)).getPk_inicial() * 1000));
					float longi = (float) (((V_tramo_carretera_bean)lsttramo_carretera.get(i)).getLongitud()*1000);
