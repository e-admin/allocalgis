...
public class Validacion_cuadro15 {
	...
	private static org.apache.log4j.Logger logger = org.apache.log4j.Logger
			.getLogger(Validacion_cuadro15.class);
	...
	public static void validacion(Connection connection, StringBuffer str, ArrayList lstValCuadros){
		...
		try
        {
			...
			String sql_sin_nucleo = "select * from v_deposito_enc_sin_nucleo";
			...
			ArrayList lstdeposito_enc_sin_nucleo = new ArrayList();
			...
			while (rs.next()) {	
				...
				if(rs.getString("LIMPIEZA")!=null &&!rs.getString("LIMPIEZA").equals(""))
					...
					deposito_enc_bean.setLimpieza(0);
				deposito_enc_bean.setContador(rs.getString("CONTADOR"));
				lstdeposito_enc_sin_nucleo.add(deposito_enc_bean);
			...
			if(lstValCuadros.contains("v01")){
				...
				for (int i = 0; i < lstdeposito_enc_sin_nucleo.size(); i++)
	            {
					int count = 0;
					V_deposito_enc_bean  deposito_enc_bean  = (V_deposito_enc_bean)lstdeposito_enc_sin_nucleo.get(i);					
					if (count == 0){
						if (contTexto == 0)
						 {
							 str.append(Messages.getString("errorMPT") + " " +Messages.getString("cuadro15.V_01") + "\n");
