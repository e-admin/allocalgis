 ...
 */
package com.gestorfip.ui.rpm.tool;
...
import com.gestorfip.ui.rpm.images.IconLoader;
...
public class RPMTool extends DragTool {
	...
	public static final ImageIcon ICON = IconLoader.icon("rpm.gif");
...
//    }
    ...
    protected void gestureFinished() throws Exception {
    	 ...
    	 FipPanelBean fipSelected = (FipPanelBean) appContext.getBlackboard().get(ConstantesGestorFIP.FIP_TRABAJO);
    	 String srs = fipSelected.getSrs();
...
//    	 ConfiguracionGestor config = (ConfiguracionGestor)appContext.getBlackboard().get(ConstantesGestorFIP.CONFIG_VERSION_CONSOLE_UER);
    	 ...
    	 for(int i=0; i<versiones.length; i++){
    		 if(versiones[i].getId() == config.getIdVersion()){
    			 ...
    			 if(versiones[i].getVersion() == ConstantesGestorFIP.VERSIONCONSOLA_UER_1_86){
    				//Consola 1.86v
    			    url = urlConsoleUERWS+"/lanzaderas/arbolambitosVIS/arbolambVIS.jsp?debugMode=on&rutaServ="+urlConsoleUERWS+"/&srs="+srs+"&x="+geometry.getCoordinate().x+"&y="+geometry.getCoordinate().y+"&idAmbito="+String.valueOf(idAmbitoTrabajo.intValue());;
    ...
    private static String getLinuxDesktop(){
	      ...
	      //solo se averigua el entorno de escritorio una vez, despues se almacena en la variable est�tica
