...
public class SaveInventario extends Save {
	...
	int totalElementosRepetidos = 0;
	int totalElementosPotencialmenteRepetidos = 0;

	TreeMap documentosPorInventario = new TreeMap();
	TreeMap documentosRepetidos = new TreeMap();
	...
	public SaveInventario(java.awt.Frame parent, boolean modal,
		...
		totalElementosRepetidos = 0;
		totalElementosPotencialmenteRepetidos = 0;
		documentosPorInventario.clear();
		documentosRepetidos.clear();
	...
	/*private void saveFiles(TaskMonitorDialog progressDialog, StringBuffer sb)
			throws Exception {
		...
		Collection<DocumentBean> documentos= getDocuments();
			...
			int totalElementos=documentos.size();
			int errores=0;
...
//				BufferedWriter document = new java.io.BufferedWriter(new java.io.FileWriter(lastDirectory+"\\"+documentBean.getFileName()));			
					...
					try {
						...
						logger.error("No se ha podido guardar el adjunto del inventario:"+documentBean.getFileName());
				document.flush();
				document.close();
				try {
					File file = new File(lastDirectory + "\\"
							+ documentBean.getFileName());
					file.delete();
				} catch (Exception e1) {
				}
						errores++;
...
//				}
		...
		if (errores > 0) {
			logger.error("Se han producido " + errores + "/" + totalElementos
					+ " errores al salvar los adjuntos del inventario");
			sb.append(totalElementos - errores);
			throw new Exception("Se han producido " + errores + "/"
					+ totalElementos
					+ " errores al salvar los adjuntos del inventario");
		}
...
//			}
	...
	private boolean saveFile(DocumentBean documentBean) throws Exception {
		byte[] content = documentBean.getThumbnail();

		File file2 = new File(lastDirectory + "\\" + documentBean.getFileName());
		if (file2.exists())
			totalElementosRepetidos++;
		
		
		if (documentosRepetidos.containsKey(documentBean.getFileName()))
			totalElementosPotencialmenteRepetidos++;		
		else
			documentosRepetidos.put(documentBean.getFileName(), documentBean.getFileName());		


		OutputStream document = new FileOutputStream(lastDirectory + "\\"
				+ documentBean.getFileName());
		try {
			document.write(documentClient.getAttachedByteStream(documentBean));
			return true;
		} catch (Exception e) {
			logger.error("No se ha podido guardar el adjunto del inventario:"
					+ documentBean.getFileName());
			document.flush();
			document.close();
			try {
		...
		if (jRadioAllBienes.isSelected()) {
			...
			inmueblesUrbanos = inventarioClient
							.getBienesInventario(Const.ACTION_GET_INMUEBLES,
									...
									listafeatures, configParameters,true);
			inmueblesUrbanos=getBienesActualVersion(inmueblesUrbanos);
			
			if (progressDialog.isCancelRequested()) return "";
			...
			derechosReales=inventarioClient.getBienesInventario(
					Const.ACTION_GET_DERECHOS_REALES,
							...
							null, null, listafeatures, configParameters,true);
			...
			credYDer = inventarioClient
					.getBienesInventario(Const.ACTION_GET_CREDITOS_DERECHOS,
							...
							null, listafeatures, configParameters,true);
...
//			
			...
			bienesRevertibles = inventarioClient
					.getBienesRevertibles(null, null, filtros, configParameters,true);
			...
			if(bienes!=null){
				for (BienBean bien : bienes) {
					...
					if (bien instanceof CreditoDerechoBean) {
						try {
							bien = (BienBean) inventarioClient
									.getBienInventario(
											Const.ACTION_GET_CREDITO_DERECHO,
											Const.PATRON_CREDITOS_DERECHOS_PERSONALES,
											bien.getId(),
											bien.getRevisionActual(),
					...
					if (bien instanceof DerechoRealBean) {
						try {
							bien = (BienBean) inventarioClient
									.getBienInventario(
											Const.ACTION_GET_DERECHO_REAL,
											Const.PATRON_DERECHOS_REALES,
											bien.getId(),
											bien.getRevisionActual(),
											bien.getRevisionExpirada());
						} catch (Exception e) {
						...
						if (bien.getTipo().equals(
							...
							try {
								bien = (BienBean) inventarioClient
										.getBienInventario(
												Const.ACTION_GET_INMUEBLE,
												Const.PATRON_INMUEBLES_URBANOS,
												bien.getId(),
												bien.getRevisionActual(),
												bien.getRevisionExpirada());
							} catch (Exception e) {
					...
					if (bien instanceof SemovienteBean) {
						try {
							bien = (BienBean) inventarioClient
									.getBienInventario(
											Const.ACTION_GET_SEMOVIENTE,
											Const.PATRON_SEMOVIENTES,
											bien.getId(),
											bien.getRevisionActual(),
											bien.getRevisionExpirada());
						} catch (Exception e) {
					...
					if (bien instanceof VehiculoBean) {
						try {
							bien = (BienBean) inventarioClient
									.getBienInventario(
											Const.ACTION_GET_VEHICULO,
											Const.PATRON_VEHICULOS,
											bien.getId(),
											bien.getRevisionActual(),
											bien.getRevisionExpirada());
						} catch (Exception e) {
					...
					if (bien instanceof ViaBean) {
						...
						if (Const.PATRON_VIAS_PUBLICAS_RUSTICAS
								.equals(((ViaBean) bien).getTipo())) {
							try {
								bien = (BienBean) inventarioClient
										.getBienInventario(
												Const.ACTION_GET_VIA,
												Const.PATRON_VIAS_PUBLICAS_RUSTICAS,
												bien.getId(),
												bien.getRevisionActual(),
												bien.getRevisionExpirada());
							} catch (Exception e) {
							...
							try {
								bien = (BienBean) inventarioClient
										.getBienInventario(
												Const.ACTION_GET_VIA,
												Const.PATRON_VIAS_PUBLICAS_URBANAS,
												bien.getId(),
												bien.getRevisionActual(),
												bien.getRevisionExpirada());
							} catch (Exception e) {
					...
					if (bien instanceof MuebleBean) {
						if (Const.PATRON_MUEBLES_HISTORICOART
								.equals(((MuebleBean) bien).getTipo())) {
							try {
								bien = (BienBean) inventarioClient
										.getBienInventario(
												Const.ACTION_GET_MUEBLE,
												Const.PATRON_MUEBLES_HISTORICOART,
												bien.getId(),
												bien.getRevisionActual(),
												bien.getRevisionExpirada());
							} catch (Exception e) {
					...
					if (bien instanceof ValorMobiliarioBean) {
						try {
							bien = (BienBean) inventarioClient
									.getBienInventario(
											Const.ACTION_GET_VALOR_MOBILIARIO,
											Const.PATRON_VALOR_MOBILIARIO,
											bien.getId(),
											bien.getRevisionActual(),
											bien.getRevisionExpirada());
						} catch (Exception e) {
		...
		if (totalElementos != totalElementosProcesados) {
			logger.error("Se han producido "
					+ (totalElementos - totalElementosProcesados) + "/"
					+ totalElementos
					+ " errores al salvar los adjuntos del inventario.Main");
			ErrorDialog.show(progressDialog, "ERROR", messages
					.getString("inventario.saveinventario.documentos.error"),
					"Se han producido "
							+ (totalElementos - totalElementosProcesados) + "/"
							+ totalElementos
							+ " errores al salvar los adjuntos del inventario.Main \n"+
							"Total documentos sin repetir:"+(totalElementos-totalElementosPotencialmenteRepetidos)+"\n"+
							"Total documentos repetidos:"+totalElementosPotencialmenteRepetidos);
		}
		if (totalElementosRepetidos > 0)
			logger.info("Total de elementos que tienen la misma imagen:"
					+ totalElementosRepetidos);
		if (totalElementosPotencialmenteRepetidos > 0)
			logger.info("Total de elementos que tienen la misma imagen:"
					+ totalElementosPotencialmenteRepetidos);

		/*Iterator it = documentosPorInventario.entrySet().iterator();
		while (it.hasNext()) {
			Map.Entry pairs = (Map.Entry) it.next();
			System.out.println(pairs.getKey() + " = " + pairs.getValue());
		}*/
		/*
		 * StringBuffer salida = new StringBuffer(""); try { if
		 * (progressDialog.isCancelRequested()) return ""; saveFiles(
		 * progressDialog,salida); }catch (Exception ex) {
		 * 
		 * logger.error(
		 * "Se ha producido un error al guardar los documentos adjuntos", ex);
		 * ErrorDialog .show(progressDialog, "ERROR",
		 * messages.getString("inventario.saveinventario.documentos.error"),
		 * StringUtil.stackTrace(ex)); } finally{ //clear documents
		 * documents.clear(); }
		 */
...
//		saveDatosAdicionales(bienRevertible.getObservaciones(), elementoPatrimonio)
	 ...
	 * @throws Exception 
	 */
	private int[] saveListadoDocumentos(Documentable bien,
		...
		if(documentos!=null){
			for (DocumentBean documentoBean : documentos){
				...
				totalElementos++;
				try {
					// System.out.println("Datos:"+documentoBean.getFileName()+" "+((BienBean)bien).getNumInventario());
					...
					if (documentosPorInventario.get(numInventario) != null) {
						Integer elementos = (Integer) documentosPorInventario
								.get(numInventario);
						documentosPorInventario.put(numInventario,
								elementos + 1);
					} else {
						documentosPorInventario.put(numInventario, 1);
					}
					boolean resultado = saveFile(documentoBean);
					if (resultado) {
						numDocumentosProcesados++;
						totalElementosProcesados++;
					}
					else{
						logger.error("Error al obtener el adjunto");
					}
				} catch (Exception e) {
					logger.error("Error al obtener el adjunto");
				}
				...
				// document = new java.io.BufferedWriter(new
				// java.io.FileWriter(lastDirectory+"\\"+documentBean.getFileName()));
				// document.write(documentClient.getAttachedByteStream(documentBean).toString());
				//
				// }
			}
		}
		int resultado[] = { documentos.size(), numDocumentosProcesados };
		return resultado;
	...
	private void insertarDatosCreditoDerecho(CreditoDerechoBean creditoYDerecho,
		...
		// cat_cred_der
		if (creditoYDerecho.getConcepto() != null) {
		...
		if (creditoYDerecho.getTipo() != null) {
		...
		if (creditoYDerecho.getSubClase() != null) {
		...
		datos_credito.appendChild(cat_subclase);
			...
			cat_subclase_tipo.setTextContent(Estructuras
					.getListaSubclaseCredito().getDominioPadre());
			// cat_subclase//id
...
//		insertarObservacion(observacionesED, datos_inmuebles,"edificabilidad_descripcion");
	...
	private void saveSeguro(Seguro seguro, Element elementoPatrimonio) {
		...
		if(seguro!=null){
			...
			poliza.setTextContent(seguro.getPoliza()!=null?formateador12.format(seguro.getPoliza()):Long.toString(0));
...
//
	...
	private void insertarDatosBienRevertible(BienRevertible bienRevertible,
		...
		int[] numDocumentosBienRevertible = saveListadoDocumentos(
				bienRevertible, datos_bienes_revertibles);

		if (resumen.get("bienes_revertibles" + DOCUMENTOS + TOTALES) != null) {
			int totalDocumentosActuales = (Integer) resumen
					.get("bienes_revertibles" + DOCUMENTOS + TOTALES);
			resumen.put("bienes_revertibles" + DOCUMENTOS + TOTALES,
					numDocumentosBienRevertible[0]+totalDocumentosActuales);
		} else {
			resumen.put("bienes_revertibles" + DOCUMENTOS + TOTALES,
					numDocumentosBienRevertible[0]);
		}

		if (resumen.get("bienes_revertibles" + DOCUMENTOS + PROCESADOS) != null) {
			int totalDocumentosActuales = (Integer) resumen
					.get("bienes_revertibles" + DOCUMENTOS + PROCESADOS);
			resumen.put("bienes_revertibles" + DOCUMENTOS + PROCESADOS,
					numDocumentosBienRevertible[1] + totalDocumentosActuales);
		} else {
			resumen.put("bienes_revertibles" + DOCUMENTOS + PROCESADOS,
					numDocumentosBienRevertible[1]);
		}
	...
	private void insertarDatosLote(Lote lote, Element datos)
		...
		if (resumen.get("lotes" + DOCUMENTOS + TOTALES) != null) {
			int totalDocumentosActuales = (Integer) resumen.get("lotes"
					+ DOCUMENTOS + TOTALES);
			resumen.put("lotes" + DOCUMENTOS + TOTALES, numDocumentosLotes[0]+totalDocumentosActuales);
		} else {
			resumen.put("lotes" + DOCUMENTOS + TOTALES, numDocumentosLotes[0]);
		}

		if (resumen.get("lotes" + DOCUMENTOS + PROCESADOS) != null) {
			int totalDocumentosActuales = (Integer) resumen.get("lotes"
					+ DOCUMENTOS + PROCESADOS);
			resumen.put("lotes" + DOCUMENTOS + PROCESADOS,
					numDocumentosLotes[1] + totalDocumentosActuales);
		} else {
			resumen.put("lotes" + DOCUMENTOS + PROCESADOS,
					numDocumentosLotes[1]);
		}
...
//			}
	...
	private void insertarInmuebles(Collection<InmuebleBean> inmuebles,
		...
		for (InmuebleBean inmueble : inmuebles) {// &&
			...
			if (!inmueble.getNombre().equals(""))
				logger.info("End Time: Inmueble:"+inmueble.getNumInventario()+" Nombre:" + inmueble.getNombre() + " :"+ (endMils - startMils) / 1000 + " segundos");
			else if (inmueble.getDescripcion()!=null)
				logger.info("End Time: Inmueble:"+inmueble.getNumInventario()+" Descripcion:" + inmueble.getDescripcion() + " :"+ (endMils - startMils) / 1000 + " segundos");
			else
				logger.info("End Time: Inmueble:" + inmueble.getNumInventario() + " :"+ (endMils - startMils) / 1000 + " segundos");
	...
	private void insertarVias(Collection<ViaBean> bienes,
		...
		for (ViaBean vias : bienes) {// &&
			...
			long endMils = Calendar.getInstance().getTimeInMillis();
			/*
			 * logger.info("End Time:" + vias.getNombre() + " :" + (endMils -
			 * startMils) / 1000 + " segundos");
			 */
		}
		//Guarda el numero de elementos procesados
	...
	private void insertarVehiculos(Collection<VehiculoBean> bienes,
		...
		for (VehiculoBean vehiculo : bienes) {// &&
			...
			long endMils = Calendar.getInstance().getTimeInMillis();
			/*
			 * logger.info("End Time:" + vehiculo.getNombre() + " :" + (endMils
			 * - startMils) / 1000 + " segundos");
			 */
		}
		//Guarda el numero de elementos porcesado
	...
	private void insertarSemovientes(Collection<SemovienteBean> bienes,
		...
		for (SemovienteBean semoviente: bienes) {// &&
			...
			long endMils = Calendar.getInstance().getTimeInMillis();
			/*
			 * logger.info("End Time:" + semoviente.getNombre()+ " :" + (endMils
			 * - startMils) / 1000 + " segundos");
			 */
		}
		//Guarda el numero de elementos porcesado
	...
	private void insertarLotes(Collection<Lote> lotes,
		...
		for (Lote lote: lotes) {// &&
			...
			long endMils = Calendar.getInstance().getTimeInMillis();
			/*
			 * logger.info("End Time:" + lote.getNombre_lote()+" :" + (endMils -
			 * startMils) / 1000 + " segundos");
			 */
		}
		//Guarda el numero de elementos porcesado
	...
	private void insertarValoresMobiliarios(Collection<ValorMobiliarioBean> valoresMoviliario,
		...
		// Guarda el numero de elementos porcesado
		resumen.put(Const.PATRON_VALOR_MOBILIARIO + BIENES + PROCESADOS,
				numeroElemento);
		progressDialog.report(getStringWithParameters(messages,
...
private void insertarResumen(TaskMonitorDialog progressDialog, Element patrimonio) {
		...
		bienes.appendChild(vehiculos);
			...
			//vias_urbanas
			Element vias_urbanas = doc.createElement("vias_urbanas");
			bienes.appendChild(vias_urbanas);

			Element totalBienViasUrbanas = doc.createElement("total");
			vias_urbanas.appendChild(totalBienViasUrbanas);
			totalBienViasUrbanas.setTextContent(Integer.toString(resumen.get(Const.PATRON_VIAS_PUBLICAS_URBANAS+BIENES+TOTALES)));
			totalBienes+=resumen.get(Const.PATRON_VIAS_PUBLICAS_URBANAS+BIENES+TOTALES);
			Element procesadosViasUrbanas=doc.createElement("procesados");
			vias_urbanas.appendChild(procesadosViasUrbanas);
			procesadosViasUrbanas.setTextContent(Integer.toString(resumen.get(Const.PATRON_VIAS_PUBLICAS_URBANAS+BIENES+PROCESADOS)));
			procesadosBienes+=resumen.get(Const.PATRON_VIAS_PUBLICAS_URBANAS+BIENES+PROCESADOS);
			
			//vias_rusticas
			Element vias_rusticas= doc.createElement("vias_rusticas");
			bienes.appendChild(vias_rusticas);

			Element totalBienViasRusticas = doc.createElement("total");
			vias_rusticas.appendChild(totalBienViasRusticas);
			totalBienViasRusticas.setTextContent(Integer.toString(resumen.get(Const.PATRON_VIAS_PUBLICAS_RUSTICAS+BIENES+TOTALES)));
			totalBienes+=resumen.get(Const.PATRON_VIAS_PUBLICAS_RUSTICAS+BIENES+TOTALES);
			Element procesadosViasRusticas=doc.createElement("procesados");
			vias_rusticas.appendChild(procesadosViasRusticas);
			procesadosViasRusticas.setTextContent(Integer.toString(resumen.get(Const.PATRON_VIAS_PUBLICAS_RUSTICAS+BIENES+PROCESADOS)));
			procesadosBienes+=resumen.get(Const.PATRON_VIAS_PUBLICAS_RUSTICAS+BIENES+PROCESADOS);
		...
		documentos.appendChild(otros_mueblesDoc);
			...
			totalDocOtMu.setTextContent(resumen.get(Const.PATRON_BIENES_MUEBLES+ DOCUMENTOS
					+ TOTALES) != null ? Integer.toString(resumen
							.get(Const.PATRON_BIENES_MUEBLES + DOCUMENTOS
									+ TOTALES)) : Integer.toString(0));
			totalDocumentos+=resumen.get(Const.PATRON_BIENES_MUEBLES 
					+ DOCUMENTOS + TOTALES) != null ? resumen
							.get(Const.PATRON_BIENES_MUEBLES  + DOCUMENTOS + TOTALES) : 0;
			Element procesadosDocOtMu=doc.createElement("procesados");
			otros_mueblesDoc.appendChild(procesadosDocOtMu);
			procesadosDocOtMu.setTextContent(resumen.get(Const.PATRON_BIENES_MUEBLES
						+ DOCUMENTOS + PROCESADOS) != null ? Integer
							.toString(resumen.get(Const.PATRON_BIENES_MUEBLES 
									+ DOCUMENTOS + PROCESADOS)) : Integer.toString(0));
			procesadosDocumentos+=resumen.get(Const.PATRON_BIENES_MUEBLES
					+ DOCUMENTOS + TOTALES) != null ? resumen
							.get(Const.PATRON_BIENES_MUEBLES + DOCUMENTOS + PROCESADOS) : 0;
		...
		documentos.appendChild(vehiculosDoc);
			...
			Element ViasUrbanasDoc = doc.createElement("vias_urbanas");
			documentos.appendChild(ViasUrbanasDoc);

			Element totalDocViasUrbanas = doc.createElement("total");
			ViasUrbanasDoc.appendChild(totalDocViasUrbanas);
			totalDocViasUrbanas.setTextContent(resumen.get(Const.PATRON_VIAS_PUBLICAS_URBANAS+ DOCUMENTOS
					+ TOTALES) != null ? Integer.toString(resumen
							.get(Const.PATRON_VIAS_PUBLICAS_URBANAS + DOCUMENTOS
									+ TOTALES)) : Integer.toString(0));
			totalDocumentos+=resumen.get(Const.PATRON_VIAS_PUBLICAS_URBANAS
					+ DOCUMENTOS + TOTALES) != null ? resumen
							.get(Const.PATRON_VIAS_PUBLICAS_URBANAS  + DOCUMENTOS + TOTALES) : 0;
			Element procesadosDocViasUrbanas=doc.createElement("procesados");
			ViasUrbanasDoc.appendChild(procesadosDocViasUrbanas);
			procesadosDocViasUrbanas.setTextContent(resumen.get(Const.PATRON_VIAS_PUBLICAS_URBANAS
						+ DOCUMENTOS + PROCESADOS) != null ? Integer
							.toString(resumen.get(Const.PATRON_VIAS_PUBLICAS_URBANAS
									+ DOCUMENTOS + PROCESADOS)) : Integer.toString(0));
			procesadosDocumentos+=resumen.get(Const.PATRON_VIAS_PUBLICAS_URBANAS
					+ DOCUMENTOS + TOTALES) != null ? resumen
							.get(Const.PATRON_VIAS_PUBLICAS_URBANAS+ DOCUMENTOS + PROCESADOS) : 0;
							
			//vias publicas rusticas
			Element ViasRusticasDoc = doc.createElement("vias_rusticas");
			documentos.appendChild(ViasRusticasDoc);

			Element totalDocViasRusticas = doc.createElement("total");
			ViasRusticasDoc.appendChild(totalDocViasRusticas);
			totalDocViasRusticas.setTextContent(resumen.get(Const.PATRON_VIAS_PUBLICAS_RUSTICAS+ DOCUMENTOS
					+ TOTALES) != null ? Integer.toString(resumen
							.get(Const.PATRON_VIAS_PUBLICAS_RUSTICAS + DOCUMENTOS
									+ TOTALES)) : Integer.toString(0));
			totalDocumentos+=resumen.get(Const.PATRON_VIAS_PUBLICAS_RUSTICAS
					+ DOCUMENTOS + TOTALES) != null ? resumen
							.get(Const.PATRON_VIAS_PUBLICAS_RUSTICAS  + DOCUMENTOS + TOTALES) : 0;
			Element procesadosDocViasRusticas=doc.createElement("procesados");
			ViasRusticasDoc.appendChild(procesadosDocViasRusticas);
			procesadosDocViasRusticas.setTextContent(resumen.get(Const.PATRON_VIAS_PUBLICAS_RUSTICAS
						+ DOCUMENTOS + PROCESADOS) != null ? Integer
							.toString(resumen.get(Const.PATRON_VIAS_PUBLICAS_RUSTICAS
									+ DOCUMENTOS + PROCESADOS)) : Integer.toString(0));
			procesadosDocumentos+=resumen.get(Const.PATRON_VIAS_PUBLICAS_RUSTICAS
					+ DOCUMENTOS + TOTALES) != null ? resumen
							.get(Const.PATRON_VIAS_PUBLICAS_RUSTICAS+ DOCUMENTOS + PROCESADOS) : 0;	
				...
				totalDocReVe.setTextContent(resumen.get("bienes_revertibles"+ DOCUMENTOS
						+ TOTALES) != null ? Integer.toString(resumen
								.get("bienes_revertibles" + DOCUMENTOS
										+ TOTALES)) : Integer.toString(0));
				totalDocumentos+=resumen.get("bienes_revertibles"
						+ DOCUMENTOS + TOTALES) != null ? resumen
		...
		resumen_contenido.appendChild(totales);
			...
			//TODO Revisar que valor poner aqui
			//total_documentos.setTextContent(Integer.toString(totalElementos));
	...
	private void insertarDatosComunes(BienBean inmuebleBean,
		...
		try {
			Element fecha_adquisicion = doc.createElement("fecha_adquisicion");
			bienesInventario.appendChild(fecha_adquisicion);
			fecha_adquisicion.setTextContent(dateToDDMMYYY(inmuebleBean
					.getFechaAdquisicion()));
		} catch (Exception e) {
		}

		patrimonio_municipal_suelo.setTextContent(inmuebleBean
				.getPatrimonioMunicipalSuelo() ? "1" : "0");
		// fecha_aprobacion_pleno
		...
		int[] numDocumentosInmueble = saveListadoDocumentos(inmuebleBean,
				bienesInventario);

		if (resumen.get(inmuebleBean.getTipo() + DOCUMENTOS + TOTALES) != null) {
			int totalDocumentosActuales = (Integer) resumen.get(inmuebleBean
					.getTipo() + DOCUMENTOS + TOTALES);
			resumen.put(inmuebleBean.getTipo() + DOCUMENTOS + TOTALES,
					numDocumentosInmueble[0] + totalDocumentosActuales);
		} else {
			resumen.put(inmuebleBean.getTipo() + DOCUMENTOS + TOTALES,
					numDocumentosInmueble[0]);
		}

		if (resumen.get(inmuebleBean.getTipo() + DOCUMENTOS + PROCESADOS) != null) {
			int totalDocumentosActuales = (Integer) resumen.get(inmuebleBean
					.getTipo() + DOCUMENTOS + PROCESADOS);
			resumen.put(inmuebleBean.getTipo() + DOCUMENTOS + PROCESADOS,
					totalDocumentosActuales + numDocumentosInmueble[1]);
		} else {
			resumen.put(inmuebleBean.getTipo() + DOCUMENTOS + PROCESADOS,
					numDocumentosInmueble[1]);
		}
