...
public class LoadInventario extends Load {
	...
	protected void aceptar() {
		try {
			...
			logger.error("Se ha producido un error al comprobar el fichero con su esquema",
					saex);
	...
	private void insertar(Object bien) throws Exception {
		...
		try {
			...
			if (DOCUMENT_SERVER_PATH == null){
				int numeroDocumentosBien=((Documentable) bien).getDocumentos().size();
				actualizaDatosDocError(bien, ((Documentable) bien).getDocumentos().size());
				documentosError+=numeroDocumentosBien;
				throw ex;
			}
			if (ex.getCause() != null
	...
	private void updateVersion() {
		try {
			...
			AppContext.getApplicationContext().getBlackboard().put(
					UserPreferenceConstants.VERSION, version);
	...
	private void actualizaDatosDocError(Object bien, int num) {
		documentosOK -= num;
		logger.info("Actualizando documentos:" + documentosOK	+ " Numero a decrementar:" + num);
		if (bien instanceof Lote) {
			numDocLote -= num;
			return;
		}
