...
BEGIN
	...
	IF EXISTS (SELECT * FROM LAYERS WHERE name = 'EIEL_Indicadores_III_Suministros') THEN	
		IF NOT EXISTS (select * from maps_layerfamilies_relations where ID_LAYERFAMILY=(select id_layerfamily from layerfamilies_layers_relations where id_layer=(select id_layer from layers where name='EIEL_Indicadores_III_Suministros'))) THEN
	...
	IF NOT EXISTS (SELECT * FROM LAYERS WHERE name = 'EIEL_Indicadores_III_Suministros') THEN	
		/*
		 LocalGIS Layer Export v0.1 20111122
		 SQL para generar los SQL de creació® ¤e capas a partir de una instalació® ¥xistente (Ej. para crear SQL de creació® ¤e capas a partir de capas ya creadas en entorno DEV)
		...
		*/
		...
		-- 4b Creació® £olumnas
			 --Añ¡¤©r COLUMN_DOMAINS donde corresponda
