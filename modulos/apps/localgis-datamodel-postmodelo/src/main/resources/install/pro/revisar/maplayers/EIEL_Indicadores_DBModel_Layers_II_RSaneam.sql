...
INSERT INTO maps_layerfamilies_relations (ID_MAP,ID_LAYERFAMILY,POSITION,ID_ENTIDAD) values ((select id_map from maps m, dictionary d where m.id_name=d.id_vocablo and d.traduccion='EIEL_Indicadores_II_Red de saneamiento'),1,0);
...
order by a.position;
*/
-- Â¡ADEVERTENCIA! Ejecutar en lugar de 5a para la segunda capa en adelante sobre la misma table
...
WHERE t.name='eiel_indicadores_d_rsaneam'  and
     ...
     and a.id_layer=(SELECT id_layer FROM layers where name='EIEL_Ind_II_RSaneam_EstadoRed') 
     and a.id_column=c.id and a.id_alias=d.id_vocablo
order by a.position;
