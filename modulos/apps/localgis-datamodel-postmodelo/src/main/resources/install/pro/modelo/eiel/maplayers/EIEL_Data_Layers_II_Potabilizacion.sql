...
BEGIN
	...
	IF EXISTS (SELECT * FROM LAYERS WHERE name = 'EIEL_Indicadores_II_Potabilizacion') THEN	
		IF NOT EXISTS (select * from maps_layerfamilies_relations where ID_LAYERFAMILY=(select id_layerfamily from layerfamilies_layers_relations where id_layer=(select id_layer from layers where name='EIEL_Indicadores_II_Potabilizacion'))) THEN
	...
	IF NOT EXISTS (SELECT * FROM LAYERS WHERE name = 'EIEL_Indicadores_II_Potabilizacion') THEN	
		/*
