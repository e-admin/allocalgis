...
BEGIN
	...
	IF EXISTS (SELECT * FROM LAYERS WHERE name = 'EIEL_Ind_II_RSaneam_saneamau') THEN	
		IF NOT EXISTS (select * from maps_layerfamilies_relations where ID_LAYERFAMILY=(select id_layerfamily from layerfamilies_layers_relations where id_layer=(select id_layer from layers where name='EIEL_Ind_II_RSaneam_saneamau' LIMIT 1))) THEN
			...
			insert into maps_layerfamilies_relations (ID_MAP,ID_LAYERFAMILY,POSITION,ID_ENTIDAD) values (currval('seq_maps'),(select id_layerfamily from layerfamilies_layers_relations where id_layer=(select id_layer from layers where name='EIEL_Ind_II_RSaneam_saneamau' LIMIT 1) LIMIT 1),0,0); 
			   

			insert into layers_styles (id_map,id_layerfamily,id_layer, id_style, stylename, isactive,position,ID_ENTIDAD,ISVISIBLE,ISEDITABLE) values (currval('seq_maps'),(select id_layerfamily from layerfamilies_layers_relations where id_layer=(select id_layer from layers where name='EIEL_Ind_II_RSaneam_saneamau' LIMIT 1) LIMIT 1),(select id_layer from layers where name='EIEL_Ind_II_RSaneam_saneamau' LIMIT 1),(select id_styles from layers where name='EIEL_Ind_II_RSaneam_saneamau' LIMIT 1),'"EIEL_Ind_II_RSaneam_saneamau:_:default:EIEL_Ind_II_RSaneam_saneamau"', true,0,0,true,true);
		END IF;
	END IF;
	
	IF NOT EXISTS (SELECT * FROM LAYERS WHERE name = 'EIEL_Ind_II_RSaneam_saneamau') THEN	
		/*
		 ...
		 No contempla la creaci󮠤e mapas ni familias de capas
