...
public class CatastroServlet extends HttpServlet
{
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException
    {
        ...
        try{
            ...
            logger.error("[CatastroServlet : doPost ]: en if cogiendo la query");
            ...
            switch (query.getAction())
            {
                ...
                case ConstantesRegExp.ACTION_BORRAR_EXPEDIENTE:
                {
                    Expediente exp= (Expediente)query.getParams().get(ConstantesRegExp.OBJETO_EXPEDIENTE);
                    geoConn.borrarExpediente(oos,exp, userSesion );

                    //PRUEBAS SERVICIOS WEB PeticionSWCatastro.peticionCrearExpSinParcelas(exp, userSesion.getUserPrincipal().getName());
                    break;
                }
                case ConstantesRegExp.ACTION_IS_PERMISOS_CONSULTA_INFO_CATASTRAL_FISICO_ECONOMICO:
                {
                	geoConn.isPermisosConsultaInformacionCatastralFisicoEconomico(oos,userSesion);
                    break;
                }
                case ConstantesRegExp.ACTION_IS_PERMISOS_CONSULTA_INFO_CATASTRAL_TITULARIDAD:
                {
                    geoConn.isPermisosConsultaInformacionCatastralTitular(oos,userSesion);
                    break;
                }
                ...
                case ConstantesRegExp.ACTION_GET_EXPEDIENTES_USUARIO_FIN_ENTADA_MASIVO:
                {
                    String idUsuario = userSesion.getIdUser();
                    ArrayList estadosArray = (ArrayList)query.getParams().get(ConstantesRegExp.OBJETO_LISTA_ESTADOS_EXP);
                    String convenio = (String) query.getParams().get(ConstantesRegExp.STRING_CONVENIO);
                    String modoTrabajo = (String) query.getParams().get(ConstantesRegExp.STRING_MODO_TRABAJO);
                    ...
                    geoConn.getExpedientesEstadosUsuario(oos,idUsuario, estadosArray, convenio, userSesion.getIdMunicipio(),isAcoplado);
                    break;
                }
                ...
                case ConstantesRegExp.ACTION_GET_LIST_ENTIDADES_GENERADORAS:
                {
                    geoConn.getLstEntidadGeneradora(oos);
                    break;
                }
                ...
                case ConstantesRegExp.ACTION_GET_VIA_POR_NOMBRE_Y_CODIGO:
                {
                    String nombre = ((String)query.getParams().get(ConstantesRegExp.STRING_NOMBRE_VIA));
                    Integer codvia = ((Integer)query.getParams().get(ConstantesRegExp.STRING_CODIGO_VIA));
                    geoConn.getViaPorNombreYCodigo(oos, nombre, codvia.intValue(),userSesion.getIdMunicipio());
                    break;
                }
                case ConstantesRegExp.ACTION_GET_VIA_POR_CODIGO_VIA:
                {
                    Integer codigoMunicipio = ((Integer)query.getParams().get(ConstantesRegExp.CODIGO_MUNICIPIO_INE));
                    Integer codigoViaIne = ((Integer)query.getParams().get(ConstantesRegExp.STRING_CODIGO_VIA));
                    geoConn.getViaPorCodigo(oos, codigoMunicipio,codigoViaIne);
                    break;
                }
                ...
                case ConstantesRegExp.ACTION_GET_CODIGO_DIGITO_CONTROL_DNI:
                {
                    geoConn.getCodigoDigitoControlDni(oos);
                    break;
                }
                ...
                case ConstantesRegExp.ACTION_MODIFICAR_EXPEDIENTE:
                {
                	...
                	Expediente exp= (Expediente)query.getParams().get(ConstantesRegExp.OBJETO_EXPEDIENTE);
                	Boolean isCatastroTemporal = (Boolean)query.getParams().get(ConstantesRegExp.STRING_ESCRIBE_CATASTRO_TEMPORAL);
                	Boolean isExpSitFinales = (Boolean)query.getParams().get(ConstantesRegExp.STRING_IS_EXP_SITUACION_FINAL);
                	AppContext aplicacion = (AppContext) AppContext.getApplicationContext();
                	aplicacion.getApplicationContext().getBlackboard().put("catastroTemporal", isCatastroTemporal);
                	aplicacion.getApplicationContext().getBlackboard().put("expSitFinales", isExpSitFinales);
                    geoConn.modificarExpediente(oos,exp, "satec", userSesion.getIdUser() );
                    
                	break;
                }
                case ConstantesRegExp.ACTION_ELIMINAR_FINCA_CATASTRO_TEMPORAL:
                {
                		Expediente exp= (Expediente)query.getParams().get(ConstantesRegExp.OBJETO_EXPEDIENTE);
                		ArrayList listaRefElimCatastroTemporal = (ArrayList)query.getParams().get(ConstantesRegExp.OBJETO_LISTA_REFERENCIAS_CATASTRALES);
                		for (int i=0; i<listaRefElimCatastroTemporal.size(); i++){
                			FincaCatastro finca = (FincaCatastro)listaRefElimCatastroTemporal.get(i);
                			String refCatastral = finca.getRefFinca().getRefCatastral();
                			geoConn.eliminarFincaCatastroTemporal(exp, refCatastral);
                		}
                ...
                case ConstantesRegExp.ACTION_ACTUALIZAR_OVC_ACOPLADO:
                {
                    ...
                    if(modoTrabajo.equalsIgnoreCase(DatosConfiguracion.MODO_TRABAJO_ACOPLADO))  {
                    	String cuerpo = "";
                    	 ...
                    	 if(convenio.equalsIgnoreCase(DatosConfiguracion.CONVENIO_TITULARIDAD) &&
                         	...
                         	ArrayList xmlBIExp = geoConn.getXmlParcelasFX_CCExp(oos, expObj);
                         	cuerpo = PeticionSWCatastro.crearCuerpoVARPADRequest(oos, expObj, xmlBIExp);
...
//                         else{
//                        	 ArrayList xmlParcelasExp = geoConn.getXmlParcelasFX_CCExp(oos, expObj);
//                        	 cuerpo =  PeticionSWCatastro.crearCuerpoFinEntradaRequest(oos, expObj, xmlParcelasExp);
//                        	 
//                         }
          
                    	  oos.writeObject(cuerpo);
                    }
                	break;
                }
                case ConstantesRegExp.ACTION_EXPORTACION_MASIVA:{
                    ...
                    if(modoTrabajo.equalsIgnoreCase(DatosConfiguracion.MODO_TRABAJO_ACOPLADO))  {
                       ...
                       // expedientes = geoConn.getExpedientesExportacionMasiva(oos, userSesion.getIdMunicipio(),convenio);
                        ...
                        ArrayList listaExpedientes= (ArrayList)query.getParams().get(ConstantesRegExp.OBJETO_LISTA_EXP);
                        boolean todosExpedientes = (Boolean)query.getParams().get(ConstantesRegExp.BOOLEAN_EXPORTAR_TODOS_EXPEDIENTE);
                        ...
                        if(expedientes.isEmpty()) 
                            ...
                            if(convenio.equalsIgnoreCase(DatosConfiguracion.CONVENIO_TITULARIDAD)){
                                ...
                                if(!listaExpFiscEconomic.isEmpty()){
                                	// se identifica que se va a escribir el ficehero fin de entrada
                                	// para que escriba los tags correspondientes dependiendo de si el 
                                	// expediente es de situaciones finales o de variaciones
                                	AppContext.getApplicationContext().getBlackboard().put("catastroTemporal", false);
                                	buildFINEntradaRemote(oos, listaExpFiscEconomic, directorio, nombreFicheroFinEntrada, modoTrabajo, geoConn, userSesion.getIdMunicipio(),ficheroExportacion);
                ...
                case ConstantesRegExp.ACTION_ACTUALIZA_CATASTRO_TEMPORAL:
                {
                    ...
                    geoConn.completaReferenciasToXMLBDRefCat(exp, convenio, userSesion.getIdMunicipio());
                    oos.writeObject(exp);
                    
                    break;
                }
                case ConstantesRegExp.ACTION_ESCRIBIR_CATASTRO_TEMPORAL_VARIACIONES:
                {
                    String convenio = (String)query.getParams().get(ConstantesRegExp.STRING_CONVENIO);
                    Expediente exp = (Expediente)query.getParams().get(ConstantesRegExp.OBJETO_EXPEDIENTE);
                    Boolean isCatastroTemporal = (Boolean)query.getParams().get(ConstantesRegExp.STRING_ESCRIBE_CATASTRO_TEMPORAL);
                ...
                //TODO volver a activar esta accion
...
//                	geoConn.insertarDatosSalida(oos, udsa, insertarExpediente, exp);
                	geoConn.insertarDatosSalida(oos, udsa, insertarExpediente, exp, userSesion.getIdMunicipio(), userSesion.getIdEntidad());
                	break;
                }
                case ConstantesRegExp.ACTION_GET_ID_EXPEDIENTE:
                {                	
                	Expediente exp = (Expediente)query.getParams().get(ConstantesRegExp.OBJETO_EXPEDIENTE);
                	geoConn.getIdExpedienteBD(oos, exp.getNumeroExpediente());
                	break;
                }
    ...
    private void buildVARPAD(ObjectOutputStream oos, ArrayList expedientes, String directorio, String nombreFichero, String modoTrabajo, COperacionesDGC geoConn, String idMunicipio ) throws Exception{
    ...
    private void buildVARPADRemote(ObjectOutputStream oos, ArrayList expedientes, String directorio, String nombreFichero, String modoTrabajo, COperacionesDGC geoConn, String idMunicipio, FicheroExportacion ficheroExportacion) throws Exception{
        ...
        CabeceraVARPAD cabecera = geoConn.creaCabeceraVARPADMasivo(oos, expedientes, modoTrabajo);
        cabecera.setDescripcionDelFichero(fichero.getDescripcion());
    ...
    private void buildFINEntrda(ObjectOutputStream oos, ArrayList expedientes, String directorio, String nombreFichero, String modoTrabajo, COperacionesDGC geoConn, String idMunicipio ) throws Exception{

        Fichero fichero = new Fichero();
        fichero.setIdTipoFichero(Fichero.FIN_ENTRADA);
        fichero.setNombre(nombreFichero);
        fichero.setDescripcion(Fichero.DESCRIPCION_FIN_ENTRADA);
    ...
    private void buildFINEntradaRemote(ObjectOutputStream oos, ArrayList expedientes, String directorio, String nombreFichero, String modoTrabajo, COperacionesDGC geoConn, String idMunicipio, FicheroExportacion ficheroExportacion ) throws Exception{
        ...
        cabecera.setDescripcionDelFichero(fichero.getDescripcion());
        cabecera.setNombreFicheroAsignadoPorDGC(fichero.getNombre());
