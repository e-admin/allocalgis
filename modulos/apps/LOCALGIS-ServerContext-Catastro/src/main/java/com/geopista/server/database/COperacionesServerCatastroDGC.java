/**
 * COperacionesServerCatastroDGC.java
...
import com.geopista.app.catastro.model.beans.ConstantesRegistroExpModel;
...
public class COperacionesServerCatastroDGC
{
	private static org.apache.log4j.Logger logger = org.apache.log4j.Logger.getLogger(COperacionesServerCatastroDGC.class);
				...
				if (existExpediente(exp,finca.getRefFinca().getRefCatastral())){
					...
					if (finca.getFxcc()!=null && finca.getFxcc().getDXF()!=null){
						...
						if (finca.getLstImagenes() != null && finca.getLstImagenes().size()>0){
							...
							String lstImagenes = EdicionUtilsCatastroServer.getImagenCatastroXML(finca.getLstImagenes());
	...
	private CabeceraWSCatastro creaCabeceraWSBD(Expediente exp, String tipo, 
		...
		if(tipo.equals(ConstantesRegistroExpModel.TAG_CREACION_EXPEDIENTE_REQUEST)){
			cabecera.setTipoFichero(CabeceraWSCatastro.TIPO_XML_CREACION_EXPEDIENTE);
			cabecera.setActualizaCatastro(false);
		}
		else if(tipo.equals(ConstantesRegistroExpModel.TAG_CONSULTA_CATASTRO_REQUEST)){
			cabecera.setTipoFichero(CabeceraWSCatastro.TIPO_XML_CONSULTA_CATASTRO);
			cabecera.setActualizaCatastro(false);
		}
		else if(tipo.equals(ConstantesRegistroExpModel.TAG_ACTUALIZA_CATASTRO_REQUEST)){
		...
		else if(tipo.equals(ConstantesRegistroExpModel.TAG_CONSULTA_ESTADO_EXPEDIENTE_REQUEST)){
		...
		try{
			...
			while (rs.next())
			{
				...
				if (!nif.startsWith("E") ||
					...
					GeopistaCommonUtils.completarConCeros(idMun, 3);


					idProv = GeopistaCommonUtils.completarConCeros(idProv.trim(), 2);

					idMun = GeopistaCommonUtils.completarConCeros(idMun.trim(), 3);					
...
//		String sSQL= "select id_entidad_generadora from expediente where id_expediente="+refExpediente;
	...
	private ArrayList getConstrucciones(FincaCatastro finca) throws Exception
	{
		...
		try
		{
			...
			while(rs.next())
			{
				...
				construccionCatastro.setNumOrdenConstruccion(GeopistaCommonUtils.completarConCeros(rs.getString("numero_orden_construccion"), 4));
		...
		try
		{
			...
			if(rs.next())
			{
				...
				if (elemRep.getId() != null && elemRep.getId().length()<18){
					elemRep.setId(GeopistaCommonUtils.completarConCeros(elemRep.getId(), 18));
				}
	...
	private Expediente completarFincaCatastroExpedienteDB(Expediente exp, String idMunicipioFinca) throws Exception{
		...
		try {
			...
			while(rs.next()) {
				...
				String idM = GeopistaCommonUtils.completarConCeros(String.valueOf(idMunicipio), 5);               
	...
	{       
		...
		{         
			...
			String codProvincia = EdicionUtilsCatastroServer.paddingString(String.valueOf(fc.getDirParcela().getProvinciaINE()),
					TAM_CODPRO,'0',true);
			String codMunicipio = EdicionUtilsCatastroServer.paddingString(String.valueOf(fc.getDirParcela().getMunicipioINE()),
					TAM_CODMUNI,'0',true);
			...
			if (udsa.getFincaCatastro()!=null){
				if(udsa.getFxcc()!=null){
					if(udsa.getFxcc().getDXF()!=null){
						...
						geometriaParcela = ImportarUtilsServerCatastro.obtenerGeometriaParcela(dxf);
	...
	throws Exception
	{
		...
		pers.getDerecho().setPorcentajeDerecho(EdicionUtilsCatastroServer.strToFloat(linea.substring(54, 57)+"."+linea.substring(57,59)));
		...
		pers.getDomicilio().setKilometro(EdicionUtilsCatastroServer.strToDouble(linea.substring(284,287)+"."+linea.substring(287, 289)));
