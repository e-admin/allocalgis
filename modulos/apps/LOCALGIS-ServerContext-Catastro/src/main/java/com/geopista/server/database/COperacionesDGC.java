...
public class COperacionesDGC
{
	...
	private static final String SRID_INSERT_BBDD_GEOGRAFICAS = "4258"; 
	/**
	...
	public void crearExpediente(ObjectOutputStream oos, Expediente exp, String idUser) throws Exception{
		try{
			if(!existeExpediente(exp.getNumeroExpediente())){
				...
				exp.setIdTecnicoCatastro(idUser);
				insertarDireccionDB(exp.getDireccionPresentador(), exp.getIdMunicipio());
				String idEntGenString = String.valueOf(exp.getEntidadGeneradora().getIdEntidadGeneradora());
				int idEntGen = Integer.parseInt(idEntGenString);
				getEntidadGeneradoraBD(exp, null, true,idEntGen);
	...
	public void borrarExpediente(ObjectOutputStream oos, Expediente exp, Sesion sesion) throws Exception{
		try{
			if(existeExpedienteBD(exp.getNumeroExpediente())){
				
				if (!checkPermission(sesion,ConstantesRegExp.ACL_CATASTRO,ConstantesRegExp.STRING_PERMISO_BORRAR_EXP,connection) && 
						!sesion.getIdUser().equalsIgnoreCase(exp.getIdTecnicoCatastro())){
					throw new PermissionException("No puede ejecutar la operación sin tener el permiso"+" " + ConstantesRegExp.STRING_PERMISO_BORRAR_EXP 
							+ " o el usuario no es el propietario del expediente");
				}
				...
				if(exp.getListaReferencias()!=null && exp.getListaReferencias().size()>0){
					for(int i = 0;i<exp.getListaReferencias().size();i++){
						...
						if(exp.getListaReferencias().get(i) instanceof FincaCatastro)
							borrarFincaCatastroExpedienteDB(exp, i);
						else if(exp.getListaReferencias().get(i) instanceof BienInmuebleCatastro)
							borrarBienInmuebleCatastroExpedienteBD(exp, i);
					}
				}
				
				Object o2= borrarExpedienteDB(exp);
	...
	public void isPermisosConsultaInformacionCatastralFisicoEconomico(ObjectOutputStream oos,Sesion sesion) throws Exception{
		try{
			boolean isPermisoConsulta = false;
			if (checkPermission(sesion,ConstantesRegExp.ACL_CATASTRO,ConstantesRegExp.STRING_PERMISO_FISICO_ECONOMICO,connection)){
				isPermisoConsulta =  true;
			}
			Boolean boolPermiso = new Boolean(isPermisoConsulta);
			oos.writeObject(boolPermiso);

		}
		catch(Exception e){
			logger.error("Consultar Informacion Catastral: "+ e.getMessage());
			throw e;
		}
	}	
	
	public void isPermisosConsultaInformacionCatastralTitular(ObjectOutputStream oos,Sesion sesion) throws Exception{
		try{
			boolean isPermisoConsulta = false;
			if (checkPermission(sesion,ConstantesRegExp.ACL_CATASTRO,ConstantesRegExp.STRING_PERMISO_TITULARIDAD,connection)){
				isPermisoConsulta =  true;
			}
			Boolean boolPermiso = new Boolean(isPermisoConsulta);
			oos.writeObject(boolPermiso);
	...
	private GeopistaAcl getPerfil(long idAcl, Connection conn, Sesion sSesion) throws Exception{
        ...
        vParametros.add(4,sSesion.getIdUser());
        ...
        CResultadoOperacion rQueryPermisos=COperacionesControl.ejecutarQuery(
                ...
                vParametros);
        GeopistaAcl geopistaAcl = new GeopistaAcl(sSesion.getUserPrincipal(),sNombreAcl,sDescripcionAcl);
        GeopistaAclEntry entry= new GeopistaAclEntry(sSesion.getUserPrincipal());

        if (rQueryPermisos.getVector()!=null&&rQueryPermisos.getVector().size()>0)
        {
            logger.debug("El usuario "+sSesion.getUserPrincipal().getName()+ " tiene permisos "+rQueryPermisos.getVector().size()+" para el acl "+geopistaAcl.getAclName());
            ...
            logger.debug("El usuario "+sSesion.getUserPrincipal().getName()+ " NO tiene permisos para el acl "+geopistaAcl.getAclName()+" Descripcion:"+geopistaAcl.getAclDescripcion());
        geopistaAcl.addEntry(sSesion.getUserPrincipal(),entry);
        return geopistaAcl;
	}
	
	private boolean checkPermission(Sesion sSesion,long lACL,String sPerm,Connection con) throws Exception{
		logger.debug("Buscando el acl:"+lACL);
		GeopistaAcl acl = getPerfil(lACL,con, sSesion);
//		GeopistaAcl acl=sSesion.getPerfil(lACL,con);
	...
	public void eliminarFincaCatastroTemporal(Expediente expediente, String refCatastral) throws Exception
	{
		PreparedStatement ps= null;
		Connection conn= null;
				...
				for(int i = 0;i<exp.getListaReferencias().size();i++)
				{
					if(exp.getListaReferencias().get(i) instanceof FincaCatastro)
					{
						...
						if(exp.isTipoTramitaExpSitFinales()){
							...
							modificaFincaExpedienteVariacionesDB(exp, i);
						}
						else{
					...
					else if(exp.getListaReferencias().get(i) instanceof BienInmuebleJuridico)
					{
						...
						if (bij.isDelete())
						{
							...
							modificaBienInmuebleExpedienteVariacionesBD(exp,i);
						}
					}
				}
			}
	...
	/**
	 * Metodo que devuelve todos los expedientes de un usario que tengan el estado pasado por por parametro, en caso de que sea la
	 ...
	 * @throws Exception Las Excepciones registradas
	 * */
	public void getExpedientesEstadosUsuario(ObjectOutputStream oos, String idUsuario, ArrayList estadosArray, String convenio,
			String idMunicipio, boolean isAcoplado)throws Exception
			{
		try
		{
			for(Iterator it= getExpedientesEstadosUsuarioDB(idUsuario, estadosArray, convenio, idMunicipio, isAcoplado).iterator();it.hasNext();)
			{
	...
	/**
	 * Funcion que devuelve un arrayList con los objetos EntidadGeneradora de las entidades generadoras que hay en BD. 
	 * El objeto oos se utiliza para escribir el resultado que se devolvera en el response del servlet.
	 *
	 * @param oos Objeto donde se escribe el resultado
	 * @throws Exception  Las Excepciones registradas
	 * */
	public void getLstEntidadGeneradora(ObjectOutputStream oos)throws Exception
	{
		try
		{
			ArrayList listaDatos= (ArrayList)getLstEntidadGeneradoraBD();
			oos.writeObject(listaDatos);
		}
		catch(Exception e)
		{
			logger.error("getLstEntidadGeneradora: "+ e.getMessage());
			oos.writeObject(new ACException(e));
			throw e;
		}
	...
	public void getViaPorNombreYCodigo(ObjectOutputStream oos, String nombre, int codVia, String idMunicipio)throws Exception
	{
		try
		{
			DireccionLocalizacion dir= (DireccionLocalizacion)getViaPorNombreYCodigoBD(nombre, codVia, idMunicipio);
			oos.writeObject(dir);
		}
		catch(Exception e)
		{
	...
	/**
	 * Metodo que devuelve una via buscandola por el codigo de la via y el municipio. El objeto oos se
	 * utiliza para escribir el resultado que se devolvera en el response del servlet.
	 *
	 * @param oos Objeto donde se escribe el resultado
	 * @throws Exception  Las Excepciones registradas
	 */
	public void getViaPorCodigo(ObjectOutputStream oos, Integer codigoMunicipio, Integer codigoViaIne)throws Exception
	{
		try
		{
			DireccionLocalizacion dir= (DireccionLocalizacion)getViaPorCodigoBD(codigoMunicipio, codigoViaIne);
			oos.writeObject(dir);
		}
		catch(Exception e)
		{
	...
	/**
	 * Metodo que devuelve una hash en la que la key es el resto y el valor es la letra asociada del dni
	 *  El objeto oos se utiliza para escribir el resultado que se devolvera en el response del servlet.
	 *
	 * @param oos Objeto donde se escribe el resultado
	 * @throws Exception  Las Excepciones registradas
	 */
	public void getCodigoDigitoControlDni(ObjectOutputStream oos)throws Exception
	{
		try
		{
			Hashtable hashDatos= getCodigoDigitoControlDniBD();
			oos.writeObject(hashDatos);
		}
		catch(Exception e)
		{
			logger.error("getCodigoDigitoControlDni: "+ e.getMessage());
			oos.writeObject(new ACException(e));
			throw e;
		}
	}
	 ...
	 * @throws Exception
	 */
	public ArrayList getExpedientesExportacionMasiva(ObjectOutputStream oos, String idMunicipio, String convenio,
													ArrayList listaExpedientes, boolean todosExpedientes) throws Exception
	{
		try
		{

			return getExpedientesExportacionMasivaBD(idMunicipio, convenio, listaExpedientes, todosExpedientes );
	...
	public void completaReferenciasToXMLBD(Expediente exp, String convenio, String idMunicipio) throws Exception{
		...
		try {
			...
			if(convenio.equalsIgnoreCase(DatosConfiguracion.CONVENIO_TITULARIDAD))
				...
				if (exp.getTipoExpediente().getConvenio()== null){
					String convenioExp = getConvenioExpediente(exp);
					exp.getTipoExpediente().setConvenio(convenioExp);
				}
	 ...
	 * @throws Exception
	 */
	public void completaReferenciasToXMLBDRefCat(Expediente exp, String convenio, String idMunicipio) throws Exception{

		try {
			
			if(convenio.equalsIgnoreCase(DatosConfiguracion.CONVENIO_TITULARIDAD))
				exp = completarBienInmuebleCatastroExpedienteDBRefCat(exp, idMunicipio);
				...
				if (exp.getTipoExpediente().getConvenio().equalsIgnoreCase(DatosConfiguracion.CONVENIO_FISICO_ECONOMICO))
					exp = completarFincaCatastroExpedienteDBRefCat(exp, idMunicipio);
				else if(exp.getTipoExpediente().getConvenio().equalsIgnoreCase(DatosConfiguracion.CONVENIO_TITULARIDAD))
					exp = completarBienInmuebleCatastroExpedienteDBRefCat(exp, idMunicipio);
			}
	...
	public void getIdExpedienteBD(ObjectOutputStream oos, String numExpediente)throws Exception
	{
		try
		{
			Long idExpediente= getIDExpPorNumExp(numExpediente);
			oos.writeObject(idExpediente);
		}
		catch(Exception e)
		{
			logger.error("existeBIEn: "+ e.getMessage());
	...
	private Expediente crearExpedienteDB(Expediente exp) throws Exception {
		...
		" Cod_Poblacion_Notaria, Cod_Notaria, Anio_protocolo_Notarial, Protocolo_notarial, isexpsitfinales, modo_acoplado) " +
		...
		try{
			...
			ps.setBoolean(30, exp.isTipoTramitaExpSitFinales());
			ps.setBoolean(31, exp.isModoAcoplado());
			ps.execute();
			conn.commit();            
		}
		catch (Exception e){
	...
	private Boolean borrarExpedienteDB(Expediente exp) throws Exception {
		
		if(exp==null)
			return new Boolean(false);
		
		
		String sSQL = "select id_expediente from expediente_fichero where id_expediente=?"; 

		String sqlOracle = "select id_expediente from expediente_fichero where id_expediente=?";
		...
		try{
			...
			while(rs.next()){
				...
				sSQL = "delete from expediente_fichero where id_expediente=?"; 

				sqlOracle = "delete from expediente_fichero where id_expediente=?";
			...
			sSQL= "delete from expediente where id_expediente=?";

			sqlOracle = "delete from expediente where id_expediente=?";
	...
	private void borrarFincaCatastroExpedienteDB(Expediente exp, int i)throws Exception {
		if(exp!=null){
			...
			FincaCatastro existeF = existeParcelaEnBD(refFinca);
			if(existeF!=null){
				String sSQL= "delete from expediente_finca_catastro where id_expediente=? and ref_catastral=?";
				PreparedStatement ps= null;
				ResultSet rs = null;
				Connection conn= null;
				...
				try{
					...
					sSQL = "select id_expediente, referencia from catastro_temporal where id_expediente=? and referencia =?";
					ps= conn.prepareStatement(sSQL);
					ps.setLong(1,exp.getIdExpediente());
					ps.setString(2,refFinca);
					rs= ps.executeQuery();
					while (rs.next())
					{
						sSQL = "delete from catastro_temporal where id_expediente=? and referencia =?";
						ps= conn.prepareStatement(sSQL);
						ps.setLong(1,exp.getIdExpediente());
						ps.setString(2,refFinca);
						ps.execute();
	...
	private void borrarBienInmuebleCatastroExpedienteBD(Expediente exp, int i) throws Exception {
		...
		if(exp!=null){
			...
			BienInmuebleCatastro bi = existeBIEnBD(idBI);
			if(bi!=null) {
				String sSQL= "delete from Expediente_BienInmueble where id_expediente=? and ID_BienInmueble=?";
				PreparedStatement ps= null;
				ResultSet rs = null;
				Connection conn= null;
				try {
	 ...
	 * @throws Exception
	 */
	private Collection getExpedientesEstadosUsuarioDB(String idUsuario, ArrayList estadosArray, String convenio, String idMunicipio, boolean isAcoplado) throws Exception {
		...
		if((idUsuario!=null)&&(!idUsuario.equals(""))){
			...
			String sSQL = "select * from expediente where id_municipio = " +idMunicipio + " and modo_acoplado = " +isAcoplado ;

			
			if(estadosArray != null && !estadosArray.isEmpty()){
				sSQL= sSQL + " and (";
				
				for(int index=0; index<estadosArray.size(); index++){
					sSQL= sSQL +" id_estado=" + estadosArray.get(index) + " or";
			...
			try
			{
				...
				while (rs.next())
				{
					...
					exp.setTipoTramitaExpSitFinales(rs.getBoolean("isexpsitfinales"));
...
//			}
	 ...
	 * @throws Exception
	 */
	private ArrayList getLstEntidadGeneradoraBD() throws Exception
	{
		ArrayList aux= new ArrayList();
		String sSQL= "select * from entidad_generadora";
		PreparedStatement ps= null;
		Connection conn= null;
		ResultSet rs= null;
		try
		{
			...
			while(rs.next())
			{
				...
				entidadGeneradora.setCodigo(rs.getInt("codigo"));
				entidadGeneradora.setDescripcion(rs.getString("descripcion"));
				entidadGeneradora.setIdEntidadGeneradora(rs.getInt("id_entidad_generadora"));
				entidadGeneradora.setNombre(rs.getString("nombre"));
				entidadGeneradora.setTipo(rs.getString("tipo"));
				
				aux.add(entidadGeneradora);
	...
	private Collection getBienInmuebleCatastroBuscadosBD(String idMunicipio,String patron) throws Exception
	{
		...
		try
		{
			...
			if(CPoolDatabase.isPostgres(conn))
			{
				sSQL ="select bien_inmueble.identificador, bien_inmueble.poligono_rustico, bien_inmueble.parcela, bien_inmueble.primer_numero, bien_inmueble.id_via, bien_inmueble.clase_bieninmueble from bien_inmueble left join parcelas on (bien_inmueble.parcela_catastral=parcelas.referencia_catastral and parcelas.id_municipio='"+ idMunicipio+"'" +
						") where ((bien_inmueble.identificador like upper('%" + patron + "%')) or ( bien_inmueble.identificador is null)) " +
	...
	private Collection getFincaCatastroBuscadasPorDirDB(String idMunicipio,String patronNombreVia, String patronTipoVia) throws Exception
	{
		...
		" vias ON parcelas.id_via=vias.id where" +
		" parcelas.referencia_catastral IS NOT NULL and parcelas.id_municipio= '"+ idMunicipio+"' "
	...
	private Collection getBienInmuebleCatastroBuscadosPorDirDB(String idMunicipio,String patronNombreVia, String patronTipoVia) throws Exception
	{
		...
		try
		{
			...
			if(CPoolDatabase.isPostgres(conn))
			{
				...
				" from bien_inmueble,vias, parcelas where bien_inmueble.id_via=vias.id and parcelas.referencia_catastral " +
	...
	private boolean existeExpedienteBD(String numeroExp) throws Exception
	{
		PreparedStatement ps= null;
		Connection conn= null;
		ResultSet rs= null;
	 ...
	 * @throws Exception
	 * */
	private DireccionLocalizacion getViaPorCodigoBD(Integer codigoMunicipio, Integer codigoVia) throws Exception
	{
		PreparedStatement ps= null;
		Connection conn= null;
		ResultSet rs= null;
		...
		try
		{
			String query = "select * from vias where id_municipio=" + Integer.valueOf(codigoMunicipio) + 
			" and id =" + Integer.valueOf(codigoVia);
			conn= CPoolDatabase.getConnection();
			ps=conn.prepareStatement(query);
			rs=ps.executeQuery();
	...
	private DireccionLocalizacion getViaPorNombreYCodigoBD(String nombre, int codVia, String idMunicipio) throws Exception
	{
		PreparedStatement ps= null;
		Connection conn= null;
		ResultSet rs= null;
		...
		{
			String query = "select * from vias where id_Municipio=" + Integer.valueOf(idMunicipio) + " and nombrecatastro = '" + 
				nombre.toUpperCase() + "' and codigocatastro = " +codVia;
			conn= CPoolDatabase.getConnection();
			ps=conn.prepareStatement(query);
			rs=ps.executeQuery();
			resultado = new DireccionLocalizacion();
	...
	/**
	 * Metodo que devuelve una Hashtable en la que la key es el resto y el valor es la letra asociada del dni,
	 *
	 * @return Hashtable en la que la key es el resto y el valor es la letra asociada del dni
	 * @throws Exception
	 * */
	private Hashtable getCodigoDigitoControlDniBD() throws Exception
	{
		Hashtable aux= new Hashtable();
		ArrayList codigos = new ArrayList();
		ArrayList nombres = new ArrayList();
		String sSQL= "select * from digitoControlDni";
		PreparedStatement ps= null;
		Connection conn= null;
		ResultSet rs= null;
		try
		{
			...
			{
				//TODO esto mirar y mejorar
				aux.put(rs.getString("resto"),rs.getString("letra"));
			}
	...
	private Hashtable getCodigoNombreMunicipioBD(String codigoProvincia) throws Exception
	{
		...
		String sSQL= "select id_ine, nombreoficial,id from municipios where id_provincia='"+codigoProvincia +
	...
	private String getConvenioExpediente(Expediente exp)throws Exception
	{
		String convenio = null;
		String sSQL = null;
		
		sSQL= "select convenio from tipo_expediente where codigo_tipo_expediente='" + exp.getTipoExpediente().getCodigoTipoExpediente()+"'";
...
//	public FincaCatastro obtenerInfoFinca(FincaCatastro fc, long idExpediente)throws Exception {
//		
//		String strElemf = null;
//		final ArrayList instancias = new ArrayList();
//		String SQL = "select xml from catastro_temporal where referencia=? and id_expediente=?";
//		PreparedStatement ps= null;
//		Connection conn= null;
//		ResultSet rs = null;
//
...
//			conn= CPoolDatabase.getConnection();
//			ps= conn.prepareStatement(SQL);
//			ps.setString(1, fc.getRefFinca().getRefCatastral());
//			ps.setLong(2, idExpediente);
//			
//			rs = ps.executeQuery();
//			if (rs.next()){
//				strElemf = rs.getString("xml");
//			}
//			
//			if (strElemf!=null)
//			{
...
//    }
	...
	private void modificaFincaExpedienteVariacionesDB(Expediente exp, int i)throws Exception {
	...
	private void modificaBienInmuebleExpedienteVariacionesBD(Expediente exp, int i) throws Exception{
		if(exp!=null){
			String sSQL = "";
			PreparedStatement ps= null;
			Connection conn= null;
	...
	private Collection getBienesInmueblesBuscadasPorTitularBD(String patronNif, String idMunicipio) throws Exception
	{
		...
		try
		{
			...
			while(rs.next())
			{
				...
				{
					if(idVia!=-1){
						sSQL = "select vias.tipovianormalizadocatastro, vias.nombrecatastro from vias where id=" + idVia
						+ " and id_municipio="+Integer.parseInt(idMunicipio);
						ps= conn.prepareStatement(sSQL);
						rsVia= ps.executeQuery();
						if(rsVia.next())
	 ...
	 * @throws Exception
	 * */
	private ArrayList getExpedientesExportacionMasivaBD(String idMunicipio, String convenio, ArrayList listaExpedientes, boolean todosExpedientes) throws Exception {
		...
		" and (id_estado="+ConstantesRegExp.ESTADO_FINALIZADO + " or id_estado="+ ConstantesRegExp.ESTADO_MODIFICADO+ ") " +
		"union " + " select * from expediente where id_municipio = " +idMunicipio +
		...
		try{
			...
			while(rs.next()) {
				...
				if(todosExpedientes){
					// se rellenan con todos los expedientes
					expedientes.add(rellenarExpediente(rs, conn, convenio, idMunicipio));
				}
				else{
					//se rellena solo con los expedientes que hayan sido seleccionados en la
					// tabla de busqueda
					if(listaExpedientes != null && listaExpedientes.contains(TypeUtil.getSimpleLong(rs, "id_expediente"))){
	...
	private Expediente rellenarExpediente (ResultSet rs, Connection conn, String convenio, 
	 ...
	 * @throws Exception
	 * */
	private ArrayList getExpedientesExportacionMasivaListaExpedientesBD(String idMunicipio, String convenio,
															ArrayList listaExpedientes, ArrayList listaEstados) throws Exception {
		ArrayList expedientes = new ArrayList();
		...
		if(listaEstados != null && !listaEstados.isEmpty()){
			...
			for(int index=0; index<listaEstados.size(); index++){
				sSQL= sSQL +" id_estado=" + listaEstados.get(index) + " or";
		...
		try{
			...
			while(rs.next()) {
				...
				if(listaExpedientes.contains(TypeUtil.getSimpleLong(rs, "id_expediente"))){
			...
			try {
				...
				if(modoTrabajo.equalsIgnoreCase(DatosConfiguracion.MODO_TRABAJO_ACOPLADO)){
					...
					*/
					...
					sSQL = "select count(*) as numeroFincas from parcelas p, expediente_finca_catastro ep where p.referencia_catastral=ep.ref_catastral and p.fecha_baja is null and"
						+" ep.id_expediente in(select id_expediente from expediente where id_estado="+ConstantesRegExp.ESTADO_FINALIZADO +
						" and id_expediente = "+exp.getIdExpediente()+")";
					...
					/* select count(*) as numeroTitulares from persona where (parcela_catastral || numero_cargo || digito_control1 || 
						 digito_control2) in (select b.identificador from bien_inmueble b where parcela_catastral in 
						 (select p.referencia_catastral from parcelas p, expediente_finca_catastro ep where 
						 p.referencia_catastral=ep.ref_catastral and p.fecha_baja is null and ep.id_expediente 
						 in(select id_expediente from expediente where id_estado=6 and id_expediente=2150
					)))*/
	...
	private Persona getRepresentante(String nifRepresentante, String id_bienInmueble) throws SQLException
	{
		...
		try {
			...
			while (rs.next())
			{
				...
				idProv = completarConCeros(idProv, 2);

				idMun = completarConCeros(idMun, 3);
	...
	private ArrayList getTitular(String referenciaExpediente, String id_bienInmueble) throws SQLException
	{
		...
		if (id_bienInmueble.length()<20){
			completarConCeros(id_bienInmueble, 20);
		}
		...
		try{
			...
			while (rs.next())
			{
				...
				if (!nif.startsWith("E") ||
					...
					completarConCeros(idMun, 3);


					idProv = completarConCeros(idProv.trim(), 2);

					idMun = completarConCeros(idMun.trim(), 3);					
	...
	{
		String idEntidadGeneradora = null;
		//String sSQL= "select id_entidad_generadora from expediente where numero_expediente='"+refExpediente+"'";
	 ...
	 * @throws Exception 
	 */
	private Expediente completarBienInmuebleCatastroExpedienteDBRefCat(Expediente exp, String idMunicipio) throws Exception
	{
		ArrayList lstBienesInmuebles= new ArrayList();
		...
		try {
			...
			while(rs.next()){
				...
				for(int j=0; j<exp.getListaReferencias().size(); j++)
				{
					BienInmuebleCatastro bi = (BienInmuebleCatastro)exp.getListaReferencias().get(j);
					if(bi.getIdBienInmueble().getIdBienInmueble().equals(rs.getString("identificador"))){
	...
	private ArrayList getConstrucciones(FincaCatastro finca) throws Exception
	{
		...
		try
		{
			...
			while(rs.next())
			{
				...
				construccionCatastro.setNumOrdenConstruccion(rs.getString("numero_orden_construccion"));
		...
		try
		{
			...
			if(rs.next())
			{
				...
				if (elemRep.getId() != null && elemRep.getId().length()<18){
					elemRep.setId(completarConCeros(elemRep.getId(), 18));
				}
	...
	private Expediente completarFincaCatastroExpedienteDB(Expediente exp, String idMunicipioFinca) throws Exception{
		...
		try {
			...
			while(rs.next()) {
				...
				String idM = completarConCeros(String.valueOf(idMunicipio), 5);               
	...
	private Expediente completarFincaCatastroExpedienteDBRefCat(Expediente exp, String idMunicipioFinca) throws Exception{
		...
		try {
			...
			while(rs.next()) {
				for(int j=0; j<exp.getListaReferencias().size(); j++)
				{
					FincaCatastro fincaCat = (FincaCatastro)exp.getListaReferencias().get(j);
					if(fincaCat.getRefFinca().getRefCatastral().equals(rs.getString("referencia_catastral"))){
	...
	private static final Integer TYPE_DATE = new Integer(Types.DATE);
	...
	throws Exception
	{
		insertarListaDatos(lstDatos, insertarExpediente, exp, null);
	}
	/**
	 * 
	 * @param lstDatos
	 ...
	 */
	private void insertarListaDatos(ArrayList lstDatos, boolean insertarExpediente, Expediente exp, String idMunicipio) //throws DataException
	throws Exception
	{
		...
		if (lstDatos!=null)
		{
			...
			{            
				try
				{
					...
					{
						insertarDatosSuelo((SueloCatastro)o, insertarExpediente, exp, idMunicipio);
					}
					else if (o instanceof UnidadConstructivaCatastro)
					{
						insertarDatosUC ((UnidadConstructivaCatastro)o, insertarExpediente, exp, idMunicipio);
					}
					else if (o instanceof BienInmuebleJuridico)
					{
						insertarDatosBIJ ((BienInmuebleJuridico)o, insertarExpediente, exp, idMunicipio);
					}
					else if (o instanceof ConstruccionCatastro)
					{
						insertarDatosConstruccion ((ConstruccionCatastro)o, insertarExpediente, exp, idMunicipio);
					}
					else if (o instanceof Cultivo)
					{
						insertarDatosCultivo ((Cultivo)o, insertarExpediente, exp, idMunicipio);
					}
					else if (o instanceof RepartoCatastro)
					{
						insertarDatosReparto ((RepartoCatastro)o, insertarExpediente, exp, idMunicipio);
					}
	...
	throws Exception
	{  
		insertarDatosBIJ(bij, insertarExpediente, exp, null);
	}

	private void insertarDatosBIJ(BienInmuebleJuridico bij, boolean insertarExpediente, Expediente exp, String idMunicipio) //throws DataException
	throws Exception
	{   
		try
		{   
			...
			if (insertarExpediente && exp!=null)
				insertarDatosExpediente (exp, idMunicipio);  
			...
			if(bij.getBienInmueble()!=null && 
				...
				if (bij.getBienInmueble().getRepresentante().getNif()!=null)
				{
					...
					if(bij.getBienInmueble().getRepresentante().getDomicilio().getCodigoPostal() == null){
						lstValores.add(null);
					}
					else{
						lstValores.add(new Integer(bij.getBienInmueble().getRepresentante().
				...
				if(bij.getBienInmueble().getDomicilioTributario().getCodigoPostal() == null){
					lstValores.add(null);
				}
				else{
	...
	throws Exception
	{       
		insertarDatosReparto(reparto, insertarExpediente, exp, null);
	}
	 ...
	 * @throws DataException 
	 */
	private void insertarDatosReparto(RepartoCatastro reparto, boolean insertarExpediente, Expediente exp, String idMunicipio) 
	//throws DataException
	throws Exception
	{        
...
//		lstValores.add(elemreparto.getNumCargo());
//		lstTipos.add(TYPE_VARCHAR); 
		lstValores.add(new Integer(elemreparto.getNumCargo()));            
		lstTipos.add(TYPE_INTEGER);   
		...
		lstValores.add(reparto.getIdOrigen().getRefCatastral());            
		lstTipos.add(TYPE_VARCHAR);  
		lstValores.add(new Integer(reparto.getNumOrdenConsRepartir()));            
		lstTipos.add(TYPE_INTEGER);  
	...
	throws Exception
	{
		insertarDatosSuelo(sc, insertarExpediente, exp, null);
	}

	private void insertarDatosSuelo(SueloCatastro sc, boolean insertarExpediente, Expediente exp, String idMunicipio) 
	//throws DataException
	throws Exception
	{
		...
		{      
			...
			lstValores.add(sc.getDatosExpediente().getCodigoEntidadRegistroDGCOrigenAlteracion());
			lstTipos.add(TYPE_INTEGER);
	...
	throws Exception
	{
		insertarDatosUC(uc, insertarExpediente, exp, null);
	}

	private void insertarDatosUC(UnidadConstructivaCatastro uc, boolean insertarExpediente, Expediente exp, String idMunicipio)
	...
	throws Exception
	{
		insertarDatosConstruccion(construc, insertarExpediente, exp, null);
	}

	private void insertarDatosConstruccion(ConstruccionCatastro construc, boolean insertarExpediente, Expediente exp, String idMunicipio)
	...
	throws Exception
	{
		...
		{      
			...
			lstValores.add(new Integer(construc.getNumOrdenConstruccion()));            
			lstTipos.add(TYPE_INTEGER);       
			...
			lstValores.add(construc.getDatosEconomicos().getCorrectorApreciacion());            
			lstTipos.add(TYPE_FLOAT);      
	...
	throws Exception
	{ 
		insertarDatosCultivo(cultivo, insertarExpediente, exp, null);
	}
	/**
	 * 
	 * @param cultivo
	 ...
	 * @throws DataException
	 */
	private void insertarDatosCultivo(Cultivo cultivo, boolean insertarExpediente, Expediente exp, String idMunicipio) //throws DataException
	throws Exception
	{       
		try
		{     
	...
	{        
		...
		PreparedStatement s = null;
		ResultSet r = null;
		String codProvincia = paddingString(String.valueOf(dir.getProvinciaINE()),
				TAM_CODPRO,'0',true);

		String codMunicipioINE = paddingString(String.valueOf(dir.getMunicipioINE()),
				TAM_CODMUNI,'0',true);
	...
	{     
		...
		if (dir.getCodParaje() != null && !dir.getCodParaje().equals("") 
			...
			try{
				...
				String codProvincia = paddingString(String.valueOf(dir.getProvinciaINE().trim()), 
						2,'0', true);

				String codMunicipio = paddingString(String.valueOf(dir.getMunicipioINE().trim()),
						3,'0',true);

				String codParaje = paddingString(String.valueOf(dir.getCodParaje().trim()),
						5,'0',true);
	...
	throws Exception
	{       
		insertarDatosFinca (fc, insertarExpediente, exp, null);
	}
	 ...
	 * @throws DataException
	 */
	public void insertarDatosFinca (FincaCatastro fc, boolean insertarExpediente, Expediente exp, String idMunicipio)
	//throws DataException
	throws Exception
	{       
		try
		{         
			...
			String codProvincia = paddingString(String.valueOf(fc.getDirParcela().getProvinciaINE()),
					TAM_CODPRO,'0',true);
			String codMunicipio = paddingString(String.valueOf(fc.getDirParcela().getMunicipioINE()),
					TAM_CODMUNI,'0',true);
			...
			if(fc.getDirParcela().getCodigoPostal()==null){
				lstValores.add(null);
			}
			else{
			...
			//Si expediente distinto de null insertar datos expediente_finca_catastro
			if (insertarExpediente && exp!=null){
				
				r=null;
				s=null;
				
				s = conn.prepareStatement("select id_expediente, ref_catastral from expediente_finca_catastro where id_expediente=? and ref_catastral=?");
				s.setLong(1,exp.getIdExpediente());
				s.setString(2,fc.getRefFinca().getRefCatastral());
				r = s.executeQuery();
				...
				if (!r.next()){
					...
					r=null;
					s=null;
					s = conn.prepareStatement("insert into expediente_finca_catastro (id_expediente, ref_catastral) values (?,?)");

					lstValores.clear();
					lstTipos.clear();

					lstValores.add(exp.getIdExpediente());
					lstTipos.add(TYPE_NUMERIC); 
	...
	throws Exception
	{      
		return insertarDatosExpediente(datosExpediente, null);
	}
	 ...
	 * @throws DataException 
	 */
	public Long insertarDatosExpediente(Expediente datosExpediente, String idMunicipio) //throws DataException
	throws Exception
	{       
		...
		if(idExpediente!=null)
			...
			"protocolo_notarial=?, isexpsitfinales=? where id_expediente=?");
		else
		{
			...
			s = conn.prepareStatement("insert into expediente (id_entidad_generadora, id_estado, id_tecnico_catastro," +
					...
					"codigo_descriptivo_alteracion, descripcion_alteracion, nif_presentador, nombre_completo_presentador, " +
					"num_bienesinmuebles_urb, num_bienesinmuebles_rus, num_bienesinmuebles_esp, fecha_de_cierre, " +
					"cod_provincia_notaria, cod_poblacion_notaria, cod_notaria, anio_protocolo_notarial, protocolo_notarial, isexpsitfinales, " +
		...
		if (datosExpediente.getIdMunicipio() == 0 && idMunicipio != null && !idMunicipio.equals("")){
			lstValores.add(new Long(idMunicipio));
		}
		else{
		...
		lstTipos.add(TYPE_VARCHAR);
		lstValores.add(datosExpediente.isTipoTramitaExpSitFinales());
		lstTipos.add(TYPE_BOOLEAN);
	 ...
	 * @throws DataException 
	 */
	private Integer busquedaSridEntidad(String idEntidad)throws Exception
	{
		Integer srid = null;
		...
		s = conn.prepareStatement("select srid from entidad_supramunicipal where id_entidad=?");
		s.setInt(1,Integer.parseInt(idEntidad));
		r = s.executeQuery();
		if (r.next())
			srid = new Integer(r.getInt(1));       
	 ...
	 * @throws DataException 
	 */
	private HashMap transformGeometryGeograficas(Geometry geometriaParcela, Integer sridEntidad)throws Exception
	{

		HashMap hashGeometry = new HashMap();
		...
		s = conn.prepareStatement("select astext(transform (GeometryFromText(?,?),?)), srid(transform (GeometryFromText(?,?),?))");
		
		s.setString(1, ((Polygon)(geometriaParcela)).toString());
		s.setInt(2, sridEntidad);
		s.setInt(3, Integer.parseInt(SRID_INSERT_BBDD_GEOGRAFICAS));
		s.setString(4, ((Polygon)(geometriaParcela)).toString());
		s.setInt(5, sridEntidad);
		s.setInt(6, Integer.parseInt(SRID_INSERT_BBDD_GEOGRAFICAS));
		
		r = s.executeQuery();
		if (r.next())
			hashGeometry.put("GEOMETRY", new String(r.getString(1)));
			hashGeometry.put("SRID", new String(r.getString(2)));
	...
	public void insertarDatosSalida (ObjectOutputStream oos,UnidadDatosIntercambio udsa, boolean insertarExpediente, 
			Expediente exp) throws Exception {
		insertarDatosSalida (oos, udsa, insertarExpediente, exp, null, null);
	}
	 ...
	 */
	public void insertarDatosSalida (ObjectOutputStream oos,UnidadDatosIntercambio udsa, boolean insertarExpediente, 
			Expediente exp, String idMunicipio, String idEntidad) throws Exception //throws DataException

			{  
		try {
			
			String iddEntidad= String.valueOf(AppContext.getIdEntidad());
			
			Integer sridEntidad = null;
			...
			if (udsa.getFincaCatastro()!=null){
				if(udsa.getFxcc()!=null){
					if(udsa.getFxcc().getDXF()!=null){
						...
						geometriaParcela = ImportarUtils.obtenerGeometriaParcela(dxf);
						if(geometriaParcela!=null){
							// se busca el SRID de la entidad generadora de la tabla entidad_supramunicipal
							sridEntidad = busquedaSridEntidad(idEntidad);
							HashMap hashGeometry = transformGeometryGeograficas(geometriaParcela, sridEntidad.intValue());
							
							com.vividsolutions.jts.io.WKTReader a = new com.vividsolutions.jts.io.WKTReader();
							String wktGeometry = hashGeometry.get("GEOMETRY") + "," + hashGeometry.get("SRID");
							Geometry g = a.read(wktGeometry);
							g.setSRID(Integer.parseInt((String)hashGeometry.get("SRID")));
							
							insertarDatosFinca(g, udsa.getFincaCatastro(), insertarExpediente, exp, idMunicipio);
						}
						else{
							insertarDatosFinca(udsa.getFincaCatastro(), insertarExpediente, exp, idMunicipio);
						}
					}else
						insertarDatosFinca(udsa.getFincaCatastro(), insertarExpediente, exp, idMunicipio);
				}
			...
			insertarListaDatos(udsa.getLstSuelos(), insertarExpediente, exp, idMunicipio);
			udsa.setLstSuelos(null);
			insertarListaDatos(udsa.getLstUCs(), insertarExpediente, exp, idMunicipio);     
			udsa.setLstUCs(null);
			insertarListaDatos(udsa.getLstBienesInmuebles(), insertarExpediente, exp, idMunicipio); 
			udsa.setLstBienesInmuebles(null);
			insertarListaDatos(udsa.getLstCultivos(), insertarExpediente, exp, idMunicipio);
			udsa.setLstCultivos(null);
			insertarListaDatos(udsa.getLstConstrucciones(), insertarExpediente, exp, idMunicipio);
			udsa.setLstConstrucciones(null);
			insertarListaDatos(udsa.getLstRepartos(), insertarExpediente, exp, idMunicipio);   
...
//			if (insertarExpediente && exp != null){
//				completaReferenciasToXMLBD(exp, convenio, idMunicipio);
//			}
	...
	throws Exception
	{  
		insertarDatosFinca (geometriaFinca, fc, insertarExpediente, exp, null);
	}
	public void insertarDatosFinca (Geometry geometriaFinca, FincaCatastro fc, boolean insertarExpediente, Expediente exp, String idMunicipio)
	...
	{       
		...
		{         
			...
			String codProvincia = paddingString(String.valueOf(fc.getDirParcela().getProvinciaINE()),
					TAM_CODPRO,'0',true);
			String codMunicipio = paddingString(String.valueOf(fc.getDirParcela().getCodigoMunicipioDGC()),
					TAM_CODMUNI,'0',true);
			String codCompletoMunicipio = codProvincia + codMunicipio;
