/**
 * GeopistaPrintableOcupaciones.java
...
public class GeopistaPrintableOcupaciones extends javax.swing.JPanel implements Printable{
    ...
    public final static int FICHA_OCUPACIONES_CONSULTA = 1;
    public final static int FICHA_OCUPACIONES_MODIFICACION = 2;
    public final static int FICHA_OCUPACIONES_PLANOS = 3;
    public final static int FICHA_OCUPACIONES_UTILIDADES = 4;
	...
	public int print(Graphics graphics, PageFormat pageFormat, int pageIndex) throws PrinterException {
        ...
        {
            switch (tipoFicha) {
                case FICHA_OCUPACIONES_CONSULTA:
                case FICHA_OCUPACIONES_MODIFICACION:
                case FICHA_OCUPACIONES_PLANOS:
                case FICHA_OCUPACIONES_UTILIDADES:
                    rbI18N = com.geopista.app.ocupaciones.CMainOcupaciones.literales;
                    break;
            }

            Graphics2D g2d = (Graphics2D)graphics;
            init();
            pintarEncabezadoPagina(g2d,pageFormat);
...
//                                Object oEstruct = null;
                            if ((sValor != null) && (!sValor.equals(""))) {
                                switch (tipoFicha) {
                                    ...
                                    case FICHA_OCUPACIONES_PLANOS:
                                    case FICHA_OCUPACIONES_UTILIDADES:
                                        if (sDomSource.equalsIgnoreCase("TiposOcupacion")) {
                                            Hashtable hDomainNodes = com.geopista.app.ocupaciones.Estructuras.getListaTipoOcupacion().getLista();
                                            for (Enumeration _e=hDomainNodes.elements();_e.hasMoreElements();)
                                            {
                                                ...
                                                if (auxDomain.getPatron().equalsIgnoreCase(sValor))
                                                    sValor = ((DomainNode)com.geopista.app.ocupaciones.Estructuras.getListaTipoOcupacion().getDomainNode(sValor)).getTerm(com.geopista.app.ocupaciones.CMainOcupaciones.literales.getLocale().toString());
                                            }
                                        }
                                        else if (sDomSource.equalsIgnoreCase("ViasNotificacion"))
                                            sValor = ((DomainNode)com.geopista.app.ocupaciones.Estructuras.getListaViasNotificacion().getDomainNode(sValor)).getTerm(com.geopista.app.ocupaciones.CMainOcupaciones.literales.getLocale().toString());
                                        else if (sDomSource.equalsIgnoreCase("DocumentacionTipoNotificacion")) {
                                            Hashtable hDomainNodes = com.geopista.app.ocupaciones.Estructuras.getListaEstadosNotificacion().getLista();
                                            for (Enumeration _e=hDomainNodes.elements();_e.hasMoreElements();)
                                            {
                                                DomainNode auxDomain= (DomainNode)_e.nextElement();
                                                if (auxDomain.getPatron().equalsIgnoreCase(sValor))
                                                    sValor = auxDomain.getTerm(com.geopista.app.ocupaciones.CMainOcupaciones.literales.getLocale().toString());
                                            }
                                        }
                                        else
                                            sValor = "";
                   ...
                   if (xmlHoja.getNodeName().equalsIgnoreCase("SuperTitulo"))
                        printSuperTitulo(g2d,pageFormat, pageIndex,sValor);
                    else if (xmlHoja.getNodeName().equalsIgnoreCase("Cabecera"))
                        printCabecera(g2d,pageFormat, pageIndex, sValor);
