...
public class ExportImportDialog extends JDialog {
	...
	private Checkbox checkExportData;
...
//	private String currentName;
	...
	public static final int MODE_BORRADO = 3;
...
//				this.setTitle(this.getTitle() + " \"" + currentName + "\"");
		...
		else if(mode==MODE_BORRADO){
			this.setTitle(I18N.get("GestorCapas","layerutil.exportimport.title.borrar"));
			getAcceptButton().setText(I18N.get("GestorCapas","layerutil.exportimport.button.borrar"));
			getDataExportImportPanel().setBorder(new TitledBorder(null, I18N.get("GestorCapas","layerutil.exportimport.button.borrar"), TitledBorder.LEFT, TitledBorder.TOP, null, Color.DARK_GRAY));
			//getContentPane().add(getImportConditionsPanel());
		}
	...
	public boolean isCheckExportDataChecked(){
		return checkExportData.getState();
	}
	...
	private void acceptButtonActive(){
		getAcceptButton().setEnabled(((getNumSelectedLayer()>0) && (isCheckExportDataChecked() || isCheckExportImportTablesChecked() || isCheckExportImportDomainsChecked() || isCheckExportImportLayerChecked() || isCheckExportImportLayerFamiliesChecked())));
	}

	public void selectLayersByName(List<String> layerNames){
		getLayerCheckBoxList().getModel().getSize();	
		int size = listModel.getSize();
		for(int i = 1; i < size; ++i){
			Object obj = listModel.getElementAt(i);
			if(obj instanceof JCheckBox){
				JCheckBox checkbox = (JCheckBox) obj;
				Iterator<String> itLayerNames = layerNames.iterator();
				while(itLayerNames.hasNext()){
					String layerName = itLayerNames.next();
					String layerTableName = ((LayerTable) cmbLayer.getItemAt(i)).getLayer().getDescription();					
					if(layerTableName != null && layerTableName.equals(layerName)){
	...
	private JPanel getDataExportImportPanel() {
		if (dataExportImportPanel == null) {
			...
			dataExportImportPanel.setBounds(250, 68, 150, 141);	
			...
			dataExportImportPanel.add(getCheckExportData());	
		}
...
//			}
	...
	private Checkbox getCheckExportData(){
		if(checkExportData==null){
			checkExportData = new Checkbox();
			checkExportData.setState(true);
			checkExportData.setFont(new Font("Dialog", Font.PLAIN, 11));
			checkExportData.setBounds(15, 105, 112, 22);
			checkExportData.setLabel(I18N.get("GestorCapas","layerutil.exportimport.checkbox.datos"));
			checkExportData.addItemListener(new ItemListener() {
				public void itemStateChanged(ItemEvent e) {
			    	  acceptButtonActive();
				  }
	        }); 
