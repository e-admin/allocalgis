...
public class JDialogDBColumn extends JDialog
{
    ...
    {   
        if (colrow!=null)
        {
            ...
            this.cmbTipo.setSelectedItem(LCGIII_GeopistaUtil.translateBDtoComboItem(colrow.getColumnaBD().getType()));
    ...
    {
        if (type.equalsIgnoreCase(ColumnDB.COL_INTEGER))
            return "int4";
        else if (type.equalsIgnoreCase(ColumnDB.COL_CHAR))
            return "bpchar";
        else if (type.equalsIgnoreCase(ColumnDB.COL_BOOLEAN))
            return "bool";
        else 
    ...
    private JComboBox getCmbTipo()
    {
        if (cmbTipo == null)
        {
            ...
            cmbTipo.addItem("");
            cmbTipo.addItem(ColumnDB.COL_NUMERIC);            
            cmbTipo.addItem(ColumnDB.COL_INTEGER);
            cmbTipo.addItem(ColumnDB.COL_VARCHAR);
            cmbTipo.addItem(ColumnDB.COL_CHAR);
            cmbTipo.addItem(ColumnDB.COL_DATE);
            cmbTipo.addItem(ColumnDB.COL_GEOMETRY);
            cmbTipo.addItem(ColumnDB.COL_BYTEA);
            cmbTipo.addItem(ColumnDB.COL_TIMESTAMP);
            cmbTipo.addItem(ColumnDB.COL_BOOLEAN);
    ...
    protected void cmbTipo_actionPerformed(ActionEvent e)
    {
        if (spinnerTamanio!=null &&
                (cmbTipo.getSelectedItem().toString().equals(ColumnDB.COL_NUMERIC)
                        || cmbTipo.getSelectedItem().toString().equals(ColumnDB.COL_VARCHAR)
                        || cmbTipo.getSelectedItem().toString().equals(ColumnDB.COL_CHAR)))
        ...
        if (spinnerPrecision!=null && cmbTipo.getSelectedItem().toString().equals(ColumnDB.COL_NUMERIC))
    ...
    public ColumnRow getColumnRow ()
    {
        if(bAceptar)
        {
            ...
            if (!txtNombreSistema.getText().trim().equals(""))
            {
                ...
                colRow.getColumnaSistema().setlongitud(Integer.valueOf(spinnerTamanio.getValue().toString()).intValue());
            }
            //else
            //{
