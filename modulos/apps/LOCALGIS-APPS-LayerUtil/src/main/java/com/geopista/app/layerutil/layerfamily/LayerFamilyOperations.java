...
public class LayerFamilyOperations
{
     ...
     * @throws DataException Si se produce un error de acceso a datos 
     */
    public int comprobarLayerFamily(LayerFamilyTable newLayerfamily) throws DataException
    {        
        int coalesce = 0;  
        try
        { 
            ...
            //String query = "select Coalesce(count(*), 0)  from layerfamilies, dictionary where layerfamilies.id_name = dictionary.id_vocablo and (locale =? or locale ='es_ES') and dictionary.traduccion=?";
            PreparedStatement s = null;
            ResultSet r = null;
            s = conn.prepareStatement("LMcomprobarlayerfamilies");
            s.setString(1,locale);
            if (newLayerfamily.getHtNombre().get(locale) != null)
            		s.setString(2,newLayerfamily.getHtNombre().get(locale).toString());
            else
            	s.setString(2,newLayerfamily.getHtNombre().get("es_ES").toString());
            
            r = s.executeQuery();  
            if (r.next())           
               coalesce = r.getInt("coalesce");      
