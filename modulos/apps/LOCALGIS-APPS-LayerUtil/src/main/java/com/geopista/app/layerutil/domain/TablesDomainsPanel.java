...
public class TablesDomainsPanel extends JPanel implements FeatureExtendedPanel, TreeSelectionListener{
    ...
    private void initComponents()
    {
        ...
        try{
            ...
            //aplicacion.login();            
            com.geopista.app.administrador.init.Constantes.url= aplicacion.getString(UserPreferenceConstants.LOCALGIS_SERVER_URL)+ WebAppConstants.ADMINISTRACION_WEBAPP_NAME;
//            com.geopista.app.administrador.init.Constantes.idMunicipio=Integer.parseInt(AppContext.getApplicationContext().getString(UserPreferenceConstants.DEFAULT_MUNICIPALITY_ID));
            ...
            com.geopista.app.administrador.init.Constantes.Locale = UserPreferenceStore.getUserPreference(UserPreferenceConstants.DEFAULT_LOCALE_KEY, AppConstants.DEFAULT_LOCALE, false);
            ...
            try
            {
                messages = ResourceBundle.getBundle("com.geopista.app.layerutil.language.GestorCapasi18n", new Locale(UserPreferenceStore.getUserPreference(UserPreferenceConstants.DEFAULT_LOCALE_KEY, AppConstants.DEFAULT_LOCALE, false)));
            }catch (Exception e)
            {
                messages = ResourceBundle.getBundle("com.geopista.app.layerutil.language.GestorCapasi18n", new Locale(AppConstants.DEFAULT_LOCALE));
            }            
    ...
    {	
        try{
            ...
            //Patron del dominio
            String cadena = auxDomain.getListaNodes().getFirst().getPatron();
            int valorNumerico=-1;
            if (cadena!=null && cadena.length()>4){
            String cadenaNueva = cadena.substring(0, cadena.length()-4); 
            	if (cadenaNueva.startsWith("?"))
            		cadenaNueva=cadenaNueva.substring(1);
            	try{
            		valorNumerico=Integer.parseInt(cadenaNueva,10);
            	}
	            catch (Exception e){            			            	
	            }
            }
            ...
            //Solo se mira esta condicion se el campo es de tipo varchar

            if(( valorNumerico!=-1) && 
            		(valorNumerico!=auxColumn.getlongitud()) &&
            		(Integer.parseInt(auxColumn.getAttribute().getType())==3)){
            	
                   	JOptionPane.showMessageDialog(null,"La longitud de los campos tienes extensiones diferentes.\n"
                   			+ "Seleccione un dominio de longitud '"+auxColumn.getlongitud()+"' "
                   			+ "para la columna: '"+auxColumn.getName()+"'");
                  return;
            }
    ...
    public void valueChanged (TreeSelectionEvent e){
        ...
        {
            levelActivado=false;
            idMunicipio = Integer.parseInt(AppContext.getApplicationContext().getString(UserPreferenceConstants.DEFAULT_MUNICIPALITY_ID));
