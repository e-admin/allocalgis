...
public class LabelStyle implements Style {
    ...
    //añadimos el alineado horizontal
     ...
     * returns null.
     */
    private Geometry intersection(Geometry geometry, IViewport viewport)
        throws NoninvertibleTransformException {
        return geometry.intersection(viewportRectangle(viewport));
    }
    private Geometry viewportRectangle(IViewport viewport)
        throws NoninvertibleTransformException {
    ...
    private double verticalAlignmentOffset(double scaledLabelHeight) {
        ...
        double offset =
            buffer
                + (((BasicStyle)layer.getBasicStyle()).getLineWidth() / 2d)
