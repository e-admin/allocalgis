...
public class FeatureUtil {
	...
	public static Feature toFeature(Geometry g, FeatureSchema schema) {
	    Feature feature = new GeopistaFeature(schema);
    ...
    /**
     * Copia todos los atributos menos la geometria.
     * @param a
     * @param b
     */
    public static void copySpecialAttributes(Feature a, Feature b) {
        for (int i = 0; i < a.getSchema().getAttributeCount(); i++) {
        	if (a.getSchema().getAttributeType(i)!= AttributeType.GEOMETRY)
        		b.setAttribute(i, a.getAttribute(i));
        }
    }
