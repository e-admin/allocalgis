...
/**
 * Categoria: agrupa un conjunto de layers que responden
 * al mismo criterio.
 *
 * Sobre una Category se pueden dar tres tipos de EVENTOS:
 *  Añadir layer
 *  Quitar layer
 *  Cambiar metadatos
 *
...
public class Category {
    ...
    public void remove(Layerable layerable) {
        ...
        getLayerManager().removeOrderLayer(layerable);
    }
    ...
    public void add(int index, Layerable layerable) {
    	// Modified by Lilian
    	if (index>=0){
	    	while (layerables.size()<=index){
	    		layerables.add((layerables.size()), null);
	    	}
    	}
    	if (index<0){
    		layerables.add(0, layerable);
    	}else{
    		layerables.set(index, layerable);
    	}
        
  		//ESTE METODO de fireLayerChanged tarda un tiempo que no es despreciable
        if (getLayerManager() != null) {
            //layerManager is null when Java2XML creates the object. [Jon Aquino]
        	//TODO JAVIER
