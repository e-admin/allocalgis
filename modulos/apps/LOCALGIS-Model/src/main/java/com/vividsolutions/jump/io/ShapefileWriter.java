 ...
 *<tr><td>OutputFile or DefaultValue</td><td>File name for the output .shp file</td></tr>
 *<tr><td>ShapeType</td><td>Dimentionality of the Shapefile - 'xy', 'xym' or 'xyz'.  'xymz' and 'xyzm' are the same as 'xyz' </td></tr>
 *</table><br>
 ...
 *
 *JCS columns are converted to DBF columns by:
 *
 *JCS Column        DBF column
 ...
 *
 *For more information on the DBF file format, see http://www.apptools.com/dbase/faq/qformt.htm.
 *
 *Since shape files may contain only one type of geometry (POINT, MULTPOINT, POLYLINE, POLYGON,
 *POINTM, MULTPOINTM, POLYLINEM, POLYGONM, POINTZ, MULTPOINTZ, POLYLINEZ, or POLYGONZ), the
 ...
 *for example, if the shapetype is determined to be 'POLYLINE' any POINT, MULTIPOINT, or POLYGON
 *geometries in the FeatureCollection will be replaced with a NULL geometry.
 *
 *The coordinate dimensionality can be explicitly set with a DataProperties tag of 'ShapeType':
 *'xy', 'xym', or 'xyz' ('xymz' and 'xyzm' are pseudonyms for 'xyz').  If this DataProperties is
 *unspecified, it will be auto set to 'xy' or 'xyz' based on the first non-NULL geometry having a Z coordinate.
 *
 *Since JCS and JTS do not currently support a M (measure) coordinate, it will always be set to
 *–10E40 in the shape file (type 'xym' or 'xyzm').  This value represents the Measure "no data"
 *value (page 2, ESRI Shapefile Technical Description).  Since the 'NaN' DOUBLE values for Z
 *coordinates is invalid in a shapefile, it is converted to '0.0'.
 *
...
public class ShapefileWriter implements JUMPWriter {
    ...
    public void write(FeatureCollection featureCollection, DriverProperties dp) throws IllegalParametersException, Exception{
    	write(featureCollection,dp,null,16);
    }
    
    
    public void write(FeatureCollection featureCollection, DriverProperties dp,HashMap<String,String> excludeFields,int doubleNumberOfDecimals)
        ...
        writeDbf(featureCollection, dbffname,excludeFields,doubleNumberOfDecimals);
    ...
    *(3 is for x,y,m but thats not supported yet) <br>
    ...
    void writeDbf(FeatureCollection featureCollection, String fname) throws Exception{
    	writeDbf(featureCollection,fname,null,16);
    }

    
    void writeDbf(FeatureCollection featureCollection, String fname,HashMap<String,String> excludeFields,int doubleNumberOfDecimals)
        throws Exception {
        DbfFileWriter dbf;
        FeatureSchema fs;
        int t;
        ...
        for (t = 0; t < fs.getAttributeCount(); t++) {
          ...
          //Incluido para evitar determinadas columnas
            if ((excludeFields!=null) && (excludeFields.containsKey(columnName))){
            	excludeFieldsCount++;
            	continue;
            }
            ...
            if (columnType == AttributeType.INTEGER) {
            	...
            	// [JP] The framework supports Long
...
//            	 [JP] The framework supports FLOAT
            	if (columnType == AttributeType.FLOAT) {
            fields[f] = new DbfFieldDef(columnName, 'N', 33, doubleNumberOfDecimals);
            f++;
        ...
        if (excludeFieldsCount>0){
        	for (int i=0;i<excludeFieldsCount;i++)
        		fields=(DbfFieldDef[]) org.apache.commons.lang.ArrayUtils.remove(fields,fields.length-1);
        }
        ...
        for (t = 0; t < num; t++) {
            ...
            for (u = 0; u < fs.getAttributeCount(); u++) {
                ...
                if (columnType == AttributeType.INTEGER || 
                    ...
                    // [JP] Sometimes Attributes of type INTEGER are actually Long
                    // this is a workarround for ClassCastException
                    	int value=feature.getInteger(u);
                        DBFrow.add(new Integer(value));
                    }
                } else if (columnType == AttributeType.DOUBLE ||
...
//                  [JP] Sometimes Attributes of type DOUBLE are actually Float
                    // this is a workarround for ClassCastException
                    	double value=feature.getDouble(u);
                        DBFrow.add(new Double(value));
                    }
                } else if (columnType == AttributeType.DATE) {
                    Object a = feature.getAttribute(u);
                    if (a == null) {
