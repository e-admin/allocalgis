 ...
 */
public class GeoGMLInputTemplate  extends DefaultHandler {
    ...
    public String columnName(int index) throws ParseException {
        if (loaded) {
            return ((GeopistaColumnDescription) columnDefinitions.get(index)).columnName;
        } else {
     ...
     **/
    public FeatureSchema toFeatureSchema(String readerName) throws ParseException {
        if (!(loaded)) {
            throw new ParseException(
                "requested toFeatureSchema w/o loading the template");
        }

        readerName = readerName.substring(0,readerName.length()-4) + ".sch";
        File schemaFile = new File(readerName);
        
        //si el fichero de esquema no existe cargamos la geometria con el sistema
        //de JUMP
        ...
        GeopistaSchema geopistaSchema = null;
         ...
         try {
            ...
            geopistaSchema = (GeopistaSchema) converter.read(reader, com.geopista.feature.GeopistaSchema.class);

            /*//la geometria la debe cargar del fichero sch,
            //yo creo que ya no hace falta que este en el fichero sch
            ...
            geopistaSchema.addAttribute("GEOMETRY", AttributeType.GEOMETRY,columnGeometry,GeopistaSchema.READ_WRITE);*/
            return geopistaSchema;

          }catch(FileNotFoundException e) // Si no existe el fichero se usa el esquema de JUMP
          {
            System.out.println("No existe información ampliada de esquema: "+ readerName);
          }
          catch (Exception e)
		  {
          	System.out.println("Problemas leyendo el esquema: "+ readerName);
          	e.printStackTrace();
		  }
           ...
           if (!(loaded)) {
            ...
            for (int t = 0; t < columnDefinitions.size(); t++) {
                fcmd.addAttribute(((GeopistaColumnDescription) columnDefinitions.get(t)).columnName,
                    ((GeopistaColumnDescription) columnDefinitions.get(t)).getType());
            }
            return fcmd;
    ...
    public int match(String XMLtagName, Attributes xmlAtts)
        ...
        if (loaded) {
            for (int t = 0; t < columnDefinitions.size(); t++) {
                if (((GeopistaColumnDescription) columnDefinitions.get(t)).match(
    ...
    public Object getColumnValue(int index, String tagBody, Attributes xmlAtts)
        ...
        if (((GeopistaColumnDescription) columnDefinitions.get(index)).valueType == ColumnDescription.VALUE_IS_BODY) {
            val = tagBody;
        } else {
            val = xmlAtts.getValue(((GeopistaColumnDescription) columnDefinitions.get(
                        index)).valueAttribute);
        }

        //have the value as a string, make it an object
        cd = (GeopistaColumnDescription) columnDefinitions.get(index);
    ...
    public void endElement(String uri, String name, String qName)
        ...
        try {
            ...
            if (qName.equalsIgnoreCase("column")) {
                ...
                //we're okay
                GeopistaColumnDescription colDes;

                colDes = new GeopistaColumnDescription();
