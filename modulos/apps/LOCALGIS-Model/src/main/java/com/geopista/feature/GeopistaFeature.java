...
public class GeopistaFeature extends AbstractBasicFeature
{
	...
	public Feature clone(boolean deep)
	{
	 ...
	 clone.setFireDirtyEvents(isFireDirtyEvents());
...
public boolean isTempID()
{
    ...
    public void setDirtySpecial(boolean dirty){
    	 this.dirty = dirty;
    }
    public void forceDirty(boolean dirty)
    {        
        this.dirty = dirty;
        if (layer != null)
        {
        ...
        if (super.getSchema() instanceof GeopistaSchema)
        {
            ...
            } else
            {
                ...
                if (localIsDirty == true)
                {
                	...
                	System.out.println("El elemento no esta modificado");
                }
            }
        } else
        {
        ...
        if (getSchema() instanceof GeopistaSchema)
        {
            ...
            switch (layerGeometryType)
                ...
                case Table.MULTIPOLYGON:
                    ...
                    {
                    	logger.error("Tipo de geometria no permitida:"+geometry.getClass());
...
// Do nothing
	...
	{
		//Solucion incidencia MODELO 196. Se exportan incorrectamente los elementos de tipo double. Se exportan
		//sin decimales
        Object currentAttribute = getAttribute(attributeIndex); 
        
        if(currentAttribute instanceof Number){
        	try{
        		return ((Number) currentAttribute).doubleValue();
        	}
        	catch (Exception e){
        		return ((Number) currentAttribute).intValue();
        	}
    ...
    public boolean setGeometryEmpty()
    {
    	if (getSchema() instanceof GeopistaSchema)
    	{
    		...
    		if(geometryIndex==-1)
    			logger.error("La capa no tiene geometría asociada. Revisa la estructura de la capa");
