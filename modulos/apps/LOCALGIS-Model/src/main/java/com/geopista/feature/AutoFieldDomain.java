...
public class AutoFieldDomain extends Domain {
	...
	public Object getRightValue(Feature feature, String attName)
	{
		...
		if (type.equals(AutoFieldDomain.AUTOENV_VAR))
			{
				if (attName == null)
					return UserPreferenceStore.getUserPreference(expression,null,true);
				...
				//Si no incluimos esto, inserta siempre en el municipio 34083.
				if (expression.equals("geopista.DefaultCityId"))
					return String.valueOf(AppContext.getIdMunicipio());
				
				else return UserPreferenceStore.getUserPreference(expression,null,true);
			}
		else
