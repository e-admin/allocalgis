...
public class ReferenciarViasPanel extends javax.swing.JPanel implements WizardPanel {
	...
	private ArrayList<GeopistaDatosImportarPadronConDomicilio> viasSinReferenciar;
	private GeopistaDatosImportarPadronConDomicilio elementListSelected;
	private JPanel jPanelInfo = null;
	private JPanel jPanelDatos = null;
	public static HashMap<GeopistaDatosImportarPadronConDomicilio, ArrayList<PoliciaCoincidencias>> listaCoincidencias = new HashMap<GeopistaDatosImportarPadronConDomicilio, ArrayList<PoliciaCoincidencias>>();
	...
	public Connection getDBConnection()
	{
		...
		try
		{
			...
			String sConn = app.getString(UserPreferenceConstants.LOCALGIS_DATABASE_URL);
	...
	private void asociarVias()
	{
		...
		GeopistaDatosImportarPadronConDomicilio datos = (GeopistaDatosImportarPadronConDomicilio) jTableViasSinReferenciar.getValueAt(rowViaOrigen,NoGeoRefTableModel.ROW_NOMBRE);
		...
		if (listaCoincidencias.get(datos) == null){
			listaCoincidencias.put(datos,new ArrayList<PoliciaCoincidencias>());
		}
	...
	private void escogerAsociacion()
	{
		...
		elementListSelected = (GeopistaDatosImportarPadronConDomicilio)auxList.getElementAt(selectedRow);

	}
	/*
	 * Elimina la entrada en la tabla origen
	 */
	public void eliminar(GeopistaDatosImportarPadronConDomicilio  datos,int row)
	{
	...
	public void eliminarAsociacion()
	{
		...
		GeopistaDatosImportarPadronConDomicilio devuelve = (GeopistaDatosImportarPadronConDomicilio)jListViasAsociadas.getSelectedValue();
				...
				new Thread(new Runnable()
				{
					public void run()
					{
						...
						{ 
							...
							//javier aragon
							viasSinReferenciar = (ArrayList<GeopistaDatosImportarPadronConDomicilio>) wizardContext.getData("PortalesSinDomicilio");
							actualizarModeloOrigen(viasSinReferenciar);
						} catch (Exception e)
						{
							e.printStackTrace();
	...
	public void exitingToRight() throws Exception {
		...
		if (!asociaciones.isEmpty()){
			...
			Iterator<GeopistaDatosImportarPadronConDomicilio> it = listaCoincidencias.keySet().iterator();
			while (it.hasNext()){
				GeopistaDatosImportarPadronConDomicilio datos = it.next();
				//int idDomicilio = operacionesBD.findDomicilio(datos.getData());
				ArrayList<PoliciaCoincidencias> numsPolicia = listaCoincidencias.get(datos);
				String numPolicia = ((PortalGeorreferenciado)numsPolicia.get(0).getDatos().get(0)).getPortal();
				Iterator<Integer> idsDomicilioIterator =  datos.getIdDomicilio().iterator();
				while(idsDomicilioIterator.hasNext()){
					Integer id = operacionesBD.findIdPortal(datos.getData().getIdVia(), numPolicia);
					operacionesBD.updateIdPortalAtDomicilio(id,idsDomicilioIterator.next());
				}
			}
		}
	}
...
class RendererAsociacionesPanelFinal extends DefaultListCellRenderer
{
			...
			Object value, int i, boolean s, boolean f)
	{
		...
		GeopistaDatosImportarPadronConDomicilio valor = (GeopistaDatosImportarPadronConDomicilio)value;
		...
		label.setText(valor.getData().getTipoVia()+" "+valor.getData().getNombreCortoVia()+" =====> "+result);
...
class RendererOrigenNombre implements TableCellRenderer{
	...
	public Component getTableCellRendererComponent(JTable table,
		...
		if (value!=null)
			aux = new JLabel(valor.getData().getNombreCortoVia());
		if (isSelected)
