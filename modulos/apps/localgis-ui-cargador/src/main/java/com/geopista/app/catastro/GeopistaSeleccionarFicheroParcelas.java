/**
 * GeopistaSeleccionarFicheroParcelas.java
...
*/
	...
	public static boolean encontrarCampo(String nombreCampo, FeatureSchema esquema)
	{
...
*/
	...
	public static GeopistaLayer loadLayer(GeopistaEditor geopistaEditor, String absolutePath, Blackboard blackboard, AppContext aplicacion) throws Exception{
		GeopistaLayer layer08;
		try{
			layer08 = (GeopistaLayer) blackboard.get("capaParcelasInfoReferencia");
	        if (layer08 != null){
	        	geopistaEditor.getLayerManager().remove(layer08);
	        }
	        layer08 = (GeopistaLayer) geopistaEditor.loadData(absolutePath,aplicacion.getI18nString("importar.informe.parcelas"));
    ...
    public static boolean validate(GeopistaLayer layer, String cadenaTexto[], AppContext aplicacion){
	    boolean hayErroresFila = false;   
		...
		try{
	        // Obtener el esquema
	        FeatureSchema esquema = layer.getFeatureCollectionWrapper().getFeatureSchema();
	        ...
	        if (campoMasaCorrectos && campoHojaCorrecto && campoParecelaCorrecto && campoTipoCorrecto && campoFechaAltaCorrecto && campoFechaBajaCorrecto){
	            // A partir de aqui hay que verificar que no hay nulos y es del tipo correcto los valores.
	            List listaLayer = layer.getFeatureCollectionWrapper().getFeatures();
	            Iterator itLayer = listaLayer.iterator();
	            ...
	            while (itLayer.hasNext()){
	                ...
	                String parcela = f.getString("PARCELA");
	                String tipo = f.getString("TIPO");
	                // Comprobamos que no sea nulo y sea una U o una R
	                if ((!tipo.equals("U")) && (!tipo.equals("R")) && (!tipo.equals("D")) && (!tipo.equals("X"))){
	                    // Solo puede haber una R  una U una D o una X.
	                    cadenaTexto[0] = cadenaTexto[0] + aplicacion.getI18nString("importar.informe.parcelas.rustico")+ masa+ hoja+ parcela+ aplicacion.getI18nString("importar.informe.parcelas.fin.rustico");
	                    hayErroresFila = true;
	                    hayErroresFilas = true;
	                } else{
	                    ...
	                    try{
	                        ...
	                        if (f.getString("FECHABAJA").equals("99999999")){
	                            ...
	                            if ((f.getString("FECHABAJA")) == null){
	                                ...
	                                try{
	                                    ...
	                                    hayErroresFila = true;
	                                    hayErroresFilas = true;
	                                    cadenaTexto[0] = cadenaTexto[0]+ aplicacion.getI18nString("importar.informe.parcelas.fecha.baja")
	                        ...
	                        // En la fecha de alta
	                        cadenaTexto[0] = cadenaTexto[0]+ aplicacion.getI18nString("importar.informe.parcelas.fecha.alta.validacion")
	                                + masa+ hoja+ parcela+ aplicacion.getI18nString("importar.informe.parcelas.fin.rustico");
	                        exc.printStackTrace();
	                        hayErroresFila = true;
	                        hayErroresFilas = true;
	            ...
	            cadenaTexto[0] = cadenaTexto[0]+ aplicacion.getI18nString("importar.informacion.ficheros.no.correctos");
	            hayErroresFilas = true;
	        }
		} catch (Exception e){
			hayErroresFilas = true;
