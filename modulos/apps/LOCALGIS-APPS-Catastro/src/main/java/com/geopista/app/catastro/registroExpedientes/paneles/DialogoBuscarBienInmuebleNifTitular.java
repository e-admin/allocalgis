/**
 * DialogoBuscarBienInmuebleNifTitular.java
    ...
    private void cargaDatosTabla()
    {
        ...
        for(int i=0; i< referenciasCatastrales.size();i++)
        {
            ...
            if(referenciasCatastrales.get(i) instanceof Persona)
            {
                ...
                System.out.println("NIF: " + datos[i][0]);
                BienInmuebleCatastro bienInmueble = persona.getBienInmueble();
                auxDir = bienInmueble.getDomicilioTributario();
                datos[i][1] = bienInmueble.getIdBienInmueble().getIdBienInmueble();
                System.out.println("Ref catastral: " + datos[i][1]);
            }

            if(Estructuras.getListaTipos().getDomainNode(auxDir.getTipoVia())!=null)
            {
                datos[i][2] = Estructuras.getListaTipos().getDomainNode(auxDir.getTipoVia())
            ...
            System.out.println("Tipo: " + datos[i][2]);
            datos[i][3] = auxDir.getNombreVia();
            System.out.println("Nombre Via: " + datos[i][3]);
            ...
            System.out.println("Numero: " + datos[i][4]);
        }
                    ...
                    public void run()
                    {
                        try
                        {
                            ...
                            try
                            {
                                ...
                                //Todo mirar convenio
                                referenciasCatastrales= (ArrayList)ConstantesRegExp.clienteCatastro.getBienInmuebleBuscadosPorTitular(patron);
                                cargaDatosTabla();
                            }
                            catch(ACException e)
                            {
