 ...
 * @author yraton
 */
public class InhumacionJDialog extends javax.swing.JDialog {
	
	private org.apache.log4j.Logger logger= org.apache.log4j.Logger.getLogger(InhumacionJDialog.class);

	private static final long serialVersionUID = 1L;

	private int patronInhumacion = 2;

	private static final String patron = "GD-2";
	
	private String operacion;
	private String tipo;
    private ApplicationContext aplicacion;
    private javax.swing.JFrame desktop;
    private InhumacionBean inhumacion;
	...
	private Vector vDomainTipoInhumacion;
    ...
    public InhumacionJDialog (JFrame desktop, String locale, String operacion,String tipo, UnidadEnterramientoBean unidadSelected, ArrayList listaDifuntos,  ArrayList listaTarifas) throws Exception{
    	super(desktop);
    ...
    public InhumacionJDialog(JFrame desktop, String locale) throws Exception{
        super(desktop);
        this.desktop= desktop;
        this.locale= locale;
        inicializar();
    ...
    private void inicializar() {
        ...
        tipoInhumacionJComboBox = new javax.swing.JComboBox();
        ...
        vDomainTipoInhumacion = Estructuras.getListaCombosTipoInhumacionesSorted(locale);
        tipoInhumacionJComboBox = new JComboBox(vDomainTipoInhumacion);
        for (int i = 0; i < vDomainTipoInhumacion.size(); i++) {
  		DomainNode node = (DomainNode) vDomainTipoInhumacion.get(i);
  		if (node.getPatron().equalsIgnoreCase(String.valueOf(getPatroninhumacion()))){
  			tipoInhumacionJComboBox.setSelectedIndex(i);
  		}
  	}
        
        
        tipoInhumacionJComboBox.addActionListener(new java.awt.event.ActionListener(){
  		public void actionPerformed(ActionEvent e){
  			JComboBox cb = (JComboBox)e.getSource();
  			DomainNode dNode= (DomainNode) cb.getSelectedItem();
  			 int patron = Integer.parseInt(dNode.getPatron());
  			 if (patron != 0){
  				 setPatronInhumacion(patron);
  			 }
  		}
  	});
        ...
        tipoInhumacionJLabel.setText("Tipo Inhumacion"); //(301, 68, -1, -1));
        DatosDefuncionJPanel.add(tipoInhumacionJLabel, new org.netbeans.lib.awtextra.AbsoluteConstraints(605, 68, -1, -1));
        DatosDefuncionJPanel.add(tipoInhumacionJComboBox, new org.netbeans.lib.awtextra.AbsoluteConstraints(702, 65, 150, -1)); //(389, 65, 153, -1)

        fechaInhumacionJLabel.setText("Fecha Inhumacion"); // (301, 94, -1, -1)
        ...
        DatosDefuncionJPanel.add(informeJScrollPane, new org.netbeans.lib.awtextra.AbsoluteConstraints(18, 143, 834, 38));

        informeInhumacionJLabel.setText("Informe de Inhumacion:");
...
//	} 
    	...
    	inhumacion= null;
    		...
    		actualizarDatosInhumacion(inhumacion);
      }
      fireActionPerformed();
  }
  
  
  public void actualizarDatosInhumacion(InhumacionBean inhumacion){
      
  	if (inhumacion==null || inhumacion.getTipo() == null) return;
      
  	inhumacion.setEntidad(entidadJTextField.getText().trim());
  	inhumacion.setNombreCementerio(cementerioJTextField.getText().trim());
  	
  	inhumacion.setTipo_contenedor(getPatronContenedor());
  	inhumacion.setTipo_inhumacion(getPatroninhumacion());
  	inhumacion.setFecha_inhumacion(fechaInhumacionJTextField.getDate());
  	inhumacion.setCodigo(codigoJTextField.getText().trim());
  	inhumacion.setInforme_inhumacion(informeJTextArea.getText().trim());
  	
  	DifuntoBean difunto = null;
  	if (getDifuntoSeleccionado()!= null){
  		difunto = getDifuntoSelected();
  	}else if (inhumacion.getDifunto() != null){
  		difunto = inhumacion.getDifunto();
  	}
    ...
    //2.tarifa
  	...
  	}else if (inhumacion.getTarifa() != null){
  		tarifa = inhumacion.getTarifa();
  	}else{
  		tarifa = new TarifaBean();
  	}
    tarifa.setTipo_tarifa(Const.TARIFA_GDIFUNTOS);

  	inhumacion.setTarifa(getTarifaSelected());
  	
  	inhumacion.setBonificacion(bonificaionJTextField.getText().trim());
  	inhumacion.setPrecio_final(precioJTextField.getText().trim());
      
  	//Por ultimo hago el set
	setInhumacion(inhumacion);
	 ...
	 */
  public void load(InhumacionBean inhumacionElem, boolean editable) {
	if (inhumacionElem == null)
			return;
		
	cementerioJTextField.setText(inhumacionElem.getNombreCementerio() != null ?  inhumacionElem.getNombreCementerio() : "");
	cementerioJTextField.setEditable(false);
	entidadJTextField.setText(inhumacionElem.getEntidad() != null ?  inhumacionElem.getEntidad() : "");
	entidadJTextField.setEditable(false);
		
	setInhumacion(inhumacionElem);
      ...
      }else if (inhumacionElem.getDifunto()!= null){
    	  datosFallecimiento = inhumacionElem.getDifunto().getDatosFallecimiento();
      }
      ...
      if ((operacion.equalsIgnoreCase(Constantes.OPERACION_MODIFICAR) || 
      	...
      	loadInhumacion(inhumacionElem, editable);
  ...
  public void loadInhumacion (InhumacionBean inhumacionElem, boolean editable){
	    ...
	    //cargarmos las estructuras..para obtener el tipo de concesion 
	      ...
	      for (int i = 0; i < vDomainTipoContenedor.size(); i++) {
			DomainNode node = (DomainNode) vDomainTipoContenedor.get(i);
			if (node.getPatron().equalsIgnoreCase(String.valueOf(inhumacionElem.getTipo_contenedor()))){
				contendorJComboBox.setSelectedIndex(i);
			}
	      }
		...
		fechaInhumacionJTextField.setDate(inhumacionElem.getFecha_inhumacion() != null ? inhumacionElem.getFecha_inhumacion() : new Date());
		fechaInhumacionJTextField.setEnabled(editable);

		codigoJTextField.setText(inhumacionElem.getCodigo()!= null ? inhumacionElem.getCodigo() : "");
		codigoJTextField.setEditable(editable);

		informeJTextArea.setText(inhumacionElem.getInforme_inhumacion()!= null ? inhumacionElem.getInforme_inhumacion() : "");
		...
		TarifaBean tarifa = inhumacionElem.getTarifa();
		if (tarifa != null) loadDatosTarifa (tarifa, editable);

      bonificaionJTextField.setText(inhumacionElem.getBonificacion()!= null ? inhumacionElem.getBonificacion() : "0");
      bonificaionJTextField.setEditable(editable);
      
      precioJTextField.setText(inhumacionElem.getPrecio_final()!= null ? inhumacionElem.getPrecio_final() : "0");
      precioJTextField.setEditable(editable);
      
      for (int i = 0; i < vDomainTipoInhumacion.size(); i++) {
	    	DomainNode node = (DomainNode) vDomainTipoInhumacion.get(i);
			if (node.getPatron().equalsIgnoreCase(String.valueOf(inhumacionElem.getId_inhumacion()))){
				tipoInhumacionJComboBox.setSelectedIndex(i);
				setPatronInhumacion(inhumacionElem.getTipo_inhumacion());
			}
	      }
	       tipoInhumacionJComboBox.setEnabled(editable);
  ...
  public void loadDatosTarifa (TarifaBean tarifa, boolean editable){
    ...
    public InhumacionBean getInhumacion() {
		return inhumacion;
	}

	public void setInhumacion(InhumacionBean inhumacion) {
   ...
   private ArrayList getTarifasByServicioAndUnidad (){
	   ...
	   for (int i = 0; i < listaTarifas.size(); i++) {
		...
		//String.valueOf(patronInhumacion)
		if ((tarifa.getTipo_calculo().equals(patron)) && (tarifa.getTipo_unidad() == getUnidadSelected().getTipo_unidad())){
    ...
    public int getPatroninhumacion() {
		return patronInhumacion;
	}


	public void setPatronInhumacion(int patronInhumacion) {
		this.patronInhumacion = patronInhumacion;
	}
    ...
    private javax.swing.JComboBox tipoInhumacionJComboBox;
