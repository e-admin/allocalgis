/**
 * CircleInformationGeoMarketingPlugIn.java
...
import com.localgis.app.gestionciudad.plugins.geomarketing.dialogs.SelectRadiusDialog;
...
import com.localgis.app.gestionciudad.plugins.geomarketing.utils.SelectItemsByCircleToolGeoMarketing;
...
public class CircleInformationGeoMarketingPlugIn extends ThreadedBasePlugIn{

	private double radius = -1;
	private static Geometry jtsGeometry = null;
	private boolean circleGeomarketingButtonAdded = false;
	private static AppContext aplicacion = (AppContext) AppContext.getApplicationContext();

	static SelectItemsByCircleToolGeoMarketing circleTool= null;
...
//		}
		...
		SelectRadiusDialog dialog = new SelectRadiusDialog(context.getWorkbenchGuiComponent().getMainFrame());
		if (dialog.wasOKPressed()){
			radius = dialog.getSelectedRadius();
			/*
			 * Gets Geometry of a circle of radius equals to this.radius.
			 */
			if (radius > 0){
				// this tool is only for one click!
				circleTool= new SelectItemsByCircleToolGeoMarketing(context.getWorkbenchContext(), radius);
				circleTool.activate(context.getLayerViewPanel());
				context.getLayerViewPanel().setCurrentCursorTool(circleTool);
				//				 while (circleTool.isActive()){
				//					 jtsGeometry = circleTool.getCircleGeometry();
				//				 }
	...
	public static void onCircleSelectFinish(){
		try{
			jtsGeometry = circleTool.getCircleGeometry();
	...
	public ImageIcon getIcon() {
		return IconLoader.icon(I18N.get("geomarketing","localgis.geomarketing.plugins.circle.icon"));
	}
	...
	public void initialize(PlugInContext context) throws Exception {
		...
		}, I18N.get("geomarketing","localgis.geomarketing.plugins.menu.circle.title"), false, null,
...
//				.add(checkFactory.createExactlyNFeaturesMustBeSelectedCheck(1))
	...
	{
		if (!circleGeomarketingButtonAdded )
		{
			toolbox.addToolBar();
			CircleInformationGeoMarketingPlugIn explode = new CircleInformationGeoMarketingPlugIn();                 
			toolbox.addPlugIn(explode, null, explode.getIcon());
			toolbox.finishAddingComponents();
			toolbox.validate();
			circleGeomarketingButtonAdded = true;
		}
	}
