...
public void layout(boolean changed) {
	...
	if (!alwaysShowScroll) {
		hVisible = needHScroll(contentRect, false);
		vVisible = needVScroll(contentRect, hVisible);
	...
	Rectangle hostRect = getHostRectangle(hVisible, vVisible);
	if (expandHorizontal) {
...
private boolean needHScroll(Rectangle contentRect, boolean vVisible) {
	...
	Rectangle hostRect = getHostRectangle(false, vVisible);
...
private boolean needVScroll(Rectangle contentRect, boolean hVisible) {
	...
	Rectangle hostRect = getHostRectangle(hVisible, false);
...
/**
 * 	Actived the automatic scrol when focus changed and the focused control are not visible. 
 * 
 * @param active	If true the scroll when change focus is actived else is not active.
 */
public void setAutoScroll(boolean active) {
	autoScrollIsActive = active;

	if (autoScrollIsActive == true) {
		addFocusListenerToAllChildren(this, focusListenerToScroll);
	}
}

boolean isAutoScrollActivate() {
	return autoScrollIsActive;
}
	...
	if (parent != null) {
    	...
    	for (int i =0; i < childrens.length ; i++) {
    		if (Composite.class.isAssignableFrom(childrens[i].getClass())) {
    			addFocusListenerToAllChildren( (Composite)childrens[i], listener);
    		} else {
...
private Point getNeedMoving(Rectangle viewArea, Rectangle controlRect) {
	...
	FocusListener focusListenerToScroll = new FocusListener() {
		public void focusGained(FocusEvent e) {
			if (autoScrollIsActive == false ) {
				return;
			}
			// Get the focused child of the content
			Control focusedControl = (Control) e.widget;
			while (focusedControl.getParent() != content) {
			...
			// Get the position of the Control in the contents.
			Point controlPosition = getContent().toDisplay(focusedControl.getLocation());
			controlPosition = getContent().toControl(controlPosition);
		
			// Calculate the rectangle area of the FocusControl and the content
			...
			Rectangle viewArea				= new Rectangle(-content.getLocation().x, -content.getLocation().y, getClientArea().width, getClientArea().height);
		
			if( !viewArea.intersection(boundsFocusedControl).equals(boundsFocusedControl)) {
				Point movingNeed = getNeedMoving(viewArea, boundsFocusedControl);
				
				Point location = new Point(content.getLocation().x + movingNeed.x, content.getLocation().y + movingNeed.y);
				ScrollBar vBar = getVerticalBar ();
				if (vBar!=null) {
				...
				content.setLocation (location.x, location.y);
