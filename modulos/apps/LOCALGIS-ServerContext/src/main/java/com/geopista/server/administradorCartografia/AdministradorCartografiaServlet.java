/**
 * AdministradorCartografiaServlet.java
...
//          //Connection conn=CPoolDatabase.getConnection();
            //if (conn!=null){
            	...
            	if (getServletContext().getServletContextName().equals("Geopista")){
		            logger.info("Cargando dominios para el contexto:"+getServletContext().getServletContextName());
		            if(GeopistaDomains.hmMunicipios.size()==0)
		                reloadDomains();
		            logger.info("Dominios cargados para el contexto:"+getServletContext().getServletContextName() );
            	}
            	else{
            		 logger.info("No cargamos los dominios para el contexto:"+getServletContext().getServletContextName() );
            	}
            //}
            //else{
...
//                            iUpdatedFeature=geoConn.insertFeature(iMunicipio,sLayer,sLocale,aUpload[0],oos,sesion,bValidateData);
                            iUpdatedFeature=geoConn.insertFeature(iMunicipio,sLayer,sLocale,aUpload[0],oos,sesion,bValidateData, srid,params);
                            ...
                            iUpdatedFeature=geoConn.updateFeature(iMunicipio,sLayer,sLocale,aUpload[0],aUpload[0].isDeleted(),oos,sesion,bValidateData, srid,params);
                ...
                case Const.ACTION_MAP_UPLOAD:
                    IACMap acMap=(IACMap)params.get(Const.KEY_MAP_UPLOAD);       
                    ...
                    else if ((acMap.getId()==null||acMap.getId().equals("")) && acMap.getIdEntidad()!=0){
                ...
                case Const.ACTION_MAP_UPLOAD_MUNI:
                    acMap=(IACMap)params.get(Const.KEY_MAP_UPLOAD);
...
//                    int idMunicipio=(Integer)params.get(Const.KEY_MUNICIPIO);
                	if (validar(iEntidad)){
            			throw new Exception("Formato entidad incorrecto");
...
//                	idMunicipio=(Integer)params.get(Const.KEY_MUNICIPIO);
                ...
                case Const.ACTION_MUNICIPIO_GEOMETRY:
                    ...
                    break;
                case Const.ACTION_EXPORT_SHP_MAPAS:
                	String exportSHPIdEntidad=(String)params.get(Const.KEY_EXPORT_SHP_IDENTIDAD);
                	oos.writeObject(geoConn.getMapas(sLocale, exportSHPIdEntidad));
                  break;
                case Const.ACTION_EXPORT_SHP_LAYERS_MAPAS:
                	ExportMapBean exportMapBean=(ExportMapBean)params.get(Const.KEY_EXPORT_SHP_MAP_BEAN);
                	oos.writeObject(geoConn.getLayersMapa(sLocale, exportMapBean));
                  break;
                case Const.ACTION_LAYER_EXPORT_SHP:
                    ...
                    List<Municipio> listaMunicipios = geoConn.getMunicipiosEntidad(((String)params.get(Const.KEY_EXPORT_SHP_IDENTIDAD)));
                	sesion.setIdMunicipio(listaMunicipios.get(0).getId()+"");

                    geoConn.returnLayer(iEntidad,sLayer,sLocale,g,
                                                         (FilterNode)params.get(Const.KEY_FILTER),
                                                         oos,sesion,bValidateData,listaMunicipios, srid);
                    endMils=Calendar.getInstance().getTimeInMillis();
        			logger.info("Tiempo Total carga:"+sLayer+" :"+(endMils-startMils)+" mils"+" IdMunicipio:"+sesion.getIdMunicipio());
                    break;
					
					case Const.ACTION_WMS_TEMPLATE_LIST:
                	geoConn.returnWMSTemplateList(oos,Integer.parseInt(sesion.getIdMunicipio()));
                    break;
                    
                case Const.ACTION_WMS_TEMPLATE:
                	int id=Integer.parseInt((String)params.get(Const.KEY_WMS_TEMPLATE));
                	geoConn.returnWMSTemplate(oos,id);
                    break;
                    
                case Const.ACTION_WMS_TEMPLATE_MAPS:
                	int template_id=Integer.parseInt((String)params.get(Const.KEY_WMS_TEMPLATE));
                	geoConn.returnWMSTemplateMaps(oos,template_id);
                    break;
                    
                case Const.ACTION_DELETE_WMS_TEMPLATE:
                	int t_id=Integer.parseInt((String)params.get(Const.KEY_WMS_TEMPLATE));
                	geoConn.deleteWMSTemplate(oos, t_id, Integer.parseInt(sesion.getIdMunicipio()));
                    break;
                case Const.ACTION_UPDATE_WMS_TEMPLATE:
                	IWMSTemplate wmsTemplate=(IWMSTemplate)params.get(Const.KEY_WMS_TEMPLATE_COMPLETE_EDIT);
                	geoConn.updateWMSTemplate(oos,wmsTemplate);
                	break;
                case Const.ACTION_ADD_WMS_TEMPLATE:
                	IWMSTemplate wmsTemplateToAdd=(IWMSTemplate)params.get(Const.KEY_WMS_TEMPLATE_COMPLETE_ADD);
                	geoConn.addWMSTemplate(oos,wmsTemplateToAdd,Integer.parseInt(sesion.getIdMunicipio()));
                	break;            
    ...
    private boolean validar(int idEntidad){
		if (idEntidad<0)
			return false;
		else
			return true;
