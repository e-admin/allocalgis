...
public class CWorkflow {
    ...
    {
		try {
        	//Vector expedientes = COperacionesDatabase.getSearchedExpedientes(new Hashtable(),null, tiposLicencia);
            Vector expedientes = COperacionesDatabase.getSearchedExpedientesSilencioAdmin(new Hashtable(),null, tiposLicencia);
    		...
    		for (int i = 0; i < expedientes.size(); i++)
            {
				...
				if (fechaEfectiva.before(fechaActual)) {
					...
					COperacionesDatabase.insertEventoExpediente(expedienteLicencia, CWorkflowLiterales.EVENTOS_SILENCIO_ADMINISTRATIVO, false, true);
				}
			}
			return true;
		} catch (Exception ex) {
	...
	public static boolean checkEventosPlazosExpiracion(Vector tiposLicencia)
    {
	    try
        {
			Vector expedientes = COperacionesDatabase.getSearchedExpedientesExpirables(false,null,tiposLicencia);
			...
			for (int i = 0; i < expedientes.size(); i++)
            {
				...
				CWorkflowLine workflowLine = COperacionesDatabase.getWorkflowLine(expedienteLicencia.getEstado().getIdEstado(),
				...
				if (fechaEfectiva.before(fechaActual)) {
					...
					COperacionesDatabase.insertEventoExpediente(expedienteLicencia, CWorkflowLiterales.EVENTOS_PLAZO, false, false);
				}
			}
			return true;
		} catch (Exception ex) {
		...
		*/
		try {
		    ...
		    if (estado.getIdEstado()!=CConstantesComando.ESTADO_EJECUCION && expedienteLicencia.getSilencioAdministrativo()!=null &&expedienteLicencia.getSilencioAdministrativo().equals("1"))
            {
                ...
                COperacionesDatabase.actualizaResolucion(expedienteLicencia);
                ...
                if ((solicitud != null) &&  (solicitud.getTipoLicencia().getIdTipolicencia() == CConstantesComando.TIPO_OBRA_MAYOR) )
                {
                   if  (solicitud.getTipoObra().getIdTipoObra() == CConstantesComando.tipoObraDemolicion)
                        ...
                        COperacionesDatabase.actualizarSubparcelas(solicitud.getReferenciasCatastrales(), solicitud.getIdMunicipio(),CConstantesComando.SOLAR);
                  if  (solicitud.getTipoObra().getIdTipoObra() == CConstantesComando.tipoObraNuevaPlanta)
                        /** por silencio administrativo la resolucion es favorable */
                        COperacionesDatabase.actualizarSubparcelas(solicitud.getReferenciasCatastrales(), solicitud.getIdMunicipio(),CConstantesComando.NUEVA_PLANTA);
            ...
            COperacionesDatabase.updateEstadoExpediente(estado.getIdEstado(), expedienteLicencia.getNumExpediente(),
                    true, new Integer(CConstantesComando.TIPO_FINALIZACION_SILENCIO_ADMINISTRATIVO));
            
            Connection connection = null;
    		connection = CPoolDatabase.getConnection();
    		
    		COperacionesDatabase.insertHistoricoExpediente(expedienteLicencia, "workflow", true, connection);
			COperacionesDatabase.insertEventoExpediente(expedienteLicencia, new Integer(expedienteLicencia.getTipoLicenciaDescripcion()).intValue(),
			...
			if (expedienteLicencia.getVU().equals("1")){
				COperacionesDatabase.insertHitoTelematico(expedienteLicencia,new Integer(expedienteLicencia.getTipoLicenciaDescripcion()).intValue());
			}
	...
	public static boolean procesaPlazoExpiracion(CExpedienteLicencia expedienteLicencia, int idPlazoEstado)
    {
		try {
            ...
            */
			...
			CResultadoOperacion resultadoOperacion = COperacionesDatabase.getExpedienteLicencia(expedienteLicencia.getNumExpediente(), null, null, null);
			...
			expedienteLicencia.setEstado(estado);
            logger.info("expedienteLicencia.getEstado().getIdEstado(): "+expedienteLicencia.getEstado().getIdEstado());
            //COperacionesDatabase.updateEstadoExpediente(idPlazoEstado, expedienteLicencia.getNumExpediente(), true, null);
            COperacionesDatabase.updateEstadoExpediente(idPlazoEstado, expedienteLicencia.getNumExpediente(), false, null);
            
            Connection connection = null;
    		connection = CPoolDatabase.getConnection();
    		
    		COperacionesDatabase.insertHistoricoExpediente(expedienteLicencia, "workflow", true, connection);
            COperacionesDatabase.insertEventoExpediente(expedienteLicencia, new Integer(expedienteLicencia.getTipoLicenciaDescripcion()).intValue(), true, false, connection);
			...
			//**************************************
			if (expedienteLicencia.getVU().equals("1")){
				COperacionesDatabase.insertHitoTelematico(expedienteLicencia,solicitudLicencia.getTipoLicencia().getIdTipolicencia());
			}
    ...
    */
	...
	public static boolean checkPlazosExpiracion(Vector tiposLicencia) {
		...
		try {
			...
			Vector expedientes = COperacionesDatabase.getSearchedExpedientesExpirables(true,null,tiposLicencia);
			logger.debug("expedientes.size(): " + expedientes.size());
			...
			for (int i = 0; i < expedientes.size(); i++) {
				...
				CWorkflowLine workflowLine = COperacionesDatabase.getWorkflowLine(connection, expedienteLicencia.getEstado().getIdEstado(),
