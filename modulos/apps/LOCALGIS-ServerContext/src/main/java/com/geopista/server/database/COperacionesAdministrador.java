/**
 * COperacionesAdministrador.java
...
public class COperacionesAdministrador{
	...
	public final static int DIAS_AVISO = 15;
	public final static int PERIODICIDAD = 365;
          ...
          try
          {
                ...
                /* ResultSet rsSQL;
                 ...
                 if (iResult>0)
                 {
                     ...
                     for (int j=0;j<rol.getPermisos().gethPermisosSorted().size();j++){
         				//for (Enumeration e = listaPermisos.gethPermisos().elements(); e.hasMoreElements();) {
         				//Permiso permiso = (Permiso) e.nextElement();
         				Permiso auxPermiso = (Permiso) rol.getPermisos().gethPermisosSorted().get(j);
    ...
    public static CResultadoOperacion ejecutarDeleteRol(Rol rol, String sIdEntidad)
      {
          ...
          try
          {
                 ...
                 preparedStatement.setInt(1,Integer.parseInt(rol.getId()));
                 rsSQL=preparedStatement.executeQuery();
                 if (rsSQL.next())
                 {
                 ...
                 else
                 {
                      ...
                      preparedStatement.setInt(1,Integer.parseInt(rol.getId()));
                      preparedStatement.setInt(2,Integer.parseInt(sIdEntidad));
                      rsSQL=preparedStatement.executeQuery();
    ...
    public static CResultadoOperacion ejecutarNewRol(Rol rol, String sIdUser, String sIdEntidad)
    {
         ...
         try
         {
           ...
           /*  if (sIdUser != null)
             ...
             //Tipo sistema o creada (puede borrarse)
        ...
        try
        {
          ...
          //BUG. PG8.4. Casting de tipos
            ...
            if ((user.isPasswordCambiado() && mismoUsuario)){
            	...
            	preparedStatement.setInt(13,Integer.parseInt(user.getId()));
            }
            else
            	preparedStatement.setInt(12,Integer.parseInt(user.getId()));
            ...
            else
            {
                ...
                for (int j=0;j<user.getPermisos().gethPermisosSorted().size();j++){
     				//for (Enumeration e = listaPermisos.gethPermisos().elements(); e.hasMoreElements();) {
     				//Permiso permiso = (Permiso) e.nextElement();
     				Permiso auxPermiso = (Permiso) user.getPermisos().gethPermisosSorted().get(j);
    ...
    public static CResultadoOperacion getUserPassword(String userName)
    {
	    Connection connection = null;
	    PreparedStatement preparedStatement = null;
	    ResultSet rsSQL = null;
	    ...
	    try
	    {
	        ...
	        connection.setAutoCommit(false);
	        preparedStatement = connection.prepareStatement("select password from iuseruserhdr where upper(name)=upper(?)");
	        preparedStatement.setString(1,userName);
	        rsSQL=preparedStatement.executeQuery();
	        if (rsSQL.next())
	        {
	            resultado=new CResultadoOperacion(true, (new EncriptarPassword((String)rsSQL.getObject("password")).desencriptar()));
	        }
	        else{
	        	resultado=new CResultadoOperacion(false, "Error al recuperar password");
	        }
	    }catch(Exception e){
	    	  java.io.StringWriter sw=new java.io.StringWriter();
              java.io.PrintWriter pw=new java.io.PrintWriter(sw);
       ...
       public static CResultadoOperacion ejecutarNewDomainNode(DomainNode nodo)
       {
            ...
            try
            {
                ...
                logger.info("Creando nuevo domainnode: "+preparedStatement.toString());
                preparedStatement.execute();
                
                LCGIII_GeopistaUtil.generarSQL(preparedStatement);

                connection.commit();
                logger.info("Nuevo Domain con id: "+lIdDomainNode+" creado con exito");
                ...
                try
                {
                    ...
                    do
                    {
                        ...
                        try
                        {
                            ...
                            //TODO REVISAR. La asignacion de dominios 
                            if (nodo.getIdDomain()!=null){
                            	logger.info("El nodo dispone ya de un identificador de dominio:"+nodo.getIdDomain());
                            }
                            ...
                            preparedStatement.setLong(1,Long.parseLong(nodo.getIdDomain()));
                            ...
                            logger.info("Creando nuevo dominio: "+preparedStatement.toString());
                            preparedStatement.execute();
                            ...
                            try{preparedStatement.close();} catch (Exception ex2) {}
                            insertado=true;
                            logger.debug("New Domain con id: "+nodo.getIdDomain()+" creado con exito");
                            resultado=new CResultadoOperacion(true, new Long(nodo.getIdDomain()).toString());
                        }catch(Exception e)
                        {
        ...
        public static CResultadoOperacion ejecutarDeleteDomainNode(DomainNode nodo)
        {
              ...
              try
              {
                     ...
                     try
                     {
                        ...
                        preparedStatement.setInt(1, Integer.parseInt(nodo.getIdNode()));
                        rsSQL=preparedStatement.executeQuery();
                        while (rsSQL.next())
                        {
        ...
        public static CResultadoOperacion ejecutarDeleteDomain(Domain nodo)
        {
              ...
              try
              {
                     ...
                     preparedStatement.setInt(1,Integer.parseInt(nodo.getIdDomain()));
                     preparedStatement.execute();
    ...
    public static CResultadoOperacion ejecutarDeleteEntidad(Entidad entidad) {
        ...
        try {
            ...
            preparedStatement.setInt(1, Integer.parseInt(entidad.getId()));
            rsSQL=preparedStatement.executeQuery();
            if (rsSQL.next()) {
                try {
                    rsSQL.close();
    ...
    public static CResultadoOperacion ejecutarUpdateEntidad(Entidad entidad) {
        ...
        try {
            ...
            preparedStatement.setInt(7, Integer.parseInt(entidad.getId()));
    ...
    public static CResultadoOperacion ejecutarNewMunicipio(Municipio municipio) {
        Connection connection = null;
        PreparedStatement preparedStatement = null;
        ResultSet rsSQL = null;
        CResultadoOperacion resultado = null;
        try {
            logger.debug("Inicio añadir municipio: " + municipio.getNombre());
            connection = CPoolDatabase.getConnection();
            if (connection == null) {
                logger.warn("No se puede obtener la conexión");
                return new CResultadoOperacion(false, "No se puede obtener la conexión");
            ...
            connection.setAutoCommit(false);
            preparedStatement = connection
                    .prepareStatement("insert into municipios (id, id_provincia, id_catastro, id_ine,nombreoficial,srid,srid_proyeccion) values (?,?,?,?,?,?,?)");
            preparedStatement.setLong(1, Long.parseLong(municipio.getId()));
            
            if (municipio.getId().length()==5){
                preparedStatement.setString(2, municipio.getId().substring(0,2));
                preparedStatement.setString(3, municipio.getId().substring(2,5));
                preparedStatement.setString(4, municipio.getId().substring(2,5));
            }
            else{
                preparedStatement.setString(2, municipio.getId().substring(0,1));
                preparedStatement.setString(3, municipio.getId().substring(1,4));
                preparedStatement.setString(4, municipio.getId().substring(1,4));            	
            }
            preparedStatement.setString(5, municipio.getNombre());
            preparedStatement.setString(6, municipio.getSrid());
            preparedStatement.setInt(7, Integer.parseInt(municipio.getSrid()));
            preparedStatement.execute();
            connection.commit();
            logger.debug("Municipio con id: " + municipio.getId() + " creado con exito");
            resultado = new CResultadoOperacion(true, new Long(municipio.getId()).toString());

        } catch (Exception e) {
            java.io.StringWriter sw = new java.io.StringWriter();
            java.io.PrintWriter pw = new java.io.PrintWriter(sw);
            e.printStackTrace(pw);
            logger.error("ERROR al CREAR el municipio " + sw.toString());
            resultado = new CResultadoOperacion(false, e.getMessage());
            try {
                connection.rollback();
            } catch (Exception ex2) {
