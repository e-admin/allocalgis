...
public class JDBCLoginModule implements LoginModule {
	...
	public boolean login() throws LoginException {
	   if (callbackHandler == null)
		...
		try {
			...
			//TRUCO para que pase si viene por openam
			if (webCredential.toString().equalsIgnoreCase("_#openam#_"))
				logger.info("Autenticacion positiva por OpenAM. Password correcta. Usuario:"+webUserName);
			else{
				authState = credential.check(webCredential);
	            if (!authState)
	                logger.warn("Autenticacion negativa. Password incorrecta. Usuario:"+webUserName);
	            else
	                logger.info("Autenticacion positiva. Password correcta. Usuario:"+webUserName);
			}
	...
	public void initialize(Subject subject,
		...
		try {
			...
			//userQuery += " and "+dbUserTable+"."+dbUserTableUserField+"="+SSOConstants.SSO_NAME+" and "+dbUserTable+"."+dbUserTableCredentialField+"="+SSOConstants.SSO_PASSWORD;
...
//			Code.debug("rolesQuery = " + rolesQuery);
	...
	public void loadUser(String userName)
		...
		if(!userName.equals(SSOConstants.SSO_USERNAME)){
	    	...
	    	if (resultado.getVector().size()>0) {
	            ...
	            try
	            {
			    	dbCredential = (new EncriptarPassword((String) auxVector.get(0))).desencriptar();
	            }catch(Exception e)
	            {
	        ...
	        vParametros.add(0,Integer.parseInt(sIdField));
	        vParametros.add(1,Integer.parseInt(sIdField));
	        vParametros.add(2,Integer.parseInt(sIdField));
	
			resultado = COperacionesControl.ejecutarQuery(rolesQuery, vParametros);
			if (!resultado.getResultado() || (resultado.getVector() == null))
	        {
	            logger.info("El usuario "+userName+" no tiene ningún permiso. "+rolesQuery);
	...
	//		roleGroup = new JAASGroup(JAASGroup.ROLES);
			...
			dbCredential = SSOConstants.SSO_PASSWORD;
			roleGroup = new LocalgisJAASGroup(LocalgisJAASGroup.ROLES);
			roleGroup.addMember(new JAASRole("Geopista.Common.Actions"));
			roleGroup.addMember(new JAASRole("Geopista.Administracion.Login"));
		}
	}
