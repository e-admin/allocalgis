				...
				new Thread(new Runnable() {
					public void run() {
						try {
							...
							// NUEVO
							if (!AlfrescoManagerUtils.isAlfrescoClientActive() && !AlfrescoManagerUtils.isAlfrescoUuid(auxDocumento.getId(), auxDocumento.getIdMunicipio()))
								 if (auxDocumento.getFileName().startsWith("http"))
		                            	auxDocumento.setContent(UtilsResource.getBytesFromResource(auxDocumento));
								 else
		...
		try {
			...
			String nombreFicheroTemporal=null;
			 if (auxDocumento.getFileName().startsWith("http"))
				 ...
				 nombreFicheroTemporal = System.getProperty("java.io.tmpdir")+ auxDocumento.getFileName();
			// NUEVO
			if (!AlfrescoManagerUtils.isAlfrescoClientActive()) {
		...
		progressDialog.addComponentListener(new ComponentAdapter() {
			public void componentShown(ComponentEvent e) {
				new Thread(new Runnable() {
					public void run() {
						try {
							if (!AlfrescoManagerUtils.isAlfrescoClientActive() && !AlfrescoManagerUtils.isAlfrescoUuid(document.getId(), document.getIdMunicipio())) {
