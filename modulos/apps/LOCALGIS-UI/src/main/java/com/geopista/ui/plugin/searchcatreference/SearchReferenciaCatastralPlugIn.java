/**
 * SearchReferenciaCatastralPlugIn.java
  ...
  public ImageIcon getIcon() {
    ...
    public void run(TaskMonitor monitor, PlugInContext context) throws Exception
    {
        ...
        if(searchFeatures.size()==0)
        {
            ...
            IGeopistaLayer geopistaLayer = acClient.searchByAttribute((GeopistaMap) context.getTask(),
                    (GeopistaLayer) currentLayer, UserPreferenceStore.getUserPreference(
                            UserPreferenceConstants.DEFAULT_LOCALE_KEY, "es_ES", true), fn, REFERENCIA_CATASTRAL, 
        ...
        if(searchFeatures.size()==1)
        {
            ...
            if (context.getWorkbenchContext().getIWorkbench().getGuiComponent()  instanceof GeopistaEditor){
            	 ...
            	 ((GeopistaEditor) context.getWorkbenchContext().getIWorkbench().
            			 getGuiComponent()).fireGeopistaSelectionChanged((AbstractSelection) context.getLayerViewPanel().getSelectionManager().getFeatureSelection());
            }            	    
