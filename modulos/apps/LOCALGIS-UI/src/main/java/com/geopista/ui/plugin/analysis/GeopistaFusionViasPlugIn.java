...
public class GeopistaFusionViasPlugIn extends AbstractPlugIn implements ThreadedPlugIn
{
     ...
     * @return
     */
    public static Hashtable fusionVias(Layer capaTramosVias, Layer capaVias, TaskMonitor monitor, Hashtable targetHash, boolean isGraphic){
    	
		GeopistaLayer capaViaDatabase = (GeopistaLayer) capaVias;
		GeopistaServerDataSource geopistaServerDataSource = (GeopistaServerDataSource) capaViaDatabase.getDataSourceQuery().getDataSource();
    	
		//si hay Vias de otras importaciones las eliminamos ya que tenemos todos los tramos de vias en la capa.
		if(capaViaDatabase.getFeatureCollectionWrapper().getFeatures().size()>0){
			if(isGraphic)
				monitor.report(aplicacion.getI18nString("GeopistaImportarToponimosPanel.BorrandoRegistros"));
			else
				System.out.println(aplicacion.getI18nString("GeopistaImportarToponimosPanel.BorrandoRegistros"));
			Iterator lastFeaturesIter = capaViaDatabase.getFeatureCollectionWrapper().getFeatures().iterator();
			...
			try{
				geopistaServerDataSource.getConnection().executeUpdate(capaViaDatabase.getDataSourceQuery().getQuery(), capaViaDatabase.getFeatureCollectionWrapper().getUltimateWrappee(), monitor);
			} catch (Exception e){
				// como no podemos lanzar Exception la paramos aqui.
				logger.error("execute()", e);
			}
			
			capaViaDatabase.getFeatureCollectionWrapper().removeAll(capaViaDatabase.getFeatureCollectionWrapper().getFeatures());
		}
    	...
    	if (capaTramosVias.getFeatureCollectionWrapper().getFeatureSchema() instanceof GeopistaSchema && capaVias.getFeatureCollectionWrapper().getFeatureSchema() instanceof GeopistaSchema){
              // lista de features de la capa tramos vias
              List sourceFeatures = capaTramosVias.getFeatureCollectionWrapper().getFeatures();
              // sacamos el esquema de la capa origen
              GeopistaSchema sourceFeatureSchema = (GeopistaSchema) capaTramosVias.getFeatureCollectionWrapper().getFeatureSchema();
              // sacamos el esquema de la capa vias
              GeopistaSchema targetSchema = (GeopistaSchema) capaVias.getFeatureCollectionWrapper().getFeatureSchema();

              // buscamos el nombre de los attributos que vamos a utilizar
              String numviaAttributeName = sourceFeatureSchema.getAttributeByColumn("numvia");
              String codigoCatastroAttributeName = targetSchema.getAttributeByColumn("codigocatastro");
              String nombreCatastroAttributeName = targetSchema.getAttributeByColumn("nombrecatastro");
              String tipoViaNormalizadoCatastroAttributeName = targetSchema.getAttributeByColumn("tipovianormalizadocatastro");
              String tipoViaIneAttributeName = targetSchema.getAttributeByColumn("tipoviaine");

              // comenzamos la iteracion por cada uno de los tramos de vias
              Iterator sourceFeaturesIter = sourceFeatures.iterator();
              int currentFusion = 1;
              if(isGraphic)
            	  monitor.report(aplicacion.getI18nString("ProcesandoTramosVia"));
              else
            	  logger.info(aplicacion.getI18nString("ProcesandoTramosVia"));
              while (sourceFeaturesIter.hasNext()){
            	  if(isGraphic)
            		  monitor.report(currentFusion++, sourceFeatures.size(), aplicacion.getI18nString("GeopistaFusionVias"));
            	  else
            		  logger.info(aplicacion.getI18nString("GeopistaFusionVias")+" "+(currentFusion++)+"/"+sourceFeatures.size());
                  Object currentSourceFeature = sourceFeaturesIter.next();

                  if (currentSourceFeature instanceof GeopistaFeature){
                      GeopistaFeature currentSourceGeopistaFeature = (GeopistaFeature) currentSourceFeature;
                      String currentNumViaValue = currentSourceGeopistaFeature.getString(numviaAttributeName);
                      try{
                          // comprobamos con esta conversión si el Numero de vía es realmente un entero y si no nos saltamos la iteración
                          Integer.parseInt(currentNumViaValue);
                          // buscamos el numero de via en el hashtable y si no existe lo insertamos, si ya existe realizamos la fusion de vias.                         
                          Hashtable featureData = (Hashtable) targetHash.get(currentNumViaValue);
                          if (featureData == null){
                              	ArrayList geometryList = new ArrayList();
                              	featureData = new Hashtable(); 
                              	Geometry sourceGeom1 = currentSourceGeopistaFeature.getGeometry();
                          ...
                          // si encontramos algún error pasamos a la siguienet feature
                          logger.error("run(TaskMonitor, PlugInContext)", e);
                      }
                  }
              }
         ...
         /**     
  			// We need to add the vias insertadas en la base de datos gracias a Carvia
  			try{

  				Connection conn = aplicacion.getConnection();

  				int idMunicipio = Integer.parseInt(aplicacion.getString("localgis.DefaultCityId"));

  				std = conn.prepareStatement("selectViasSinRepresentacion");
         ...
         */     
              ...
              while(targetHashIterator.hasNext()){
                  ...
                  try{
	                  ...
	                  if (currentGeometryList!=null){
	                	  calle=gf.createMultiLineString((LineString[]) currentGeometryList.toArray(calleArray));
	                  }
	                  
	                  GeopistaFeature targetFeature = new GeopistaFeature(targetSchema);
	
					//copiamos los atributos de una capa a otra; numvia pasa a Codigo Catastro	
	                  targetFeature.setAttribute(codigoCatastroAttributeName,currentIdVia);	
	                  // Denominacion pasa a Nombre Catastro
	                  if(currentFeatureData.get("denominacion")!=null && currentFeatureData.get("denominacion").toString()!= null && currentFeatureData.get("denominacion").toString().indexOf(" ")==2){
	                	  targetFeature.setAttribute(nombreCatastroAttributeName,currentFeatureData.get("denominacion").toString().substring(currentFeatureData.get("denominacion").toString().indexOf(" ")).trim());
	                	  targetFeature.setAttribute(tipoViaNormalizadoCatastroAttributeName,currentFeatureData.get("denominacion").toString().substring(0,currentFeatureData.get("denominacion").toString().indexOf(" ")).trim());	
	                  }else{
	                	  targetFeature.setAttribute(nombreCatastroAttributeName,currentFeatureData.get("denominacion"));	
	                	  // En tipo via normalizado catastro introducimos el valor NE (No Especificado) hasta que sepamos el valor real
	                	  targetFeature.setAttribute(tipoViaNormalizadoCatastroAttributeName,NO_ESPECIFICADO);	
	                  }
	                  if (currentFeatureData.get("tipovianormalizadocatastro")!=null){
	                	  targetFeature.setAttribute(tipoViaNormalizadoCatastroAttributeName, currentFeatureData.get("tipovianormalizadocatastro"));
	                  }
	                  // En tipo via INEdo catastro introducimos el valor NE (No Especificado) hasta que sepamos el valor real
	                  targetFeature.setAttribute(tipoViaIneAttributeName,NO_ESPECIFICADO);	
	                  // La geometria es la misma
	                  if (calle!=null){
	                	  targetFeature.setGeometry(calle);
	                  }
    ...
    public static void executeFusion(Layer capaTramosVias, Layer capaVias, TaskMonitor globalMonitor, Hashtable targetHash, boolean isGraphic){
        // sacamos el esquema de la capa origen
        GeopistaSchema sourceFeatureSchema = (GeopistaSchema) capaTramosVias.getFeatureCollectionWrapper().getFeatureSchema();
        // sacamos el esquema de la capa vias
        GeopistaSchema targetSchema = (GeopistaSchema) capaVias.getFeatureCollectionWrapper().getFeatureSchema();        
        final String numViaTramosViaAttributeName = sourceFeatureSchema.getAttributeByColumn("numvia");
        final String idViaTramosViaAttributeName = sourceFeatureSchema.getAttributeByColumn("id_via");
        final String idRoutesAttributeName = targetSchema.getAttributeByColumn("id");
        
    	  Hashtable localTargetHash = new Hashtable(targetHash);
          int currentPosition = 0;
          Collection insertKeysFeatures = localTargetHash.keySet();
          Iterator insertKeysFeaturesIter = insertKeysFeatures.iterator();
          boolean isFiringEventsCapaVias = capaVias.getLayerManager().isFiringEvents();
          List arlNotAddedFeatures = new ArrayList();
          capaVias.getLayerManager().setFiringEvents(false);
          try{
        	  while (insertKeysFeaturesIter.hasNext()){
        		  if (isGraphic && globalMonitor != null){
        			  globalMonitor.report(currentPosition++,insertKeysFeatures.size(),aplicacion.getI18nString("GeopistaMostrarCallejeroPanel.CreandoVia"));
        		  }else if(!isGraphic){
        			  logger.info(aplicacion.getI18nString("GeopistaMostrarCallejeroPanel.CreandoVia")+" "+(++currentPosition)+"/"+insertKeysFeatures.size());
        		  }
                  Object currentKey = insertKeysFeaturesIter.next();
                  GeopistaFeature currentInsertFeature = (GeopistaFeature) localTargetHash.get(currentKey);
                  capaVias.getFeatureCollectionWrapper().add(currentInsertFeature);

        	  }
        	  
          } finally{
        	  capaVias.getLayerManager().setFiringEvents(isFiringEventsCapaVias);
          }
          GeopistaServerDataSource geopistaServerDataSource = (GeopistaServerDataSource) capaTramosVias.getDataSourceQuery().getDataSource();

          try{ 
        	  geopistaServerDataSource.getConnection().executeUpdate(capaVias.getDataSourceQuery().getQuery(),capaVias.getFeatureCollectionWrapper(), globalMonitor);
          } catch (Exception e){
                  // como no podemos lanzar Exception la paramos aqui.
        	  logger.error("execute()", e);
          }
   /*       capaVias.getLayerManager().setFiringEvents(false);
          // Now that we have added the Vias with Geometry, we need to add the ones without any geometry to the layer
          for (int i=0; i<arlNotAddedFeatures.size(); i++){
        	  capaVias.getFeatureCollectionWrapper().add((GeopistaFeature) arlNotAddedFeatures.get(i));
          }
          capaVias.getLayerManager().setFiringEvents(true);
    */      

          if (isGraphic && globalMonitor != null){
        	  globalMonitor.report(currentPosition++,insertKeysFeatures.size(),aplicacion.getI18nString("GeopistaMostrarCallejeroPanel.ProcesandoVias"));
          }else if(!isGraphic){
        	  	logger.info(aplicacion.getI18nString("GeopistaMostrarCallejeroPanel.ProcesandoVias")+" "+(currentPosition++)+"/"+insertKeysFeatures.size());
          }
          ...
          while (insertKeysFeaturesIter.hasNext()){
        	  Object currentKey = insertKeysFeaturesIter.next();
        	  GeopistaFeature currentInsertFeature = (GeopistaFeature) localTargetHash.get(currentKey);
        	  // buscamos todos los tramos con el mismo numero de via para insertar en el attributo id de via el valor devuelto por el administrador de cartografia
              Collection sourceFeaturesWithEqualsIdVia = GeopistaUtil.searchByAttribute(capaTramosVias,numViaTramosViaAttributeName, currentKey.toString());
              Iterator sourceFeaturesWithEqualsIdViaIter = sourceFeaturesWithEqualsIdVia.iterator();
              boolean isFiringEvents = capaTramosVias.getLayerManager().isFiringEvents();
              capaTramosVias.getLayerManager().setFiringEvents(false);
              try{
            	  while (sourceFeaturesWithEqualsIdViaIter.hasNext()){
                          Object currentSectionsRoutesFeatureObject = sourceFeaturesWithEqualsIdViaIter.next();
                          if (currentSectionsRoutesFeatureObject instanceof GeopistaFeature){
                              GeopistaFeature currentSectionsRoutesFeature = (GeopistaFeature) currentSectionsRoutesFeatureObject;
                              currentSectionsRoutesFeature.setAttribute(idViaTramosViaAttributeName,currentInsertFeature.getAttribute(idRoutesAttributeName));
                              currentSectionsRoutesFeature.setDirty(true);
                          }
            	  }
              } finally{
          ...
          try{
        	  geopistaServerDataSource.getConnection().executeUpdate(capaTramosVias.getDataSourceQuery().getQuery(),capaTramosVias.getFeatureCollectionWrapper(),globalMonitor);
          } catch (Exception e){
                  // como no podemos lanzar Exception la paramos aqui.
                  logger.error("execute()", e);
          }
