...
public class SynchronizeAutoCADPlugIn extends ThreadedBasePlugIn
{
    		...
    		while (sourceLayersIterator.hasNext())
    		{
    			...
    			if (currenLayer instanceof GeopistaLayer)
    			{
    				...
    				try{
    					...
    					Integer srid_destino=Integer.parseInt((String)AppContext.getApplicationContext().getBlackboard().get(Const.KEY_SRID),10);

    					GeopistaLayer systemLayer= admCarClient.loadLayer(null, currentGeopistaLayer.getSystemId(), 
    							"es_ES", null, FilterLeaf.equal("1", new Integer(1)), true, srid_destino, null);
    ...
    {   
    	...
    	final String dirBase = UserPreferenceStore.getUserPreference(
