 ...
 *
 */
public class FileGeoreferencePanel06 extends javax.swing.JPanel implements WizardPanel {
    ...
    private ViasCollector localizado = null;  //  @jve:decl-index=0:
    private GeopistaLayer capa = null;  //  @jve:decl-index=0:
    ...
    public FileGeoreferencePanel06(String id, String nextId, PlugInContext context2) {
	...
	public void enteredFromLeft(Map dataMap) {
		...
		if((String)wizardContext.getData("completar") == null){
			localizado = (ViasCollector)wizardContext.getData("localizado");
			...
			if (capa.isLocal()){
				...
				//System.out.println("Indice de la capa "+wizardContext.getData("indiceCapa"));
				try {
				...
				while(recorreDatos2.hasNext()){// recorre todos los datos
					...
					while(recorreFeatures.hasNext()){//recorre todas las features
						...
						valueFichero = ((lineaDatos.get(campoFichero) != null)? lineaDatos.get(campoFichero).toString() : "");
						valueCapa = ((feature.getAttribute(campoCapa) != null)? feature.getAttribute(campoCapa).toString(): "");
						if(valueFichero.equals(valueCapa)){// Si coincide el valor con el fichero
							encontrado = true;
							break;
				...
				while(recorreDatos2.hasNext()){// recorre todos los datos
					...
					HashMap listFeatures=new HashMap();
					while(recorreFeatures.hasNext() && lineaDatos.get(campoFichero) != null){//recorre todas las features
						GeopistaFeature feature = (GeopistaFeature)recorreFeatures.next();
						if (lineaDatos.get(campoFichero)!=null && feature.getAttribute(campoCapa)!=null){
						if(lineaDatos.get(campoFichero).equals(feature.getAttribute(campoCapa).toString())){// Si coincide el valor con el fichero
							...
							while(listaCamposFichero.hasMoreElements()){
								...
								if(feature.getSchema().hasAttribute(valor)){
									listFeatures.put(campoCapa,feature);
					...
					//Una vez que las hemos modificado actualizamos atributos.
					capa.getLayerManager().setFiringEvents(true);
					Iterator it=listFeatures.keySet().iterator();
					while (it.hasNext()){
						String clave=(String)it.next();
						GeopistaFeature feature=(GeopistaFeature)listFeatures.get(clave);
						feature.forceDirty(true);
					}
    ...
    public void addLayerAttribute() throws SecurityException, NoSuchMethodException, IllegalArgumentException, InstantiationException, IllegalAccessException, InvocationTargetException
    {
        ...
        HashMap typeData = null;
        validator = new SchemaValidator(null);
        
        if(capa.isLocal() && localizado != null && localizado.getDataType()!=null){    
        	typeData = localizado.getDataType(); 
        }
        ...
        if(capa.isLocal() && typeData != null){
        	 Set valores = typeData.keySet();
        ...
        if(capa.isLocal() && typeData != null){//SOLO CUANDO LA CAPA ES LOCAL###############################################################
            ...
            Iterator recorreLocalizado = ((ArrayList)localizado.getListaVias()).iterator();
            while(recorreLocalizado.hasNext()){
                GeopistaFeature fNew=null;
                Via datosCoincidencia = (Via)recorreLocalizado.next();
                Hashtable dataLine = datosCoincidencia.getData();
                fNew = new GeopistaFeature(newSchema);
                Set values = typeData.keySet();
                Iterator listaCamposDatos = values.iterator();
                while(listaCamposDatos.hasNext()){
                    	String key= (String)listaCamposDatos.next();
                    	if(typeData.get(key) == null)
                    		typeData.put(key, AttributeType.STRING);
                        AttributeType attribute=(AttributeType)typeData.get(key);
                    ...
                    fNew.setGeometry(datosCoincidencia.getFirstGeometry());
                    CoordinateSystem crsDestino = layerManager.getCoordinateSystem();
                    CoordinateSystem crsOrigen = PredefinedCoordinateSystems.getCoordinateSystem(fNew.getGeometry().getSRID());
                    Reprojector.instance().reproject(fNew.getGeometry(), crsOrigen, crsDestino);
                    fNew.setLayer(capa);
                    ArrayList errores = validateFeature(fNew,capa);
                    
                    if (errores.size() > 0){
                    	String nuevoError = I18N.get("Georreferenciacion","georeference.panel06.insertionError")+" ==> "+datosCoincidencia.getNombre()+"\n";
            ...
            while(recorreLocalizado.hasNext()){
                ...
                fNew.setGeometry(datosCoincidencia.getFirstGeometry());
                CoordinateSystem crsDestino = capa.getLayerManager().getCoordinateSystem();
                CoordinateSystem crsOrigen = PredefinedCoordinateSystems.getCoordinateSystem(fNew.getGeometry().getSRID());
                Reprojector.instance().reproject(fNew.getGeometry(), crsOrigen, crsDestino);
                fNew.setLayer(capa);
                ...
                if (errores.size() > 0){
                	String nuevoError = "";
                	nuevoError = I18N.get("Georreferenciacion","georeference.panel06.insertionError")+" ==> "+datosCoincidencia.getNombre();
                	this.jTextArea.append(nuevoError);
                	Iterator recorreErrores = errores.iterator();
                	while (recorreErrores.hasNext()){
                		nuevoError = "\n\t";
    ...
    private void sesgaAtributos(GeopistaSchema newSchema, Hashtable dataLine) {
	    ...
	    while(listaCampos.hasMoreElements()){
	    	...
	    	while (recorreListaAtributos.hasNext()){
	    		nombreAtributo = (String)((com.geopista.feature.Attribute)recorreListaAtributos.next()).getName();
