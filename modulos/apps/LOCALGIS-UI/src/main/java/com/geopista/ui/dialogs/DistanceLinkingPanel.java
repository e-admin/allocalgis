...
public class DistanceLinkingPanel extends JPanel
{
	...
	private static final String REPORTING_LAYER_NAME =  aplicacion.getI18nString("lyrPoliceStreetLayer");
	private static final String ATTRIBUTE_ID_ON_TARGET = DistanceLinkingPanel.class.getName()+"ATTRIBUTE_ID_ON_TARGET";
	private static final String BUFFERLAYER_NAME =  aplicacion.getI18nString("lyrBufferLayer");
	private static final String ATTRIBUTE_ID_ON_SOURCE = DistanceLinkingPanel.class.getName()+"ATTRIBUTE_ID_ON_SOURCE";
	private static final String ERRORLAYER_NAME =  aplicacion.getI18nString("lyrErrorLayer");
    ...
    protected static void applyArrowStyles(Layer layer) {
...
//        }
    ...
    protected static void applyErrorStyles(Layer layer) {
    ...
    int cont = 0;
    	...
    	//Para cargar SHP es preciso definir un sridDefecto y un sridInicial. No 
    	//se utilizan para probar con ficheros SHP.
    	String sridDefecto="25830";
    	String sridInicial="25830";
    	try{
	    	...
	    	sridInicial = geoConn.getSRIDDefecto(false, Integer.parseInt(iSesion.getIdEntidad()));
	    	//String sridDefecto = geoConn.getSRIDDefecto(true, Integer.parseInt(iSesion.getIdEntidad()));
	    	sridDefecto = geoConn.getSRIDDefecto(false, Integer.parseInt(iSesion.getIdEntidad()));
		...
		FeatureSchema featureSchema1=capaNumerosPolicia.getFeatureCollectionWrapper().getFeatureSchema();
		
		GeopistaSchema schema=null;
		if (featureSchema1 instanceof GeopistaSchema)
			schema = (GeopistaSchema) capaNumerosPolicia.getFeatureCollectionWrapper().getFeatureSchema();
    //    String attrMuni = schema.getAttributeByColumn("id_municipio");
		...
		String attrMunicipio=null;
		if (schema!=null)
			schema.getAttributeByColumn("id_municipio");
	...
	//				    }*/
    			...
    			listFeatures.add(targetFeature);
    		}
    	}
...
//			}
    		...
    		if (featureSchema instanceof GeopistaSchema){
    			...
    			//Si el atributo es nulo posiblemente sea un fichero shapefile, en ese caso todo lo que
    			//va en el fichero se considera que pertenece al municipio.
    			if (sourceFeature.getAttribute(attributeName)!=null){
    		...
    		// origen
    	{
    		...
    		GeopistaFeature sourceFeature = (GeopistaFeature) policeFeaturesIter.next();
    		...
    		if (featSchema instanceof GeopistaSchema){
    			...
    			attributeName = geopistaSchema.getAttributeByColumn("id_municipio");
    			...
    			continuar=true;
		   	 	...
		   	 	if (continuar || sMunicipio.equals(sourceFeature.getAttribute(attributeName).toString())){
		    		// Obtiene las features target candidatas
		    		ArrayList targetCandidateFeatures = (ArrayList) targetToSourceLinkingMap.get(sourceFeature);
		    		...
		    		while (targetCandidateIter.hasNext()) // Itera calles
		    			...
		    			{
		    			candidateTargetFeature=(GeopistaFeature) targetCandidateIter.next();
		    			if (monitor.isCancelRequested()) return;
		    			...
		    			Feature targetFeature = (Feature) finalRelation.get(sourceFeature);
		    			...
		    			coordenadas[1] = sourceFeature.getGeometry().getCoordinate();
		    			coordenadas[0] = geometryMidPoint(targetFeature.getGeometry(), sourceFeature.getGeometry());
...
//	             errorLayer.getLabelStyle().setEnabled(true);
    ...
    private void correctionConnect(ActionEvent e)
    {
      ...
      reportLayer.getFeatureCollectionWrapper().removeAll(reportLayer.getFeatureCollectionWrapper().getFeatures());
      errorLayer.getFeatureCollectionWrapper().removeAll(errorLayer.getFeatureCollectionWrapper().getFeatures());
      ...
      while(featuresSourceIter.hasNext())
      {
          ...
          Coordinate[] coordenadas = new Coordinate[2];
          coordenadas[0] = geometryMidPoint(featureStreet.getGeometry(),actualFeaturesSource.getGeometry());
          GeometryFactory geometryFactory = new GeometryFactory();
          coordenadas[1] = actualFeaturesSource.getGeometry().getCoordinate();
          LineString arrow = geometryFactory.createLineString(coordenadas);
    ...
    private static Coordinate geometryMidPoint(Geometry workGeometry, Geometry policePoint)
    {
	...
	public static Object getFromBlackboard(String key, PlugInContext localContext)
	{
		try
		{
				ILayerViewPanel lyrView = localContext.getWorkbenchGuiComponent()
	...
	public static void putToBlackboard(String key, Object value, PlugInContext localContext)
	{
		try
		{
				Blackboard bk = localContext.getWorkbenchGuiComponent()
...
private Layer getTargetLayer()
{
	...
	private static ArrayList nearestFeaturesBrute2(Feature sourceFeature, FeatureCollection collectionWorkFeatures)
	{
		Geometry sourceFeatureGeometry = sourceFeature.getGeometry();
	...
	public static void resetAll(PlugInContext localContext, Blackboard bk)
	{
		...
		putToBlackboard(FINAL_LINKING_ARRAY,null, localContext);
	}
...
//	{return null;}
	...
	protected void setFields(TaskMonitor monitor, ActionEvent e)
	{
		...
		try
		{
			...
			while (finalRelations.hasNext())
			{
				...
				if (expresion==null)
					logger.error("La expresion de asignacion es nula verifique que ha seleccionado el campo en ambos desplegables");
			 ...
			 if(geopistaServerDataSource!=null && geopistaServerDataSource instanceof GeopistaServerDataSource){
			     ...
			     if (geopistaServerDataSource.getProperties() != null) {
			         ...
			         geopistaServerDataSource.getProperties().put("selectedImportEPSG", originalEPSG);
			     } else {
			         Map properties = new HashMap();
			         properties.put("srid_destino", Integer.valueOf(sridDefecto));
			         properties.put("selectedImportEPSG",originalEPSG);
	...
	public static void connect(Layer capaNumerosPolicia, Layer capaCallejero, PlugInContext localContext, FeatureSchema initialFeatureSchema, FeatureCollection initialErrorFeature, FeatureCollection initialReportDataset, TaskMonitor monitor, HashMap arrowFeatureRelation, Blackboard bk, boolean isGraphic)
    {     
        if (capaNumerosPolicia == null){
        	if(isGraphic)
        		JOptionPane.showMessageDialog((Component) localContext.getWorkbenchGuiComponent(),aplicacion.getI18nString("ConnectPoliceStreetPlugIn.CapaPoliciasNoCargada"));
        	else
        		System.out.println(aplicacion.getI18nString("ConnectPoliceStreetPlugIn.CapaPoliciasNoCargada"));
        	return;
        }
        if (capaCallejero == null){
        	if(isGraphic)
        		...
        		System.out.println(aplicacion.getI18nString("ConnectPoliceStreetPlugIn.CapaViasNoCargada"));
        	return;
        }      
        ILayerViewPanel layerViewPanel=null;
        if(isGraphic)
        	layerViewPanel= localContext.getLayerViewPanel();      
        Layer errorLayer = null;
        Layer reportLayer = null;   
        ...
        // Crea las capas de informes
        // =======================================
	        reportLayer = localContext.getWorkbenchGuiComponent().getActiveTaskComponent().getLayerViewPanel().getLayerManager().getLayer(REPORTING_LAYER_NAME);       
	        if (reportLayer == null){      	
	            initialReportDataset = new FeatureDataset(initialFeatureSchema);
	        	reportLayer = localContext.addLayer(StandardCategoryNames.WORKING,REPORTING_LAYER_NAME, initialReportDataset);
	        	applyArrowStyles(reportLayer);
	        ...
	        else{ // Si existen las capas de informe al usuario hay que vaciarlas
	        	reportLayer.getFeatureCollectionWrapper().removeAll(reportLayer.getFeatureCollectionWrapper().getFeatures());
	        }       
	        errorLayer = localContext.getLayerManager().getLayer(ERRORLAYER_NAME);
	        if (errorLayer == null){
	        ...
	        firing = localContext.getLayerManager().isFiringEvents();
	        localContext.getLayerManager().setFiringEvents(false);   
        //}
        try{
        	FeatureCollection targetFeaturesCollection = capaCallejero.getFeatureCollectionWrapper().getUltimateWrappee();       
        	Collection callesCollection = capaCallejero.getFeatureCollectionWrapper().getFeatures();
        	// Crea una collection para alojar las features sin clasificar
        	FeatureCollection sinClasificar = new FeatureDataset(capaNumerosPolicia.getFeatureCollectionWrapper().getFeatureSchema());
        	sinClasificar.addAll(capaNumerosPolicia.getFeatureCollectionWrapper().getFeatures());
        	// introduce todos los origenes en la capa de error     	
        	errorLayer.getFeatureCollectionWrapper().addAll(capaNumerosPolicia.getFeatureCollectionWrapper().getFeatures());        
        	Map targetToSourceLinkingMap = new HashMap();// enlace 1:n      
        	TreeMap finalRelation = new TreeMap(); // borra anteriores resultados.
	        putToBlackboard(FINAL_LINKING_ARRAY, finalRelation, localContext);
	        // repasa las features destino para extraer los buffer y filtrar      
	        List allPoliceNumberFeatures = capaNumerosPolicia.getFeatureCollectionWrapper().getFeatures();
	        int iteraciones = 0;
	        int cont = 0;
	        ...
	        while (policeFeaturesIter.hasNext()){
	        	if(isGraphic){
	        		if (monitor.isCancelRequested()) 
	        			return;
	        		monitor.report(cont++,allPoliceNumberFeatures.size(),aplicacion.getI18nString("Revisandoelementosdestinodelenlace"));
	        	}else
	        		System.out.println(aplicacion.getI18nString("Revisandoelementosdestinodelenlace")+" "+(cont++)+"/"+allPoliceNumberFeatures.size());
	        	Feature sourceFeature = (Feature) policeFeaturesIter.next();     		
	        	// Busca todas la feature del grupo destino más cercana
	        	ArrayList actualNearestFeature = nearestFeaturesBrute2(sourceFeature, targetFeaturesCollection);       		
	        	// Asociamos las features candidatas NumeroPolicia<->calles
	        	targetToSourceLinkingMap.put(sourceFeature,actualNearestFeature);       		
	        }       	
	        // Genera la representación visual de los enlaces y clasifica entre enlazados, ambiguos y sin asignar
	        policeFeaturesIter = allPoliceNumberFeatures.iterator();        	
	        GeometryFactory geometryFactory = new GeometryFactory();
	        FeatureSchema featureSchema = new FeatureSchema();
	        featureSchema.addAttribute("GEOMETRY", AttributeType.GEOMETRY);
	        FeatureCollection arrows = new FeatureDataset(featureSchema);      	
	        cont = 0;
	        while (policeFeaturesIter.hasNext()){ // Revisa las features origen
	        	GeopistaFeature sourceFeature = (GeopistaFeature) policeFeaturesIter.next();
	        	// Obtiene las features target candidatas
	        	ArrayList targetCandidateFeatures = (ArrayList) targetToSourceLinkingMap.get(sourceFeature);
	        	if(isGraphic)
	        		monitor.report(cont++,allPoliceNumberFeatures.size(),aplicacion.getI18nString("Etiquetandolosenlacesrealizadosyloserrores"));
	        	else
	        		System.out.println(aplicacion.getI18nString("Etiquetandolosenlacesrealizadosyloserrores")+" "+(cont++)+"/"+allPoliceNumberFeatures.size());
	        	Iterator targetCandidateIter = targetCandidateFeatures.iterator();
	        	ArrayList arrowArray = new ArrayList(); // Lista de geometrías para etiquetar
	        	ArrayList matchedArray = new ArrayList(); // Lista de features que se han emparejado para esta SourceFeature
	        	GeopistaFeature candidateTargetFeature=null;
	        	while (targetCandidateIter.hasNext()){ // Itera calles candidatas para esta SourceFeature        			
	        		candidateTargetFeature=(GeopistaFeature) targetCandidateIter.next();
	        		if(isGraphic){
	        			if (monitor.isCancelRequested()) 
	        				return;
	        	...
	        	// aún no existe en los enlaces finales por lo que se incluye y se genera la pista visual.
	        	Coordinate[] coordenadas = new Coordinate[2];
	        	coordenadas[0] = sourceFeature.getGeometry().getCoordinate();
	        	coordenadas[1] = geometryMidPoint(targetFeature.getGeometry(), sourceFeature.getGeometry());
	        	LineString arrow = geometryFactory.createLineString(coordenadas);
	        	// Crea la Feature de la flecha
	        	Feature featureArrow = new BasicFeature(arrows.getFeatureSchema());
	        	featureArrow.setGeometry(arrow);       			
	        	arrows.add(featureArrow);
	        	arrowArray.add(featureArrow);       			
	        	// Hastable que contiene las relaciones finales, cada sourceFeature con la targetFeature que le corresponde
	        	finalRelation.put(sourceFeature, targetFeature);
	        	arrowFeatureRelation.put(sourceFeature, featureArrow);    			
	        	reportLayer.getFeatureCollectionWrapper().addAll(arrowArray);
	        	errorLayer.getFeatureCollectionWrapper().removeAll(matchedArray); // se eliminan de la capa de errores
	...
	public static void set(Layer sourceL, String expresion, TaskMonitor monitor, FeatureExpressionPanel featureExpressionPanel, String attributeIDonSource, PlugInContext localContext, Blackboard bk, boolean isGraphic) throws Exception
	{
		if(expresion==null || expresion.equals(""))
			expresion= (String) getFromBlackboard(EXPRESION_FORMULA, localContext);
		TreeMap finalRelation=(TreeMap) getFromBlackboard(FINAL_LINKING_ARRAY, localContext);
		if (finalRelation==null)
			return;
		...
		if(getFromBlackboard(SELECTEDSOURCELAYER, localContext,bk)!=null)*/
		Layer sourceLayer = sourceL;
		if(sourceL==null)
			sourceLayer = (Layer) getFromBlackboard(SELECTEDSOURCELAYER, localContext);
		boolean isFiringEvents = sourceLayer.getLayerManager().isFiringEvents();
		try{  
			sourceLayer.getLayerManager().setFiringEvents(false);
			while (finalRelations.hasNext()){
				if(isGraphic)
					...
					System.out.println(aplicacion.getI18nString("TransfiriendoDatosEnlazados")+" "+(cont++)+"/"+finalRelation.size());
				Map.Entry entry= (Entry) finalRelations.next();
				Feature target = (Feature)entry.getValue();
				Feature source = (Feature)entry.getKey();
				expParser.setFeature(target);
		...
		if(geopistaServerDataSource!=null && geopistaServerDataSource instanceof GeopistaServerDataSource){
			try{
				geopistaServerDataSource.getConnection().executeUpdate(sourceLayer.getDataSourceQuery().getQuery(),sourceLayer.getFeatureCollectionWrapper().getUltimateWrappee(), monitor);
			}catch(Exception e1){	
				throw e1;
			}
		}
