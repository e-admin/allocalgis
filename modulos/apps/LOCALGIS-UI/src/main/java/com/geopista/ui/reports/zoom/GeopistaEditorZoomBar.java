 ...
 */
package com.geopista.ui.reports.zoom;
...
public class GeopistaEditorZoomBar extends JPanel {

	private static AppContext aplicacion = (AppContext) AppContext.getApplicationContext();
	private int printAreaWidthInPixels = -1;
	private int printAreaHeightInPixels = -1;
	...
	throws NoninvertibleTransformException {
		...
		slider.addMouseMotionListener(new MouseMotionAdapter() {
			...
			public void mouseDragged(MouseEvent e) {
				try {
					layerViewPanel().erase((Graphics2D) ((LayerViewPanel)layerViewPanel()).getGraphics());
					drawWireframe();
					...
					if (scaleBarRenderer != null) {
						scaleBarRenderer.paint(
								(Graphics2D) ((LayerViewPanel)layerViewPanel()).getGraphics(),
								getScale());
					}
	...
	private void drawWireframe() throws NoninvertibleTransformException {
		Graphics2D g = (Graphics2D) ((LayerViewPanel)layerViewPanel()).getGraphics();
		g.setColor(Color.lightGray);
		g.setStroke(stroke);
		g.draw(getWireFrame());
	}
	private static final String SEGMENT_CACHE_KEY = GeopistaEditorZoomBar.class.getName() + " - SEGMENT CACHE";
		...
		AffineTransform transform =
			Viewport.modelToViewTransform(
					...
					new Point2D.Double(
							...
							((LayerViewPanel)layerViewPanel()).getSize().getHeight());
	...
	private static final String SCALE_KEY = GeopistaEditorZoomBar.class.getName() + " - SCALE";
	private static final String CENTRE_KEY = GeopistaEditorZoomBar.class.getName() + " - CENTRE";
	//Store centre-locked flag on blackboard rather than field because there could
	//be several zoom bars [Jon Aquino]
	
	private static final String CENTRE_LOCKED_KEY = GeopistaEditorZoomBar.class
			.getName()
			+ " - CENTRE LOCKED";
	private static final String MIN_EXTENT_KEY = GeopistaEditorZoomBar.class
			.getName()
			+ " - MIN EXTENT";
	private static final String USER_DEFINED_MIN_SCALE = GeopistaEditorZoomBar.class
			.getName()
			+ " - USER DEFINED MIN SCALE";
	private static final String USER_DEFINED_MAX_SCALE = GeopistaEditorZoomBar.class
			.getName()
			+ " - USER DEFINED MAX SCALE";
	private static final String MAX_EXTENT_KEY = GeopistaEditorZoomBar.class
			.getName()
			+ " - MAX EXTENT";
	...
	private ILayerViewPanel layerViewPanel() {
		// Puesto que esto es un GeopistaEditor, el layerviewpanelproxy es el 
		// mismo. No contiene un internal frame
		return ((LayerViewPanelProxy) frame).getLayerViewPanel();
	}
	...
	private void updateLabel() throws NoninvertibleTransformException {
		...
		scalelabel.setText("(1:"
				+ NumberFormat.getIntegerInstance()
						.format(getNormalizedPrintScale()) + ")");
	}
	...
	private double getNormalizedPrintScale() {
		double horizontalScale;
		
		if (printAreaHeightInPixels != -1 && printAreaWidthInPixels != -1){
			horizontalScale = ScaleUtils.getReportPrintEnvelopeScale(layerViewPanel(), 
				printAreaHeightInPixels, printAreaWidthInPixels);
		}
		else { 
			horizontalScale = ScaleUtils.getReportPrintEnvelopeScale(layerViewPanel(), 
				layerViewPanel().getHeight(), layerViewPanel().getWidth());
		}
	...
	private void updateSliderLabels() throws NoninvertibleTransformException {
		...
		for (int i = 0; i < slider.getWidth(); i++) {
			...
			int sliderValue = ((BasicSliderUI) slider.getUI())
					.valueForXPosition(i);
			JLabel label = new JLabel(chooseGoodIncrement(toScale(sliderValue))
					.toString());
	...
	public void setPrintAreaDimensions(int pixelsWidth, int pixelsHeight){
		this.printAreaWidthInPixels = pixelsWidth;
		this.printAreaHeightInPixels = pixelsHeight;
	}
