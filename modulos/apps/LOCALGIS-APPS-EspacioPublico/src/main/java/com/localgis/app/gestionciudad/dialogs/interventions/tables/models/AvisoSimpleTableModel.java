...
//		IconLoader.icon("flechas.gif").toString(),
	...
	public Object getValueAt(int row, int col) {
		...
		case 3: 
			try{
				return ConstantesEspacioPublico.DateFormat.format(lstElementos.get(row).getStartWarning().getTime());
			} catch (Exception e) {
		...
		case 4: 
			try{
				if (lstElementos.get(row).getNextWarning() != null){
					return ConstantesEspacioPublico.DateFormat.format(lstElementos.get(row).getNextWarning().getTime());	
				} else{
					return "*";
				}
