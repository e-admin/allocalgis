 ...
 *
 */
public class LocalGISGestionCiudad extends CMain implements ISecurityPolicy{
	...
	public static final String idApp="Espacio Publico";
...
//			Municipio municipio = (new OperacionesAdministrador(ConstantesObraCivil.localgisObraCivil)).getMunicipio(new Integer(ConstantesObraCivil.IDMunicipioCompleto).toString());
//			if (municipio!=null)
//			{
//				ConstantesObraCivil.Municipio = municipio.getNombre();
//				ConstantesObraCivil.Provincia= municipio.getProvincia();
//				ConstantesObraCivil.idMunicipio = municipio.getId();
//				ConstantesObraCivil.idProvincia= municipio.getIdprovincia();
//
//				com.geopista.security.GeopistaPrincipal principal= com.geopista.security.SecurityManager.getPrincipal();
//				if(principal!=null)
//					setTitle(getTitle()+ " - " + ConstantesObraCivil.Municipio + " (" + ConstantesObraCivil.Provincia+") - " + I18N.get("LocalGISEIEL","localgiseiel.cauthdialog.jlabelnombre") + principal);
//			}
	...
	{	
		...
		addComponentListener(new java.awt.event.ComponentAdapter()
		{
			...
			{
				UtilsEspacioPublico.ajustaVentana(aplicacion.getMainFrame(), DIM_X, DIM_Y+50);
			}
		});
...
//				ConstantesObraCivil.localgisObraCivil = aplicacion.getString(UserPreferenceConstants.LOCALGIS_SERVER_URL)+"eiel";
...
//				ConstantesLocalGISEIEL.Locale = aplicacion.getString(UserPreferenceConstants.DEFAULT_LOCALE_KEY,"es_ES");
				...
				try
				{
					Constantes.idMunicipio = ConstantesEspacioPublico.IDMunicipioCompleto = 
...
//			}
	...
	public boolean applySecurityPolicy(GeopistaAcl acl, GeopistaPrincipal principal)
	{
		try
		{
			...
			ConstantesEspacioPublico.principal = principal;
	...
	private void setPermisos(Enumeration<GeopistaPermission> e)
	{
		...
		ConstantesEspacioPublico.permisos = permisos;
	}
	...
	private JMenuItem getJMenuImprimirMapa(){
		...
		imprMenuItem.addActionListener(new ActionListener()
		{
			public void actionPerformed(ActionEvent e)
			{
				...
				UtilsEspacioGeopistaMap.setMapToReload(idMapa, true);
...
//					int idMapa = new Integer(AppContext.getApplicationContext().getString("id.mapa.planeamiento.informe.urbanistico")).intValue();
	...
	{	
				...
				new Thread(new Runnable()
				{
					public void run()
					{
						...
						{        
							...
							// Arreglar lo del boton cancelar
							jif = encapsulaEnJInternalFrame(getJPanelGestionCiudad(), "");
		...
		if (cancelada) {
			cancelada = false;
			System.out.println("Parando aplicacion");
			stopApp();
			return;
		}
    	...
    	if ((mostrarMapa!=null) && (mostrarMapa.equals("false"))){
    		logger.info("No cargamos el mapa");
    	}
    	else{
				...
				new Thread(new Runnable()
				{
					public void run()
					{
						...
						{        
							...
							c.setCursor(Cursor.getPredefinedCursor(Cursor.WAIT_CURSOR));
							// Arreglar lo del boton cancelar
							jif = encapsulaEnJInternalFrame(getPrintJPanelGestionCiudad(), "Imprimir Mapas De Gestión Ciudad");
							llamadaAInternalFrame(jif, false, true, true);
							c.setCursor(Cursor.getPredefinedCursor(Cursor.DEFAULT_CURSOR));
							c.setCursor(Cursor.getDefaultCursor());
							jif.setMaximizable(false);
...
//							String title = ConstantesObraCivil.clienteLocalGISEIEL.getNombredMapa(idMap);
							String title ="";
	...
	private void menuBarSetEnabled(boolean b, JFrame desktop)
	{
		try
		{           
