...
import com.localgis.app.gestionciudad.dialogs.documents.dialogs.utils.DocumentInterface;
import com.localgis.app.gestionciudad.dialogs.documents.dialogs.utils.DocumentPanel;
import com.localgis.app.gestionciudad.dialogs.documents.dialogs.utils.JDialogLocal;
import com.localgis.app.gestionciudad.dialogs.documents.dialogs.utils.JFrameOpciones;
import com.localgis.app.gestionciudad.dialogs.documents.dialogs.utils.JPanelComentarios;
...
public class GestionDocumentalJPanel extends JPanel  implements DocumentInterface{
	...
	public void inicializar(final Object obj) throws Exception{
		...
		documentClient= new GestionCiudadDocumentClient(aplicacion.getString(UserPreferenceConstants.LOCALGIS_SERVER_ADMCAR_SERVLET_URL) + ServletConstants.DOCUMENT_SERVLET_NAME);
	...
	private void visualizarDocumento(){
		...
		try{
			...
			if (new File(auxDocument.getFileName()).isAbsolute())
				selectedFile= new File(auxDocument.getFileName()).getAbsolutePath();
			else 
			{
				String pathFile = "";
				try {
					pathFile = System.getProperty("java.io.tmpdir");
					(new File (pathFile)).toPath().resolve(File.pathSeparator);
				} catch (InvalidPathException e) {
					logger.error("Invalid path : ", e);
					throw new RuntimeException(e);
				}
				selectedFile = pathFile + auxDocument.getFileName();
