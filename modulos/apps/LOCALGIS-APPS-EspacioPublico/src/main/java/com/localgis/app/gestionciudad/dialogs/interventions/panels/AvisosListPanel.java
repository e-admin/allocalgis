...
public class AvisosListPanel extends JPanel{
	...
	private void onPaginaSiguienteButtonDo() {
		String usuario = AppContext.getApplicationContext().getString(UserPreferenceConstants.LOCALGIS_LOGIN_LAST_USERNAME);
	...
	private void onPaginaAnteriorButtonDo() {
		String usuario = UserPreferenceStore.getUserPreference(UserPreferenceConstants.DEFAULT_LOGIN_USERNAME, "", true);
		int idMunicipio = AppContext.getIdMunicipio();
		if( (usuario != null && !usuario.equals("")) && (idMunicipio > 0) ){
			if (pagina - 1 >= 0){
				this.pagina = pagina - 1;
					...
					new Thread(new Runnable()
					{
						public void run()
						{
							try {
								...
								progressDialog.report("Recargando red '" +  ConstantesEspacioPublico.NOMBRE_RED + "'.");
								...
								if (networkMgr.getNetwork(ConstantesEspacioPublico.NOMBRE_RED) != null)
									networkMgr.eraseNetwork(ConstantesEspacioPublico.NOMBRE_RED);

								RouteNetworksUtils.leerGrafodeBase(ConstantesEspacioPublico.NOMBRE_RED, progressDialog, context);
			...
			if (dialog.wasOKPressed()){
				...
				try {
					WSInterventionsWrapper.addIntervention(nuevoAviso, 
							AppContext.getApplicationContext().getString(UserPreferenceConstants.LOCALGIS_LOGIN_LAST_USERNAME), 
