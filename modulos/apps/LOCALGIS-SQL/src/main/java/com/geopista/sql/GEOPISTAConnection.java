...
public class GEOPISTAConnection implements Connection{
	...
	GEOPISTAConnection(String sConnString){
		this.sUrl=sConnString.substring(sConnString.indexOf(AppContext.getApplicationContext().getString(UserPreferenceConstants.DEFAULT_PROTOCOL)));
	}
		...
		try {
			...
			try
			{
				...
				{	
					...
					{
						logger
						.error("executeStatement(GEOPISTAPreparedStatement) - Resultado:"
								+ rs.getResultado()
								+ " Cadena:"
								+ resultado);
					}
...
//			    e.printStackTrace();
//			    System.out.println(" Cadena devuelta por Admcar:"+resultado);
			    rs=null;
			    throw new SQLException(resultado, e);
			    }
