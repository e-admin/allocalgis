...
public class TablesDBOperations{
    ...
    //private static final String pass = aplicacion.getUserPreference(UserPreferenceConstants.LOCALGIS_DATABASE_DIRECTCONNECTION_PASS,"",false);
    ...
    public boolean crearTablaBD(Table table) throws DataException{    
        ...
        try{ 
            ...
            .append(nomTable).append("\" TO consultas; GRANT SELECT, UPDATE, INSERT, DELETE ON TABLE \"")
            .append(nomTable).append("\" TO modelo;");
        ...
        try{             
            ...
            if (oriTable!=null && oriTable.getDescription()!=null){
                if (!oriTable.getDescription().equals(modTable.getDescription())){
                    ...
                    //operaciones.crearSecuencia(oriTable.getDescription());
                    //crearSecuencia(oriTable.getDescription());
                    String sequenceName=crearSecuencia2(oriTable.getDescription());
                    ...
                    query.append("ALTER TABLE \""+sequenceName)
     ...
     */
    public String crearSecuencia2(String nombreTabla) throws DataException
    {   
    	PreparedStatement s = null;
        boolean bRes = false;
        try{
        	...
        	if (!existeSecuenciaInRelName("seq_"+nombreTabla)){
        		if (!existeSecuenciaInRelName("seq_"+nombreTabla.toLowerCase())){
        			String query = "CREATE SEQUENCE \"public\".\"seq_"+ nombreTabla.toLowerCase()
        		            + "\" INCREMENT 1 MINVALUE 1 START 1 CACHE 1;";            
		            ...
		            s.executeUpdate();
		            LCGIII_GeopistaUtil.generarSQL(s);   
		            return "seq_"+nombreTabla.toLowerCase();
        		}
        		else{
        			return "seq_"+nombreTabla.toLowerCase();
        		}
     ...
     * @return
     */
    public boolean existeSecuenciaInRelName(String nombreSecuencia) throws DataException{
    ...
    public boolean crearColumnaBD(Table table, ColumnRow colrow) throws DataException{   
        ...
        try{ 
            ...
            .append(LCGIII_GeopistaUtil.translateBDtoComboItem(colrow.getColumnaBD().getType()));
            ...
            if ((colrow.getColumnaBD().getDefaultValue()!=null)&&(!colrow.getColumnaBD().getDefaultValue().trim().equals(""))){
                query.append("ALTER TABLE \"").append(nomTable).append("\" ALTER COLUMN \"").append(colrow.getColumnaBD().getName());
                
                String tipo = colrow.getColumnaBD().getType();
                
                //Hay un error con los setval que vienen sin el caracter ")" al final. Lo añadimos
                
                if ((colrow.getColumnaBD().getDefaultValue()!=null) && (colrow.getColumnaBD().getDefaultValue().startsWith("nextval"))){
                	if (!colrow.getColumnaBD().getDefaultValue().endsWith(")")){
                		colrow.getColumnaBD().setDefaultValue(colrow.getColumnaBD().getDefaultValue()+")");
                		 if (tipo.equalsIgnoreCase("numeric") ||
     	                        tipo.equalsIgnoreCase("int4") || tipo.equalsIgnoreCase("float8")){
     	                	query.append("\" SET DEFAULT ")
     		                .append(colrow.getColumnaBD().getDefaultValue()).append(";");
     	                }
     	                else{
     		                query.append("\" SET DEFAULT '")
     		                .append(colrow.getColumnaBD().getDefaultValue()).append("';");
        ...
        if (!colrowOri.getColumnaBD().getType().equalsIgnoreCase(colrowMod.getColumnaBD().getType())
            ...
            for (int i=0; i< colsBD.length; i++){
                ...
                if (i!=pos-1 && pos>0){   
                    ...
                    .append("\" AS ").append(LCGIII_GeopistaUtil.translateBDtoComboItem(colrowMod.getColumnaBD().getType()));
            ...
            for (int i=0; i<colsBD.length; i++){
                ...
                .append(LCGIII_GeopistaUtil.translateBDtoComboItem(cbd.getType()));
     ...
     * @return Posición de la columna, o 0 en caso de no encontrarla
     */
    ...
    public int crearColumnaSistema(Table table, ColumnRow colrow) throws DataException{
        ...
        try{
        	if(colrow.getColumnaSistema().getIdColumn()==0){
	    		if ((conn == null) || (conn.isClosed()))
	    			conn = getDBConnection();
 ...
 //       List lstColumnas = new ArrayList();
    ...
    public void crearScriptInsertExportImport(FileOutputStream salidaInsert, String nombreTabla, int idEntidad) throws SQLException {
    	ResultSet r = null;
    	PreparedStatement s = null;
    	boolean isColumnMunicipio=false;
		try {
			StringBuffer columnas = new StringBuffer();
			HashMap columnsDB = obtenerListaColumnasBD(nombreTabla);
			if (columnsDB != null) {
				...
				while (it.hasNext()) {
					Map.Entry e = (Map.Entry) it.next();
					if (((ColumnDB) e.getValue()).getName().equals("id_municipio")){
						isColumnMunicipio=true;
					}
					if (((ColumnDB) e.getValue()).getName().equals("GEOMETRY")){
						columnas.append("'GeometryFromText(\\'' || astext(\"GEOMETRY\") || '\\', ' || getsrid(\"GEOMETRY\")::text || ')' as \"GEOMETRY\"");
					}
					else if (((ColumnDB) e.getValue()).getName().equals("GEOMETRYold50")){
						columnas.append("'GeometryFromText(\\'' || astext(\"GEOMETRYold50\") || '\\', ' || getsrid(\"GEOMETRY\")::text || ')' as \"GEOMETRYold50\"");
					}
					else
						columnas.append("\""+((ColumnDB)e.getValue()).getName()+"\"");
					if (it.hasNext())
				...
				String sql=null;
				if (isColumnMunicipio){
					if (idEntidad!=0)
					sql = "SELECT "+columnas+" FROM \""+nombreTabla+"\" where id_municipio IN (select id_municipio from entidades_municipios where id_entidad='"+idEntidad+"');";
					else
		       ...
		       int elementosExportados=0;
		        while (r.next() ) {
		            ...
		            for (int i = 0; i < numColumnas; i++) {
		                ...
		                else if (nombreColumnas[i].equals("GEOMETRYold50")){
		                	insert.append(r.getObject(i+1));
		                }
		                else{
		                	if (nombreColumnas[i].equals("id")){
		        ...
		        logger.info("Elementos exportados:"+elementosExportados);
	        }
        } catch (DataException e1) {
			// TODO Auto-generated catch block
			e1.printStackTrace();
