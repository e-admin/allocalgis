...
public class MapRequest {
  ...
  /**Tabla hash en la que tenemos para cada capa su estilo:
   * Nombre capa-Nombre estilo*/
  private HashMap layerStyles;
  ...
  public Map getLayerStyles(){
	  return Collections.unmodifiableMap(this.layerStyles);
  }
  ...
  public void setFormat( String format ) /*throws IllegalArgumentException */{
   ...
   /* String[] formats = service.getCapabilities().getMapFormats();
    ...
    String formats_desc=formats[0];
    for (int i = 1; i < formats.length; i++)
		{
		formats_desc =formats_desc + ", "+formats[i];
		}
    throw new IllegalArgumentException("Formato necesario:"+format+" no soportado en la lista de disponibles:"+formats_desc);
  */}
  ...
  public void setLayerStyles(HashMap layerStyles){
  ...
  private  String stylesToString(){
	  ...
	  if(layerStyles!=null){
	   while( it.hasNext() ) {
		      ...
		      String styleName=(String) this.layerStyles.get(layerName);
		      if(styleName!=null)
 ...
 */
  ...
  public URL getURL() throws MalformedURLException {	  
      ...
      String sts=stylesToString();
      if(!sts.equals(""))
      urlBuf.append("&STYLES="+sts);
      if( transparent ) {
      ...
      if( time != null ) {
          urlBuf.append( "&TIME=" + time );
      }/*else{
    	  SimpleDateFormat formato1 = new SimpleDateFormat("yyyy-MM-dd");
    	  SimpleDateFormat formato2 = new SimpleDateFormat("kk:mm:ss");
          urlBuf.append( "&TIME=" + formato1.format(new Date())+"T"+formato2.format(new Date()));
      }*/
      this.time = null;
    
	  logger.info("getURL():"+urlBuf.toString());

      System.out.println("URL solicitada: "+urlBuf.toString());
