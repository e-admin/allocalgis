/**
 * CementeriosDocumentConnection.java
...
public class CementeriosDocumentConnection{
    ...
    public void checkPermissionLock(String idLayer, String idFeature, Sesion userSesion) throws Exception{
        ...
        try{
            ...
            if (!(iLock==Const.LOCK_FEATURE_OWN ||
                  iLock==Const.LOCK_LAYER_OWN   ||
                  iLock==Const.LOCK_LAYER_LOCKED)){
                String sMsg=null;
                switch (iLock){
                    case Const.LOCK_LAYER_OTHER:
                        sMsg="locked: layer "+idLayer;
                        break;
                    case Const.LOCK_FEATURE_OTHER:
                        sMsg="locked: feature";
                        break;
                    default:
                        sMsg="Lock error";
    ...
    private int canLockFeature(int iMunicipio, String sLayer, int iFeature, int iUser) throws Exception{
        ...
        if(iFeatureLock!=-1)
            return (iFeatureLock==iUser?Const.LOCK_FEATURE_OWN
                                       :Const.LOCK_FEATURE_OTHER);
        ...
        try{
            ...
            rs=ps.executeQuery();
            if (rs.next())
                iRet=(rs.getInt("user_id")!=iUser)? Const.LOCK_LAYER_OTHER
                                                  : Const.LOCK_LAYER_OWN;
        }finally{
            try{rs.close();}catch(Exception e){};
            try{ps.close();}catch(Exception e){};
            try{conn.close();CPoolDatabase.releaseConexion();}catch(Exception e){};
   ...
   /**
    ...
    public void borrarFicherosEnDisco(Object key, Collection c){
        try{
            ...
            for (int i=0; i<docs.length; i++){
                DocumentBean doc= (DocumentBean)docs[i];
                if (!documentManager.asociadoAOtroElem(doc.getId(), key))
 		...
 		try {
            ...
            for (int i=0; i<documentos.length; i++){
                ...
                if (doc.getId() == null){  //if (doc.getId() == -1){
                    ...
                    if (!documentManager.existeLink(key, superpatron, patron, doc.getId())){
                        /** Enlazamos */
