...
public class LocalGISEIELUtils
{
			...
			boolean editable, String visible, String novisible)
	{
		...
		catch(Exception e)
		{
			...
			logger.error("Error al cargar el mapa EIEL: ",e);
			return false;
		}
	}
...
//				geopistaEditor.loadMapEIEL("geopista:///"+id_map);
	...
	public static void storeNucleosMunicipio(){
		...
		HashMap nucleosAllMunicipio=new HashMap();
		Collection c=null;
        try {
			c = InitEIEL.clienteLocalGISEIEL.getNucleosEIEL();
	        ...
	        while (it.hasNext()){
				...
				nucleosAllMunicipio.put(nucleoEIEL.getCodMunicipio()+"_"+clave,nucleoEIEL);
	 ...
	 * @return
	 */
	public static void storeNucleosAllMunicipios(){
		
		//HashMap nucleosMunicipio=new HashMap();
		HashMap nucleosAllMunicipio=new HashMap();
		
		HashMap nucleosPorMunicipio=new HashMap();
		Collection c=null;
        try {
    		c = InitEIEL.clienteLocalGISEIEL.getAllNucleosEIEL();
			//Los ordenamos alfabeticamente		
			Object[] arrayNodos= c.toArray();
	    	Arrays.sort(arrayNodos,new NodoComparatorByDenominacion());
	        ...
	        while (it.hasNext()){
				...
				//Almacenamos la informacion por municipio para tener un listado de nucleos de un municipio
				ArrayList listaNucleos;
				if (nucleosPorMunicipio.get(nucleoEIEL.getCodMunicipio())!=null){
					listaNucleos=(ArrayList)nucleosPorMunicipio.get(nucleoEIEL.getCodMunicipio());
					listaNucleos.add(nucleoEIEL);
										
				}
				else{
					listaNucleos=new ArrayList();
					listaNucleos.add(nucleoEIEL);						
				}
				nucleosPorMunicipio.put(nucleoEIEL.getCodMunicipio(), listaNucleos);	
				
				
			}
	        
	        Iterator it2=nucleosPorMunicipio.keySet().iterator();
	        while (it2.hasNext()){
	        	String claveIdMunicipio=(String)it2.next();
	        	ArrayList list=(ArrayList)nucleosPorMunicipio.get(claveIdMunicipio);
	        	 Object[] arrayNodos2= list.toArray();
	        	 //Arrays.sort(arrayNodos2,new NodoComparatorByEntidad());
	        	 Arrays.sort(arrayNodos2,new NodoComparatorByDenominacion());
	        	 list.clear();
				 for (int i = 0; i < arrayNodos2.length; ++i) {
					LCGNucleoEIEL nucleoEIEL = (LCGNucleoEIEL) arrayNodos2[i];
					list.add(nucleoEIEL);
				 }
	        }		        
			...
			aplicacion.getBlackboard().put("HASH_NUCLEOS_ALL_MUNICIPIOS",nucleosAllMunicipio);
			aplicacion.getBlackboard().put("HASH_NUCLEOS_POR_MUNICIPIO",nucleosPorMunicipio);
    ...
    public static void inicializarConstantesMunicipio(){
		...
		com.geopista.app.administrador.init.Constantes.url = AppContext	.getApplicationContext().getString(UserPreferenceConstants.LOCALGIS_SERVER_URL)+ WebAppConstants.GEOPISTA_WEBAPP_NAME;

		ConstantesLocalGISEIEL.idProvincia = GeopistaUtil.completarConCeros(String.valueOf(AppContext.getIdMunicipio()),5).substring(0, 2);
    ...
    public static void actualizarInformacionMunicipioSeleccionado(String idMunicipio, EditorPanel editorPanel){
    	...
    	try {
			...
			if (AppContext.getApplicationContext().getBlackboard().get(UserPreferenceConstants.SEL_MUNI_AUTO)!=null
					&& ((Boolean)AppContext.getApplicationContext().getBlackboard().get(UserPreferenceConstants.SEL_MUNI_AUTO)).booleanValue()){
			    ...
			    if (geopistaLayer!=null){
			    	...
			    	CoordinateSystem inCoord = PredefinedCoordinateSystems.getCoordinateSystem(Integer.parseInt(GEOGRAPHICS_ETRS89));
