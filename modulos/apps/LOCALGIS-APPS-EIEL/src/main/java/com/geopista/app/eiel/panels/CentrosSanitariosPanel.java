...
public class CentrosSanitariosPanel extends InventarioPanel implements FeatureExtendedPanel
{
    ...
    private ComboBoxEstructuras jComboBoxUci = null;
    private JTextField jTextFieldCamas = null;    
    ...
    private JLabel jLabelUci = null;
    private JLabel jLabelCamas = null;
     ...
     * 
     */
    public CentrosSanitariosPanel()
    {
        super();
        initialize();
    }
    
    public CentrosSanitariosPanel(CentrosSanitariosEIEL pers)
    {
        super();
        initialize();
        loadData (pers);
    ...
    public void loadData(CentrosSanitariosEIEL elemento)
    {
        if (elemento!=null)
        {
            ...
            if (elemento.getUci()!= null){            	
            	jComboBoxUci.setSelectedPatron(elemento.getUci());
            }
            else{
            	jComboBoxUci.setSelectedIndex(0);
            } 
            
            if (elemento.getNumCamas() != null){            	
            	jTextFieldCamas.setText(elemento.getNumCamas().toString());
            }
            else{
            	jTextFieldCamas.setText("");
            } 
        	...
        	jTextFieldClave.setText(ConstantesLocalGISEIEL.CENTROSSANITARIOS_CLAVE);
    ...
    {
    	Object object = AppContext.getApplicationContext().getBlackboard().get("centrossanitarios_panel");    	
    	if (object != null && object instanceof CentrosSanitariosEIEL){    		
    		CentrosSanitariosEIEL elemento = (CentrosSanitariosEIEL)object;
            //Datos identificacion   
        	if (elemento.getClave() != null){
        		jTextFieldClave.setText(elemento.getClave());
        	}
    ...
    public CentrosSanitariosEIEL getCentrosSanitarios (CentrosSanitariosEIEL elemento)
    {
        ...
        if (okPressed)
        {
            if(elemento==null)
            {
            	elemento = new CentrosSanitariosEIEL();
            }
            ...
            if (jComboBoxUci.getSelectedPatron()!=null)
            	elemento.setUci((String) jComboBoxUci.getSelectedPatron());
            else elemento.setUci("");
            
            if (jTextFieldCamas.getText()!=null && !jTextFieldCamas.getText().equals("")){
            	elemento.setNumCamas(new Integer(jTextFieldCamas.getText()));
            }
            else{
            	elemento.setNumCamas(null);
            }
    ...
    {      
        ...
        this.setName(I18N.get("LocalGISEIEL","localgiseiel.centrossanitarios.panel.title"));
        
        this.setLayout(new GridBagLayout());
        this.setSize(CentrosSanitariosDialog.DIM_X,CentrosSanitariosDialog.DIM_Y);
...
//		     	jComboBoxEntidad.setEditable(true);
    ...
    private ComboBoxEstructuras getJComboBoxUci()
    { 
        if (jComboBoxUci == null)
        {
            Estructuras.cargarEstructura("eiel_uci");
            jComboBoxUci = new ComboBoxEstructuras(Estructuras.getListaTipos(),
                    null, aplicacion.getString(UserPreferenceConstants.DEFAULT_LOCALE_KEY,"es_ES"), false);
        
            jComboBoxUci.setPreferredSize(new Dimension(100, 20));
        }
        return jComboBoxUci;        
    }
    
    private JTextField getjTextFieldCamas()
    {
    	if (jTextFieldCamas == null)
    	{
    		jTextFieldCamas = new TextField(10);
    		jTextFieldCamas.addCaretListener(new CaretListener()
    		{
    			public void caretUpdate(CaretEvent evt)
    			{
    				EdicionUtils.chequeaLongYCharCampoEdit(jTextFieldCamas, 10, aplicacion.getMainFrame());
    			}
    		});
    	}
    	return jTextFieldCamas;
    }
    ...
    public CentrosSanitariosPanel(GridBagLayout layout)
    {
        super(layout);
        initialize();
    }
    ...
    private JPanel getJPanelDatosInformacion()
    {
        ...
        {   
        	...
        	jLabelUci = new JLabel("", JLabel.CENTER);
        	jLabelUci.setText(I18N.get("LocalGISEIEL", "localgiseiel.panels.label.uci"));
            
        	jLabelCamas = new JLabel("", JLabel.CENTER);
        	jLabelCamas.setText(I18N.get("LocalGISEIEL", "localgiseiel.panels.label.camas"));
            ...
            jPanelInformacion.add(jLabelUci,
                    new GridBagConstraints(2, 3, 1, 1, 0.1, 0.1,
                            GridBagConstraints.EAST, GridBagConstraints.HORIZONTAL,
                            new Insets(5, 5, 5, 5), 5, 0));
            
            jPanelInformacion.add(getJComboBoxUci(), 
                    new GridBagConstraints(3, 3, 1, 1, 0.1, 0.1,
                            GridBagConstraints.CENTER, GridBagConstraints.HORIZONTAL,
                            new Insets(5, 5, 5, 5), 0, 0));
            
            jPanelInformacion.add(jLabelCamas,
                    new GridBagConstraints(0, 4, 1, 1, 0.1, 0.1,
                            GridBagConstraints.EAST, GridBagConstraints.HORIZONTAL,
                            new Insets(5, 5, 5, 5), 5, 0));
            
            jPanelInformacion.add(getjTextFieldCamas(), 
            ...
            jPanelInformacion.add(getjTextFieldObservacion(), 
                    new GridBagConstraints(3, 6, 2, 1, 0.1, 0.1,
                            GridBagConstraints.CENTER, GridBagConstraints.HORIZONTAL,
                            new Insets(5, 5, 5, 5), 0, 0));
	...
	public void loadDataIdentificacion(){
		...
		if (obj != null && obj instanceof FeatureDialog){
			...
			Feature feature = featureDialog.get_fieldPanel().getModifiedFeature();
	        	...
	        	if (feature.getAttribute(esquema.getAttributeByColumn("orden_sa"))!=null){
	        		orden_sa=(feature.getAttribute(esquema.getAttributeByColumn("orden_sa"))).toString();
	        	}
	        	
	        	EdicionOperations operations = new EdicionOperations();
	        	loadData(operations.getPanelCentrosSanitariosEIEL(clave, codprov, codmunic, entidad, nucleo, orden_sa));
	        	
	        	loadDataIdentificacion(clave, codprov, codmunic, entidad, nucleo, orden_sa);       	
			}
		}
	
	
	public void loadDataIdentificacion(String clave, String codprov,
			String codmunic, String entidad, String nucleo, String orden_sa) {
        ...
        if (orden_sa != null){
    		jTextFieldOrden.setText(orden_sa);
    	}
    	else{
    		jTextFieldOrden.setText("");
    	}
