		...
		if (!((NucleosAbandonadosEIELTableModel)((TableSorted)getJTableListaNucleos().getModel()).getTableModel()).getData().isEmpty()
			...
			if (selectedItem != null && selectedItem instanceof NucleosAbandonadosEIEL){
				if (confirm(I18N.get("LocalGISEIEL","localgiseiel.register.tag2"), 
					...
					try {
						Collection lstFeatures = InitEIEL.clienteLocalGISEIEL.getIdsFeatures(selectedItem, ConstantesLocalGISEIEL.NUCLEOS_ABANDONADOS);
						...
						InitEIEL.clienteLocalGISEIEL.eliminarElemento(selectedItem, lstFeatures, idLayer, ConstantesLocalGISEIEL.NUCLEOS_ABANDONADOS );
						resultado = true;
					} catch (Exception e) {
						e.printStackTrace();
						resultado = false;
