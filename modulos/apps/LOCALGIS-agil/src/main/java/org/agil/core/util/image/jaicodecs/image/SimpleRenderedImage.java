...
public abstract class SimpleRenderedImage implements RenderedImage {
	...
	public WritableRaster copyData(WritableRaster dest) {
	  ...
	  public static ColorModel createColorModel(SampleModel samplemodel){
		...
		if(samplemodel instanceof ComponentSampleModel)
			return getDefaultColorModel(samplemodel.getDataType(), i);
		if(samplemodel instanceof SinglePixelPackedSampleModel)
		{
			SinglePixelPackedSampleModel singlepixelpackedsamplemodel = (SinglePixelPackedSampleModel)samplemodel;
			int ai[] = singlepixelpackedsamplemodel.getBitMasks();
			int j = 0;
			int k = 0;
			int l = 0;
			int i1 = 0;
			...
			int ai1[] = singlepixelpackedsamplemodel.getSampleSize();
			int k1 = 0;
			for(int l1 = 0; l1 < ai1.length; l1++)
				k1 += ai1[l1];

			return new DirectColorModel(k1, j, k, l, i1);
		}
		if(isBinary(samplemodel))
		{
			...
			return new IndexColorModel(1, 2, abyte0, abyte0, abyte0);
		} else
		{
			return null;
		}
	...
	public static ColorModel getDefaultColorModel(int i, int j)
	{
		if(i < 0 || i == 2 || i > 5 || j < 1 || j > 4)
		{
			return null;
		} else
		{
			ColorSpace colorspace = j > 2 ? ColorSpace.getInstance(1000) : ColorSpace.getInstance(1003);
			boolean flag = j == 2 || j == 4;
			byte byte0 = flag ? ((byte) (3)) :(byte) 1;
			return RasterFactory.createComponentColorModel(i, colorspace, flag, false, byte0);
		}
	}
	 public static boolean isBinary(SampleModel samplemodel)
	{
		return (samplemodel instanceof MultiPixelPackedSampleModel) &&
		((MultiPixelPackedSampleModel)samplemodel).getPixelBitStride() == 1 ;
	 }
