...
* caso contrario se muestra un pawasOKPressednel simple.
...
public abstract class ExtendedPanelsDialog extends JDialog implements FeatureDialogHome
{
    ...
    {
    	_okCancelPanel.setOKPressed(okPressed);
    }
...
//	 Obtenemos paneles suplementarios
	...
	if (extendedForms != null) for (Enumeration e = extendedForms
		...
		boolean addExtraForms=true;
		//Se elimina el tab de Inventario,ya que se duplicaba.
		if (vExtraForms != null){
		if (aplicacion.getBlackboard().get("ShowSimpleAtributes")==null ) {
	 
			Iterator it = extendedForms.iterator();
			while(it.hasNext()){
				FeatureExtendedForm fef = (FeatureExtendedForm) it.next();
				Iterator ite = vExtraForms.iterator();
				while(ite.hasNext()){
					FeatureExtendedForm fefe = (FeatureExtendedForm) ite.next();
					if(fef.getClass().toString().equals(fefe.getClass().toString())){
						ite.remove();
					}
				}
			}
