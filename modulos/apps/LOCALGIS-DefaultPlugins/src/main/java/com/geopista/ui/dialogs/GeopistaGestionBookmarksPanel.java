/**
 * GeopistaGestionBookmarksPanel.java
	...
	public void irAMarcador(GeopistaBookmark elemento){
		...
		try
		{
			...
			try {
				...
				if (workbenchFrame!=null && workbenchFrame instanceof GeopistaEditor){				
					 GeopistaEditor geopistaeditor=(GeopistaEditor) context.getWorkbenchGuiComponent();
					 geopistaeditor.getLayerViewPanel().getViewport().zoom(envelope);	         
				}
			} catch (Exception e) {
			}
