...
public class LocalGISWarning implements Serializable {
	...
	private GregorianCalendar nextValuePeriod;
	...
	public String toString(){
		...
		if(period != null)
			resultGeneral = "{"+period+"}"+resultGeneral;
	...
	public LocalGISWarning(String data) throws ConfigurationException{
		...
		for(int i = 0;i<incidents.length && recursiveIncident == null;i++){
			...
			if(incident.contains("{")){
				buildPeriod(incident.substring(1, incident.indexOf("}")));
			}else{
				dates = incident.substring(1, incident.indexOf("]"));
				times = incident.substring(incident.indexOf("(")+1,incident.indexOf(")"));
			}
	...
	private void buildPeriod(String period) throws ConfigurationException{
		this.period = period;
		String[] data = period.split("-");
		if(start == null)
			throw new ConfigurationException("Period available only with start date");
		GregorianCalendar calendar = (GregorianCalendar)start.clone();
		for(int i = 0;i< data.length;i++){
			String character = data[i].substring(data[i].length()-2);
			Integer amount = new Integer(data[i].substring(0,data[i].length()-2));
			...
			if(character.equalsIgnoreCase("s"))
				calendar.add(GregorianCalendar.SECOND, amount);
		}
		this.nextValuePeriod = calendar;
	}

	public boolean inPeriod(Date baseStartDate){
		
		return inPeriod(baseStartDate,new Date(System.currentTimeMillis()));
		
	}
	public boolean inPeriod(Date baseStartDate,Date actualDate){
		if(baseStartDate.before(actualDate)){
			if(nextValuePeriod.getTime().before(actualDate))
				return true;
		}
		return false;
	}
