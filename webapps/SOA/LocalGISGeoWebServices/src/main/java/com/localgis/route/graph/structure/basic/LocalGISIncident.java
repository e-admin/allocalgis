...
public class LocalGISIncident implements Serializable {
	...
	public RecursiveIncident getRecursivencident(){
		return recursiveIncident;
	}
	
	public void setReursiveIncident(RecursiveIncident recursiveIncident){
		this.recursiveIncident = recursiveIncident;
	}
	
	public LocalGISIncident(int incidentType){
		this.incidentType = incidentType;
	}
	public LocalGISIncident(){
	...
	public LocalGISIncident(String data) throws ConfigurationException{
		...
		for(int i = 0;i<incidents.length && recursiveIncident == null;i++){
			...
			if(dates.contains("-")&& dates.split("-").length > 0 && dates.split("-")[0].split("/").length>2){
				//puede ser de formato "DD/MM/YY-DD/MM/YY"
				String[] date = dates.split("-");
				String[] time = times.split("-");
				if(dates.equals("-")){
					if(time.length > 0)
						recursiveIncident = new RecursiveIncident(data);
				}
				else if(date[0].split("/").length == 3 || date[1].split("/").length == 3){
	...
	public boolean equals(Object o) {
		if(o instanceof LocalGISIncident){
			if(((LocalGISIncident) o).getIncidentType() == this.getIncidentType())
				return true;
		}
		
		return false;
	}
	private static Logger LOGGER = Logger.getLogger(LocalGISIncident.class);
}
