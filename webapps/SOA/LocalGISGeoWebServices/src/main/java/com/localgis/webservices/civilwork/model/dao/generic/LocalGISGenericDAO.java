...
public class LocalGISGenericDAO {
	...
	public void addWarning(Connection connection, LocalGISNote warning) throws SQLException{
		...
		if(warning instanceof LocalGISIntervention){
			...
			if(warning instanceof LocalGISPlanesObra){
				warningType = WarningTypes.PLANESOBRA;
			}else{
				if(warning instanceof LocalGISNote){
	...
	public LocalGISNote getWarning(Connection connection,Integer idWarning,Integer idMunicipio,Integer idUser, Integer idEntidad) throws SQLException, ConfigurationException {
		LocalGISNote note = null;
		String entidad = "";
		if (idEntidad != null){
			entidad = "civil_work_warnings.id_municipio in (select id_municipio from entidades_municipios where id_entidad = ?) AND ";
		}
		else{
			entidad = "civil_work_warnings.id_municipio = ? AND ";
		}
	...
	public LocalGISNote getWarning(Connection connection,Integer idWarning,Integer idMunicipio,Integer idUser) throws SQLException, ConfigurationException {
		return getWarning(connection,idWarning,idMunicipio,idUser,null);
	}
	...
	public static String buildOrderByColumns(ArrayList<OrderByColumn> orderColumns) {
		...
		while(it.hasNext()){
			...
			if (column != null && !column.getColumn().isEmpty()){
				if(result.equals(""))
					result = " ORDER BY ";
