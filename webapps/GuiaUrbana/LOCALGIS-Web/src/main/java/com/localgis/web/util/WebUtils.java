...
public class WebUtils {
    ...
    public static SortedSet joinAndSortLayers(List layersIncluded, List localgisLayers, List localgisWMSLayers, LocalgisMap localgisMap, String urlGetMapRequests, String urlGetFeatureInfoRequests, String urlGetLegendGraphicsRequests,
        ...
        if (localgisWMSLayers != null) {
            ...
            while (itWMSLayers.hasNext()) {
                ...
                LayerOpenlayers layerOpenLayer=buildLayerOpenlayers(localgisMapServerLayer,projection);
                boolean resultado=result.add(layerOpenLayer);
                if (!resultado)
                	System.out.println("Elemento no añadido");
     ...
     * @return Una capa de Openlayers
     */
    public static LayerOpenlayers buildLayerOpenlayers(LocalgisMapServerLayer localgisMapServerLayer,String projection) {
        LayerOpenlayers layerOpenlayers = new LayerOpenlayers();
        ...
        //layerOpenlayers.setPosition(new Integer(0));
        layerOpenlayers.setPosition(localgisMapServerLayer.getPosition());
        ...
        urlGetFeatureInfoRequests += "REQUEST=GetFeatureInfo&EXCEPTIONS=application/vnd.ogc.se_xml&INFO_FORMAT=gml&FORMAT="+localgisMapServerLayer.getFormat()+"&QUERY_LAYERS="+localgisMapServerLayer.getLayers()+"&LAYERS="+localgisMapServerLayer.getLayers()+"&VERSION="+layerOpenlayers.getVersion();
        if (projection==null)
            urlGetFeatureInfoRequests += "&SRS="+localgisMapServerLayer.getSrs();
        else
            urlGetFeatureInfoRequests += "&SRS="+projection;
