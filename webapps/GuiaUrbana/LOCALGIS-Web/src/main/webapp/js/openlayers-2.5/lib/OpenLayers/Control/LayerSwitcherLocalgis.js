...
//        this.dataExtLbl.style.display = (containsOverlays) ? "" : "none"; 
    ...
    loadContents: function() {
     ...
     // Creacion de un div modify para los botones de modificar las capas
        ...
        //Creación de los elemento que controlarán las imágenes al lado del nombre
        ...
        this.modify.appendChild(menosTrans);//Se introducen dentro las imágenes
        ...
        this.buttonExtLayer.value = "Añadir Capas Externas";
        ...
        OpenLayers.Event.observe(upButton, "click", 
                OpenLayers.Function.bindAsEventListener(
                		function capaUp(e){  
                			...
                			//Esta variable guarda el índice de la capa dentro del contenedor
                			var idx = this.map.getLayerIndex(this.map.activeLayer);
                			//Esta condición impide que intentemos ascender una capa que no debería ascender más
                			//if(idx<(cont-2) && idx>3){
							if(idx<(cont-4) && idx>3){
                				this.map.raiseLayer(this.map.activeLayer, 1);
                			}
        ...
        OpenLayers.Event.observe(downButton, "click", 
                OpenLayers.Function.bindAsEventListener(
                		function capaDown(e){
                			...
                			//Esta variable guarda el índice de la capa dentro del contenedor
                			var idx = this.map.getLayerIndex(this.map.activeLayer);
                			//La capa con el índice número 1 es la de más abajo
                			//La penúltima capa es la única que no debería bajar
                			if(idx<(cont) && idx>4){
                				this.map.raiseLayer(this.map.activeLayer, -1);   
                			}
                			if (e != null) {
