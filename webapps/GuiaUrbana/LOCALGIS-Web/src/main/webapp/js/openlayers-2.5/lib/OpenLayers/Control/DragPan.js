...
OpenLayers.Control.DragPan = OpenLayers.Class(OpenLayers.Control, {
    ...
    */
    ...
    panMapDone: function(xy) {
        ...
        try{
        	if(!_isIE)this.map.zoomTo(this.map.getZoom()-0);
       		
        	if(esRuta)
        	{
        		//this.map.setCenter(this.map.getLonLatFromViewPortPx(xy), this.map.getZoom(), this.handler.dragging,true);
        		
        		bounds = this.map.getExtent();
        		center = bounds.getCenterLonLat();
        		this.map.setCenter(center, this.map.getZoomForExtent(bounds),false,true);
        	}
