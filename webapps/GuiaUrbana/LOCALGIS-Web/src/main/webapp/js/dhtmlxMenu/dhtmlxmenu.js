...
dhtmlXMenuObject.prototype._showContextMenu = function(x, y, zoneId) {
	...
	*/
	...
	try
	{
		...
		var alturaZona = parseInt(document.getElementById(divPrimaria).offsetHeight);
		var topMenu = y;
		if(alturaZona - topMenu > 0)this._showPolygon(this.idPrefix+this.topId, "bottom");
		else this._showPolygon(this.idPrefix+this.topId, "top");
...
dhtmlXMenuObject.prototype._doOnContextBeforeCall = function(e, cZone) {
	...
	/*if(!_isIE)
	{
		if(e.target.tagName == "svg"){
			var parent = document.getElementById(e.target.id).parentNode.id;
	...
	// scroll settings
	var p = (e.srcElement||e.currentTarget);
