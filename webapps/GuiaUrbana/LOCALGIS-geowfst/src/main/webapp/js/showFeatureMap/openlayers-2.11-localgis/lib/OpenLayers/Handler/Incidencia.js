OpenLayers.Handler.Incidencia = OpenLayers.Class.create();
	OpenLayers.Handler.Incidencia.prototype =
	  OpenLayers.Class.inherit(OpenLayers.Handler.Feature, 
	  {
		...
		{    
			...
			var feature = null;
			for (var i = 0; i < this.layer.incidencias.length; i++) 
			{
				if (this.layer.incidencias[i].icon.imageDiv.firstChild == node) 
				{
					feature = this.layer.incidencias[i];
					break;
				}
			}
			var selected = false;
		...
		CLASS_NAME: "OpenLayers.Handler.Incidencia"
	});