...
import es.uji.dsign.cms.CMSSignedHelper;
import es.uji.dsign.cms.CMSUtils;
...
public class MyCMSSignedDataGenerator    extends CMSSignedGenerator
{
    ...
    private class SignerInf
    {
            ...
            throws IOException, SignatureException, InvalidKeyException, NoSuchProviderException, NoSuchAlgorithmException, CertificateEncodingException, CMSException
        {
            ...
            Signature           sig = CMSSignedHelper.INSTANCE.getSignatureInstance(signatureName, Security.getProvider(sigProvider));
            

            byte[]      hash = null;
            
            if ( _myhash == null ){
            	MessageDigest       dig = CMSSignedHelper.INSTANCE.getDigestInstance(digestName, Security.getProvider(sigProvider));
            	if (content != null)
            	{
