...
public class MainWindow {
	...
	private JComboBox  signFormatList=null;
	private JLabel labelCertificateType = null;
	...
	private SignatureApplet sa;
	
	public MainWindow(AppHandler aph, SignatureApplet sa) 
	throws Exception {
		_aph= aph;	
	...
	JPanel getMainFrame() throws Exception {
	    if (mainPanel == null) {
	        this.mainPanel = getMainContentPane();
	        sa.add(this.mainPanel);
	    }
	    return mainPanel;
	}
...
//	}
	...
	throws Exception {
		if (mainContentPane == null) {
			...
			//***************************
			//Formato de firma (fjgarcia)
			//***************************
			JLabel labelSignFormatList = new JLabel();
			labelSignFormatList.setBounds(new Rectangle(5, 398, 236, 18));
			labelSignFormatList.setText(LabelManager.get("LABEL_SIGN_SELECTION"));
			
			String[] data = {"XADES-BES","XADES-T","XADES-C","XADES-X","XADES-XL","RAW","CMS","PDF"};
			signFormatList = new JComboBox(data);
			signFormatList.setBounds(new Rectangle(170, 398, 110, 20));
			signFormatList.setVisible(true);
			
			labelCertificateType = new JLabel();
			labelCertificateType.setBounds(new Rectangle(100, 208, 500, 20));
			labelCertificateType.setText("");
			
			// Accion a realizar cuando el JComboBox cambia de item seleccionado.
			signFormatList.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent e) {
					labelCertificateType.setText("Firma Seleccionada:"+signFormatList.getSelectedItem().toString());
					if ((signFormatList.getSelectedItem().toString().equalsIgnoreCase("XADES-C"))||
	                		(signFormatList.getSelectedItem().toString().equalsIgnoreCase("XADES-X"))||
	                		(signFormatList.getSelectedItem().toString().equalsIgnoreCase("XADES-XL"))){
						labelCertificateType.setText(LabelManager.get("FIRMA_XADES_INFO"));
	                }
	                else if (signFormatList.getSelectedItem().toString().equalsIgnoreCase("XADES-T")){
	                	labelCertificateType.setText(LabelManager.get("FIRMA_XADES_INFO2"));
	                }
				}
			});
			...
			mainContentPane.add(labelSignFormatList,null);
			mainContentPane.add(signFormatList,null);
			mainContentPane.add(labelCertificateType,null);
...
//	}
	...
	public String getSignFormat(){
		return (String)signFormatList.getSelectedItem();
	}
	
	public void setInfoCertificate(String text){
		labelCertificateType.setText(text);
	}
}
