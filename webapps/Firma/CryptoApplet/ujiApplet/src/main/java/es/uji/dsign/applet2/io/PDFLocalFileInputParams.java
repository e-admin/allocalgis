...
public class PDFLocalFileInputParams extends AbstractData implements InputParams
{
	...
	public PDFLocalFileInputParams(SignatureApplet sa) {
	    _base = sa;
		count= 1;
		initialized= true;
        ...
        fileChooser.setFileFilter(new FileFilter() {
            public boolean accept(File f) {
                if (f != null && (f.isDirectory() || f.getName().toLowerCase().endsWith(".pdf"))) {
                    return true;
                } else { 
                    return false;
                }
